...
public class ScoreValidator {
    ...
    private static ResourceBundle resexception;
    ...
    /**
     * Return true if an expression has valid coding item function syntax.<br>
     * syntax - func: (coding ([item name], [dictionary name], [apply] auto coding Y/N))
     *
     * @param expression
     * @param errors
     * @param allVariables
    ...
    public static boolean isValidCodingExpression(String expression, StringBuffer errors, ArrayList<String> allVariables) {
        if (expression == null || expression.length() < 1) {            
            errors.append("expression_is_empty");
            return false;
        }
...
//            else if (ScoreUtil.isOperator(c)) {
//                if (!ScoreValidator.noCommaEnds(token)) {
//                    // errors.append(token + " should not contain comma" +"; ");
//                    errors.append(token + " " + "should_not_contain_comma") + "; ");
//                }
//                token = token.trim();
//                if (token.length() > 0 && !ScoreValidator.isNumber(token) && !allVariables.contains(token)) {
//                    allVariables.add(token);
//                }
//                finalExpression += token + c;
//                token = "";
//            }
                ...
                if (isCodingFunc(token)) {
                    err.delete(0, err.length());
                    if (!isValidCodingFunction(contents, info, token, err, allVariables)) {
                        errors.append(err);
                    }
                    // fake a result to carry on syntax check
                    finalExpression += "0";
                    ...
                    if (token.length() > 1) {
                        // errors.append(token + " " + "is not a valid function
                        // or has not been supported yet" +"; ");
                        // errors.append("error on line 139");
                        errors.append(token + " " + resexception.getString("function_is_invalid_or_not_supported") + "; ");
                        // carry on syntax check
                        err.delete(0, err.length());
                        ArrayList<String> variables = new ArrayList<String>();
                        if (!isValidCodingFunction(contents, info, token, err, variables)) {
                            errors.append(err);
                        }
                        // fake a function result to carry on syntax check
                        finalExpression += "0";
                ...
                token = token.trim();
              //why not a number check here? copied from calculations logic
                if (token.length() > 0 && !isNumber(token) && !allVariables.contains(token)) {
                    allVariables.add(token);
                }
                finalExpression += token + c;
...
//                    // errors.append("The character"+" " + c + " " + "should not
//                    // be followed by the character " + next + "; ");
//                    // errors.append("hit this error instead: " + c + next);
//                    errors.append("the_character") + " " + c + " " + "should_not_followed_by_character") + " "
//                        + next + "; ");
//                }
//            }
            info.pos++;
        ...
        if (info.level != 0) {
            // errors.append("Expression"+" " + exp + " "+"is invalid because
            // parenthesises are not correctly paired"+"; ");
            errors.append("expression" + " " + exp + " " + resexception.getString("is_invalid_because_wrong_paired_parenthesises"));
        }
        finalExpression += token;
...
/**
 * Checks if function specifies after "func:" is "coding     
 * @param contents
 * @param info
 * @param func
 * @param errors
 ...
 * @return
 */
    public static boolean isValidCodingFunction(char[] contents, ScoreUtil.Info info, String func, StringBuffer errors, ArrayList<String> allVariables) {
    	//ToDo add validation for minimum 3 function parameters at start
        int originalLevel = info.level;
        info.pos++;
        info.level++;
        String token = "";
        ...
        while (info.pos < contents.length) {
            ...
            if (c == ')') {
                if (!noCommaEnds(token)) {
                    // System.out.println(token + " should not contain comma" +"; ");
                    errors.append(token + " " + resexception.getString("should_not_contain_comma") + "; ");
                }
                info.level--;
                // end of the function, marked by the equal level
...
//                        // is invalid; ");
//                        errors.append(err);
                    funcVariables.add(currentExpression);
                    ++argCount;
                    //validate total number of arguments
                    if (!isValidArgumentsCnt(argCount)){
                    	errors.append(resexception.getString("coding_invalid_arg_count"));
                    }
//                    }
                    token = "";
                    break;
                ...
                // again, it is either the start of a function or an expression
                if (token != null && isCodingFunc(token)) {
                    err.delete(0, err.length());
                    ArrayList<String> variables = new ArrayList<String>();
                    if (!isValidCodingFunction(contents, info, token, err, variables)) {
                        errors.append(err);
                    ...
                    if (token.length() > 1) {
                        // errors.append(token + " is not a valid function or
                        // has not been supported yet"+"; ");
                        // errors.append("found error on line 287");
                        errors.append(token + " " + resexception.getString("function_is_invalid_or_not_supported") + "; ");
                        // carry on syntax check
                        err.delete(0, err.length());
                        ArrayList<String> variables = new ArrayList<String>();
...
//            boolean isNextSign = isValidSign(next, nextnext, func);
//            if (!isNextSign) {
//                // errors.append("got this far: func: " + func);
////                if (!isValidOrder(c, next, func)) { //&& !isValidUrl(contents, token, func)) {
////                    // errors.append("One character"+" " + c + " "+"has been
////                    // followed by a wrong character"+" " + next + "; ");
////                    errors.append("the_character") + " " + c + " " + "should_not_followed_by_character") + " "
...
//            // errors.append("Function"+" " + func + "() should have 2 arguments
//            // only"+"; ");
//            errors.append("function") + " " + func + " " + "should_have_2_arguments_only") + "; ");
//        }
...
//        }
  ...
  public static boolean isValidArgumentsCnt(Integer argCnt){
	  return (argCnt >= 3 );
  }
    ...
    /**
     * Return true if a string matches "coding"
     *
     * @param token
     * @return
     */
    public static boolean isCodingFunc(String token) {
        return token.equalsIgnoreCase("coding");
    }

...
public class RuleSetDao extends AbstractDomainDao<RuleSetBean> {
    ...
    @SuppressWarnings("unchecked")
    public ArrayList<RuleSetBean> findByItemId(Integer itemId, StudyBean study) {
        String query = "from " + getDomainClassName() + " ruleSet  where ruleSet.itemId = :itemId and ruleSet.studyId = :studyId ";
        org.hibernate.Query q = getCurrentSession().createQuery(query);

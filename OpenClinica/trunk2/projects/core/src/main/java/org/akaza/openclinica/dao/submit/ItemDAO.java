...
public class ItemDAO<K extends String,V extends ArrayList> extends AuditableEntityDAO {
    ...
    public ArrayList findByNameCrfVersionAndStudy(int studyId, int crfId, String name) {
        ItemBean eb = new ItemBean();
        this.setTypesExpected();
        ...
        variables.put(new Integer(2), new Integer(crfId));
        variables.put(new Integer(3), name);

        String sql = digester.getQuery("findByNameCrfVersionAndStudy");
        ArrayList alist = this.select(sql, variables);

...
public class ViewDiscrepancyNoteServlet extends SecureController {
    ...
    public static final String ALLOWED_DISCREPANCIES = "allowedDiscrepancies";
    public static final String POPULATE_DISCREPANCIES = "populateDiscrepancies";
    ...
    protected void processRequest() throws Exception {
      ...
      //  request.setAttribute(DIS_TYPES, DiscrepancyNoteType.toArrayList());
        ...
        HashMap<String,List<String>> permitDiscrepancies =( HashMap<String,List<String>>) request.getSession().getAttribute("permitDiscrepancies");
     
      
        doPopulateDiscrepancies(request,permitDiscrepancies);
        //TODO:Clean up the following code for hardcoded roles, its not required anymore
        if (currentRole.getRole().equals(Role.RESEARCHASSISTANT) || currentRole.getRole().equals(Role.INVESTIGATOR)) { 
          //  ArrayList<ResolutionStatus> resStatuses = new ArrayList();
           ...
           // request.setAttribute(RES_STATUSES, resStatuses);  
            ...
            resStatuses = ResolutionStatus.toArrayList();
...
//            StudySubjectBean notessub = (StudySubjectBean) ssdao.findByPK(noteSubId);
    ...
    private void doPopulateDiscrepancies(HttpServletRequest request, HashMap<String, List<String>> permitDiscrepancies) {
 Set<String> discrepancyTypes =  permitDiscrepancies.keySet();
 HashMap<DiscrepancyNoteType,List<String>> allowedDiscrepancies=  new HashMap<DiscrepancyNoteType,List<String>>();
 HashMap<String,ArrayList<ResolutionStatus>> populateDiscrepancies=  new HashMap<String,ArrayList<ResolutionStatus>>();
 Iterator<String> itResolution;   
 DiscrepancyNoteType discrepancyType;

 ArrayList<DiscrepancyNoteType> disTypes = new ArrayList<DiscrepancyNoteType>();
 ArrayList<ResolutionStatus> resStatuses = new ArrayList<ResolutionStatus>();
 
        //the following is for populating the createNew Thread ...
        for(String dnType:discrepancyTypes){
           List<String>tempStatus = new ArrayList<String>();
           List<String> tempResStatus = new ArrayList<String>();
           resStatuses = new ArrayList<ResolutionStatus>();
           String tempResS=null;
           discrepancyType = DiscrepancyNoteType.getByName(dnType);//key
           disTypes.add(discrepancyType);
           tempStatus =  permitDiscrepancies.get(dnType);//list of res status
           
           itResolution = tempStatus.iterator();
           
           while(itResolution.hasNext())
           {
               tempResS =  (String)itResolution.next();
               tempResStatus.add(tempResS);
               resStatuses.add(ResolutionStatus.getByName(tempResS));
           }
           allowedDiscrepancies.put(discrepancyType, tempResStatus);
           populateDiscrepancies.put(dnType,resStatuses);
        }
        
        request.setAttribute(DIS_TYPES, disTypes);
        
        request.setAttribute(ALLOWED_DISCREPANCIES,allowedDiscrepancies);
        request.setAttribute(POPULATE_DISCREPANCIES,populateDiscrepancies);

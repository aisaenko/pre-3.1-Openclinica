...
public class ViewSectionDataEntryServlet extends DataEntryServlet {
    ...
    //JN:Commenting out, not required anymore
...
*/
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
       ...
       sb = (SectionBean) sdao.findByPK(sectionId);
        ...
        //@pgawade Remove the medical code items from the list of items displayed in a section of CRF        
        if (null != dsb.getDisplayItemGroups()){
        	List<DisplayItemWithGroupBean> displayItemGroups = dsb.getDisplayItemGroups();
        	//List<DisplayItemWithGroupBean> displayItemGroupsCodesRemoved = new ArrayList<DisplayItemWithGroupBean>();
        	DisplayItemWithGroupBean displayItemWithGroupBean = null;
        	for (int i=0; i<displayItemGroups.size(); i++ ){
        		displayItemWithGroupBean = displayItemGroups.get(i);        		
	    		//repeating grp items
	    		if(displayItemWithGroupBean.isInGroup()){
	    			if( (null != displayItemWithGroupBean) && (null != displayItemWithGroupBean.getItemGroup())){
	        			DisplayItemGroupBean displayItemGroupBean = displayItemWithGroupBean.getItemGroup();
	        			if( (null != displayItemGroupBean) && (null != displayItemGroupBean.getItems())){
	        				List<DisplayItemBean> items = displayItemGroupBean.getItems();
	        				DisplayItemBean item = null;
	        				for(int j=0; j<items.size(); j++){
	        					item = items.get(j);
	        					if( (null != item) && (null != item.getMetadata())){
	        						ItemFormMetadataBean itemMetadata = item.getMetadata();
	        						if((null != itemMetadata) && (null != itemMetadata.getResponseSet())){
	        							if(itemMetadata.getResponseSet().getResponseType().getId() == ResponseType.MEDICAL_CODE.getId()){
	        								items.remove(item);//remove the code item
	        							}
	        						}
	        		...
	        		if( (null != displayItemWithGroupBean) && (null != displayItemWithGroupBean.getItemGroups())){
	    			List<DisplayItemGroupBean> displayItemGroupBeanLst = displayItemWithGroupBean.getItemGroups();
		    			if( (null != displayItemGroupBeanLst) && (0 != displayItemGroupBeanLst.size())){
		    				for(int k=0; k<displayItemGroupBeanLst.size(); k++){
		    					DisplayItemGroupBean displayItemGroupBean = displayItemGroupBeanLst.get(k);
		    					if((null != displayItemGroupBean) && (displayItemGroupBean.getItems() != null)){
		    						List<DisplayItemBean> items = displayItemGroupBean.getItems();
		            				DisplayItemBean item = null;
		            				for(int j=0; j<items.size(); j++){
		            					item = items.get(j);
	    			...
	    			//non-repeating grp items
	        		if( (null != displayItemWithGroupBean) && (null != displayItemWithGroupBean.getSingleItem())){
	        			DisplayItemBean singleItem = displayItemWithGroupBean.getSingleItem();
	        			if((null != singleItem) && (null != singleItem.getMetadata())){
	        				ItemFormMetadataBean itemMetadata = singleItem.getMetadata();
							if((null != itemMetadata) && (null != itemMetadata.getResponseSet())){
								if(itemMetadata.getResponseSet().getResponseTypeId() == ResponseType.MEDICAL_CODE.getId()){
									displayItemGroups.remove(displayItemWithGroupBean);//remove the code item
								}
							}
	        			}

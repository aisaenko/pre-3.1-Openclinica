...
import org.akaza.openclinica.web.util.MenuItem;
...
//get menu and find what page - Home
                PageMenu pageMenu = (PageMenu) request.getSession().getAttribute(PageMenu.PAGE_MENU_UI_CONSTANT);
                MenuItem homePage = pageMenu.getNavBar().get(0);
                if ( homePage.getHomePageURL().equals("ListStudySubjects")) {
                    setupListStudySubjectTable();
                }
                if (homePage.getHomePageURL().contains("viewAllSubjectSDVtmp")) {
                    setupSubjectSDVTable();
                } else if (homePage.getHomePageURL().equals("StatisticsPage")) {
                    if (currentStudy.getStatus().isPending()) {
                        response.sendRedirect(request.getContextPath() + Page.MANAGE_STUDY_MODULE);
                        return;
                    }
                    //htaycher: replace to normal tables
...
//                + "<a href=\"UpdateProfile\">" + respage.getString("user_profile") + " </a>");
    ...
    private void setupStudySiteStatisticsTable_new() {

        List<StudyBean> studies = (List<StudyBean>) getStudyDAO().findAll(currentStudy.getId());
        List<String[]> studySiteStatistics = new ArrayList<String[]>(studies.size());
        
        for (StudyBean studyBean : studies) {
            // Get number of subjects enrolled at a specific study or site
            Integer countofStudySubjectsAtStudyOrSite = getStudySubjectDAO().getCountofStudySubjectsAtStudyOrSite(studyBean);
            ...
            String[] theItem = {studyBean.getName(),
            		String.valueOf(countofStudySubjectsAtStudyOrSite)
            , String.valueOf(studyBean.getExpectedTotalEnrollment())
            , String.valueOf(percentage) + "%"};
            studySiteStatistics.add(theItem);
        }

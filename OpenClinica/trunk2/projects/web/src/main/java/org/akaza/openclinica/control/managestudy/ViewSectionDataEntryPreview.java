...
public class ViewSectionDataEntryPreview extends DataEntryServlet {
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //@pgawade - change to refrain medical code items from displaying in the CRF preview. They will present in the CRF spreadsheet 
        // with response type as 'hidden-code' (response type = 11)
        Integer rowToremove = null;
        if(null != itemsMap){
        	for (Map.Entry<Integer, Map<String, String>> entry : itemsMap.entrySet()) {
        	
        		Integer rowNum = entry.getKey();
        		Map<String, String> item = entry.getValue();
        		if (null != item && item.size() != 0){
        			if((item.containsKey("response_type")) && (item.get("response_type")!= null)
        			&& (item.get("response_type").equalsIgnoreCase(ResponseType.MEDICAL_CODE.getName()))){
//        				itemsMap.remove(rowNum);
        				rowToremove = new Integer(rowNum.intValue());	
        			}
        		}        	
        	}
        }
        if(null != rowToremove)	{
        	itemsMap.remove(rowToremove);
        }

...
<table cellpadding="0" border="0" class="shaded_table cell_borders  hrow">
  <tr valign="top" ><th><fmt:message key="dataset_name" bundle="${resword}"/>:</th><td class="table_cell_top">
  <c:out value="${dataset.name}"/>
  </td></tr>
  <tr valign="top" ><th><fmt:message key="dataset_description" bundle="${resword}"/>:</th><td class="table_cell">
  <c:out value="${dataset.description}"/>
  </td></tr>
  <tr valign="top"><th><fmt:message key="item_status" bundle="${resword}"/>:</th><td class="table_cell">
  <c:out value="${dataset.datasetItemStatus.description}"/>
  </td></tr>
  <tr valign="top" ><th><fmt:message key="study_name" bundle="${resword}"/>:</th><td class="table_cell">
  <c:out value="${extractBean.parentStudyName}"/>
   </td></tr>
  <tr valign="top"><th><fmt:message key="protocol_ID" bundle="${resword}"/>:</th><td class="table_cell">
  <c:out value="${extractBean.parentProtocolId}"/>
  </td></tr>
  <tr valign="top"><th><fmt:message key="date" bundle="${resword}"/>:</th><td class="table_cell">
  <fmt:formatDate value="${extractBean.dateCreated}" pattern="${dteFormat}"/>
  </td></tr>
  <tr valign="top"><th><fmt:message key="subjects" bundle="${resword}"/>:</th><td class="table_cell">
...
<table cellpadding="0" border="0" class="shaded_table cell_borders  hrow">
 <tr valign="top">
    ...
    <th><fmt:message key="protocol_ID_site_ID" bundle="${resword}"/></th>
    <c:if test="${dataset.showSubjectDob}">
      <c:choose>
        <c:when test="${study.studyParameterConfig.collectDob =='1'}">
         <th><fmt:message key="date_of_birth" bundle="${resword}"/></th>
        </c:when>
        <c:when test="${study.studyParameterConfig.collectDob =='3'}">
         <th><fmt:message key="date_of_birth" bundle="${resword}"/></th>
        </c:when>
        <c:otherwise>
          <th><fmt:message key="year_of_birth" bundle="${resword}"/></th>
        </c:otherwise>
      </c:choose>
    </c:if>
    ...
    <c:if test="${dataset.showSubjectGroupInformation}">
	   ...
	   <c:forEach var="groupClass" items="${extractBean.studyGroupClasses}">
			...
			<th><c:out value="${groupClass.name}"/></th>
			<%--</c:if>--%>
	   </c:forEach>
    </c:if>
    ...
    <c:forEach var="eventHeader" items="${extractBean.eventHeaders}">
       <th><c:out value="${eventHeader}"/>&nbsp;</th>
      </c:forEach>
    <c:forEach var="itemName" items="${extractBean.itemNames}">
      <th><a href="javascript: openDocWindow('ViewItemDetail?itemId=<c:out value="${itemName.item.id}"/>')"><c:out value="${itemName.itemHeaderName}"/></a></th>
    </c:forEach>
   </tr>

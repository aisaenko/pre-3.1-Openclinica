...
<div class="table_title"><a href="javascript:leftnavExpand('siteProperties');">
...
<br>
	...
	<div class="table_title" style="width:300px;float:left"><fmt:message key="view_site_event_definitions" bundle="${resword}"/></div>
...
<c:set var="defCount" value="${defCount+1}"/>
	&nbsp&nbsp&nbsp&nbsp<a href="javascript:leftnavExpand('sed<c:out value="${defCount}"/>');">
        <img id="excl_sed<c:out value="${defCount}"/>" src="images/bt_Expand.gif" border="0">
         <span class="section_link"> <c:out value="${definition.name}"/></span></a>
		<c:choose>
...
<table border="0" cellpadding="0" width="700"   class="shaded_table  cell_borders hrow">
 <tr valign="top">
    <th width="90px" class="table_header_row"><fmt:message key="name" bundle="${resword}"/></th>
    ...
    <!-- <th><fmt:message key="enforce_decision_conditions" bundle="${restext}"/></td>-->
    <th><fmt:message key="default_version" bundle="${resword}"/></th>
     ...
     <th><fmt:message key="selected_verions" bundle="${resword}"/></th>
...
<br><br>
    <c:choose>
        <c:when test="${userBean.sysAdmin}">
            <input type="button" onclick="confirmExitWithMessage('ListSite', '<fmt:message key="sure_to_exit_no_transaction" bundle="${resword}"/>');"  name="cancel" value="<fmt:message key="exit" bundle="${resword}"/>   " class="button_medium"/>    
        </c:when>
        <c:otherwise>
...
<br>
 <c:choose>
  <c:when test="${fromListSite=='yes'}">
   <p><a href="#" onClick="history.go(-1)" class="go_back_to_"><fmt:message key="go_back_to_site_list" bundle="${resword}"/></a></p>   
  </c:when>
  <c:otherwise>
    <c:choose>
      <c:when test="${isAdminServlet == 'admin'}">
       <p><a href="ListStudy"  class="go_back_to_"><fmt:message key="go_back_to_study_list" bundle="${resword}"/></a></p>
      </c:when>
      <c:otherwise>
         <p><a href="MainMenu"  class="go_back_to_"><fmt:message key="go_back_home" bundle="${resword}"/></a></p>
      </c:otherwise>
    </c:choose>
  </c:otherwise>
 </c:choose>

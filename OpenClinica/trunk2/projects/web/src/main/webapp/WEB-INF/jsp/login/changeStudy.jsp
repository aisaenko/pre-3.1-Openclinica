...
<jsp:useBean scope="request" id="studies" class="java.util.HashMap"/>
...
<p><fmt:message key="please_choose_study_list" bundle="${restext}"/>:</P>
    <div class="box_T"><div class="box_L"><div class="box_R"><div class="box_B"><div class="box_TL"><div class="box_TR"><div class="box_BL"><div class="box_BR">
     <table border="0" cellpadding="0" cellspacing="0" width="100%">
       <c:forEach var="entry" items="${studies}" varStatus = "status">
           <c:set var="statusId" value="${entry.key.status.id}"/>
        <c:choose>
         <c:when test="${study.id == entry.key.id}">

           <c:choose>
            <c:when test="${entry.key.parentStudyId > 0}">
           <!--    <c:if test="${!studyRole.invalid}"> -->
                 <tr>
                   <td class="table_cell_no_border">&nbsp;&nbsp;<img src="images/bullet.gif">
                   <input type="radio" checked name="studyId" value="<c:out value="${entry.key.id}"/>" <c:if test="${statusId==4}">disabled="true"</c:if>>
                   <c:out value="${entry.key.name}"/>
                       <c:if test="${statusId==4}">(Design)&nbsp;</c:if>
                       (<c:out value="${entry.value.role.i18nDescription}" />)</td>
                 </tr>
              <!--   </c:if>-->
            </c:when>
            <c:otherwise>
                <tr>
                 <td class="table_cell_no_border">
                     <input type="radio" checked name="studyId" value="<c:out value="${entry.key.id}"/>">
                 <b><c:out value="${entry.key.name}"/> (<c:out value="${entry.value.role.i18nDescription}" />)</b></td>
                 </tr>
          ...
          <c:choose>
            <c:when test="${entry.key.parentStudyId > 0}">
               <!--<c:if test="${!studyRole.invalid}">-->
                 <tr>
                  <td class="table_cell_no_border">&nbsp;&nbsp;<img src="images/bullet.gif">
                      <input type="radio" name="studyId" value="<c:out value="${entry.key.id}"/>" <c:if test="${statusId==4}">disabled="true"</c:if>>
                      <c:out value="${entry.key.name}"/>
                      <c:if test="${statusId==4}">(Design)&nbsp;</c:if>
                      (<c:out value="${entry.value.role.i18nDescription}" />)</td>
                 </tr>
            ...
            <c:otherwise>
                 <tr>
                  <td class="table_cell_no_border">
                      <input type="radio" name="studyId" value="<c:out value="${entry.key.id}"/>">
                  <b><c:out value="${entry.key.name}"/> (<c:out value="${entry.value.role.i18nDescription}" />)</b></td>
                 </tr>

...
<script language="JavaScript" src="includes/CalendarPopup.min.js"></script>
...
function setStatus(typeId,filter1,nw,ud,rs,cl,na) {
	...
	resStatusAllowed = (allowedDiscrepancies.get(typeId).substring(1,allowedDiscrepancies.get(typeId).length-1)).split(",");
	var i=0;
	for(i=0;i<resStatusAllowed.length;i++)
	{
	

		if(resStatusAllowed[i].trim()==na){
		objtr2.disabled = true;
			objtr2.options[i]=new Option(na, '5');
		}
		 if(resStatusAllowed[i].trim()== nw){
		objtr2.options[i]=new Option(nw, '1');
		}		
		 if(resStatusAllowed[i].trim()== rs){
		objtr2.options[i]=new Option(rs, '3');
		}
 if(resStatusAllowed[i].trim()==cl){
		 //alert("matched closed");
		objtr2.options[i]=new Option(cl, '4');
		}
		 if(resStatusAllowed[i].trim()== ud){
		 //alert("matched updated");
		objtr2.options[i]=new Option(ud, '2');
		}
...
<div class="textbox_center">
	...
	<div class="dnBoxCol2 dnBoxText">
		<span id="description">
			<input type="text" name="description" value="<c:out value="${discrepancyNote.description}"/>" class="formfieldXL">

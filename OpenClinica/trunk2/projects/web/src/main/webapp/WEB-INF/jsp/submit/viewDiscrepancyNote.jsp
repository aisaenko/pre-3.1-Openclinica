...
<c:set var="typeID1" value="${discrepancyTypes[0].id}"/>
...
HashMap = function(){
  ...
  var allowedDiscrepancies = new HashMap();


	<c:forEach var="entry" items="${allowedDiscrepancies}">
	<c:set var="discrepancyTypeId" value="${entry.key.id}"/>

	allowedDiscrepancies.put("${discrepancyTypeId}",'${entry.value}');
	</c:forEach>
...
<body class="popup_BG" style="margin: 0px 12px 0px 12px;" onload="window.scrollTo(0,'<c:out value="${y}"/>');javascript:setStatusWithId('<c:out value="${typeID1}"/>','0','<c:out value="${whichResStatus}"/>','<fmt:message key="New" bundle="${resterm}"/>','<fmt:message key="Updated" bundle="${resterm}"/>','<fmt:message key="Resolution_Proposed" bundle="${resterm}"/>','<fmt:message key="Closed" bundle="${resterm}"/>','<fmt:message key="Not_Applicable" bundle="${resterm}"/>');javascript:refreshSource('<c:out value="${refresh}"/>', '/ViewNotes?');">
...
<table border="0" cellpadding="0" cellspacing="0" style="float:left;">
    ...
    <tr>
	<td valign="top">
		      <table border="0" cellpadding="0" cellspacing="0" class="shaded_table no_width_defined">
            	<tr>
...
<table border="0" cellpadding="0" cellspacing="0" class="shaded_table no_width_defined" style="border-collapse:true;">
    <tbody>
        <tr>
            <td>
                <!-- These DIVs define shaded box borders -->
                       <table border="0" cellpadding="0" cellspacing="0" width="600">
                            ...
                            <tr class="aka_stripes">
                                ...
                                <td class="aka_header_border" width="25%"><fmt:message key="type" bundle="${resword}"/>: <b><c:out value="${note.value.disType.name}"/></b></td>
                             	<c:set var="disTypeName" value="${note.value.disType.name}"/>
								<c:out value="${disTypeName}"/>
                             	<c:set var="permittedResStatuses" value="${populateDiscrepancies[disTypeName]}"/>
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" width="600" id="thread<c:out value="${count}"/>">
                            ...
                            <c:if test="${!study.status.locked}">
                            	...
                            	<td class="table_cell_left" colspan="4" align="right">
                            			...
                            			<c:forEach var="status" items="${permittedResStatuses}">
                        					
											
											<c:choose>
                        					<c:when test="${status.id == 2}">
                        						<input class="button_medium" type="button" id="resStatus${sindex}${note.value.id}" value="<fmt:message key="updaate_note" bundle="${resterm}"/>" onclick="javascript:boxShowWithDefault('<c:out value="${note.value.id}"/>','<c:out value="${sindex}"/>','<c:out value="${status.id}"/>','<c:out value="${status.name}"/>','<fmt:message key="Updated" bundle="${resterm}"/>');/*scrollToElement('<c:out value="submitBtn${note.value.id}"/>');*/"/>
            								</c:when>
            								<c:when test="${status.id == 3}">
                        						<input class="button_medium" type="button" id="resStatus${sindex}${note.value.id}" value="<fmt:message key="Propose_Resolution" bundle="${resterm}"/>" onclick="javascript:boxShowWithDefault('<c:out value="${note.value.id}"/>','<c:out value="${sindex}"/>','<c:out value="${status.id}"/>','<c:out value="${status.name}"/>','<fmt:message key="Resolution_Proposed" bundle="${resterm}"/>');"/>
            								</c:when>
            								<c:when test="${status.id == 4}">
                        						<input class="button_medium" type="button" id="resStatus${sindex}${note.value.id}" value="<fmt:message key="close_note" bundle="${resterm}"/>" onclick="javascript:boxShowWithDefault('<c:out value="${note.value.id}"/>','<c:out value="${sindex}"/>','<c:out value="${status.id}"/>','<c:out value="${status.name}"/>','<fmt:message key="Closed" bundle="${resterm}"/>');"/>
            								</c:when>
                        					</c:choose>

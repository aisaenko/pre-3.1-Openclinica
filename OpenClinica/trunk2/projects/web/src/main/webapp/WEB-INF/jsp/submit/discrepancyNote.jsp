...
function boxShowWithDefault(id, index, defaultId, defaultValue,statusVal){
	...
	if(objSelect != null) {
		if(defaultValue==statusVal){
			//alert("inser");
			objSelect.options[1]=new Option(defaultValue, '1');
			objSelect.selectedIndex = 1;
			objSelect.options[1].value=defaultId;
		}
	}
...
function setStatusWithId(typeId,id,filter1,nw,ud,rs,cl,na) {
	...
	var resStatusAllowed = new Array();
...
style="float:left;display:<c:out value="${displayAll}"/>" id="${boxId}">
	...
	<span id="res1${parentId}">
			...
			<c:out value="${permittedResStatuses}"/>
			<div class="dnBoxCol2">
				...
				<c:choose>
					<c:when test="${(parentId>0 || whichResStatus==2 && discrepancyNote.discrepancyNoteTypeId==3) || whichResStatus==1}">
						<c:set var="resStatuses" value="${permittedResStatuses}"/>
					</c:when>
					<c:otherwise>

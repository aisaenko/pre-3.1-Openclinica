...
<table cellpadding="0" border="0" class="shaded_table  " width="900">
<tr>
		<td class="formlabel"><fmt:message key="job_name" bundle="${resword}"/>:</td>
		<td class="text"><div><input type="text" name="jobName" size="30" value="<c:out value="${jobName}"/>"/></div>
	...
	<tr>
		...
		<td class="text"><div><input type="text" name="jobDesc" size="60" value="<c:out value="${jobDesc}"/>"/>
	...
	<tr>
		...
		<td class="text"><select name="dsId" class="formfieldXL">
	...
	<tr>
		<td class="formlabel" style="padding-top:4px;"><fmt:message key="period_to_run" bundle="${resword}"/>: </td>
		<td class="text"><div>
		<input type="radio" name="periodToRun" value="daily"
			<c:if test="${periodToRun == 'daily'}">		checked		</c:if>	/>
		&nbsp;<fmt:message key="daily" bundle="${resword}"/>
		<br><input type="radio" name="periodToRun" value="weekly"	<c:if test="${periodToRun == 'weekly'}">checked	</c:if>/>
				&nbsp;<fmt:message key="weekly" bundle="${resword}"/>
		...
		<jsp:include page="../showMessage.jsp"><jsp:param name="key" value="periodToRun"/></jsp:include>
	</tr>
	...
	<tr>
		<td class="formlabel" style="padding-top:4px;"><fmt:message key="file_formats" bundle="${resword}"/>:</td>
		<td class="text"><div>  
		 <c:forEach var="extract" items="${extractProperties}">
		      <input type="radio" name="formatId" value="<c:out value="${extract.id}"/>"       <c:if test="${formatId == extract.id}">	checked	</c:if>	/>&nbsp;
                    <c:choose>
	...
	<tr>
		...
		<td class="text"><div><input type="text" name="contactEmail" size="60" value="<c:out value="${contactEmail}"/>"/></div>
...
<table>
	<tr>
		...
		<form class="in-button" action='ViewJob' method="POST">

...
public class UnlockEventDefinitionServlet extends SecureController {
  ...
  public void mayProceed() throws InsufficientPermissionException {
    ...
    throw new InsufficientPermissionException(Page.LIST_DEFINITION_SERVLET, "not admin", "1");
  ...
  public void processRequest() throws Exception {
    ...
    if (StringUtil.isBlank(idString)) {
      addPageMessage("Please choose a study event definition to unlock.");
      forwardPage(Page.LIST_DEFINITION_SERVLET);
    } else {
      if ("confirm".equalsIgnoreCase(action)) {
        if (!sed.getStatus().equals(Status.LOCKED)) {
          addPageMessage("This Study Event Definition cannot be unlocked."
        ...
        String emailBody = "The Study Event Definition " + sed.getName()
            + " has been unlocked for the Study " + currentStudy.getName()
            + ". Subject event data is as it was before it was locked - available for viewing, "
            + "querying, and extraction, as well as for addition and modification.";
  ...
  private void sendEmail(String emailBody) throws Exception {
    ...
    try {
      ...
      em.process(ub.getEmail().trim(), EmailEngine.getAdminEmail(),
          "Unlock Study Event Definition", emailBody);

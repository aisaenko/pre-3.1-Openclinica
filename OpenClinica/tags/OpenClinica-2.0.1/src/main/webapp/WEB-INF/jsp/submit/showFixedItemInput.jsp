...
<c:if test='${inputType == "text"}'>
	<input type="text" name="input<c:out value="${itemId}" />" value="<c:out value="${displayItem.metadata.responseSet.value}"/>" disabled style="background:white;color:#4D4D4D;"/>
</c:if>
...
<c:if test='${inputType == "single-select"}'>
	<select name="input<c:out value="${itemId}"/>" class="formfield" disabled style="background:white;color:#4D4D4D;">
		<c:forEach var="option" items="${displayItem.metadata.responseSet.options}">
			<c:choose>

...
import org.akaza.openclinica.logic.core.*;
...
public abstract class DataEntryServlet extends SecureController {
  ...
  */
  protected void processRequest() throws Exception {
    ...
    //YW 11-07-2007, data entry could not be performed if its study subject has been removed. 
...
//  Get the study then the parent study
    ...
    if (!isSubmitted) {
      ...
      boolean validate =
       ...
       //We can conclude that the user is trying to save data; therefore, set a request
      //attribute indicating that default values for items shouldn't be displayed
      ...
      for (int i = 0; i < allItems.size(); i++) {
        ...
        if (diwg.isInGroup()) {
          ...
          if (validate) {
            ...
            //TODO next marker tbh 112007
        	  ...
        	  formGroups = loadFormValueForItemGroup(
        			  dgb, dbGroups, formGroups, eventDefinitionCRFId
        	  );
        	  logger.info("form group size after validation " + formGroups.size());
          }
          ...
          if (validate) {
            ...
            logger.info("input VALIDATE " + itemName + ": " + fp.getString(itemName));
            //dib.loadFormValue(fp.getString(itemName));
            logger.info("input " +itemName+" has a response set of "+ 
            		dib.getMetadata().getResponseSet().getOptions().size()+" options");
          } else {
            String itemName = getInputName(dib);
            logger.info("input NONVALIDATE " + itemName + ": " + fp.getString(itemName));
    	  ...
    	  if (diwb.isInGroup()) {
    		  ...
    		  for (int j=0; j<dgbs.size(); j++) {
    			  ...
    			  for(DisplayItemBean displayItem : items){
    				  ...
    				  if(isChanged(idb,oldItemdata))	changedItems.add(itemName);
    			  }
				  first = false;
    		  }
    	  } else {
    		  ...
    		  String value = idb.getValue();
    		  scoreItems.put(itemName, ib);
    		  //for items which are not in any group, their ordinal is set as 1
    		  TreeSet<Integer> ordinalset = new TreeSet<Integer>();
    		  ordinalset.add(1);
    		  itemOrdinals.put(itemId, ordinalset);
              ...
              for (int j = 0; j < children.size(); j++) {
            	  ...
            	  if(isChanged(child.getData(),oldItemdata))	changedItems.add(itemName);
              }
    	  }
      }
      
      //do calculation for 'calculation' and 'group-calculation' type items 
      ...
      for (int i = 0; i < allItems.size(); i++) {
    	  ...
    	  if (diwb.isInGroup()) {
    		  ...
    		  for (int j=0; j<dgbs.size(); j++) {
    			  ...
    			  for(DisplayItemBean displayItem : items){
    				  ...
    				  if(responseTypeId==8 || responseTypeId==9) { 
    					  ...
    					  String value = doCalculation(displayItem,scoreItems,scoreItemdata,itemOrdinals,err,displayItem.getData().getOrdinal());
    					  displayItem.loadFormValue(value);
    		    		  if(isChanged(displayItem.getData(),oldItemdata))	changedItems.add(displayItem.getItem().getName());
			        	  String inputName = "";
			  ...
			  if(responseTypeId==8 || responseTypeId==9) {
	    		  StringBuffer err = new StringBuffer();
	    		  String value = doCalculation(dib,scoreItems,scoreItemdata,itemOrdinals,err,1);
	    		  dib.loadFormValue(value);
	    		  if(isChanged(dib.getData(),oldItemdata))	changedItems.add(dib.getItem().getName());
	          ...
	          for (int j = 0; j < children.size(); j++) {
	        	  ...
	        	  if(resTypeId==8 || resTypeId==9) {
		    		  ...
		    		  String cvalue = doCalculation(child,scoreItems,scoreItemdata,itemOrdinals,cerr,1);
		    		  child.loadFormValue(cvalue);
		    		  if(isChanged(child.getData(),oldItemdata))	changedItems.add(child.getItem().getName());
  ...
  protected boolean getInputBeans() throws InsufficientPermissionException {
    ...
    if (ecb == null) {
      ...
      if (eventCRFId > 0) {
        ...
        //YW 11-12-2007 << if interviewer or/and interview date has/have been updated for study/site from "blank" to "pre-populated"
        // But at this point, this update only shows on web page and will not be updated to database. 
...
//  adding this code from below, since we want to pass a null values list 
    ...
    //>>TBH below for loop is a lot of repeated work, trying to shave some time here, 12-1-2007
    for (int i = 0; i < repeatMax; i++) {
      ...
      // get the values from the manually created rows first- not from the rep model 
    ...
    //>>TBH taking the nullvalues list out of the for loop, since it should be the same for all display beans
//    nullValuesList = formBeanUtil.getNullValuesByEventCRFDefId(
//            eventDefCRFId,
//            sm.getDataSource());
    ...
    for (int i = 0; i < repeatMax; i++) {
      ...
      //let's get the values from the rep model, which includes the first row and any new rows
    ...
    //checks how many rows are manually created, not added by repetition model
    int manualRows =0;
    for (int j = 0; j < formGroups.size(); j++) {
      DisplayItemGroupBean formItemGroup = (DisplayItemGroupBean) formGroups.get(j);
      //logger.info("begin formGroup Ordinal:" + formItemGroup.getOrdinal());
              ...
              for (DisplayItemBean dib : dbItemGroup.getItems()) {
                ...
                for (DisplayItemBean formDib : formItemGroup.getItems()) {
                  if (formDib.getItem().getId() == dib.getItem().getId()) {
                    ...
                    //tbh removed below line so as not to log so much, 112007
  ...
  /*
   * Perform validation for calculation and group-calculation type. 
   * <br>
   * Pre-condition: passed DisplayItemBean parameter has been loaded with value.
   * 
   * @param sv
   * @param dib
   * @param inputName
   ...
   */
  protected DisplayItemBean validateCalcTypeDisplayItemBean(ScoreItemValidator sv,
		  DisplayItemBean dib, String inputName) {
    ...
    if (hasGroup) {
      ...
      if(includeUngroupedItems){
        ...
        newDisplayBean = formBeanUtil.createDisplaySectionWithItemGroups(
          sb.getId(), ecb
          .getCRFVersionId(), ecb.getStudyEventId(),
          sm,
  ...
  private ArrayList getParentDisplayItems(boolean hasGroup,
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      DisplayItemBean dib = (DisplayItemBean)
        displayItems.get(new Integer(idb.getItemId()));
      if (dib != null) {
        dib.setData(idb);
   ...
   //for the non-reviewed sections, no item data in DB yet, need to
    ...
    for (int ii = 0; ii < allEDCs.size(); ii++) {
        ...
        if (!edcBean.isRequiredCRF()) {
      	logger.info("found one non required CRF: "+edcBean.getCrfName()+
    ...
    for (int i = 0; i < allCRFs.size(); i++) {
      ...
      logger.info("-- looking at a CRF: "+ec.getName()+" "+
    		  ec.getCrf().getName()+" "+
    		  ec.getCrf().getId());
      //if clause kind of not right since none of the above fields are set in the dao, tbh
    ...
    if (eventCompleted && (allCRFs.size() >= allEDCsize)) {//was allEDCs.size(), tbh
    	if (!allRequired && (allEDCsize != 0)) {//what if there are no required CRFs, and all CRFs have been finished?
    		addPageMessage("All Required CRFs have been completed.  "+
    				"You can update the Study Event's status to 'complete'"+
    				" by following the 'Edit Study Event' link.");
    	} else {
  ...
  protected List<DisplayItemWithGroupBean> createItemWithGroups(
    ...
    logger.info("set with nullValuesList of : "+nullValuesList.toString());
    ...
    if (hasItemGroup) {
      ...
      ArrayList data = iddao.findAllActiveBySectionIdAndEventCRFId(sb.getId(), ecb.getId());
       //BWP 12/2/07>> set a flag signaling to data entry JSPs that data is involved
      ...
      logger.info("just got data using section id "+sb.getId()+" and event crf id "+ecb.getId());
      ...
      for(DisplayItemGroupBean itemGroup : dsb.getDisplayFormGroups()) {
        ...
        for (int i=0; i<data.size();i++) {
          ...
          if(idb.getItemId() == firstItem.getItem().getId()) {
            ...
            digb.setItems(dibs);
            logger.info("line 2299, set with dibs list of : "+dibs.size());
              ...
              for (int j = 0; j < data.size(); j++) {
                ...
                if ((idb.getItemId() == dib.getItem().getId()) && !idb.isSelected()) {
                  ...
                  if (shouldLoadDBValues(dib)) {
                    //logger.info("+++should load db values is true, set value");
                    dib.loadDBValue();
                    logger.info("+++data loaded: "+idb.getName()+": "+idb.getValue());
                  }
                  break;
                }
              }
   ...
   * @return
   */ 
  private List<DisplayItemBean> processInputForGroupItem(
		  FormProcessor fp, 
		  List<DisplayItemBean> dibs,
		  int i,
		  DisplayItemGroupBean digb, 
   ...
   * @return
   */
  public final String getGroupItemManualInputName(
      DisplayItemGroupBean digb,
      int ordinal,
      DisplayItemBean dib) {
  ...
  /**
 	 ...
 	 * 
 	 * <p>Notice: both parameter 'itemdata' and parameter 'errors' might be updated in this method.
 	 * 
 	 * @param displayItems
 	 * @param items
 	 * @param itemdata
 	 ...
 	 */
 	public String doCalculation(DisplayItemBean displayItem, HashMap<String,ItemBean> items, 
 			HashMap<String,String> itemdata,
 			HashMap<Integer,TreeSet<Integer>> itemOrdinals, StringBuffer errors, int ordinal) {
 		if(itemdata==null) {
			logger.severe("In DataEntryServlet doCalculation(), itemdata map is empty!");
 			errors.append("Calculation cannot be started because needed items are empty" + "; ");
 			return "";
 		}
 		String value = "";
		...
		if(type==8) {
			parsedExp = parser.parse(rob.getValue(), ordinal);
		} else if(type==9) {
			...
			parsedExp = parser.parse(rob.getValue(),itemOrdinals);
		}
		if(parser.getErrors().length()>0) {
			errors.append(parser.getErrors());
		} else {
			value = ScoreUtil.eval(parsedExp, errors);
			ItemDataType idt = ib.getDataType();
			if (value==null || value.length() == 0) {
				value = "";
				String exp = rob.getValue();
				exp = exp.replace("##", ",");
				//errors.append("Result is empty in" + " " + exp + "; ");
				errors.append(resexception.getString("result_is_empty_in") + " " + exp + "; ");
			} else if (idt.equals(ItemDataType.INTEGER)) {
				try {
					...
					String exp = rob.getValue();
					exp = exp.replace("##", ",");
					//errors.append("Number was expected in" + " " + exp + " : " + value + "; ");
					errors.append(resexception.getString("number_expected_in") + " " + exp + ": " + value + "; ");
					value = "";
				}
			} 
			else if(idt.equals(ItemDataType.REAL)) {
 	...
 	protected boolean isChanged(ItemDataBean idb, HashMap<Integer,String> oldItemdata) {
 		String value = idb.getValue();
 		if(!oldItemdata.containsKey(idb.getId())) return true;
 		else {
 			String oldValue = oldItemdata.get(idb.getId());
 			if(oldValue!=null) {
 				if(value==null)	return true;
 				else if(!oldValue.equals(value))	return true;

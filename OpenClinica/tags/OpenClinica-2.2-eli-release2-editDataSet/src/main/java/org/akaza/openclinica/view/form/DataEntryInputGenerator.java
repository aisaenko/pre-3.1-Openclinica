...
public class DataEntryInputGenerator implements InputGenerator{
  ...
  private static final Logger logger = Logger.getLogger(
    "org.akaza.openclinica.view.form.DataEntryInputGenerator");
...
* Element object. ToDO: this code needs to be refactored to separate the domain
* or business rules from the rest of the parameters (e.g., isHorizontal) */
  public Element createCheckboxTag(Element tdCell, Integer itemId,
    ...
    for (Object responseOptBean : options) {
      ...
      //add br tag to before input element if the checkbox element is not horizontal  and
...
* Element object.*/
  public Element createRadioButtonTag(Element tdCell,
    ...
    for (Object responseOptBean : options) {
      ...
      if((! hasSavedData) && defaultValue != null &&
        (forDefVal.equalsIgnoreCase(defaultValue) ||
      ...
      //add br tag to before input element if the radio element is not horizontal  and
...
* child element of the select tag. */
  ...
  public Element createSingleSelectTag(Element tdCell, Integer itemId,
                                       ...
                                       String databaseValue, boolean hasSavedData) {
  ...
  * Element object.*/
  public Element createTextareaTag(Element tdCell, Integer itemId,
    ...
    if(dbValue != null && dbValue.length() > 0) {
      ...
      //This is necessary to prevent JDOM from creating a textarea tag that doesn't contain
  ...
  /**
   * Calculation response type has been treated as disabled text html input type, currently - (ywang, 1/10/2007) 
   * 
   * @param tdCell
   * @param itemId
   * @param tabNumber
   ...
   * @return
   */
  public Element createCaculationTag(Element tdCell,
          Integer itemId,
          ResponseSetBean responseSet,
          boolean isDateType,
          String dbValue, boolean hasSavedData) {
	  //for i18n date formats
	  String pattn="";
  ...
  public Element getDateWidgetForCell(Integer itemId){
    ...
    sbuilder.append(itemId).append("','").append(pattn).append("'); return false;");

  	...
  	while (it.hasNext()) {
  		...
  		// YW >>
			...
			Integer subjectStatusId = (Integer) row.get("subject_status_id");
	  		...
	  		//this code will not support 2.0.1 upgrades, since we are looking at the item-group itself
  ...
  public DatasetBean initialDatasetData(DatasetBean db) {
	  	...
	  	while (it.hasNext()) {
	  		...
	  		if(!db.getItemMap().containsKey(key)) {
		  		...
		  		//YW 2-22-2008, CreateDatasetServlet.java shows that eventIds contains study_event_definition_ids 

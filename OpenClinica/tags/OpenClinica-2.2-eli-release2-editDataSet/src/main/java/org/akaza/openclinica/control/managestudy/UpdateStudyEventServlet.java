...
//    }
    ...
    /*
     * REMOVED can happen at any step,
     * COMPLETED can happen if the Subject Event is already complete,
     * COMPLETED can also happen if all required CRFs in the Subject Event are completed,
     * LOCKED can occur when all Event CRFs are completed, or not started, or removed,
     * LOCKED/REMOVED are only options, however, when the user is study director or study coordinator
     * SKIPPED/STOPPED?
     * 
     * Additional rules spelled out on Nov 16 2007:
     * STOPPED should only be in the list of choices after IDE has been started, i.e. not when SCHEDULED
     * SKIPPED should only be in the list before IDE has been started, i.e. when SCHEDULED
     * reminder about LOCKED happening only when CRFs are completed (not as in the above...)
     * if a status is LOCKED already, it should allow a user to set the event back to COMPLETED 
     */
    ...
    ArrayList getECRFs = (ArrayList)studyEvent.getEventCRFs();
    ...
    for (int u = 0; u < getAllECRFs.size(); u++) {
    	...
    	for (int uv = 0; uv < getECRFs.size(); uv++) {
    		...
    		logger.info("***** found: "+existingBean.getCRFVersionId()+" "+existingBean.getCrf().getId()+
    				...
    				existingBean.getStatus().getName()+" "+existingBean.getStage().getName());
    		...
    		//if (existingBean.getCRFVersionId() == ecrfBean.getDefaultVersionId()) {
    			...
    			//will this match up?  Do we need to pull it out of studyEvent.getEventCRFs()?
    			//only case that this will screw up is if there are no crfs whatsoever
    			//this is addressed in the if-clause above
    			if (!existingBean.getStatus().equals(Status.UNAVAILABLE) && 
    					edefcrfdao.isRequiredInDefinition(
    							existingBean.getCRFVersionId(), studyEvent.getId())) {
	...
	//    save discrepancy notes into DB
      ...
      if(studyEvent.getStartTimeFlag()==true) {
    	  ...
    	  switch(c.get(Calendar.AM_PM)) {
    	  	case 0 :	presetValues.put(INPUT_STARTDATE_PREFIX + "Half", "am");
    	  				break;
    	  	case 1 :	presetValues.put(INPUT_STARTDATE_PREFIX + "Half", "pm");
						break;
    	  	default:	presetValues.put(INPUT_STARTDATE_PREFIX + "Half", "");
    	  				break;
    	  }
      } else {
      ...
      if(studyEvent.getDateEnded() != null) {
    	  if(studyEvent.getEndTimeFlag()==true) {
        	  ...
        	  //Later it could be put to somewhere as a static method if necessary.
        	  switch(c.get(Calendar.AM_PM)) {
        	  	case 0 :	presetValues.put(INPUT_ENDDATE_PREFIX + "Half", "am");
        	  				break;
        	  	case 1 :	presetValues.put(INPUT_ENDDATE_PREFIX + "Half", "pm");
    						break;
        	  	default:	presetValues.put(INPUT_ENDDATE_PREFIX + "Half", "");
        	  				break;
        	  }
          } 

...
import org.akaza.openclinica.logic.core.ScoreValidator;
 ...
 *         modified by jxu
 * @version CVS: $Id: SpreadSheetTableRepeating.java 10861 2008-03-25 15:26:31Z ywang $
 */
...
public class SpreadSheetTableRepeating implements SpreadSheetTable {
  ...
  public SpreadSheetTableRepeating(FileInputStream parseStream,
                                   UserAccountBean ub,
                                   String versionName, Locale locale)
    throws IOException {
    //super();
      ...
      if (sheetName.equalsIgnoreCase("Instructions")) {
        ...
        if (sheetName.equalsIgnoreCase("Items")) {
          ...
          for (int k = 1; k < numRows; k++) {
            ...
            //YW 2-5-2008 << set default resOptions for calculation and group-calculation type
            //if(responseTypeId==8 || responseTypeId==9) {
            //	resOptions = resOptions.length()>0 ? resOptions : "can not calculate";
            //}
            //YW >>
            ...
            if (!resNames.contains(responseLabel) && (!StringUtil.isBlank(resOptions))) {
                if(responseTypeId==8 || responseTypeId==9) {
              	  //YW 1-29-2008 << only one option for "calculation" type and "group-calculation" type
                  //but do we really need this variable these two types?
              	  numberOfOptions = 1;
              	  //YW >>
                }else {
            ...
            if(responseTypeId==8 || responseTypeId==9) {
            	//right now, func is not required; but if there is func, it must be correctly spelled
            ...
            if("date".equalsIgnoreCase(dataType) && !"".equals(default_value)){
              ...
              //All database values are stored in this format? en Locale MM/dd/yyyy
              try {
                default_value = new SimpleDateFormat(
            ...
            if(default_value.length()>0 && (responseTypeId==8 || responseTypeId==9)) {
            	 errors.add("Default_value is not allowed for 'calculation' or 'group-calculation' type in DEFAULT_VALUE column at " + k + ", items worksheet.");

...
public class CreateNewStudyEventServlet extends SecureController {
  ...
  protected void processRequest() throws Exception {
    ...
    if (studySubjectId <= 0) {
    	...
    	//YW 11-08-2007, << a new study event could not be added if its study subject has been removed
...
//    Collections.sort(definitionsWithCRF);
    ...
    if (!fp.isSubmitted()) {
      ...
      //example of taking the above line and transferring to i18n on the below line, tbh
...
//        }
      ...
      if (hasScheduledEvent && !errors.containsKey(INPUT_STARTDATE_PREFIX_SCHEDULED) && !errors.containsKey(INPUT_ENDDATE_PREFIX_SCHEDULED)) {
    	  if(definitionScheduled.getId()==studyEventDefinitionId) {
    		   // here the precondition of evaluating if two fields entered a same day is that 
    		  ...
    		  if(fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED+"Date").equals(fp.getString(INPUT_STARTDATE_PREFIX+"Date"))) {
    			  //if one of start or startScheduled is datetime, then another one will be forced to input datetime
    			  ...
    			  if(!schStartTime&&!startTime || schStartTime&&startTime) {
    				  if(startScheduled.before(start)) {
    					  v.addError(errors, INPUT_STARTDATE_PREFIX_SCHEDULED, resexception.getString("input_provided_not_occure_after_previous_start_date_time"));
    				  }
    			  } else {
    				  v.addError(errors, INPUT_STARTDATE_PREFIX_SCHEDULED, "When you scheduled the same date for start date/time of two events at this time, " +
    	  ...
    	  if (!strEndScheduled.equals("")) {
    		  ...
    		  if(!fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED+"Date").equals(fp.getString(INPUT_ENDDATE_PREFIX+"Date"))) {
    			  if(endScheduled.before(startScheduled)) {
	    			  v.addError(errors, INPUT_ENDDATE_PREFIX_SCHEDULED, resexception.getString("input_provided_not_occure_after_previous_start_date_time"));
	    		  }
	    	  } else {
	    		  //if in same date, only check when both had time entered
      ...
      if (!errors.isEmpty()) {
        ...
        String prefixes[] = { INPUT_STARTDATE_PREFIX, INPUT_ENDDATE_PREFIX,
            INPUT_STARTDATE_PREFIX_SCHEDULED, INPUT_ENDDATE_PREFIX_SCHEDULED};
        ...
        if (hasScheduledEvent) {
          ...
          if (subjectMayReceiveStudyEvent(sm.getDataSource(), definitionScheduled, studySubject)) {
            ...
            //YW 3-12-2008, 2220 fix <<
            AddNewSubjectServlet.saveFieldNotes(INPUT_STARTDATE_PREFIX_SCHEDULED, fdn, dndao, 
            		studyEventScheduled.getId(), "studyEvent", currentStudy);
            AddNewSubjectServlet.saveFieldNotes(INPUT_ENDDATE_PREFIX_SCHEDULED, fdn, dndao, 
            		studyEventScheduled.getId(), "studyEvent", currentStudy);
            //YW >>
          } else {

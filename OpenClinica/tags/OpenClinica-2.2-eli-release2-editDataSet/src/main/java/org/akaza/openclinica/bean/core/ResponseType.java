...
public class ResponseType extends Term {
  ...
  private static final ResponseType[] members = { TEXT, TEXTAREA,CHECKBOX, FILE, RADIO, SELECT, SELECTMULTI, CALCULATION, GROUP_CALCULATION };
  ...
  public static ResponseType getByName(String name) {
    /* BWP, 08/26/07: made the method more protective and robust, so that
    * it handles problems with parameters such as "text " (note the space within
    * the String).*/
    if (name == null || name.length() < 1) { return ResponseType.INVALID;}
    name = name.trim();
    for (int i = 0; i < list.size(); i++) {
      ResponseType temp = (ResponseType) list.get(i);

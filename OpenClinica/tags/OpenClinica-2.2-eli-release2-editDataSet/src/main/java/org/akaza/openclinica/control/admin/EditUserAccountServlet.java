  ...
  protected void processRequest() throws Exception {
    ...
    if (!fp.isSubmitted()) {
      addEntityList("userTypes", getUserTypes(),
    		  respage.getString("the_user_could_not_be_edited_because_no_user_types"),
    	          Page.ADMIN_SYSTEM);
...
//          Page.ADMIN_SYSTEM);
      ...
      if (button.equals(resword.getString("back"))) {
        ...
        if (fp.getBoolean(INPUT_RESET_PASSWORD)) {
          ...
          if ("no".equalsIgnoreCase(fp.getString(INPUT_DISPLAY_PWD))) {
            ...
            addPageMessage(respage.getString("new_user_password")+": " + password +respage.getString("please_write_down_the_password_and_provide"));
          }
        } else {
          udao.update(user);
        }
  ...
  private void sendResetPasswordEmail(UserAccountBean user, String password) throws Exception {
    ...
    body += restext.getString("your_password_has_been_reset_on_openclinica")+ ":\n\n";
    body += resword.getString("user_name")+ ": " + user.getName() + "\n";
    ...
    em.process(user.getEmail().trim(), EmailEngine.getAdminEmail(),
    		restext.getString("your_openclinica_account_password_reset"), body);

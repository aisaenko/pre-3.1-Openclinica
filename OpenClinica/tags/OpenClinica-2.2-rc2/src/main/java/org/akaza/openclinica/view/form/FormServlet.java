...
public class FormServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
    ...
    POIFSFileSystem fs = new POIFSFileSystem(new
      ...
      path+"group_demo_nw.xls")));
    HSSFWorkbook wb = new HSSFWorkbook(fs);
    Map<String,Map> allMap = spnw.createCrfMetaObject(wb);
  /*  Map gmap = spnw.createGroupsMap(wb);
    Map map2 =  spnw.createItemsOrSectionMap(wb,"items");
    FormGroupBean fgBean = beanFactory.createFormGroupBeanFromMap(gmap);
    DisplayFormGroupBean displayGroup = new DisplayFormGroupBean();
    List itemsDisplayList = beanFactory.
      createDisplayItemBeansFromMap(map2,"group_demo");
    ...
    String crfName = (String)((Map)allMap.get("crf_info")).get("crf_name");
    Map sections = (Map)allMap.get("sections");
    Map sectionMap =  ((Map)sections.get(1));
    String sectionLabel = (String) sectionMap.get("section_label");
    formGroupsL = beanFactory.createGroupBeans(allMap.get("items"),
      allMap.get("groups"),
      sectionLabel,
      crfName);

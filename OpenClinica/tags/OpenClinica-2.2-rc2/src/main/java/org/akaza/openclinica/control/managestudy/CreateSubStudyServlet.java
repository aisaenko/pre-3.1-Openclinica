 ...
 * 
 * Modified by ywang:
 *     [10-10-2007], enable setting overidable study parameters of a sub study. 
 */
public class CreateSubStudyServlet extends SecureController {
        ...
        for (int i = 0; i<parentConfigs.size(); i++) {
          ...
          if (scg != null) {
            ...
            if (scg.getValue().getId() >0 && scg.getParameter().isOverridable()) {
              ...
              //YW 10-12-2007, set overridable study parameters for a site
  ...
  private void submitStudy() {
    ...
    //YW << here only "collectDob" and "genderRequired" have been corrected for sites.
    StudyParameterValueBean spv = new StudyParameterValueBean(); 
    StudyParameterValueBean parentSPV = (StudyParameterValueBean)spvdao.findByHandleAndStudy(parent.getId(), "collectDob");
    spv.setStudyId(study.getId());
    spv.setParameter("collectDob");
    spv.setValue(parentSPV.getValue());
    spvdao.create(spv);
    
    parentSPV = (StudyParameterValueBean)spvdao.findByHandleAndStudy(parent.getId(), "genderRequired");

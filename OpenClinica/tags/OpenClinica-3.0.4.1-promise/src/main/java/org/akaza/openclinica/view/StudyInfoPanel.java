...
public class StudyInfoPanel {
    ...
    private boolean permissions = false; // added by tbh, specifically for DE permissions, 08/2009
    ...
    public void setData(Page page, HttpSession session, HttpServletRequest request) {
        ...
        try {
            ...
            if (page.equals(Page.CREATE_DATASET_1)) {
                ...
                || page.equals(Page.CREATE_DATASET_CRF_ATTR) || page.equals(Page.CREATE_DATASET_GROUP_ATTR) || page.equals(Page.CREATE_DATASET_SELECT_ITEMS)
            ...
            } else if (page.equals(Page.MANAGE_PRIVILEGES) || page.equals(Page.UPDATE_PRIVILEGES)) {
                HashMap eventlist = (HashMap) request.getAttribute("eventlist");
                ArrayList displayData = generatePermissionsEventTree(eventlist);
                ...
                /*
                 * set up the side info panel to create the following upon entry from the ViewStudyServlet Study X Subject Y StudyEventDef Z1 StudyEventDef Z2
                 * <status-tag> CRF A1 <status-tag> CRF A2 Z1 should be collapsible/expandible, etc. We can pull things from the session and the request:
                 */
                /*
                 * StudyBean study = (StudyBean) request.getAttribute("study"); StudySubjectBean studySubject = (StudySubjectBean)
                 * request.getAttribute("studySub"); EntityBeanTable table = (EntityBeanTable) request.getAttribute("table"); EventCRFBean ecb =
                 * (EventCRFBean)request.getAttribute("eventCRF"); this.reset(); ArrayList rows = table.getRows(); ArrayList beans =
                 * DisplayStudyEventBean.generateBeansFromRows(rows); addStudyEventTree(study, studySubject, beans, ecb);
                 */
                ...
                /*
                 * pages designed to also follow the above format; check to see if they are in the session already, and does not refresh. TODO refine and test
                 */
                StudyBean study = (StudyBean) session.getAttribute("study");
                StudySubjectBean studySubject = (StudySubjectBean) request.getAttribute("studySubject");
                ArrayList beans = (ArrayList) request.getAttribute("beans");
    ...
    private ArrayList generatePermissionsEventTree(HashMap eventlist) {
        ArrayList displayData = new ArrayList();
        // Iterator keyIt = eventlist.keySet().iterator();
        // logger.info("how many events =" + eventlist.size());
        String SERVLET_NAME = "ManagePrivileges";
        int count = 0;
        for (Iterator keyIt = eventlist.keySet().iterator(); keyIt.hasNext();) {
            StudyEventDefinitionBean sed = (StudyEventDefinitionBean) keyIt.next();
            displayData.add(new StudyInfoPanelLine("Definition", sed.getName(), true, false));
            ...
            for (int i = 0; i < crfs.size(); i++) {
                CRFBean crf = (CRFBean) crfs.get(i);
                if (ordinal_crf < crfs.size()) {
                    displayData.add(new StudyInfoPanelLine("CRF", "<a href='" + SERVLET_NAME + "?crfId=" + crf.getId() + "&defId=" + sed.getId() + "'>"
                        + crf.getName() + "</a>", false, false));
                } else {
                    displayData.add(new StudyInfoPanelLine("CRF", "<a href='" + SERVLET_NAME + "?crfId=" + crf.getId() + "&defId=" + sed.getId() + "'>"
                        + crf.getName() + "</a>", false, true));

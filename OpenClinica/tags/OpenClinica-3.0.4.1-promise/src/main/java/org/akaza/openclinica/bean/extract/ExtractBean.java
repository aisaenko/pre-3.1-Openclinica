...
public class ExtractBean {
	...
	public ExtractBean(DataSource ds, Role role, int type) {
        this.ds = ds;
    ...
    public void addStudyEventData() {
        ...
        if (isfnd) {
            logger.debug("ERROR in sequence");
        } else {
            logger.debug("OK sequence");
        }// if
    ...
    protected boolean inKeys(int sedInd, int sampleOrdinal, int crfInd, int itemInd, String groupName) {
        ...
        logger.debug("running in keys on sdeInd " + sedInd + 
        ...
        try {
	        logger.debug("*** finding is permitted from " + currentDef.getId() + " " + 
	        		currentCRF.getId() + " " + currentItem.getId() + " " + currentRole.getId());
	        
	        isPermitted = permissions.isPermitted(currentDef.getId(), currentCRF.getId(), currentItem.getId(), currentRole.getId());
	        
	        logger.debug("*** found " + isPermitted + " from " + currentDef.getId() + " " + 
	        		currentCRF.getId() + " " + currentItem.getId() + " " + currentRole.getId());
        } catch (NullPointerException npe) {
        	logger.debug("*** returned a null for finding the permitted from permissions ***");
        }
        // << tbh 08/2009, and 02/2010
        if (issavedkey == null){
        	//the "True" is not found for key - set to false
        	return false;
        } else if (!isPermitted){
        	logger.debug("returned false for permissions");
        	return false;
        } else {
        	return true;
        }//if

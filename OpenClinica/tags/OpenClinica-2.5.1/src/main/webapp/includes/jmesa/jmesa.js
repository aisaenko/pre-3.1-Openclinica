// Added the line below to avoid conflict with prototype 
// Also replaced all $ with $j 
...
TableFacade.prototype.createHiddenInputFields = function(form) {
    ...
    var exists = $j(form).find(':hidden[@name=' + limit.id + '_p_]').val();
    if (exists) {
        return false;
    }
    
    if (this.worksheet.save) {
        $j(form).append('<input type="hidden" name="' + limit.id + '_sw_" value="true"/>');
    }
    
    if (this.worksheet.filter) {
        $j(form).append('<input type="hidden" name="' + limit.id + '_fw_" value="true"/>');
    }
    
    /* tip the API off that in the loop of working with the table */
    $j(form).append('<input type="hidden" name="' + limit.id + '_tr_" value="true"/>');
    
    /* the current page */
    $j(form).append('<input type="hidden" name="' + limit.id + '_p_" value="' + limit.page + '"/>');
    $j(form).append('<input type="hidden" name="' + limit.id + '_mr_" value="' + limit.maxRows + '"/>');
    
    /* the sort objects */
    var sortSet = limit.getSortSet();
    $j.each(sortSet, function(index, sort) {
        $j(form).append('<input type="hidden" name="' + limit.id + '_s_'  + sort.position + '_' + sort.property + '" value="' + sort.order + '"/>');
    });
    
    /* the filter objects */
    var filterSet = limit.getFilterSet();
    $j.each(filterSet, function(index, filter) {
        $j(form).append('<input type="hidden" name="' + limit.id + '_f_' + filter.property + '" value="' + filter.value + '"/>');
    });
...
function createDynDroplistFilter(filter, id, property, options) {
    ...
    if ($j('#dynFilterDroplistDiv').size() > 0) {
    ...
    var div = $j('#dynFilterDroplistDiv');
    div.html(html);

    var input = $j('#dynFilterDroplist');
    ...
    $j(input).change(function() {
        var changedValue = $j("#dynFilterDroplistDiv option:selected").val();
...
function addDropShadow(imagesPath, theme) {
    ...
    $j('.' + theme + ' div.wrap0').css({'background': 'url(' + imagesPath + 'shadow.gif) right bottom no-repeat', 'float':'left'});
    $j('.' + theme + ' div.wrap1').css({'background': 'url(' + imagesPath + 'shadow180.gif) no-repeat'});
    $j('.' + theme + ' div.wrap2').css({'background': 'url(' + imagesPath + 'corner_bl.gif) -18px 100% no-repeat'});
    $j('.' + theme + ' div.dropShadow').css({'background': 'url(' + imagesPath + 'corner_tr.gif) 100% -18px no-repeat'});
    
    $j('div.' + theme).append('<div style="clear:both">&nbsp;</div>');
}
...
function createWsColumn(column, id, uniqueProperties, property) {
    ...
    var input = $j('#wsColumnInput'); 
    input.focus();
    
    $j('#wsColumnInput').keypress(function(event) {
        if (event.keyCode == 13) { // press the enter key 
            var changedValue = input.val();
            cell.html(changedValue);
...
function submitWsColumn(originalValue, changedValue) {
    ...
    $j.each(props, function(key, value) {
        data += ', "up_' + key + '" : "' + value + '"';
    });
    ...
    $j.post('jmesa.wrk?', eval('(' + data + ')'), function(data) {});    
}

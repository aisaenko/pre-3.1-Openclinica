...
public class CdiscOdmXmlBean {
            ...
            for (int k = 1; k < eb.getSEDNumCRFs(i) + 1; ++k) {
                ...
                while (iter.hasNext()) {
                    ...
                    for (int j = 0; j < aa.size(); ++j) {
                        ...
                        // tii.itemName + "\" Name=\"" +
                        // stringCut(ifmBean.getLeftItemText()) + "\"
                        ...
                        if (datatype.equalsIgnoreCase("text")) {
                            line +=
                                "<ItemDef OID=\"IT." + StringEscapeUtils.escapeXml(tii.itemName) + "." + tii.crfId + "\" Name=\""
                                    + StringEscapeUtils.escapeXml(tii.itemName) + "\" DataType=\"" + datatype + "\" Length=\"" + 4000 + "\" SASFieldName=\""
                                    + sasname.getValidSasName(tii.itemName) + comment;
                        } else if (datatype.equalsIgnoreCase("integer")) {
                            line +=
                                "<ItemDef OID=\"IT." + StringEscapeUtils.escapeXml(tii.itemName) + "." + tii.crfId + "\" Name=\""
                                    + StringEscapeUtils.escapeXml(tii.itemName) + "\" DataType=\"" + datatype + "\" Length=\"" + 8 + "\" SASFieldName=\""
                                    + sasname.getValidSasName(tii.itemName) + comment;
                        } else {
                            line +=
                                "<ItemDef OID=\"IT." + StringEscapeUtils.escapeXml(tii.itemName) + "." + tii.crfId + "\" Name=\""
    ...
    private String getODMDataType(int OCDataTypeId, ItemFormMetadataBean ifmBean) {
        ...
        case 8:
            return "text";
            // INT. This includes two groups. One is real integer which could be
            // used to calculation,
            // which will be return as "integer".
            // Another is only option index for some response type like
            // multiple-select etc, which

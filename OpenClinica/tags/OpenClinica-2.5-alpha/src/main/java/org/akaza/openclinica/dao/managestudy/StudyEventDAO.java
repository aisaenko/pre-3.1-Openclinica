...
public class StudyEventDAO extends AuditableEntityDAO {
    ...
    public HashMap getEventsAndMultipleCRFVersionInformation(ArrayList rows) {
        ...
        while (it.hasNext()) {
            ...
            if (this.findDouble(returnMe, event)) {// (returnMe.containsKey(event))
                ...
                // logger.warning("putting a crf into an OLD event: " +
                // crf.getName() + " into "
                // + event.getName());
    ...
    public HashMap getEventAndCRFVersionInformation(ArrayList al) {
        ...
        while (it.hasNext()) {
            HashMap answers = (HashMap) it.next();
            logger.warning("***Study Event Def ID: " + answers.get("study_event_definition_id"));
            logger.warning("***CRF ID: " + answers.get("crf_id"));
            logger.warning("***CRFVersion ID: " + answers.get("crf_version_id"));
            event = new EntityBean();
    ...
    public HashMap findCRFsByStudyEvent(StudyEventBean seb) {
        ...
        while (it.hasNext()) {
            HashMap answers = (HashMap) it.next();
            logger.warning("***First CRF ID: " + answers.get("crf_id"));
            logger.warning("***Next CRFVersion ID: " + answers.get("crf_version_id"));
            // here's the logic:
            // grab a crf,

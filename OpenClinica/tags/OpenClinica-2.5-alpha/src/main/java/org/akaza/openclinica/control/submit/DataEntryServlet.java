...
public abstract class DataEntryServlet extends SecureController {
    ...
    protected void processRequest() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < allNotes.size(); i++) {
                ...
                if (dn.getResStatus().equals(ResolutionStatus.UPDATED)) {
                    updatedNum++;
                } else if (dn.getResStatus().equals(ResolutionStatus.OPEN)) {
                    openNum++;
                } else if (dn.getResStatus().equals(ResolutionStatus.CLOSED)) {
                    closedNum++;
                }
            ...
            for (int i = 0; i < allItems.size(); i++) {
                ...
                if (diwg.isInGroup()) {
                    ...
                    if (validate) {
                        ...
                        logger.info("input " + itemName + " has a response set of " + dib.getMetadata().getResponseSet().getOptions().size() + " options");
                    } else {
                        String itemName = getInputName(dib);
     ...
     *         the user clicks the "Save" button, <code>false</code>
     *         otherwise.
     */
     ...
     * value.
     * 
     * @param sv @param dib @param inputName @return dib
     * 
    ...
    protected DisplayItemBean validateDisplayItemBeanText(DiscrepancyValidator v, DisplayItemBean dib, String inputName) {
        ...
        if (!isNull) {
            if (StringUtil.isBlank(idb.getValue())) {
                 ...
                 * 
                 * if (customValidationString.startsWith("func:")) { try {
                 * customValidation =

...
public class ListEventDefinitionServlet extends SecureController {
    ...
    public void processRequest() throws Exception {
        ...
        for (int i = 0; i < seds.size(); i++) {
            ...
            Collection eventDefinitionCRFlist = edcdao.findAllByDefinition(sed.getId());
            Map crfWithDefaultVersion = new HashMap();
            for (Iterator it = eventDefinitionCRFlist.iterator(); it.hasNext();) {
                ...
                System.out.println("ED[" + sed.getName() + "]crf[" + crfBean.getName() + "]dv[" + crfVersionBean.getName() + "]");
                crfWithDefaultVersion.put(crfBean.getName(), crfVersionBean.getName());
            }
            sed.setCrfsWithDefaultVersion(crfWithDefaultVersion);
            System.out.println("CRF size [" + sed.getCrfs().size() + "]");
        ...
        String[] columns =
            { resword.getString("order"), resword.getString("name"), resword.getString("OID"),resword.getString("repeating"), resword.getString("type"), resword.getString("category"),
                resword.getString("populated"), resword.getString("date_updated"), resword.getString("CRFs"), resword.getString("default_version"),

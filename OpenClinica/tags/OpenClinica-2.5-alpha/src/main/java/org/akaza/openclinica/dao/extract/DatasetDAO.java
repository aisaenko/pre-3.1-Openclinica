...
public class DatasetDAO extends AuditableEntityDAO {
    ...
    public ExtractBean getDatasetData(ExtractBean eb) {
        ...
        while (it.hasNext()) {
            ...
            // YW << make sure a StudySubject belong to current study which
            // could be a site
            // otherwise it is not necessary to extract this StudySubject
            // if currentStudy is a study not a site, the all subjects belong to
            // its sites will be got
            StudySubjectBean ssbean = ssdao.findBySubjectIdAndStudy(subjectId.intValue(), eb.getStudy());
            ...
            if (studySubjectId.intValue() > 0) {
                ...
                // TODO add more columns here: add Item Group data, add Disc.
                // data, add CRF data
                // String itemGroupName = (String) row.get("item_group_name");
                // Integer itemDataId = (Integer) row.get("item_data_id");
                // String discNoteDescription = (String)
                // row.get("discrepancy_note_description");
                // Integer resolutionStatusId = (Integer)
                // row.get("discrepancy_resolution_status_id");
                // String discDetailedNotes = (String)
                // row.get("detailed_notes");
                // Integer discTypeId = (Integer)
                // row.get("discrepancy_note_type_id");
                ...
                String itemGroupName = "Ungrouped";
                String itemGroupName_temp = (String) row.get("item_group_name");
                // this code will not support 2.0.1 upgrades, since we are
                // looking at the item-group itself
                // not at the updated extract_data_table
                // changed the above line from "" to "Ungrouped", tbh 01-14-2008
                ...
                for (int g = 0; g < itemGroups.size(); ++g) {
                    if (itemGroupName_temp.equals(itemGroups.get(g).getName())) {
                        itemGroupName = itemGroupName_temp;
                        break;
                    }
                }

                eb.addStudySubjectData(studySubjectId, studySubjectLabel, dateOfBirth, gender, subjectStatusId, dobCollected, // boolean
                        // here,
                        // tbh
                        secondaryID);

                eb.addStudyEventData(studySubjectId, studyEventDefinitionName, studyEventDefinitionId, sampleOrdinal, studyEventLocation, studyEventStart,
                        studyEventEnd, crfVersionName, crfVersionStatusId, dateInterviewed, interviewerName, dateCompleted, dateValidateCompleted,
                        completionStatusId);

                // or add CRFversion data here???
                System.out.println("*** found item group name before we call add item data on 707: " + itemGroupName);
                if (!"".equals(itemGroupName)) {
                    // System.out.println("got inside the loop!");
                    eb.addItemData(studySubjectId, studyEventDefinitionId, sampleOrdinal, crfId, itemId, itemValue, itemDataOrdinal, itemGroupName);
                    if (itemDataOrdinal.intValue() > eb.getMaxItemDataBeanOrdinal()) {
                        eb.setMaxItemDataBeanOrdinal(itemDataOrdinal.intValue());
                        // System.out.println("### just updated max ordinal for
                ...
                /*
                 * eb.addDiscrepancyNoteData(itemDataId, discNoteDescription,
                 * resolutionStatusId, discDetailedNotes, discTypeId);
                 */
                // eb.addSubjectGroupData(studyGroupId,
                // studyGroupClassId);//???add to item data instead???
                // eb.addCRFVersionData();//???add to study event data
                // instead???
            }
        }

        System.out.println("count for item data at end: " + eb.getItemNames().size());
        System.out.println("count for group names at end: " + eb.getGroupNames().size());
    ...
    // // //logger.warning("** captured subject identifier:
    // "+subjectIdentifier);
    ...
    // // System.out.println("** ADDED column header for item values: " +
    // orderedItemName + "; CRF: " + CRFName + " " + CRFVersionName + "; column
    // num: " + maxCRFs + "; subject: " + subjectIdentifier);
    // // System.out.println("key is " + contentKey + " and value is " +
    // contentValue);
    // //}
    // //
     ...
     */
    public DatasetBean initialDatasetData(DatasetBean db) {
        ItemDAO idao = new ItemDAO(ds);
        String sql = db.getSQLStatement();
        setExtractTypesExpected();
        ArrayList alist = select(sql);
        Iterator it = alist.iterator();
        while (it.hasNext()) {
            HashMap row = (HashMap) it.next();
            Integer defId = (Integer) row.get("study_event_definition_id");
            String defName = (String) row.get("study_event_definition_name");
            Integer crfId = (Integer) row.get("crf_id");
            String crfName = (String) row.get("crf_name");
            Integer itemId = (Integer) row.get("item_id");
            String key = defId + "_" + itemId;
            if (!db.getItemMap().containsKey(key)) {
                ItemBean ib = (ItemBean) idao.findByPK(itemId);
                ib.setSelected(true);
                ib.setDefName(defName);
                ib.setCrfName(crfName);
                ib.setDatasetItemMapKey(defId + "_" + itemId);
                ...
                db.getItemMap().put(defId + "_" + itemId, ib);
            }
        }

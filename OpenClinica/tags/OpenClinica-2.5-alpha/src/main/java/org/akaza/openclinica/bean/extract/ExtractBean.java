...
public class ExtractBean {
    ...
    public void computeReportData(ReportBean answer) {
        ...
        if (dataset.isShowSubjectGroupInformation()) {
            ...
            for (int y = 0; y < studyGroupClasses.size(); y++) {
                ...
                //YW 10-31-2007, for SPSS, eventHeader probably doesn't match the final header which will be validated
        ...
        for (int i = 1; i <= numSEDs; i++) {
            ...
            for (int j = 1; j <= numSamples; j++) {
                ...
                if (dataset.isShowEventStatus()) {
                    String eventStatus = getColumnLabel(i, j, "SubjectEventStatus",
                            numSamples);
                    String description = getColumnDescription(i, j,
        ...
        for (int i = 1; i <= numSEDs; i++) {
            int numSamples = getSEDNumSamples(i);
        	//no need for this, as we only need to track number of crfs, tbh 03/08
            //int numSEDCRFs = getSEDNumCRFs(i);
            for (int j = 1; j <= numSamples; j++) {
                int numSEDCRFs = getSEDNumCRFs(i);
                if (dataset.isShowCRFcompletionDate()) {
                    ...
                    String crfCompletionDate = getColumnLabel(i, j,
                            "CompletionDate", numSamples);
                    String description = getColumnDescription(i, j,
                            "Completion Date for ", currentDef.getName(),
                            numSamples);// FIXME is this correct?
                ...
                if (dataset.isShowCRFstatus()) {
                    String crfStatus = getColumnLabel(i, j, "CRFVersionStatus",
                ...
                for (int k = 1; k <= numSEDCRFs; k++) {
                    ...
                    for (int l = 1; l <= numItems; l++) {
                        ...
                        for (Iterator iter = groupNames.entrySet().iterator(); iter
                            ...
                            System.out.println("*** Found a row in groupNames: key " + groupName);
                            //+ ", value "+
                            //groupCount);
                            ...
                            // !data.equals("")) {
                            //YW 10-30-2007, if an item_id doesn't match a groupName, it will not be added to item title
                    ...
                    for (int k = 1; k <= numSEDCRFs; k++) {
                        ...
                        for (int l = 1; l <= numItems; l++) {
                            ...
                            for (java.util.Iterator iter = groupNames
                                ...
                                String groupName = (String) groupEntry.getKey();
                                if (inKeys(i, j, k, l, groupName)) {
                                	System.out.println("found group name at line 703: "+groupName);
                                    Integer groupCount = (Integer) groupEntry
                                            .getValue();
                                    System.out.println("found groupCount of "+groupCount);
                                    for (int m = 1; m <= groupCount.intValue(); m++) {
    ...
    public void addStudyEventData(Integer studySubjectId,
        ...
        System.out.println("found stage: "+se.getStage().getName());
        ...
        if (checkEvent == null) {
        	...
        	System.out.println("###just CREATED key: "+key+" event: "+event.getName()+" int.name: "+eventCRF.getInterviewerName());
        } else {
...
//        	eventData.put(key, checkEvent);
        	//System.out.println("count of eventcrfs "+eventCRFs.size());
        	//System.out.println("other things about the event crf: int.name "+eventCRF.getInterviewerName()+" comp status id "+eventCRF.getCompletionStatusId()+" version name "+eventCRF.getCrfVersion().getName());
        	//System.out.println("###just UPDATED key: "+key+" event: "+event.getName()+" int.name: "+eventCRF.getInterviewerName());
        }
...
//    }
    ...
    public void addItemData(Integer studySubjectId,
		...
		System.out.println("sample ordinal: "+sampleOrdinal.toString());
		System.out.println("item data ordinal: "+itemDataOrdinal.toString());

		//the mistake here is that we are taking the number of CRFs from
		//this maxOrdinals map downstream, and that is wrong.  We need to get this number
        ...
        System.out.println("*** just put in data for "+key+" and value "+itemValue);
    ...
    private int getMaxOrdinal(int studyEventDefinitionId) {
        System.out.println("*** max ordinals: "+maxOrdinals.toString());
    ...
    private void setMaxOrdinal(int studyEventDefinitionId, int sampleOrdinal) {
        Integer key = getMaxOrdinalsKey(studyEventDefinitionId);
        System.out.println("*** set max ordinal: "+sampleOrdinal+" for key "+key.toString());
	...
	private String getSEDCRFVersionName(int h, int sedInd, int crfInd) {
		...
		System.out.println("returning the following for crf version name: "+returnMe);
		return returnMe;
	}

	/*
	 * change this to pull out the correct data entry stage for a given crf version within
	 * a study event
	 */
...
//    }
    ...
    private String getSEDCRFStatus(int h, int sedInd, int crfInd) {//BADS Flag
        ...
        try {
        	...
        	System.out.println("exception hit, status set to not scheduled");
        }
        //currentCRF.getStatus().getName();
        //
        System.out.println("STATUS: "+status.getName()+
        		" crf version: "+crfv.getStatus().getName()+
        		" data entry stage: "+stage.getName()+
        ...
        System.out.println("returning: "+crfVersionStatus);
    ...
    protected boolean inKeys(int sedInd, int sampleOrdinal, int crfInd, int itemInd, String groupName) {
        ...
        for (Iterator iter = data.entrySet().iterator(); iter.hasNext();) {
            ...
            String testKey = getDataKey(0, currentDef.getId(), sampleOrdinal, currentCRF.getId(), currentItem.getId(),
                    1, groupName).substring(2);
    ...
    private StudyEventBean getEvent(int h, int i, int j) {
        ...
        if (seb == null) {
            //System.out.println("did not find seb, h" + h + " i" + i + " j" + j + " key "+key);
            //System.out.println("dump of event data: "+eventData.toString());
            return new StudyEventBean();
        } else {
        	//debug(seb);
        	//System.out.println("FOUND seb, h" + h + " i" + i + " j" + j + " key "+key);
        	if (seb.getEventCRFs().size() > 0) {
        		EventCRFBean ecbean = (EventCRFBean)seb.getEventCRFs().get(0);
        	}
        	//System.out.println("int.name "+ecbean.getInterviewerName());
            return seb;
        }
    }
    ...
    private String getEventStart(int h, int i, int j) {
        ...
        if (seb.getStartTimeFlag()) {
            ...
            //need to pull date_time_format_string from format.properties here instead, tbh
...
//        }
//        return (completionStatusId > 0 ? SubjectEventStatus.get(completionStatusId).getName() : "");

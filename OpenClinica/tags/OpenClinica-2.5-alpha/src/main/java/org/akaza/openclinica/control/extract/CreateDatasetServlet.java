...
public class CreateDatasetServlet extends SecureController {
            ...
            if ("begin".equalsIgnoreCase(action)) {
                ...
                if (!StringUtil.isBlank(saveItems)) {
                    ...
                    if (db.getItemIds().size() == 0) {
                        ...
                        logger.warning("found preset values while setting date: " + fp.getPresetValues().toString());
    ...
    private String genAttMsg(DatasetBean db) {
        ...
        if (db.isShowSubjectDob() || db.isShowSubjectGender() || db.isShowSubjectAgeAtEvent() || // newly
            // added,
            // tbh
            db.isShowSubjectStatus() || db.isShowSubjectUniqueIdentifier()) {

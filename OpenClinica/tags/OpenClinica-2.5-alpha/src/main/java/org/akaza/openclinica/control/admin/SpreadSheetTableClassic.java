...
public class SpreadSheetTableClassic implements SpreadSheetTable {// extends
    ...
    public SpreadSheetTableClassic(FileInputStream parseStream, UserAccountBean ub, String versionName, Locale locale) throws IOException {
        // super();
            ...
            if (sheetName.equalsIgnoreCase("Instructions")) {
                ...
                if (sheetName.equalsIgnoreCase("Items")) {
                        ...
                        if (responseTypeId == 8 || responseTypeId == 9) {
                            ...
                            if (resValues.contains(":")) {
                                ...
                                if (!"func".equalsIgnoreCase(s[0].trim())) {
                                    ...
                                    htmlErrors.put(j + "," + k + ",15", "INVALID FIELD");
                                }
                            }
                        ...
                        if (dbName.equals("oracle")) {
                            sqlGroupLabel =
                                "INSERT INTO ITEM_GROUP_METADATA (" + "item_group_id,\"header\"," + "subheader, layout, repeat_number, repeat_max,"
                                    ...
                                    + "', '" + "" + "', " + 1 + ", " + 1 + ", '', 1," + versionIdString + "," + "(SELECT MAX(ITEM_ID) FROM ITEM WHERE NAME='"
                                    + itemName + "' AND owner_id = " + ub.getId() + " )," + k + ",0)";
                            ...
                            sqlGroupLabel =
                                "INSERT INTO ITEM_GROUP_METADATA (" + "item_group_id,HEADER," + "subheader, layout, repeat_number, repeat_max,"
                                    ...
                                    + "(SELECT ITEM_ID FROM ITEM WHERE NAME='" + itemName + "' AND owner_id = " + ub.getId() + " ORDER BY OID DESC LIMIT 1),"
                                    + k + ",0)";
                        }
                    ...
                    if (StringUtil.isBlank(crfName)) {
                        errors.add(resPageMsg.getString("the") + " " + resPageMsg.getString("CRF_NAME_column")
                            + resPageMsg.getString("was_blank_in_the_CRF_worksheet"));
                        htmlErrors.put(j + ",1,0", resPageMsg.getString("required_field"));
                    }
                    ...
                    if (crfId == 0) {
                        ...
                        try {
                            ...
                            logger.warning("Exception encountered with query select nextval('crf_crf_id_seq'), Message-" + e.getMessage());
                        } finally {
                            if (con != null) {
                                try {
                                    con.close();
                                } catch (SQLException e) {
                                    logger.warning("Connectin can't be closed");
                                }
                            }
                        }
                    }

...
public class RulesPostImportContainer {
     ...
     * // LOGIC
     * 
     * public HashMap<String,RuleSetBean> prepareRuleSet(){ HashMap<String,RuleSetBean>
     * ruleSetMap = new HashMap<String,RuleSetBean>(); for (RuleSetBean
     * ruleSetBean : ruleSets) { if (
     * ruleSetMap.containsKey(ruleSetBean.getTarget()) ){
     * ruleSetMap.get(ruleSetBean.getTarget()).getRuleRefs().addAll(ruleSetBean.getRuleRefs());
     * }else{ ruleSetMap.put(ruleSetBean.getTarget(), ruleSetBean); } } return
     * ruleSetMap; }
     * 
     * public HashMap<String,RuleDefBean> prepareRules(){
     * 
     * HashMap<String,RuleDefBean> ruleMap = new HashMap<String,RuleDefBean>();
     * for (RuleDefBean ruleDefBean : ruleDefs) {
     * ruleMap.put(ruleDefBean.getOID(), ruleDefBean); } return ruleMap; }
     * 
     * public void fight(){
     * 
     * HashMap<String,RuleSetBean> ruleSets = prepareRuleSet(); HashMap<String,RuleDefBean>
     * rules = prepareRules();
     * 
     * for (String key : ruleSets.keySet()) { RuleSetBean ruleSetBean =
     * ruleSets.get(key); for (RuleDefBean ruleBean : ruleSetBean.getRuleRefs()) {
     * if(rules.containsKey(ruleBean.getOID())){
     * ruleSetBean.addRuleSetRule(ruleBean); }else{ System.out.println("THE KEY
     * YOU PROVIDED DOES NOT MATCH"); } } } }
     */
}

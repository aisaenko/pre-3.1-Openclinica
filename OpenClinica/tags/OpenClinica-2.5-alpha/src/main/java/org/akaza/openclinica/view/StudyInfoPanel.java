...
public class StudyInfoPanel {
    ...
    public void setData(Page page, HttpSession session, HttpServletRequest request) {
    	...
    	System.out.println("found date format string: "+ResourceBundleProvider.getFormatBundle().getString("date_format_string"));
    	System.out.println("found date format string with locale: "+ResourceBundleProvider.getFormatBundle(locale).getString("date_format_string"));
            ...
            while (uncompIt.hasNext()) {
                ...
                if (count == uncompleted.size() - 1) {
                    if (ecb != null && ecb.getId() == dedc.getEventCRF().getId() && ecb.getCrf().getId() == dedc.getEventCRF().getCrf().getId()) {
                        // System.out.println("ecb id*******" + ecb.getId() +

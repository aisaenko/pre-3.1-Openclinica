                ...
                if (sheetName.equalsIgnoreCase("Items")) {
                    ...
                    for (int k = 1; k < numRows; k++) {
                        ...
                        if (StringUtil.isBlank(leftItemText)) {
                            // errors.add("The LEFT_ITEM_TEXT column was blank
                            // at row " + k + ", Items worksheet.");
                            // htmlErrors.put(j + "," + k + ",2", "REQUIRED
                            // FIELD");
                        ...
                        if (!StringUtil.isBlank(regexp)) {
                            ...
                            if (regexp1.startsWith("regexp:")) {
                                ...
                                // System.out.println("reg:" + finalRegexp);
                            ...
                            if (dbName.equals("oracle")) {
                                sqlGroupLabel =
                                    "INSERT INTO ITEM_GROUP_METADATA (" + "item_group_id,\"header\"," + "subheader, layout, repeat_number, repeat_max,"
                                        ...
                                        + "(SELECT MAX(ITEM_ID) FROM ITEM WHERE NAME='" + itemName + "' AND owner_id = " + ub.getId() + ")," + k + ")";
                            } else {
                                sqlGroupLabel =
                                    "INSERT INTO ITEM_GROUP_METADATA (" + "item_group_id,header," + "subheader, layout, repeat_number, repeat_max,"
                                        + " repeat_array,row_start_number, crf_version_id," + "item_id , ordinal) VALUES ("
                                        ...
                                        + versionIdString + "," + "(SELECT ITEM_ID FROM ITEM WHERE NAME='" + itemName + "' AND owner_id = " + ub.getId()
                                        + " ORDER BY OID DESC LIMIT 1)," + k + ")";

...
FROM study su, subject s, event_definition_crf edc, crf

left outer join item_group ig
on crf.crf_id = ig.crf_id, crf_version crfv,
study_event se 

left outer join subject_group_map sgmap 
on se.study_subject_id = sgmap.study_subject_id,

event_crf ec, item_data id, item it

left outer join item_group_metadata igm
on it.item_id = igm.item_id, 
study_subject ss,
...
--UPDATE extract_data_table SET item_group_name = 'Ungrouped' where item_group_name = '' or item_group_name = null;
--UPDATE extract_data_table SET item_data_ordinal = 1 where item_data_ordinal < 1;

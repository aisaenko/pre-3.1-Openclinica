			...
			<table border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<select class="formfieldM" name="resolutionStatus" onchange="document.location='ViewNotes?resolutionStatus='+this.value;">
	                     ...
	                     <option value="open" <c:choose><c:when test="${param.resolutionStatus=='open'}">selected="selected"</c:when></c:choose>><fmt:message key="New/Open" bundle="${resterm}"/>
	                     <option value="closed" <c:choose><c:when test="${param.resolutionStatus=='closed'}">selected="selected"</c:when></c:choose>><fmt:message key="Resolved/Closed" bundle="${resterm}"/>

...
<jsp:useBean scope="request" id="studySub" class="org.akaza.openclinica.bean.managestudy.StudySubjectBean"/>   
   <tr>       
     <td class="table_cell_left"><c:out value="${currRow.bean.studyEvent.studyEventDefinition.name}"/>
        (<c:out value="${currRow.bean.maximumSampleOrdinal}"/>)
     </td>
     <td class="table_cell"><fmt:formatDate value="${currRow.bean.studyEvent.dateStarted}" dateStyle="short"/>
     </td>
     ...
     <td class="table_cell">
       <table border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
		<td>
		<c:if test="${studySub.status.name != 'removed' && studySub.status.name != 'auto-removed'}">
		...
		<c:choose>
         ...
         <c:otherwise>
          <c:if test="${userRole.manageStudy && studySub.status.name != 'removed' && studySub.status.name != 'auto-removed'}">
			...
			<tr valign="top">
			  ...
			  <td class="table_cell" width="180"><c:out value="${dedc.edc.crf.name}" /></td>
				...
				<c:otherwise>
		         ...
		         <td><a href="ViewSectionDataEntry?eventDefinitionCRFId=<c:out value="${dedc.edc.id}"/>&crfVersionId=<c:out value="${dedc.edc.defaultVersionId}"/>&tabId=1&studySubjectId=<c:out value="${studySub.id}"/>"
			     onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
			     onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
		         name="bt_View1" src="images/bt_View.gif" border="0" alt="<fmt:message key="view_default" bundle="${resword}"/>" title="<fmt:message key="view_default" bundle="${resword}"/>" align="left" hspace="6"></a></td>
	...
	<tr>
		...
		<c:if test="${dec.eventCRF.updatedDate != null}">
		 <fmt:formatDate value="${dec.eventCRF.updatedDate}" dateStyle="short"/><br>
		</c:if>
		<c:choose>
		...
		<td class="table_cell" width="140">
	     ...
	     <tr valign="top">
	     <td>
			<c:if test="${!dec.eventCRF.status.deleted && !dec.eventCRF.status.locked}">
		 ...
		 <a href="ViewEventCRFContent?id=<c:out value="${studySub.id}"/>&ecId=<c:out value="${dec.eventCRF.id}"/>&eventId=<c:out value="${currRow.bean.studyEvent.id}"/>"
		    ...
		    name="bt_View1" src="images/bt_View.gif" border="0" alt="<fmt:message key="view" bundle="${resword}"/>" title="<fmt:message key="view" bundle="${resword}"/>" align="left" hspace="6"></a>
		 -->
		 <a href="ViewSectionDataEntry?eventDefinitionCRFId=<c:out value="${dedc.edc.id}"/>&ecId=<c:out value="${dec.eventCRF.id}"/>&tabId=1&studySubjectId=<c:out value="${studySub.id}"/>"
			onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
			onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img 
		    name="bt_View1" src="images/bt_View.gif" border="0" alt="<fmt:message key="view" bundle="${resword}"/>" title="<fmt:message key="view" bundle="${resword}"/>" align="left" hspace="6"></a>

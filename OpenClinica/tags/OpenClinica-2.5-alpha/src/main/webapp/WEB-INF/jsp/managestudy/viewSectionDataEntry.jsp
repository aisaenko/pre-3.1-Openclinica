...
<c:when test="${crfListPage == 'yes'}">
    <a href="ListCRF?&module=">
        <input type="button" name="exit" value="<fmt:message key="exit" bundle="${resword}"/>" class="button"/>
    </a>
</c:when>
<c:when test="${studySubjectId > 0}">
        <a href="ViewStudySubject?id=<c:out value="${studySubjectId}"/>">
        <input type="button" name="exit" value="<fmt:message key="exit" bundle="${resword}"/>" class="button"/>
        </a>
</c:when>
<c:when test="${eventId > 0}">
       <a href="EnterDataForStudyEvent?eventId=<c:out value="${eventId}"/>">
            <input type="button" name="exit" value="<fmt:message key="exit" bundle="${resword}"/>" class="button"/>
       </a>
</c:when>
<c:otherwise>
<a href="ViewCRF?crfId=<c:out value="${crfId}"/>">
     <input type="button" name="exit" value="<fmt:message key="exit" bundle="${resword}"/>" class="button"/>
</a>
</c:otherwise>
</c:choose>
        ...
        <select class="formfieldM" name="sectionSelect" size="1" onchange="gotoLink();">
            ...
            <c:forEach var="sec" items="${toc.sections}">
                <c:choose>
                    <c:when test="${studySubject != null && studySubject.id>0}">
                        <c:set var="tabUrl"
                               value="ViewSectionDataEntry?ecId=${EventCRFBean.id}&sectionId=${sec.id}&tabId=${tabCount}"/>
                    </c:when>
                    <c:otherwise>
                        <c:set var="tabUrl"
...
<c:if test='${section.section.title != ""}'>
    <tr class="aka_stripes">
        <td class="aka_header_border"><b><fmt:message key="title" bundle="${resword}"/>:&nbsp;<c:out
                value="${section.section.title}" escapeXml="false"/></b></td>
    </tr>
</c:if>
<c:if test='${section.section.subtitle != ""}'>
    <tr class="aka_stripes">
        <td class="aka_header_border"><fmt:message key="subtitle" bundle="${resword}"/>:&nbsp;<c:out
                value="${section.section.subtitle}" escapeXml="false"/></td>
    </tr>
</c:if>
<c:if test='${section.section.instructions != ""}'>
    <tr class="aka_stripes">
        <td class="aka_header_border"><fmt:message key="instructions" bundle="${resword}"/>:&nbsp;<c:out
                value="${section.section.instructions}" escapeXml="false"/></td>
    </tr>
</c:if>
            ...
            <table border="0" cellpadding="0" cellspacing="0" style="margin-bottom: 6px;">
                <tr>
                    ...
                    <td valign="bottom" nowrap="nowrap" style="padding-right: 50px">
                        ...
                        <a name="top"><fmt:message key="page" bundle="${resword}"/>: <c:out
                                value="${displayItem.pageNumberLabel}" escapeXml="false"/></a>
                    </td>
        ...
        <td class="aka_header_border" colspan="2">
            <table border="0" cellpadding="0" cellspacing="0" style="margin-bottom: 6px;">
                <tr>
                    ...
                    <td valign="bottom" nowrap="nowrap" style="padding-right: 50px">
                        <fmt:message key="page" bundle="${resword}"/>: <c:out value="${displayItem.pageNumberLabel}"
                                                                              escapeXml="false"/>
                    </td>
                    <td align="right" valign="bottom">
                    </td>
        ...
        <tbody>
                ...
                <c:choose>
                    <c:when test="${status.last}">
                        ...
                        <tr id="<c:out value="${repeatParentId}"/>" repeat="template"
                            repeat-start="<c:out value="${repeatNumber}"/>" repeat-max="<c:out value="${repeatMax}"/>">
                            <input type="hidden"
                            ...
                            <td class="aka_padding_norm aka_cellBorders">
                                <input type="hidden"
                                       ...
                                       value="yes">
                                <button type="remove" template="<c:out value="${repeatParentId}"/>"
            ...
            <tr>
                <td class="aka_padding_norm aka_cellBorders" colspan="<c:out value="${columnNum}"/>">
                    <button type="add" template="<c:out value="${repeatParentId}"/>" class="button_search"><fmt:message
                            key="add" bundle="${resword}"/></button>
                </td>
            </tr>
        </tbody>
                    ...
                    <table border="0">
                        <tr>
                            ...
                            <td valign="top" class="aka_text_block"><c:out
                                    value="${displayItem.singleItem.metadata.leftItemText}" escapeXml="false"/></td>
                            ...
                            <td valign="top"><c:out value="${displayItem.singleItem.metadata.rightItemText}"
                                                    escapeXml="false"/></td>
                        </tr>
                    ...
                    <td valign="top">
                        <table border="0">
                            <tr>
                                ...
                                <td valign="top" class="aka_ques_block"><c:out
                                        value="${childItem.metadata.questionNumberLabel}" escapeXml="false"/></td>
                                <td valign="top" class="aka_text_block"><c:out
                                ...
                                <td valign="top"><c:out value="${childItem.metadata.rightItemText}"
                                                        escapeXml="false"/></td>
                            </tr>
                                <%--BWP: try this--%>
                            <tr>

			...
			<tr valign="top">
			  ...
			  <td class="table_cell" width="180"><c:out value="${dedc.edc.crf.name}" /></td>
                ...
                <c:if test="${study.status.available}">
		         ...
		         <td><a href="ViewSectionDataEntry?eventDefinitionCRFId=<c:out value="${dedc.edc.id}"/>&crfVersionId=<c:out value="${dedc.edc.defaultVersionId}"/>&tabId=1&studySubjectId=<c:out value="${studySub.id}"/>&ecId=<c:out value="${dedc.eventCRF.id}"/>&exitTo=ViewStudySubject?id=${studySub.id}"
                        onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"

...
<c:if test='${inputType=="file"}'>
	...
	<c:otherwise>
		<a href="DownloadAttachedFile?eventCRFId=<c:out value="${section.eventCRF.id}"/>&fileName=<c:out value="${inputTxtValue}"/>" id="a<c:out value="${itemId}"/>"><c:out value="${inputTxtValue}"/></a>
		<c:choose>

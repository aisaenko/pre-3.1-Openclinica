...
public class HtmlUtils {
   ...
   * such as {12, 10, 2007} for use, among others, in creating a GregorianClandar
   * object.  The method assumes a default database format of "12/10/2007" or
   * "MM/dd/yyyy".
   * @param dateString  A String like "12/10/2007".
   * @return  An int array made up of month, day, year
   * (three components, in that order).  If the dateString argument is invalid or empty, returns
   * an empty array.
  ...
  private static int[] parseDateToArray(String dateString){
    ...
    for(int i = 0; i < intArray.length;i++) {
      try{
        ...
        //the method argument is not a valid date, or in the expected format
        intArray = new int[]{};
        return intArray;
   ...
   * GregorianCalendar object, returning a Date object for formatting. This class
   * assumes a format pattern of MM/dd/yyyy.
   * @param _date A String like "12/10/2007" . If the argument cannot
   * be parsed into three segments, then the method returns null.
  ...
  public static synchronized Date parseDateValue(String _date)  {
    ...
    if(intArr.length==0 || intArr.length < 3 || intArr[0] > 12 ||
      intArr[1] > 31)  {
      return null;
    } else   {

...
public class TestRuleServlet extends SecureController {
    ...
    void populteFormFields(FormProcessor fp) {
        ...
        String targetSess = ((String) session.getAttribute("testRulesTarget")).trim().replaceAll("(\n|\t|\r)", "");
        if (session.getAttribute("testRulesTarget") != null && !targetForm.equals(targetSess)) {

...
public class SignStudySubjectServlet extends SecureController {
    ...
    public static ArrayList getDisplayStudyEventsForStudySubject(StudySubjectBean studySub,
                                                                 DataSource ds,
                                                                 UserAccountBean ub,
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            de.setDisplayEventCRFs(getDisplayEventCRFs(ds, eventCRFs, eventDefinitionCRFs, ub, currentRole, event.getSubjectEventStatus()));
    ...
    public static boolean permitSign(StudySubjectBean studySub,
                                     DataSource ds) {
        boolean sign = true;
        StudyEventDAO sedao = new StudyEventDAO(ds);
        EventCRFDAO ecdao = new EventCRFDAO(ds);
        ...
        for (int l = 0; l < studyEvents.size(); l++) {
            ...
            for (int i = 0; i < eventCrfs.size(); i++) {
                ...
                //ArrayList discList = discDao.findAllItemNotesByEventCRF(ecrf.getId());
//                for (int j = 0; j < discList.size(); j++) {
//                    DiscrepancyNoteBean discBean = (DiscrepancyNoteBean) discList.get(j);
//                    if (discBean.getResStatus().equals(org.akaza.openclinica.bean.core.ResolutionStatus.OPEN)
//                            || discBean.getResStatus().equals(org.akaza.openclinica.bean.core.ResolutionStatus.UPDATED)) {
//                        sign = false;
//                        break;
//                    }
//                }
                EventDefinitionCRFBean edcBean = edcdao.findByStudyEventIdAndCRFVersionId(studyEvent.getId(), ecrf.getCRFVersionId());
    ...
    public static boolean signSubjectEvents(StudySubjectBean studySub,
                                            DataSource ds, UserAccountBean ub) {
    ...
    public void processRequest() throws Exception {
        ...
        if (action.equalsIgnoreCase("confirm")) {
            ...
            if ((ub.getPasswd().equals(encodedUserPass)) && (ub.getName().equals(username))) {
                if(signSubjectEvents(studySub, sm.getDataSource(), ub)){
                    //Making the StudySubject signed as all the events have become signed.
        ...
        //Don't filter for now; disc note beans are returned with eventCRFId set
        discNoteUtil.injectParentDiscNotesIntoDisplayStudyEvents(displayEvents,
                  new HashSet(),sm.getDataSource(), 0);
         //All the displaystudyevents for one subject
        request.setAttribute("displayStudyEvents",displayEvents);

        //Set up a Map for the JSP view, mapping the eventCRFId to another Map: the
        //inner Map maps the resolution status name to the number of notes for that
        //eventCRF id, as in New --> 2
        Map discNoteByEventCRFid =
          discNoteUtil.createDiscNoteMapByEventCRF(displayEvents);
     ...
     */
    public static ArrayList getDisplayEventCRFs(
            DataSource ds,
            ArrayList eventCRFs,
        ...
        for (i = 0; i < eventCRFs.size(); i++) {
            ...
            EventDefinitionCRFBean edc = edcdao.findByStudyEventDefinitionIdAndCRFId(
                    studyEventDefinitionId,
                    cb.getId());
     ...
     */
    public static ArrayList getUncompletedCRFs(
            DataSource ds,
            ArrayList eventDefinitionCRFs,
            ArrayList eventCRFs,
            SubjectEventStatus status) {
        ...
        for (i = 0; i < eventDefinitionCRFs.size(); i++) {
            ...
            //System.out.println("created dedc with edcrf "+edcrf.getCrfName()+" default version "+
            //	  edcrf.getDefaultVersionName()+", id "+edcrf.getDefaultVersionId());
            ...
            if (b == null || !b.booleanValue()) {
                ...
                //System.out.println("entered boolean loop with ev "+ev.getId()+" crf version id "+
                //	  ev.getCRFVersionId());
                ...
                /*if (dedc.getEdc().getDefaultVersionId() != dedc.getEventCRF().getId()) {
                    System.out.println("ID MISMATCH: edc name "+dedc.getEdc().getName()+
                            ", default version id "+dedc.getEdc().getDefaultVersionId()+
                            " event crf id "+dedc.getEventCRF().getId());
                }*/
            }
        }

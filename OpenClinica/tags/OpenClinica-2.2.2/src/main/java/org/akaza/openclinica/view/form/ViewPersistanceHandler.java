...
/**
 * A class that handles persistent values that must appear in forms when they are generated.
 */
public class ViewPersistanceHandler {
  ...
  public List<ItemDataBean> fetchPersistedData(int sectionId,
                                               int eventcrfId) {

    SessionManager sessionManager = new SessionManager();
    ItemDataDAO itemDataDAO = new
      ItemDataDAO(sessionManager.getDataSource());
    List<ItemDataBean> itemDataBeans =
      itemDataDAO.findAllActiveBySectionIdAndEventCRFId(sectionId,
        eventcrfId);
   ...
   * @return
   */
  public List<DisplayItemBean> loadDataIntoDisplayBeans(
    List<DisplayItemBean> disBeans, boolean hasGroupedItems) {
    ...
    //The process is straightforward if the display beans are not involved with a
    //group-type matrix table that has persistent repeated rows
    dataItem:
    for(DisplayItemBean itemBean : disBeans)  {
      for(ItemDataBean iDataBean : dataBeans) {
        //We're not handling  persistent repeated rows at this point; Just the first row of data.
    ...
    //Just return the DisplayBeans unchanged if they have persistent repeated rows.
  ...
  public boolean hasPersistentRepeatedRows(List<DisplayItemBean> itemBeans) {
    ...
    for(DisplayItemBean disBean : itemBeans) {
      for(ItemDataBean dataBean : this.getItemDataBeans()){
        if(disBean.getItem().getId() == dataBean.getItemId() &&
          dataBean.getOrdinal() > 1)  {
          return true;
        }
      }
    }
   ...
   *
   * @return
   * @see org.akaza.openclinica.view.form.ViewPersistanceHandler loadDataIntoDisplayBeans()
   */
  public SortedMap<Integer,List<ItemDataBean>> handleExtraGroupRows(){

    //handle duplicate item ids in itemdatabeans
    //A sorted map with the ordinal as the index, and the related ItemDataBean as value.
    ...
    for(ItemDataBean itemDataBean : dataBeans) {
      ...
      if(tempOrdinal > 1)  {
        ...
        if(tracker == 1) {
          ...
          //if tempOrdinal is the same as currentOrdinal, then just add the data bean
  ...
  public Map<Integer,List<DisplayItemBean>> sortDuplicatesIntoRows(
    ...
    Map<Integer,List<DisplayItemBean>> rowMap =
      new HashMap<Integer,List<DisplayItemBean>>();
  ...
  private boolean listContainsDisplayItem(List<DisplayItemBean> beanList,
                                          DisplayItemBean singleBean)  {

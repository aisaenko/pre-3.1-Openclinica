    ...
    private void addMetadataCodeList(CodeListInfo cli, String currentIndent) {
        if (cli.cls.size() > 0) {
            ...
            for (ResponseSetBean cl : sortedCL) {
                ...
                line +=
                    "<CodeList OID=\"CL." + cl.getId() + "\" Name=\"" + cl.getLabel() + "\" DataType=\"" + datatype + "\" SASFormatName=\""
                        + formatname.getValidSASFormatName(cl.getLabel(), isString) + "\">";
                ...
                if (!c0.getText().trim().equals("") && !c0.getValue().trim().equals("")) {
                    add(currentIndent + indent, "<CodeListItem CodedValue=\"" + c0.getValue() + "\">");
                    add(currentIndent + indent + indent, "<Decode>");
                    add(currentIndent + indent + indent + indent, "<TranslatedText>" + c0.getText() + "</TranslatedText>");
                    add(currentIndent + indent + indent, "</Decode>");
                    add(currentIndent + indent, "</CodeListItem>");
                }
                for (int i = 1; i < codelists.size(); ++i) {
                    ResponseOptionBean c = codelists.get(i);
                    add(currentIndent + indent, "<CodeListItem CodedValue=\"" + c.getValue() + "\">");
                    add(currentIndent + indent + indent, "<Decode>");
                    add(currentIndent + indent + indent + indent, "<TranslatedText>" + c.getText() + "</TranslatedText>");
                    add(currentIndent + indent + indent, "</Decode>");
                    add(currentIndent + indent, "</CodeListItem>");
                }

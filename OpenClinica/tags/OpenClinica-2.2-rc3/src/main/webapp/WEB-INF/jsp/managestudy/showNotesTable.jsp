			...
			<tr>
				<td valign="top" class="table_tools">
					<c:if test="${table.paginated && (table.currPageNumber > 1)}">
						<a href="<c:out value="${firstPageQuery}"/>"><img src="images/arrow_first.gif" border="0" alt="first page" title="first page"></a>
						<a href="<c:out value="${prevPageQuery}"/>"><img src="images/arrow_back.gif" border="0" alt="back" title="back"></a>
					</c:if>
				</td>
				<td valign="top" class="table_tools">
					<c:choose>
						<c:when test="${empty table.rows}"> No Pages </c:when>
						<c:otherwise> Page <c:out value="${table.currPageNumber}" /> of <c:out value="${table.totalPageNumbers}" /> </td></c:otherwise>
					</c:choose>
				<td valign="top" class="table_tools">
		...
		<td width="55%" valign="top" align="center" class="table_actions">
			...
			<table border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td valign="top">
						<input type="submit" class="button_search" value="Find" 
							<c:choose>
					...
					<select class="formfieldM" name="resolutionStatus" onchange="document.location='ViewNotes?resolutionStatus='+this.value;">
	                     <option value="" <c:choose><c:when test="${param.resolutionStatus==''}">selected="selected"</c:when></c:choose>>All
	                     <option value="open" <c:choose><c:when test="${param.resolutionStatus=='open'}">selected="selected"</c:when></c:choose>>New/Open
	                     <option value="closed" <c:choose><c:when test="${param.resolutionStatus=='closed'}">selected="selected"</c:when></c:choose>>Resolved/Closed
			    ...
			    <a href="javascript:HideGroups(0,13,10);">
							<div id="HideGroups" style="display: none">&nbsp;&nbsp;Show Less</div>
							<div id="ShowGroups" style="display: all">&nbsp;&nbsp;Show More</div>
							</a></b></td>
						...
						<c:otherwise>
						  ...
						  <c:otherwise>
							...
							<c:if test="${(table.sortingColumnInd == i) && column.showLink}">
								<c:choose>
									<c:when test="${table.ascendingSort}"><img src="images/bt_sort_ascending.gif" alt="ascending sort" title="ascending sort" /></c:when>
									<c:otherwise><img src="images/bt_sort_descending.gif" alt="descending sort" title="descending sort" /></c:otherwise>
								</c:choose>
							</c:if>
						</td>

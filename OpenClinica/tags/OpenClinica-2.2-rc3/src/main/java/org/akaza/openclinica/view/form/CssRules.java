...
/**
 * The CssRules clss encapsulates the mapping of element names or
 * element descriptions to certain CSS class names. This helps allow the
 * application of CSS classes to dynamically generated elements, in the manner of
 * td class="aka_cellBorders" .
 */
public class CssRules {
  //A Map that maps element names (e.g., td) or short descriptions (e.g., span left_item)
  ...
  static {
    ...
    loadMap("th",CssClass.headerBackground,CssClass.padding_large,
      CssClass.cellBorders);
  ...
  enum CssClass {
    revised_content, bodywidth, revised, text_block, header_border,headerBackground,
    stripes, cellBorders, sm_text, padding_norm, padding_large,
    light_stripes,date_input,sm_text_input,table_cell,table_cell_left,form_table,form_table_section_header
  }
  //This method is used during the initialization of a HashMap that maps element names
  //to one or more CSS classes, as in <input type="text" class="sm_text_input">.
  //loadMap is synchronized for thread safety and is only called during the class'
  //initialization, when it is first loaded.
  private static synchronized void loadMap(String elementDescription,
  ...
  /**This method returns a string of class names for the tagName
  *parameter.
  *@param tagName A tag such as td, TD, or tr, or a tag name plus
   * a description as in td left_item.
   * @return A String of CSS class names, as in  "headerBackground padding_large"
  */
  public static synchronized String getClassNamesForTag(String tagName) {
    ...
    for(CssClass name : classNames) {
      ++counter;
      allNames += (isApplyPrefix() ? getCssRulePrefix()+name.toString() :
        name.toString());
      if(! (counter==listSize)) allNames +=" ";
    }
    return allNames;
  }

...
/**
 * The workhorse for instrument generation, the NewInstrumentBean
 * holds some information, but also updates and inserts rows in the database, using
 * the insertToDB method.</p>
 * <P>Currently, we do not make use of dbInstructions yet.
...
public class NewCRFBean extends Object implements java.io.Serializable {
  ...
  public NewCRFBean(DataSource ds, int crfId) {
    ...
    try {
      ...
      pe.printStackTrace();
      logger.info("hit an exception in creating new crf bean;"+
                         " empty item name list exists");
  ...
  /**
   * @param itemGroupNames the itemGroupNames to set
   */
  ...
  public HashMap listVersionNames() throws OpenClinicaException {
    /* serves up a list of all crf names and versions, necessary
     for checking for duplicates.
     Added by tbh, 7-25-03 */
    ...
    try {
      con = ds.getConnection(); if (con.isClosed()) {
  ...
  public HashMap listItemNames(int crfId) throws OpenClinicaException {
    /* serves up a list of all item names in a CRF, necessary
     for checking for duplicates and creating new items.
     Added by tbh, 7-25-03 */
    PreparedStatement ps = null; Connection con = null;
    ResultSet rs = null;
    HashMap returnMe = new HashMap();
  ...
  public HashMap listGroupNames(int crfId) throws OpenClinicaException {
    /* serves up a list of all item group names in a CRF, necessary
     for checking for duplicates and creating new items.
     */
    PreparedStatement ps = null; Connection con = null;
    ResultSet rs = null;
  ...
  public void insertToDB() throws OpenClinicaException {
    /* insertToDB() will implement rollback functionality, throwing an error
       will write the error to the setErrors and redirect to the error page, rather
       than to the success page, tbh, 6-6-03
        */
    Statement s = null;
    ...
    try {
      ...
      con = ds.getConnection(); if (con.isClosed()) {
        ...
        throw new OpenClinicaException(
            "newCRFBean, insertToDB, connection not open", "1");
      }
      //commenting this out temporarily so that mistakes are not made, tbh 8-13
      ...
      for (Iterator itvl = mySet.iterator(); itvl.hasNext(); ) {
        ...
        //of course, the queries are simple enough that maybe we'll never throw
      ...
      //the below lines are temporarily commented out for instrument upload, tbh 8-13
      con.commit();
      logger.info("---end of query generation, all queries committed---");
      con.setAutoCommit(true);
      logger.info("---end of query generation, autocommit set to true---");
      //at this point we check to see if there is a active version, if not, set THIS
      ...
      if (crfId != 0) {
        ...
        rs = ps.executeQuery();
        if (rs.next()) {
          //do nothing on purpose? no, we check to see if it was null first
        ...
        rs.close();
        ps.close();
      }//hairy code--but it will serve as a 'smart assigner' to set a version active if
      ...
      try {
        ...
        this.setErrors(error);
        throw new OpenClinicaException("","");
        //catching OpenClinica exceptions in general is duplicating effort, maybe we should nix it?
      }
  ...
  public void deleteFromDB() throws OpenClinicaException {    
    ...
    try {
      ...
      con = ds.getConnection(); if (con.isClosed()) {
        ...
        error.add(msg);
        throw new OpenClinicaException(
            "newCRFBean, deleteFromDB, connection not open", "1");
      }
      ...
      /*//now need to insert all items again-no need to do this 04/12/2005 any more
      Set mySet = backupItemQueries.entrySet();
      logger.info("---start of item generation here---");
      for (Iterator itvl = mySet.iterator(); itvl.hasNext(); ) {
        Map.Entry ment = (Map.Entry)itvl.next();
        String pQuery = (String)ment.getValue();
        s = con.createStatement();

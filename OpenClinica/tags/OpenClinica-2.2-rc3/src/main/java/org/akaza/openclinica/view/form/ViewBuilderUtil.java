...
public class ViewBuilderUtil {
  ...
  /**
   *  Generate a List of HTML rows (tr tag) for a matrix type table that already has form-field
   * values from the database. In other words, the user previously created repeating rows and filled
   * them with some data.
   * @param sortedDataMap   A SortedMap where the indiex is the Item Data's ordinal (like "2"), and the
   * value is a List of ItemDataBeans associated with that ordinal. These data make up the row's content.
   * @param rowContentBeans  The DisplayItemBeans that provide the HTML fields for the rows.
   * @param tabIndex  The incremented tab index for the form section.
   * @param repeatParentId   The String id for the input name fields. See the RepeatManager class.
   * @param hasDiscrepancyMgt  A boolean indicating whether the discrepancy note icon should be displayed.
   * @return   A List of row Elements, as in new Element("tr"); containing the previously generated
   * repeating rows.
   */
    ...
    for(Integer ordinal : sortedDataMap.keySet()) {
      ...
      for(DisplayItemBean disItemBean : rowContentBeans) {
        ...
        responseName= disItemBean.getMetadata().getResponseSet().
          getResponseType().getName();
        //start horiz
        if (disItemBean.getMetadata().getResponseLayout().
          equalsIgnoreCase("horizontal") &&
          (responseName.equalsIgnoreCase("checkbox") ||
            responseName.equalsIgnoreCase("radio"))
          ) {
          //The final true parameter styfles the display of default values in CRFs (because
          ...
          Element[] elements = cellFactory.createCellContentsForChecks(responseName,
            disItemBean,
            disItemBean.getMetadata().getResponseSet().getOptions().size(),
            ++tabIndex, true);
          for (Element el : elements) {
            el = this.setClassNames(el);
            if (repeatFlag) {
              el = repeatManager.addChildRepeatAttributes(el, repeatParentId,
                disItemBean.getItem().getId(), forcedParentId);
            }
            tr.addContent(el);
          }
        ...
        td = cellFactory.createCellContents(td, responseName,
          disItemBean, ++tabIndex, hasDiscrepancyMgt, true);

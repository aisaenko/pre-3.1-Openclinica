...
public class ImportCRFDataService {
    ...
    public List<DisplayItemBeanWrapper> lookupValidationErrors(javax.servlet.http.HttpServletRequest request, ODMContainer odmContainer, UserAccountBean ub,
            HashMap<String, String> totalValidationErrors, HashMap<String, String> hardValidationErrors, ArrayList<Integer> permittedEventCRFIds) throws OpenClinicaException {
        ...
        for (SubjectDataBean subjectDataBean : subjectDataBeans) {
            ...
            for (StudyEventDataBean studyEventDataBean : studyEventDataBeans) {
                StudyEventDefinitionBean sedBean =
                    sedDao.findByOidAndStudy(studyEventDataBean.getStudyEventOID(), studyBean.getId(), studyBean.getParentStudyId());
                ArrayList<FormDataBean> formDataBeans = studyEventDataBean.getFormData();
                ...
                for (FormDataBean formDataBean : formDataBeans) {
                    ...
                    eventDefinitionCRF = eventDefinitionCRFDAO.findByStudyEventIdAndCRFVersionId(studyEvent.getId(), crfVersion.getId());
                    
                    if (permittedEventCRFIds.contains(new Integer(eventCRFBean.getId()))) {
                        
                    	// if and only if it's in the correct status do we need to generate the beans
                    	// <<tbh, 09/2008
                    	// also need to create a group name checker here for correctness, tbh
                        ItemFormMetadataDAO itemFormMetadataDAO = new ItemFormMetadataDAO(ds);
                    	for (ImportItemGroupDataBean itemGroupDataBean : itemGroupDataBeans) {
                    	    // generate a list of item beans that should be in
                            // the group
                            ItemGroupDAO itemGroupDAO = new ItemGroupDAO(ds);
                            // ItemDAO itemDAO = new ItemDAO(ds);
                            // ArrayList<ItemBean> permittedItemBeans =
                    		...
                    		if (testBean == null) {
                    			...
                    			//throw new OpenClinicaException("message here", "");
                    			// your_item_group_oid_for_form_oid
                    		}
                    		totalItemDataBeanCount += itemDataBeans.size();
                    		String usedItemOIDs = "";
                    		// the new trick here - maintain a seperate list
                            // which contains all the item oids for a group
                            // after we iterate through the ones that we have,
                            // add the ones that we do not
                            // with blanks as values, tbh 02/2009
                            int count = 0;
                    		...
                    		for (ImportItemDataBean importItemDataBean : itemDataBeans) {
                    			...
                    			usedItemOIDs += "'" + importItemDataBean.getItemOID() + "'";
                    			...
                    			if (!itemBeans.isEmpty()) {
                    				...
                    				// possible update and comparison here, tbh
                                    // @@@
                    			...
                    			// if it is not the end, add a comma to the list
                                if (itemDataBeans.size() != count) {
                                    usedItemOIDs += ", ";
                                }
                    		}
                    		// generate the rest of the beans here, tbh 02/2009
                            // but without validations
                            logger.debug("found used item oids " + usedItemOIDs);
                            ItemDAO itemDAO = new ItemDAO(ds);
                            // also need to add logic where we don't overwrite
                            // previous stuff
                            ArrayList<ItemBean> unusedItemBeans = itemDAO.findAllItemsByGroupOID(itemGroupDataBean.getItemGroupOID());
                            ArrayList<ItemBean> unusedItemBeans2 = new ArrayList<ItemBean>();
                            for (ItemBean itemBean : unusedItemBeans) {
                            
                                if (!usedItemOIDs.contains(itemBean.getOid())) {
                                    unusedItemBeans2.add(itemBean);
                                }
                             }
                            logger.debug("started with " + count + " items, now we have " + unusedItemBeans2.size() + " left");
                            for (ItemBean itemBean : unusedItemBeans2) {
                                System.out.println("   iterating through unused item beans, generating blank answers: " + itemBean.getOid());
                                // if this is an overwrite, DON'T OVERWRITE
                                // DRY
                                int groupOrdinal = 1;
                                if (itemGroupDataBean.getItemGroupRepeatKey() != null) {
                                    try {
                                ...
                                ItemDataBean itemDataCheck =
                                    itemDataDAO.findByItemIdAndEventCRFIdAndOrdinal(itemBean.getId(), eventCRFBean.getId(), 1);// groupOrdinal
                                                                                                                                                        // or
                                                                                                                                                        // this
                                                                                                                                                        // ?
                                if (itemDataCheck.getItemId() == 0) {
                                    // it's not there, we create a blank
                                    DisplayItemBean displayItemBean = new DisplayItemBean();
                                    displayItemBean.setItem(itemBean);
                                    ItemDataBean itemDataBean = createItemDataBean(itemBean, eventCRFBean, "", ub, 1);// ?
                                    ...
                                    logger.debug("found a item data bean, not creating a blank answer for " + itemBean.getOid() + " -- "
                                        + itemDataCheck.getOrdinal() + " -- " + itemDataCheck.getValue() + " -- " + itemDataCheck.getItemId());
                                }
                            }
                    	}
                    }
                    ...
                    for (Object errorKey : hardValidator.keySet()) {
                        logger.info(errorKey.toString() + " -- " + hardValidator.get(errorKey));
                    ...
                    logger.info("creation of wrapper: max ordinal found " + maxOrdinal);
                    ...
                    displayItemBeanWrapper =
                        new DisplayItemBeanWrapper(displayItemBeans, true, overwrite, validationErrors, studyEventId, crfVersionId, studyEventDataBean
                                .getStudyEventOID(), studySubjectBean.getLabel(), eventCRFBean.getCreatedDate(), crfBean.getName(), crfVersion.getName(), studySubjectBean.getOid(), studyEventDataBean.getStudyEventRepeatKey());
    ...
    private void attachValidator(DisplayItemBean displayItemBean, ImportHelper importHelper, DiscrepancyValidator v, HashMap<String, String> hardv,
        ...
        // need to add a per-subject differentator, what if its a diff subject with the same item, repeat key and ordinal???
     ...
     * 'equals'. and a few other switches...also need to keep in mind that there are non-null values that need to be taken into account
     */
    private String matchValueWithManyOptions(DisplayItemBean displayItemBean, String value, List options) {
        ...
        if (!options.isEmpty()) {
            ...
            // now getting set properly, method is line 241 e-d-crf-bean tbh
    ...
    public List<String> validateStudyMetadata(ODMContainer odmContainer, int currentStudyId) {
        ...
        try {
            ...
            if (subjectDataBeans != null) {// need to do this so as not to
                ...
                for (SubjectDataBean subjectDataBean : subjectDataBeans) {
                    ...
                    if (studySubjectBean == null) {
                        ...
                        logger.info("logged an error with subject oid " + oid);
                    }
                    ...
                    if (studyEventDataBeans != null) {
                        for (StudyEventDataBean studyEventDataBean : studyEventDataBeans) {
                            ...
                            StudyEventDefinitionBean studyEventDefintionBean =
                                studyEventDefinitionDAO.findByOidAndStudy(sedOid, studyBean.getId(), studyBean.getParentStudyId());
                            if (studyEventDefintionBean == null) {
                                ...
                                // Event in the Study.");
                                logger.info("logged an error with se oid " + sedOid + " and subject oid " + oid);
                            }
                                    ...
                                    if (crfVersionBeans != null) {
                                        for (CRFVersionBean crfVersionBean : crfVersionBeans) {
                                            if (crfVersionBean == null) {
                                                ...
                                                // Study Event.");
                                                logger.info("logged an error with form " + formOid + " and se oid " + sedOid);
                                            }
                                        }
                                    } else {
            ...
            logger.info("found an openclinica exception here");
        } catch (NullPointerException npe) {
            logger.info("found a nullpointer here");
        }

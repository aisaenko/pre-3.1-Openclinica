...
public abstract class DataEntryServlet extends SecureController {
    ...
    protected void processRequest() throws Exception {
        ...
        if (!fp.getString(GO_EXIT).equals("")) {
            ...
            logger.info("try to remove to_create_crf33333");
        ...
        if (!isSubmitted) {
            ...
            if (!errors.isEmpty()) {
                    ...
                    if (diwb.isInGroup()) {
                        ...
                        for (int j = 0; j < dgbs.size(); j++) {
                            ...
                            for (DisplayItemBean displayItem : items) {
                                ...
                                System.out.println("1062: group item value:" + displayItem.getData().getValue() + ", edit flag " + displayItem.getEditFlag());
                        ...
                        for (int j = 0; j < dbGroups.size(); j++) {
                            ...
                            if ("remove".equalsIgnoreCase(displayGroup.getEditFlag())) {
                                ...
                                for (DisplayItemBean displayItem : items) {
                                    displayItem.setEditFlag(displayGroup.getEditFlag());
                                    System.out.println("1076: set edit flag to: " + displayItem.getEditFlag() + ", value " + displayItem.getData().getValue());
                ...
                if (!success) {
                    ...
                    logger.info("try to remove to_create_crf44444");
                    session.removeAttribute("to_create_crf");
                    ...
                    if (!forwardingSucceeded) {
                        ...
                        if (markSuccessfully) {
                            ...
                            session.removeAttribute(HAS_DATA_FLAG);
                            session.removeAttribute(DDE_PROGESS);
                            logger.info("try to remove to_create_crf11111");
                            session.removeAttribute("to_create_crf");
                            ...
                            if (!section.isLastSection()) {
                                ...
                                session.removeAttribute(HAS_DATA_FLAG);
                                session.removeAttribute(DDE_PROGESS);
                                logger.info("try to remove to_create_crf22222");
                                session.removeAttribute("to_create_crf");
        ...
        if (ecb == null) {
            ...
            if (eventCRFId > 0) {
                ...
                try {
                    ...
                    logger.info("Initial: to create an event CRF.");
    ...
    protected List<DisplayItemGroupBean> loadFormValueForItemGroup(DisplayItemGroupBean digb, List<DisplayItemGroupBean> dbGroups,
        ...
        for (int i = 0; i < repeatMax; i++) {
            ...
            List<DisplayItemBean> dibs = FormBeanUtil.getDisplayBeansFromItems(itBeans, sm.getDataSource(), ecb.getCRFVersionId(), sb.getId(), nullValuesList);
            // System.out.println("+++count for dibs: " + dibs.size());
            two = System.currentTimeMillis() - timeCheck;
        ...
        System.out.println("1707: manual rows" + manualRows + "formGroup size" + formGroups.size());
        ...
        for (int j = 0; j < formGroups.size(); j++) {
            ...
            if (formItemGroup.isAuto() && formItemGroup.getFormInputOrdinal() > 0) {
                ...
                logger.info("formInputOrdinal(): " + formItemGroup.getFormInputOrdinal());
                logger.info("Ordinal(): " + formItemGroup.getOrdinal());
                ...
                // logger.info("setting ordinal to: " + formItemGroup.getFormInputOrdinal() + manualRows);
            }
        }
        ...
        for (int j = 0; j < formGroups.size(); j++) {
            ...
            if (formItemGroup.getOrdinal() == previous) {
                System.out.println("found a match btw previous and ordinal");
            ...
            System.out.println("setting previous to " + previous);
        }

        System.out.println("+++ DB group row:" + dbGroups.size());

        logger.info("+++ DB group contents: " + dbGroups.toString());
        System.out.println("+++ DB group contents: " + dbGroups.toString());
        ...
        for (int i = 0; i < dbGroups.size(); i++) {
            ...
            if (!"edit".equalsIgnoreCase(dbItemGroup.getEditFlag()) && !"initial".equalsIgnoreCase(dbItemGroup.getEditFlag())) {
                ...
                System.out.println("+++ one row removed, edit flag was " + dbItemGroup.getEditFlag());
    ...
    protected DisplaySectionBean getDisplayBean(boolean hasGroup, boolean includeUngroupedItems) throws Exception {
        ...
        edcdao = new EventDefinitionCRFDAO(sm.getDataSource());
        // I'm not sure we do need this findByStudyEventIdAndCRFVersionId or not 
        //EventDefinitionCRFBean edcbTmp = edcdao.findByStudyEventIdAndCRFVersionId(ecb.getStudyEventId(), cvb.getId());
        //if(edcbTmp.getId()==0){
        section.setEventDefinitionCRF(edcb);
        //}else{
        //  section.setEventDefinitionCRF(edcbTmp);
        //}

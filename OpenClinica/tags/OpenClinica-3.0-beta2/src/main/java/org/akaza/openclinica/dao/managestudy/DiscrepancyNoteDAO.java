...
public class DiscrepancyNoteDAO extends AuditableEntityDAO {
    ...
    public Integer getViewNotesCountWithFilter(ListNotesFilter filter, StudyBean currentStudy) {
        ...
        String sql = "select count(all_dn.discrepancy_note_id) from (";
    ...
    public Integer countAllItemDataByStudyAndUser(StudyBean study, UserAccountBean user) {
        ...
        variables.put(new Integer(1), new Integer(study.getId()));
        variables.put(new Integer(2), new Integer(study.getId()));
        variables.put(new Integer(3), new Integer(user.getId()));
    ...
    public EntityBean create(EntityBean eb) {
        ...
        if (sb.getAssignedUserId() == 0) {
            ...
            variables.put(new Integer(9), new Integer(sb.getAssignedUserId()));
        }
        // variables.put(new Integer(9), new Integer(sb.getAssignedUserId()));
    ...
    public ArrayList findAllByStudyEventWithConstraints(StudyEventBean studyEvent, StringBuffer constraints, boolean isSite) {
        this.setTypesExpected();
        ArrayList answer = new ArrayList();
        HashMap variables = new HashMap();
        variables.put(new Integer(1), new Integer(studyEvent.getId()));
        ...
        if ("oracle".equalsIgnoreCase(SQLInitServlet.getDBName())) {
            sql =
                "select ec.event_crf_id from event_crf ec, study_event se, crf_version cv, " + "(select edc.study_event_definition_id, edc.crf_id, crf.name "
                    ...
                    + "            and edc.status_id=1 and edc.hide_crf = 'true' and edc.crf_id = crf.crf_id) sedc "

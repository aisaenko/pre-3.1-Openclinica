...
public abstract class ListStudySubjectServlet extends SecureController {
    ...
    public void processRequest() throws Exception {
        ...
        //Set a subject property to determine whether to show a signed-type icon (electronic signature)
        //in the JSP view or not
        //Get all event crfs by studyevent id; then use EventDefinitionCRFDAO.isRequired to
        ...
        for(DisplayStudySubjectBean subject : displayStudySubs) {
            for(StudyEventBean event : subject.getStudyEvents()){
                if(event.getSubjectEventStatus() != null && event.getSubjectEventStatus().getId() == 3) {
                    ...
                    //determine whether the subject has any required, uncompleted event CRFs
        ...
        //new
        columnArray.add("secondary label");
        for (int i = 0; i < studyGroupClasses.size(); i++) {
            StudyGroupClassBean sgc = (StudyGroupClassBean) studyGroupClasses.get(i);
    ...
    private boolean eventHasRequiredUncompleteCRFs(StudyEventBean studyEvent) {
        ...
        if(studyEvent == null) return false;

        EventCRFDAO  eventCRFDAO = new  EventCRFDAO(sm.getDataSource());
        EventDefinitionCRFDAO  eventDefinitionDAO  = new
        ...
        //If the EventCRFBean has a completionID of 1 (indicating that it is not complete), then find
        //out whether it's required. If so, then return from the method false.
        for(EventCRFBean crfBean : crfBeans){
            if(crfBean != null && crfBean.getCompletionStatusId() == 1) {
                if(eventDefinitionDAO.isRequiredInDefinition(crfBean.getCRFVersionId(),
                  crfBean.getStudyEventId())){
                    return true;
                }
            }
        }
    ...
    public static DisplayStudyEventBean getDisplayStudyEventsForStudySubject(StudySubjectBean studySub, StudyEventBean event, DataSource ds,
                                                                             UserAccountBean ub, StudyUserRoleBean currentRole) {

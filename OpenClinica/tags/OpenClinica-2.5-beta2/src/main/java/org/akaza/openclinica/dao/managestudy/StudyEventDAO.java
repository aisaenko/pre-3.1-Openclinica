...
public class StudyEventDAO extends AuditableEntityDAO {
    ...
    public void setCRFTypesExpected() {
        /*
         * <sql>SELECT C.CRF_ID, C.STATUS_ID, C.NAME, C.DESCRIPTION, V.CRF_VERSION_ID, V.NAME, V.REVISION_NOTES FROM CRF C, CRF_VERSION V, EVENT_DEFINITION_CRF
         * EDC WHERE C.CRF_ID = V.CRF_ID AND EDC.CRF_ID = C.CRF_ID AND EDC.STUDY_EVENT_DEFINITION_ID =? </sql>
         */
        this.unsetTypeExpected();
        this.setTypeExpected(1, TypeNames.INT);
        this.setTypeExpected(2, TypeNames.INT);
    ...
    public ArrayList findAllByStudyEventDefinitionAndCrfOids(String studyEventDefinitionOid, String crfOrCrfVersionOid) {
        ...
        variables.put(new Integer(1), studyEventDefinitionOid);
        variables.put(new Integer(2), crfOrCrfVersionOid);
        variables.put(new Integer(3), crfOrCrfVersionOid);
    ...
    public StudyEventBean findAllByStudyEventDefinitionAndCrfOidsAndOrdinal(String studyEventDefinitionOid, String crfOrCrfVersionOid, String ordinal,
        ...
        HashMap<Integer, Object> variables = new HashMap<Integer, Object>();
        variables.put(new Integer(1), studyEventDefinitionOid);
        variables.put(new Integer(2), Integer.valueOf(studySubjectId));
        variables.put(new Integer(3), Integer.valueOf(ordinal));
        variables.put(new Integer(4), crfOrCrfVersionOid);
        variables.put(new Integer(5), crfOrCrfVersionOid);
    ...
    public EntityBean findByStudySubjectIdAndDefinitionIdAndOrdinal(int ssbid, int sedid, int ord) {
        this.setTypesExpected();
        HashMap variables = new HashMap();
        variables.put(new Integer(1), new Integer(ssbid));
        variables.put(new Integer(2), new Integer(sedid));
        variables.put(new Integer(3), new Integer(ord));
    ...
    public EntityBean create(EntityBean eb) {
        ...
        if (sb.getDateStarted() == null) {
            ...
            nullVars.put(new Integer(5), new Integer(TypeNames.TIMESTAMP));
            variables.put(new Integer(5), null);
        } else {
            // YW 08-16-2007 << data type changed from DATE to TIMESTAMP
            variables.put(new Integer(5), new Timestamp(sb.getDateStarted().getTime()));
        }
        if (sb.getDateEnded() == null) {
            // YW 08-16-2007 << data type changed from DATE to TIMESTAMP
            nullVars.put(new Integer(6), new Integer(TypeNames.TIMESTAMP));
            variables.put(new Integer(6), null);
        } else {
            // YW 08-16-2007 << data type changed from DATE to TIMESTAMP
            variables.put(new Integer(6), new Timestamp(sb.getDateEnded().getTime()));
        }
        ...
        variables.put(new Integer(10), sb.getStartTimeFlag());
        variables.put(new Integer(11), sb.getEndTimeFlag());
    ...
    public EntityBean update(EntityBean eb) {
        ...
        variables.put(new Integer(11), sb.getStartTimeFlag()); // YW
        // 08-17-2007,
        // start_time_flag
        variables.put(new Integer(12), sb.getEndTimeFlag()); // YW
        // 08-17-2007,
        // end_time_flag
        variables.put(new Integer(13), new Integer(sb.getId()));
    ...
    /**
     * Get the maximum sample ordinal over all study events for the provided StudyEventDefinition / StudySubject combination. Note that the maximum may be zero
    ...
    /**
     * Using the HashMaps returned from a <code>select</code> call in findCRFsByStudy, prepare a HashMap whose keys are study event definitions and whose
     * values are ArrayLists of CRF versions included in those definitions.
     * 
     * @param rows
     *            The HashMaps retured by the <code>select</code> call in findCRFsByStudy.
     * @return a HashMap whose keys are study event definitions and whose values are ArrayLists of CRF versions included in those definitions. Both the keys of
     *         the HashMap and the elements of the ArrayLists are actually EntitBeans.
     */
    ...
    // similar
    /*
     * ssachs - this is meant for use with getEventsAndMultipleCRFVersionInformation; in particular the column names "study_event_name", "crf_name" and
     * "crf_version_name" should be maintained if the SQL changes SELECT SED.name AS study_event_name , SED.study_event_definition_id , C.name AS crf_name ,
     * CV.name AS crf_version_name , CV.crf_version_id FROM study_event_definition SED , event_definition_crf EDC , crf C , crf_version CV WHERE SED.study_id = ?
     * AND SED.study_event_definition_id = EDC.study_event_definition_id AND C.crf_id = EDC.crf_id AND C.crf_id = CV.crf_id
     */

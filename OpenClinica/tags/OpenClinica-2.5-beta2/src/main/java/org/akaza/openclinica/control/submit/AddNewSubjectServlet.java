        ...
        if (!fp.isSubmitted()) {
            ...
            if (!errors.isEmpty()) {
                ...
                if (fp.getBoolean("addWithEvent")) {
                    ...
                    response.sendRedirect(response.encodeRedirectURL("ViewStudySubject?id=" + studySubject.getId()));
                    return;
    ...
    public static void saveFieldNotes(String field, FormDiscrepancyNotes notes, DiscrepancyNoteDAO dndao, int entityId, String entityType, StudyBean sb) {
        ...
        for (int i = 0; i < fieldNotes.size(); i++) {
            ...
            if ("itemData".equalsIgnoreCase(entityType)) {              
              ...
              ArrayList parentNotes = (ArrayList)dndao.findExistingNotesForItemData(dnb.getEntityId());

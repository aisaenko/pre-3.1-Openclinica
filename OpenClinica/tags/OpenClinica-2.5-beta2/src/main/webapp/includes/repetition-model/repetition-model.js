...
//RELATED TO OpenClinica 2.2=5 functionality: SEE LINES 942-50, 303-10 AND 1241
if(!window.RepetitionElement || (
  ...
  //Element addRepetitionBlock(in Node refNode);
    addRepetitionBlock : function(refNode, index){ //addRepetitionBlockByIndex functionalty enabled if @index defined
      ...
      //LINES 942-50 ARE NOT PART OF THE DISTRIBUTED LIBRARY:
      //Clear the new row's input values, so that the elements in the new rows
      //do not display any values
      if(global_addButtonClicked) clearInputElementValues(block);  //added for functions in OpenClinica 2.2
      //18. The return value is the newly cloned element.
     //"block" is an HTMLTableRowElement object
     var blockTR = block.parentNode.parentNode.childNodes[1].childNodes[1];
  var isIE = detectIEWindows(navigator.userAgent);

      if(isIE) {
          blockTR = block.parentNode.parentNode.childNodes[0].childNodes[0];
      }
          var thNode = document.createElement("TH");
          var tdNode = document.createElement("TD");
          var strNode = document.createElement("STRONG");
          var txtNode = document.createTextNode(""+this.repetitionIndex);
          strNode.appendChild(txtNode);
          thNode.style.background="#ECECEC";
          thNode.style.padding="0.6em";
          thNode.style.border="2px solid #b0b0b0";
          tdNode.style.padding="0.2em";
          tdNode.style.border="2px solid #b0b0b0";
          tdNode.appendChild(strNode);
          if(this.repetitionIndex == 1) {
              blockTR.insertBefore(thNode,blockTR.firstChild);
          }
          block.insertBefore(tdNode,block.firstChild);

     /* if((! isIE) && this.repetitionIndex == 1) {
          blockTR.innerHTML="<th class='aka_headerBackground aka_padding_large aka_cellBorders'></th>"+blockTR.innerHTML;
      }
      if(! isIE){
          block.innerHTML='<td class="aka_padding_norm aka_cellBorders"><strong>'+ (this.repetitionIndex)+'</strong></td>'+block.innerHTML;                    
      }*/
      return block;
    },
      ...
      for(var i = 0; i < btns.length; i++){
        for(var t, j = 0; t = repetitionTemplates[j]; j++){
          if(btns[i].htmlTemplate == t && t.repetitionBlocks.length >= t.repeatMax){
            ...
            //line 1221, changeBtnDisabledState() METHOD DEFINED OUTSIDE OF THIS LIBRARY, IN

...
public class AddNewSubjectServlet extends SecureController {
  ...
  protected void processRequest() throws Exception {
    ...
    //TODO l10n for dates?  Note that in some places we hard-code the YOB by using "01/01/"+yob,
	...
	StudyParameterValueBean parentSPV = 
		(StudyParameterValueBean)spvdao.findByHandleAndStudy(parentStudyId, "collectDob");
	...
	//YW >>
	//tbh
	StudyParameterValueBean checkPersonId =
		(StudyParameterValueBean)spvdao.findByHandleAndStudy(parentStudyId, "subjectPersonIdRequired");
    ...
    if (!fp.isSubmitted()) {
      ...
      //YW << record parameters' values on input page so those values could be used to compare against
      ...
      if (dobSetting.equals("1")) {// date of birth
        ...
        v.addValidation(INPUT_YOB, 
        		Validator.COMPARES_TO_STATIC_VALUE, 
      ...
      if (!uniqueIdentifier.equals("")) {
        ...
        //tbh
//        if (currentStudy.getParentStudyId() > 0) {
//        	subjectWithSameIdInParent = sdao.findByUniqueIdentifierAndStudy(
//        			uniqueIdentifier, 
//        			currentStudy.getParentStudyId());
//        	logger.info("check parent..."+currentStudy.getParentStudyId());
//        }
        //tbh
        if (subjectWithSameId.isActive()) { //|| subjectWithSameIdInParent.isActive()) {
          ...
          if (subjectWithSameId.isActive()) {
            ...
            if (subjectWithSameId.isActive()) {
              Validator.addError(errors, INPUT_UNIQUE_IDENTIFIER,
            		  resexception.getString("this_subject_with_person_ID") + uniqueIdentifier + resexception.getString("has_already_enrolled_parent_study"));

              subjectWithSameIdInCurrentStudyTree = true;
            } else {
            	//YW 11-26-2007 << check whether there is a subject with the same id in other sites of the same study
            	 subjectWithSameId = sdao.findByUniqueIdentifierAndParentStudy(uniqueIdentifier, 
                         currentStudy.getParentStudyId());
                 if (subjectWithSameId.isActive()) {
                   Validator.addError(errors, INPUT_UNIQUE_IDENTIFIER,
                 		  resexception.getString("this_subject_with_person_ID") + uniqueIdentifier + resexception.getString("has_already_enrolled_site_study"));
        ...
        for (int i=0; i<classes.size();i++) {
           ...
           if (notes.trim().length() > 255) {
             Validator.addError(errors, "notes" + i,
            	resexception.getString("notes_cannot_longer_255"));
           }
      ...
      if (!errors.isEmpty()) {
		...
		if(currentStudy.getStudyParameterConfig().getCollectDob().equals("1")) {
			...
			if(subjectWithSameId.isDobCollected() == false ) {
				...
				if(!y.equals("0001")) {
					//if year-of-birth is different from DOB's year, use year-of-birth
					if(!y.equals(d[2])) {
			...
			else if(!sdf.format(subjectWithSameId.getDateOfBirth()).toString().equals(DOB)) {
		...
		//current Study require YOB
			...
			//year of date-of-birth in the subject talbe is not availbe, YOB is used
			else {
				needUpdate = true;
				updateSubject = subjectWithSameId;
		...
		//current study require no DOB, there is no need to check date-of-birth in the subject table
		else {
			fp.addPresetValue(Y_WARNING, "false");
		}	
	...
	//YW >>  
         ...
         //YW << If showExistingRecord, which means there is a record for the subject
         //      in <subject> table, the subject only needs to be inserted into <studysubject> table.
         ...
         //YW >>	 
	         if (!StringUtil.isBlank(fp.getString(INPUT_GENDER))) {
	          ...
	          if (currentStudy.getStudyParameterConfig().getCollectDob().equals("1")) {
	            ...
	            // valid
	        	  //added the "2" to make sure that 'not used' is kept to null, tbh 102007
	            subject.setDobCollected(false);
	            ...
	            //String dobString = "01/01/" + yob;
                    String dobString = local_df.format(fakeCal.getTime());
	     ...
	     //      If input value(s) is(are) different from database, warning will be shown.
	     //      If value(s) in database is(are) empty, entered value(s) could be used;
	     //      Otherwise, value(s) in database will be used.
	     //      For date-of-birth, if database only has year-of-birth, the year in database will be used for year part
  ...
  public static void saveFieldNotes(String field, FormDiscrepancyNotes notes,
                                    DiscrepancyNoteDAO dndao, int entityId,
                                    String entityType, StudyBean sb) {

    if(notes == null || dndao == null || sb == null) {
      //BWP>> log a message here about the failed operation? using what Logger?
      return;
    }

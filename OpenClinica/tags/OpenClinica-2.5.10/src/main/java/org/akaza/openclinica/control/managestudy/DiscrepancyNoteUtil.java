...
public class DiscrepancyNoteUtil {
    ...
    public List<DiscrepancyNoteBean> getThreadedDNotesForStudy(StudyBean currentStudy, Set<Integer> resolutionStatusIds, DataSource dataSource,
        ...
        if (parentStudyId > 0) {
            ...
            ArrayList theParentItemDataNotes = new ArrayList();
            for (DiscrepancyNoteBean discNote : (ArrayList<DiscrepancyNoteBean>) parentItemDataNotes) {
                ItemDataBean idb = (ItemDataBean) iddao.findByPK(discNote.getEntityId());
                ...
                if (discNote.getStudyId() == parentStudyId && ssub.getStudyId() == currentStudy.getId()) {
                    theParentItemDataNotes.add(discNote);
                }
            }
            itemDataNotes.addAll(theParentItemDataNotes);
        }

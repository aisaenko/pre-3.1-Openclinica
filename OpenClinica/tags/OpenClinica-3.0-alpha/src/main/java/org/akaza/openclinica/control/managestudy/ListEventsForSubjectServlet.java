    ...
    public void processRequest() throws Exception {
        ...
        for (int i = 0; i < subjects.size(); i++) {
            ...
            for (int k = 0; k < displayEvents.size(); k++) {
                ...
                //Issue 3212 BWP  <<
                if(currentStudy.getParentStudyId() > 0){
                    //check each eventDefCRFBean and set its isHidden property to true, if its
                    //persistent/database-derived hideCrf is true (domain rule: hide the CRF from users logged into a site)
                    HideCRFManager hideCRFManager = HideCRFManager.createHideCRFManager();
                    hideCRFManager.optionallyCheckHideCRFProperty(dseb);

                    //remove a DisplayEventCRFBean from a DisplayStudyEventBean in the list
                    //  if it contains a hidden CRF in its event definition
                    hideCRFManager.removeHiddenEventCRF(dseb);

                    //generate a boolean request attribute indicating whether any of the CRFs
                    //should be hidden
        ...
        ArrayList allStudyRows = DisplayStudySubjectEventsRow.

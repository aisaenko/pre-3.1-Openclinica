 ...
 * user logged in at the site level cannot view/use CRFs that are marked as "hidden."
 * User: bruceperry
 * Date: Feb 9, 2009
 */
public class HideCRFManager {
    ...
    public void removeHiddenEventCRF(DisplayStudyEventBean displayStudyEventBean){
        ...
        displayStudyEventBean.setAllEventCRFs(
          suppressHiddenEventCRFBeans(eventCRFBeans));

        eventCRFBeans = displayStudyEventBean.getDisplayEventCRFs();

        displayStudyEventBean.setDisplayEventCRFs(
          suppressHiddenEventCRFBeans(eventCRFBeans));

        List<DisplayEventDefinitionCRFBean>  displayEventDefinitionCRFBeans =
          displayStudyEventBean.getUncompletedCRFs();
    ...
    private ArrayList<DisplayEventCRFBean> suppressHiddenEventCRFBeans(
      List<DisplayEventCRFBean> eventCRFBeans){

        //Must make this an ArrayList because of the displayStudyEventBean.setAllEventCRFs()
        //method definition
    ...
    public ArrayList<DisplayEventCRFBean> removeHiddenEventCRFBeans(List<DisplayEventCRFBean>
      displayEventCRFBeans){

        ArrayList<DisplayEventCRFBean> newDisplayEventCRFBeans =
          new ArrayList<DisplayEventCRFBean>();
    ...
    public ArrayList<DisplayEventDefinitionCRFBean> removeHiddenEventDefinitionCRFBeans(
      List<DisplayEventDefinitionCRFBean>
        displayEventDefinitionCRFBeans){

        ArrayList<DisplayEventDefinitionCRFBean> newDisplayEventDefinitionCRFBeans =
          new ArrayList<DisplayEventDefinitionCRFBean>();

        EventDefinitionCRFBean eventDefinitionCRFBean = new EventDefinitionCRFBean();

        for(DisplayEventDefinitionCRFBean displayEventDefinitionCRFBean :
          displayEventDefinitionCRFBeans){
    ...
    public void optionallyCheckHideCRFProperty(
    ...
    public boolean studyEventHasAHideCRFProperty(
      DisplayStudyEventBean displayStudyEventBean) {
    ...
    public void hideSpecifiedEventCRFDefBeans(
      List<EventDefinitionCRFBean> eventDefinitionCRFBeans){
    ...
    /**
     * Given a StudyEventDefinitionBean and a list of CRFBeans as parameters, this method
     * returns of new list of CRFBeans containing only unhidden CRFs (their isHideCRF property
     * is false).
     * @param studyEventBean The StudyEventDefinitionBean
     * @param crfBeans  An ArrayList of CRFBeans
     * @param dataSource A javax.sql.DataSource for the use of the EventDefinitionCRFDAO.
     * @return  An ArrayList of CRFBeans containing only "unhidden" CRFs.
     */
    public ArrayList<CRFBean> removeHiddenCRFBeans(StudyEventDefinitionBean studyEventBean,
        ...
        for(CRFBean crfBean : crfBeans) {
            tempBean = eventDefinitionCRFDAO.findByStudyEventDefinitionIdAndCRFId(studyEventBean.getId(),
              crfBean.getId());

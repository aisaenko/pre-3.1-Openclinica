...
public class OdmExtractDAO extends DatasetDAO {
    ...
    public void setSubjectEventFormDataTypesExpected(String odmVersion) {
        if ("1.2".equals(odmVersion) || "1.3".equals(odmVersion)) {
            ...
            this.setTypeExpected(8, TypeNames.INT);// study_group_class_id
            this.setTypeExpected(9, TypeNames.STRING);// study_group_class_name
            this.setTypeExpected(10, TypeNames.STRING);// study_group_name
            ...
            this.setTypeExpected(24, TypeNames.INT);// crf_status_id
            this.setTypeExpected(25, TypeNames.INT);// event_crf_id
            this.setTypeExpected(26, TypeNames.DATE);// date_interviewed
            this.setTypeExpected(27, TypeNames.STRING);// interviewer_name
        }
    }
    ...
    public HashSet<Integer> getMetadata(int studyId, MetaDataVersionBean metadata, String odmVersion) {
        ...
        while (it.hasNext()) {
            ...
            if (!itdset.contains(itId)) {
                ...
                idef.setSignificantDigits(sig > 0 ? sig : MetaDataCollector.getSignificantDigits(datatype, codes.keySet(), hasCode));
                itemDefs.add(idef);
            }
        ...
        if (odmVersion.startsWith("oc")) {
            ...
            // System.out.println("getStudyGroupClassSql=" +
            // getStudyGroupClassSql(studyId));
            rows = select(this.getStudyGroupClassSql(studyId));
            it = rows.iterator();
    ...
    protected String getStudyEventAndFormMetaSql(int studyId) {
        return "select sed.ordinal as definition_order, edc.ordinal as crf_order, edc.crf_id, cv.crf_version_id,"
            ...
            + " and edc.status_id = 1 and edc.crf_id = crf.crf_id and crf.status_id = 1 and crf.crf_id = cv.crf_id and (cv.status_id = 1 or cv.status_id = 6)"
    ...
    protected String getOCSubjectEventFormSql(String studyIds, String sedIds, String itemIds, String dateConstraint) {
        return "select ss.oc_oid as study_subject_oid, ss.label, ss.unique_identifier, ss.secondary_label, ss.gender, ss.date_of_birth,"
            ...
            + " cv.oc_oid as crf_version_oid, cv.name as crf_version, ec.status_id as crf_status_id, ec.event_crf_id, ec.date_interviewed,"
            + " ec.interviewer_name from (select study_event_id, study_event_definition_id, study_subject_id, \"location\" as se_location,"
            ...
            + getEventCrfIdsByItemDataSql(studyIds, sedIds, itemIds, dateConstraint)
            + ")) ec, crf_version cv"

...
public class ExportDatasetServlet extends SecureController {
    ...
    public void processRequest() throws Exception {
        ...
        GenerateExtractFileService generateFileService = 
        	new GenerateExtractFileService(sm.getDataSource(), request, sm.getUserBean());
...
//        eb.setDateCreated(new java.util.Date());
        ...
        if (StringUtil.isBlank(action)) {
            ...
            if ("sas".equalsIgnoreCase(action)) {
                ...
                generateFileService.createFile(SASFileName, generalFileDir, generateReport, db, sysTimeEnd, ExportFormatBean.TXTFILE);
                ...
                // the above gets us the best of both worlds - the file name, together with the file id which we can then
                // push out to the browser.  Shame that it is a long hack, though.  need to pare it down later, tbh
                // and of course DRY
                for (Iterator it=answerMap.entrySet().iterator(); it.hasNext();)  {
                	java.util.Map.Entry entry = (java.util.Map.Entry)it.next();
                	Object key = entry.getKey();
            	...
            	// removed three lines here and put them in generate file service, createSPSSFile method. tbh 01/2009
            	eb = dsdao.getDatasetData(eb, currentstudyid, parentstudy);

                eb.getMetadata();

                eb.computeReport(answer);
                // System.out.println("*** isShowCRFversion: "+db.isShowCRFversion());
                
            	// TODO in the spirit of DRY, if this works we need to remove lines 443-776 in this servlet, tbh 01/2009
            	String DDLFileName = "";
            	HashMap answerMap = generateFileService.createSPSSFile(db, 
            			eb, 
            			currentStudy, 
            			parentStudy, 
            			sysTimeBegin, 
            			generalFileDir, answer);
                //String DDLFileName = createSPSSFile(db, eb, currentstudyid, parentstudy);
                /*
                ...
                fId = generateFileService.createFile(CSVFileName, generalFileDir, answer.toString(), db, sysTimeEnd, ExportFormatBean.CSVFILE);

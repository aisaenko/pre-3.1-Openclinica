...
public class EnterDataForStudyEventServlet extends SecureController {
    ...
    //The study event has an existing discrepancy note related to its location property; this
    //value will be saved as a request attribute
    public final static String HAS_LOCATION_NOTE = "hasLocationNote";
    //The study event has an existing discrepancy note related to its start date property; this
    //value will be saved as a request attribute
    public final static String HAS_START_DATE_NOTE = "hasStartDateNote";
    //The study event has an existing discrepancy note related to its end date property; this
    //value will be saved as a request attribute
    ...
    private StudyEventBean getStudyEvent(int eventId) throws Exception {
        ...
        if (!aeb.isActive()) {
            ...
            throw new InsufficientPermissionException(Page.SUBMIT_DATA_SERVLET, resexception.getString("study_event_not_belong_study"), "1");
        }
    ...
    protected void processRequest() throws Exception {
        ...
        if(subjectStudyIsCurrentStudy && isParentStudy) {
            ...
            if(! isParentStudy) {
                ...
                allNotesforSubjectAndEvent = discrepancyNoteDAO.findAllStudyEventByStudiesAndSubjectId(
                  stParent,study,studySubjectBean.getId());
            }  else {

                 allNotesforSubjectAndEvent = discrepancyNoteDAO.findAllStudyEventByStudiesAndSubjectId(
                  currentStudy,study,studySubjectBean.getId());
        ...
        ArrayList eventDefinitionCRFs = edcdao.findAllActiveByEventDefinitionId(seb.getStudyEventDefinitionId());
        ...
        ArrayList uncompletedEventDefinitionCRFs =
          getUncompletedCRFs(eventDefinitionCRFs, eventCRFs);
        populateUncompletedCRFsWithCRFAndVersions(uncompletedEventDefinitionCRFs);

        //BWP 2816 << Attempt to provide the DisplayEventDefinitionCRF with a valid owner
        ...
        ArrayList displayEventCRFs =
          ViewStudySubjectServlet.getDisplayEventCRFs(sm.getDataSource(),
            eventCRFs, eventDefinitionCRFs, ub, currentRole, seb.getSubjectEventStatus());
       ...
       //>>
        ...
        ArrayList beans = ViewStudySubjectServlet.
          getDisplayStudyEventsForStudySubject(studySubjectBean, sm.getDataSource(), ub, currentRole);
    ...
    private void populateUncompletedCRFsWithAnOwner(List<DisplayEventDefinitionCRFBean>
      displayEventDefinitionCRFBeans){
        ...
        for(DisplayEventDefinitionCRFBean dedcBean : displayEventDefinitionCRFBeans){
            ...
            if(eventCRFBean != null && eventCRFBean.getOwner() == null &&
              eventCRFBean.getOwnerId() > 0) {
                userAccountBean = (UserAccountBean)
                  userAccountDAO.findByPK(eventCRFBean.getOwnerId());
            ...
            if(eventCRFBean != null && eventCRFBean.getOwner() == null){
                ...
                if(ownerId > 0) {
                    userAccountBean = (UserAccountBean)
                      userAccountDAO.findByPK(ownerId);
    ...
    /**
     * If DiscrepancyNoteBeans have a certain column value, then set flags that a JSP will
     * check in the request attribute. This is a convenience method called by the processRequest()
     * method.

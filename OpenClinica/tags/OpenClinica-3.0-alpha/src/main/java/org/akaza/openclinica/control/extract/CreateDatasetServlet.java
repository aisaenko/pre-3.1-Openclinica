...
public class CreateDatasetServlet extends SecureController {
        ...
        if (StringUtil.isBlank(action)) {
            ...
            if ("begin".equalsIgnoreCase(action)) {
                ...
                for (int i = 0; i < seds.size(); i++) {
                    ...
                    crfs = hideCRFs.removeHiddenCRFBeans(sed,crfs,sm.getDataSource());
                    if (!crfs.isEmpty()) {
                        events.put(sed, crfs);
                    }
                }
    ...
    private String genAttMsg(DatasetBean db) {
        ...
        if (db.isShowSubjectDob() || db.isShowSubjectGender() || db.isShowSubjectAgeAtEvent() ||
          db.isShowSubjectStatus() || db.isShowSubjectUniqueIdentifier() || db.isShowSubjectSecondaryId()) {

...
public class CreateDiscrepancyNoteServlet extends SecureController {
    ...
    protected void processRequest() throws Exception {
        ...
        /*
         * ArrayList ress = ResolutionStatus.toArrayList(); for ( int i=0; i<ress.size(); i++) { ResolutionStatus s= (ResolutionStatus)ress.get(i);
        ...
        if (!fp.isSubmitted()) {
            ...
            if (parent.getId() == 0 || isNew) {// no parent, new note thread
                ...
                // if it is a CRC then we should automatically propose a solution, tbh
			...
			// else if (parent.getAssignedUserId() > 0) {
            // note.setAssignedUserId(parent.getAssignedUserId());
            // System.out.println("found user assigned id: " + parent.getAssignedUserId());
            // } 
			else {
                // a little bit of a workaround
                note.setAssignedUserId(parent.getOwnerId());
                System.out.println("found user assigned id, in the PARENT OWNER ID: " + parent.getOwnerId() + " note that user assgined id did not work: " + assignedUserAccountId);
            }
            ...
            // request.setAttribute(USER_ACCOUNT_ID, new
            // Integer(note.getAssignedUserId()));
            // above - wrong. should be poster id:
            // request.setAttribute(USER_ACCOUNT_ID, new Integer(ub.getId()));
            // }

            request.setAttribute(USER_ACCOUNT_ID, new Integer(ub.getId()).toString());
			// problem here - when we want to propose a soln as a CRC, we need to assign this back to the OP, not the user bean.
			// address it below, tbh
			...
			if (r.equals(Role.RESEARCHASSISTANT) && (currentStudy.getId() != currentStudy.getParentStudyId())) {
				System.out.println("go to this part where we set the attr to the original poster, tbh");
				request.setAttribute(USER_ACCOUNT_ID, new Integer(note.getOwnerId()).toString());
			}
            request.setAttribute(USER_ACCOUNTS, userAccounts);
                    ...
                    if ((note.getAssignedUserId() > 0) && ("1".equals(email.trim()))) {
                        ...
                        try {
                            ...
                            message.append("<P>Dear " + assignedUser.getFirstName() + " " + assignedUser.getLastName() + ",</P><P>"
                                + respage.getString("email_header_1") + " the OpenClinica System " + respage.getString("email_header_2")
                                + " discrepancy note " + note.getDescription() + " " + respage.getString("email_header_3") + "</P>");

                            message.append("<P>Name: " + note.getDescription() + "</P>");
                            message.append("<P>Notes: " + note.getDetailedNotes() + "</P>");
                            message.append("<P>Study Subject: " + note.getStudySub().getLabel() + "</P>");
                            // add more here later, tbh
                            ...
                            em.processHtml(alertEmail.trim(), EmailEngine.getAdminEmail(), "Discrepancy Note for " + note.getDescription(), emailBodyString);
                            System.out.println("sending email to " + alertEmail);
    ...
    private ArrayList generateUserAccounts(int studyId) {
		...
		// need to add the above two lines and grab all users by the parent study id, tbh 03/2009
        ArrayList userAccounts = userAccountDAO.findAllUsersByStudy(currentStudy.getParentStudyId());

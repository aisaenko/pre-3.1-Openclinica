        ...
        for (int i = 0; i < versions.size(); i++) {
            ...
            if (versionId != null && versionId.intValue() > 0) {
                ...
                if (igmdao.versionIncluded(versionId)) {
                    ItemFormMetadataBean imfBean = ifmdao.findByItemIdAndCRFVersionId(item.getId(), versionId.intValue());
                    ...
                    ItemFormMetadataBean imfBean = ifmdao.findByItemIdAndCRFVersionIdNotInIGM(item.getId(), versionId.intValue());
                    imfBean.setCrfVersionName(version.getName());
                    CRFBean crf = (CRFBean) cdao.findByPK(version.getCrfId());
                    imfBean.setCrfName(crf.getName());
                    versionItems.add(imfBean);

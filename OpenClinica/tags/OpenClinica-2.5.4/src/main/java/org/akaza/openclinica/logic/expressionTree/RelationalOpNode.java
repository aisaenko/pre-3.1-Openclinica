...
public class RelationalOpNode extends ExpressionNode {
    ...
    String testCalculate() throws OpenClinicaSystemException {
        double x, y;
        if (ExpressionTreeHelper.isDateyyyyMMdd(left.testValue()) && ExpressionTreeHelper.isDateyyyyMMdd(right.testValue())) {
    ...
    String calculate() throws OpenClinicaSystemException {
        double x, y;
        if (ExpressionTreeHelper.isDateyyyyMMdd(left.value()) && ExpressionTreeHelper.isDateyyyyMMdd(right.value())) {
    ...
    void validate() throws OpenClinicaSystemException {
        if (!(ExpressionTreeHelper.isDateyyyyMMdd(left.value()) && ExpressionTreeHelper.isDateyyyyMMdd(right.value()))
            && !isDouble(left.value(), right.value())) {

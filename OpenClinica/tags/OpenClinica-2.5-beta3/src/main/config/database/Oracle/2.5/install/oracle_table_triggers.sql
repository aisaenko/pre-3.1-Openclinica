...
begin
  --
  newrec.ITEM_DATA_ID := newrec.ITEM_DATA_ID;
  newrec.ITEM_ID      := newrec.ITEM_ID;
  newrec.EVENT_CRF_ID := newrec.EVENT_CRF_ID;
  newrec.STATUS_ID    := newrec.STATUS_ID;
  newrec.VALUE        := newrec.VALUE;
  newrec.DATE_CREATED := newrec.DATE_CREATED;
  newrec.DATE_UPDATED := newrec.DATE_UPDATED;
  newrec.OWNER_ID     := newrec.OWNER_ID;
  newrec.UPDATE_ID    := newrec.UPDATE_ID;
  newrec.ORDINAL      := newrec.ORDINAL; 
  --
  oldrec.ITEM_DATA_ID := oldrec.ITEM_DATA_ID;
  oldrec.ITEM_ID      := oldrec.ITEM_ID;
  oldrec.EVENT_CRF_ID := oldrec.EVENT_CRF_ID;
  oldrec.STATUS_ID    := oldrec.STATUS_ID;
  oldrec.VALUE        := oldrec.VALUE;
  oldrec.DATE_CREATED := oldrec.DATE_CREATED;
  oldrec.DATE_UPDATED := oldrec.DATE_UPDATED;
  oldrec.OWNER_ID     := oldrec.OWNER_ID;
  oldrec.UPDATE_ID    := oldrec.UPDATE_ID;
  oldrec.ORDINAL      := oldrec.ORDINAL; 
  --
  --
  if inserting then
    trg_operation := 'INSERT';

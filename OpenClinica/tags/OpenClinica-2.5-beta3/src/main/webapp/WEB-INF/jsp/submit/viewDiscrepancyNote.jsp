...
<tr>
  <td valign="bottom">
    <table border="0" cellpadding="0" cellspacing="0">
     ...
     </c:choose>  =  <c:out value="${entityValue}"/>             
           </b>
			</div></div></div>
        </td>
      </tr>
 ...
 <tbody>
   <tr>
	...
	<!-- Thread title -->
		...
		<div style="float: right; padding-left: 30px;"><b>Last updated:</b> <fmt:formatDate value="${note.value.createdDate}" pattern="${dteFormat}"/> by <c:out value="${note.value.owner.name}"/></div>
		</td>
	   </tr>
	   <tr class="aka_stripes">
		...
		<td class="aka_header_border" width="25%"><b><fmt:message key="of_notes" bundle="${resword}"/>:</b> <c:out value="${note.value.numChildren+1}" /></td>
	   </tr>
	</table>

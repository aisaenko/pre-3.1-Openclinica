...
import org.akaza.openclinica.core.form.ScoreItemValidator;
...
public abstract class DataEntryServlet extends SecureController {
    ...
    /**
     * The event definition CRF bean which governs the event CRF bean into which we are entering data.
     */
    ...
    protected void processRequest() throws Exception {
        ...
        if (!ecb.isActive()) {
            throw new InconsistentStateException(Page.SUBMIT_DATA_SERVLET, resexception.getString("event_not_exists"));
        }

        // let's get the status/number of item discrepancy notes
        try {
            dndao = new DiscrepancyNoteDAO(sm.getDataSource());
            ArrayList<DiscrepancyNoteBean> allNotes = dndao.findAllTopNotesByEventCRF(ecb.getId());
            ...
            for (int i = 0; i < allNotes.size(); i++) {
                DiscrepancyNoteBean dn = allNotes.get(i);
                ArrayList children = dndao.findAllByParent(dn);
                if (!children.isEmpty()) {
                    DiscrepancyNoteBean child = (DiscrepancyNoteBean) children.get(0);
                    dn.setResStatus(child.getResStatus());
                }
                if (dn != null) {
                    if (ResolutionStatus.UPDATED.equals(dn.getResStatus())) {
                        updatedNum++;
                    } else if (ResolutionStatus.OPEN.equals(dn.getResStatus())) {
                        openNum++;
                    } else if (ResolutionStatus.CLOSED.equals(dn.getResStatus())) {
                        // if (dn.getParentDnId() > 0) {
                        closedNum++;
                        // }
                    } else if (ResolutionStatus.RESOLVED.equals(dn.getResStatus())) {
                        // if (dn.getParentDnId() > 0) {
                        resolvedNum++;
                        // }
                    } else if (ResolutionStatus.NOT_APPLICABLE.equals(dn.getResStatus())) {
                        notAppNum++;
                    }
                }
        ...
        if (!fp.getString(GO_EXIT).equals("")) {
            ...
            System.out.println("try to remove to_create_crf33333");
        ...
        if (eventDefinitionCRFId <= 0) {
            ...
            EventDefinitionCRFBean edcBean = edcdao.findByStudyEventIdAndCRFVersionId(ecb.getStudyEventId(), ecb.getCRFVersionId());
        ...
        request.setAttribute("decryptedPassword", org.akaza.openclinica.core.SecurityManager.getInstance().encrytPassword("root"));
        ...
        if (!isSubmitted) {
            ...
            for (int i = 0; i < allItems.size(); i++) {
                ...
                if (diwg.isInGroup()) {
                    ...
                    for (int j = 0; j < children.size(); j++) {
                        ...
                        // logger.info("Checking child value:
            ...
            for (int i = 0; i < allItems.size(); i++) {
                ...
                if (diwg.isInGroup()) {
                    ...
                    List<DisplayItemGroupBean> dbGroups = diwg.getDbItemGroups();
                    List<DisplayItemGroupBean> formGroups = new ArrayList<DisplayItemGroupBean>();
                    //jxu- this part need to be refined, why need to validate items again?
                    if (validate) {
                        formGroups = validateDisplayItemGroupBean(v, dgb, dbGroups, formGroups, ruleValidator, groupOrdinalPLusItemOid);
                        // formGroups = validateDisplayItemGroupBean(v, dgb, dbGroups, formGroups);
                        logger.info("form group size after validation " + formGroups.size());
                ...
                if (diwb.isInGroup()) {
                    ...
                    for (int j = 0; j < dgbs.size(); j++) {
                        ...
                        for (DisplayItemBean displayItem : items) {
                            ...
                            if (responseTypeId == 8 || responseTypeId == 9) {
                                ...
                                String value = sc.doCalculation(displayItem, scoreItems, scoreItemdata, itemOrdinals, err, displayItem.getData().getOrdinal());
                                displayItem.loadFormValue(value);
                                if (isChanged(displayItem.getData(), oldItemdata))
                                    changedItems.add(displayItem.getItem().getName());
                                String inputName = "";
                                if (displayGroup.isAuto()) {
                                    inputName = getGroupItemInputName(displayGroup, displayGroup.getOrdinal(), displayItem);
                                } else {
                                    inputName = getGroupItemManualInputName(displayGroup, displayGroup.getOrdinal(), displayItem);
                                }
                    ...
                    for (int j = 0; j < children.size(); j++) {
                        ...
                        if (resTypeId == 8 || resTypeId == 9) {
                            ...
                            String cvalue = sc.doCalculation(child, scoreItems, scoreItemdata, itemOrdinals, cerr, 1);
            ...
            if (errors.isEmpty() && shouldRunRules()) {
                ...
                if (session.getAttribute("shouldRunValidation") != null && session.getAttribute("shouldRunValidation").toString().equals("1")) {
                    session.setAttribute("shouldRunValidation", null);
                } else {
                ...
                if (!success) {
                    ...
                    if (!forwardingSucceeded) {
                        ...
                        if (markSuccessfully) {
                            ...
                            session.removeAttribute(HAS_DATA_FLAG);
                            session.removeAttribute(DDE_PROGESS);
                            System.out.println("try to remove to_create_crf11111");
                            session.removeAttribute("to_create_crf");
                            ...
                            if (!section.isLastSection()) {
                                ...
                                session.removeAttribute(HAS_DATA_FLAG);
                                session.removeAttribute(DDE_PROGESS);
                                System.out.println("try to remove to_create_crf22222");
                                session.removeAttribute("to_create_crf");
    ...
    protected void getInputBeans() throws InsufficientPermissionException {
        ...
        if (ecb == null) {
            ...
            if (eventCRFId > 0) {
                ...
                //use toCreateCRF as a flag to prevent user to submit event CRF more than once
                ...
                try {
                    ...
                    System.out.println("Initial: to create an event CRF.");
                    ...
                    throw new InsufficientPermissionException(Page.SUBMIT_DATA_SERVLET, ne.getMessage(), "1");
                }
            }
    ...
    private EventCRFBean createEventCRF() throws InconsistentStateException {
        ...
        if (ssb.getId() <= 0) {
            ...
            throw new InconsistentStateException(Page.SUBMIT_DATA_SERVLET, resexception.getString("begin_data_entry_without_event_but_subject"));
        }
        ...
        if (sedb.getId() <= 0) {
            ...
            throw new InconsistentStateException(Page.SUBMIT_DATA_SERVLET, resexception.getString("begin_data_entry_without_event_but_study"));
        }
        ...
        if (eb.getId() <= 0) {
            ...
            throw new InconsistentStateException(Page.SUBMIT_DATA_SERVLET, resexception.getString("begin_data_entry_without_event_but_CRF"));
        }
        ...
        if (aeb.getId() <= 0) {
            ...
            throw new InconsistentStateException(Page.SUBMIT_DATA_SERVLET, resexception.getString("begin_data_entry_without_event_but_especified_event"));
        }
        ...
        if (eventCRFId == 0) {// no event CRF created yet
            ...
            System.out.println("*********CREATED EVENT CRF");
        } else {
            // there is an event CRF already, only need to update
            ecb = (EventCRFBean) ecdao.findByPK(eventCRFId);
            ecb.setCRFVersionId(crfVersionId);
        ...
        if (ecb.getId() <= 0) {
            ...
            throw new InconsistentStateException(Page.SUBMIT_DATA_SERVLET, resexception.getString("new_event_CRF_not_created"));
        } else {
    ...
    protected List<DisplayItemGroupBean> loadFormValueForItemGroup(DisplayItemGroupBean digb, List<DisplayItemGroupBean> dbGroups,
        ...
        for (int i = 0; i < repeatMax; i++) {
            ...
            // rep model
            if (!StringUtil.isBlank(fp.getString(igb.getName() + "_manual" + i + ".newRow"))) {
        ...
        // eventDefCRFId,
        // sm.getDataSource());
        logger.info("+++ count for null values list: " + nullValuesList.size());
        logger.info(nullValuesList.toString() + " found with " + eventDefCRFId);
        ...
        for (int i = 0; i < repeatMax; i++) {
            ...
            List<DisplayItemBean> dibs = FormBeanUtil.getDisplayBeansFromItems(itBeans, sm.getDataSource(), ecb.getCRFVersionId(), sb.getId(), nullValuesList);
            logger.info("+++count for dibs: " + dibs.size());
            two = System.currentTimeMillis() - timeCheck;
        ...
        logger.info("manual rows" + manualRows + "formGroup size" + formGroups.size());
     ...
     * The loadFormValue method should be used for this purpose. <p/> validateDisplayItemBeanText, validateDisplayItemBeanSingleCV, and
     * validateDisplayItemBeanMultipleCV are provided to make implementing this method easy.
     *
     * @param v
    ...
    protected DisplayItemBean validateDisplayItemBeanText(DiscrepancyValidator v, DisplayItemBean dib, String inputName) {
        ...
        if (!isNull) {
            if (StringUtil.isBlank(idb.getValue())) {
                 ...
                 * customValidation = Validator.processCRFValidationFunction(customValidationString); } catch (Exception e) { e.printStackTrace(); } } else if
                 * (customValidationString.startsWith("regexp:")) { try { customValidation = Validator.processCRFValidationRegex(customValidationString); }
    ...
    protected boolean writeToDB(DisplayItemBean dib, ItemDataDAO iddao, int ordinal) {
        ...
        if (StringUtil.isBlank(dib.getEditFlag())) {
            ...
            if (!idb.isActive()) {
                ...
                System.out.println("update item update_id "+idb.getUpdater().getId());
            ...
            if ("add".equalsIgnoreCase(dib.getEditFlag())) {
                ...
                idb.setUpdater(ub);
                //idb = (ItemDataBean) iddao.updateValue(idb);
                //instead of two hits to the db, we perform an 'upsert' combining them into one
                ...
                //not setting id with just Updater() trying UpdaterId() <<tbh 08/2008
    ...
    protected DisplaySectionBean getDisplayBean(boolean hasGroup, boolean includeUngroupedItems) throws Exception {
        ...
        if (hasGroup) {
            ...
            if (includeUngroupedItems) {
                ...
                newDisplayBean =
                  formBeanUtil.createDisplaySectionWithItemGroups(sb.getId(), ecb.getCRFVersionId(), ecb.getStudyEventId(), sm, eventDefinitionCRFId);
            }
        ...
        for (int i = 0; i < displayItems.size(); i++) {
            ...
            if (shouldLoadDBValues(dib)) {
                ...
                dib.loadDBValue();
                // logger.info("just got data loaded:
                // "+dib.getData().getValue());
            }
    ...
    protected void populateNotesWithDBNoteCounts(FormDiscrepancyNotes discNotes, DisplaySectionBean section) {
        dndao = new DiscrepancyNoteDAO(sm.getDataSource());
        ...
        List<DisplayItemWithGroupBean> allItems = section.getDisplayItemGroups();
        logger.info("start to populate notes:" + section.getDisplayItemGroups().size());
            ...
            if (itemWithGroup.isInGroup()) {
                ...
                for (int i = 0; i < digbs.size(); i++) {
                    ...
                    for (DisplayItemBean dib : items) {
                        ...
                        //System.out.println("itemDataId:" + itemDataId);
                        ...
                        //System.out.println("dib note size:" + dib.getNumDiscrepancyNotes() + dib.getData().getId());
    ...
    private boolean saveItemsToMarkComplete(Status completeStatus) throws Exception {
        ...
        for (int i = 0; i < sections.size(); i++) {
            ...
            if (!isSectionReviewedOnce(sb)) {
                //ArrayList requiredItems = idao.findAllRequiredBySectionId(sb.getId());
    ...
    protected List<DisplayItemWithGroupBean> createItemWithGroups(
      DisplaySectionBean dsb, boolean hasItemGroup, int eventCRFDefId) {
            ...
            for (DisplayItemGroupBean itemGroup : dsb.getDisplayFormGroups()) {
                ...
                for (int i = 0; i < data.size(); i++) {
                    ...
                    if (idb.getItemId() == firstItem.getItem().getId()) {
                        ...
                        digb.setItems(dibs);
                        // logger.info("line 2299, set with dibs list of : " +
                        // dibs.size());
                        ...
                        for (DisplayItemBean dib : displayGroup.getItems()) {
                            for (int j = 0; j < data.size(); j++) {
                                ...
                                if (idb.getItemId() == dib.getItem().getId() && !idb.isSelected()) {
                                    ...
                                    dib.setData(idb);
                                    if (shouldLoadDBValues(dib)) {
                                        // logger.info("+++should load db values
                                        // is true, set value");
                                        dib.loadDBValue();
    ...
    public final String getGroupItemManualInputName(DisplayItemGroupBean digb, int ordinal, DisplayItemBean dib) {
        String inputName = digb.getItemGroupBean().getName() + "_manual" + ordinal + getInputName(dib);
    ...
    /**
     * This method will populate grouped and variableAndValue HashMaps 
     * grouped : Used to correctly populate group ordinals 
     * variableAndValue : Holds itemOID , value (in Form ) pairs passed in to rule processor
     *
     * @param allItems
     */
    private Container populateRuleSpecificHashMaps(List<DisplayItemWithGroupBean> allItems, Container c) {
        ...
        for (DisplayItemWithGroupBean displayItemWithGroupBean : allItems) {
            ...
            if (displayItemWithGroupBean.getSingleItem() != null) {
                if (displayItemWithGroupBean.getSingleItem().getItem().getOid() != null) {
                    ...
                    c.variableAndValue.put(displayItemWithGroupBean.getSingleItem().getItem().getOid(), displayItemWithGroupBean.getSingleItem().getData()
                      .getValue());
                }
            ...
            for (DisplayItemGroupBean itemGroupBean : displayItemWithGroupBean.getItemGroups()) {
                ...
                for (DisplayItemBean displayItemBean : itemGroupBean.getItems()) {
                    ...
                    c.variableAndValue.put(key1, displayItemBean.getData().getValue());
    ...
    private HashMap<String, ArrayList<String>> runRules(List<DisplayItemWithGroupBean> allItems, List<RuleSetBean> ruleSets, Boolean dryRun,
        ...
        if (shouldRunRules) {
            ...
            c = populateRuleSpecificHashMaps(allItems, c);
            ...
            // return getRuleSetService().runRules(ruleSets, dryRun, currentStudy, c.variableAndValue, ub);

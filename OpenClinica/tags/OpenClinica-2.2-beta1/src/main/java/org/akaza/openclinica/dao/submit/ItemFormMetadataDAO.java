...
public class ItemFormMetadataDAO extends EntityDAO {
  ...
  public ArrayList findByMultiplePKs(ArrayList ints) throws OpenClinicaException {
	...
	while (it.hasNext()) {
		...
		ItemFormMetadataBean ifmBean = 
			(ItemFormMetadataBean)this.findByPK(newInt.intValue());
		//check to make sure we have what we need
		logger.info("options: "+ifmBean.getResponseSetId()+
				" bean options list: "+ifmBean.getResponseSet().getOptions().toString());
		/*ArrayList options = ifmBean.getResponseSet().getOptions();
		Iterator robit = options.iterator();
		while (robit.hasNext()) {
			ResponseOptionBean rob = (ResponseOptionBean)robit.next();
			logger.info("rob text: "+rob.getText());
			logger.info("value: "+rob.getValue());
		}*/
		answer.add(ifmBean);
	}
	return answer;
  ...
  /*public ArrayList findAllBySectionId(int sectionId) throws OpenClinicaException {
	...
	//TODO added SQL in xml file, but not sure as to why crf version 
	//is the xml file we're using, tbh, 1-8-2005
	String sql = digester.getQuery("findAllBySectionId");
	logger.warning("found query: "+sql);
  ...
  public Collection findAll() throws OpenClinicaException {
    ...
    while (it.hasNext()) {
      ItemFormMetadataBean ifmb = (ItemFormMetadataBean) this.getEntityFromHashMap((HashMap) it
          .next());
      answer.add(ifmb);
    }
  ...
  public ArrayList findAllByItemId(int itemId){
    ...
    this.setTypeExpected(26,TypeNames.STRING);//version name
    //add more here for display, tbh 082007
    this.setTypeExpected(27,TypeNames.STRING);//group_label
    this.setTypeExpected(28,TypeNames.INT);//repeat_max
    this.setTypeExpected(29,TypeNames.STRING);//section_name
    HashMap variables = new HashMap();
    ...
    while (it.hasNext()) {
      ...
      //System.out.println(">>>added group name: "+groupLabel);
      ifmb.setSectionName(sectionName);
      //System.out.println("<<<added section name: "+sectionName);
  ...
  public ItemFormMetadataBean findByItemIdAndCRFVersionId(int itemId, int crfVersionId) {
    ...
    //EntityBean eb = this.executeFindByPKQuery("findByItemIdAndCRFVersionId", variables);

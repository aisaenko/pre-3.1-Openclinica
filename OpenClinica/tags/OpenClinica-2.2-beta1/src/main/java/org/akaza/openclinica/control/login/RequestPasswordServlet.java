...
public class RequestPasswordServlet extends SecureController {
  ...
  private void sendPassword(String passwd,UserAccountBean ubDB) throws Exception {
    ...
    StringBuffer email = new StringBuffer("Hi " + ubDB.getFirstName() + ", <br>");
    				...
    				restext.getString("your_password_has_been_reset_as") +":" + passwd).append(
    				"<br>" +  restext.getString("you_will_be_required_to_change")).append(
    				restext.getString("time_you_login_to_the_system")).append(
    				restext.getString("use_the_following_link_to_log") + ":<br>").append(
    				SQLInitServlet.getField("sysURL") );
    				
    String emailBody = email.toString();
    try {
    em.process(ubDB.getEmail().trim(), EmailEngine.getAdminEmail(), restext.getString("your_openclinica_password"),
        emailBody);

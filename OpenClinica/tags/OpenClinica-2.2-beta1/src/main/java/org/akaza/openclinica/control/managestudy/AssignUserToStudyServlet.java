...
public class AssignUserToStudyServlet extends SecureController {
  ...
  private String sendEmail(UserAccountBean u, StudyUserRoleBean sub) throws Exception {
    ...
    String body = u.getFirstName() + " " + u.getLastName() + "(" + resword.getString("username")+": " + u.getName()
        + ") " + respage.getString("has_been_assigned_to_the_study_site")
        + ((StudyBean) session.getAttribute("study")).getName() + " " + resword.getString("as") + sub.getRoleName() + ". ";
    ...
    em = new EmailEngine(EmailEngine.getSMTPHost());
    //to admin
    em.process(EmailEngine.getAdminEmail(), EmailEngine.getAdminEmail(), respage.getString("new_user_added_to_study"),
        body);

...
/**
 * This class generates a horizontal HTML table with multiple columns and headers.
 */
public class HorizontalFormBuilder extends DefaultFormBuilder {
  //The object that will provide the content for the table's headers and cells.
  ...
  //Have the form values already been saved during initial or doubledata entry?
  ...
  //The list of DisplayFormGroupBeans that provide the content for an XHTML table
  ...
  public String createMarkup() {
    ...
    if (isDataEntry) {
      ...
      itemDataBeans = persistanceHandler.fetchPersistedData(sectionBean.getId(),
        eventCRFbean.getId());
    ...
    boolean repeatFlag;
    // The CellFactory object that generates the content for HTML table TD cells.
    ...
    if(studBean != null && studBean.getStudyParameterConfig().
      getDiscrepancyManagement().equalsIgnoreCase("true")){
    ...
    for (DisplayItemGroupBean displayItemGroup : this.displayItemGroups) {
      ...
      //A Map that contains persistent, repeated rows in a matrix type table
      //the index is the Item id of the first member of the row; the value is a List
      //of item beans that make up the row
      SortedMap<Integer, List<ItemDataBean>> ordinalItemDataMap =
        new TreeMap<Integer, List<ItemDataBean>>();
      //Is this a persistent matrix table and does it already have repeated rows
      //in the database?
      boolean hasStoredRepeatedRows=false;
      boolean unGroupedTable = (displayItemGroup.getItemGroupBean().getName().
        equalsIgnoreCase(BeanFactory.UNGROUPED));
      //Load any database values into the DisplayItemBeans
      if(hasDbFormValues) {
        currentDisplayItems = persistanceHandler.loadDataIntoDisplayBeans(
          currentDisplayItems, (! unGroupedTable)
        );
        /*   The highest number ordinal represents how many repeated rows there are.
  If the ordinal in ItemDataBeans > 1, then we know that the group has
  persistent repeated rows. Get a structure that maps each ordinal (i.e., >= 2) to
  its corresponding List of ItemDataBeans. Then reiterate the existing DisplayBeans,
  with the number of new rows equaling the highest ordinal number minus 1.  For example,
  in a List of  ItemDataBeans, if the highest ordinal property among these beans is 5,
  then the matrix table has 4 repeated rows from the database.
  Provide each new row
  with its values by using the ItemDataBeans.*/
        if((! unGroupedTable) &&
          persistanceHandler.hasPersistentRepeatedRows(currentDisplayItems)){
          hasStoredRepeatedRows=true;
          //if the displayitems contain duplicate item ids, then these duplicates
          //represent repeated rows. Separate them into a Map of new rows that
      ...
      if (unGroupedTable) {
        orphanTable = formUtil.createXHTMLTableFromNonGroup(
          currentDisplayItems,
          tabindex,
          hasDiscrepancyMgt);
        //We have to track the point the tabindex has reached here
        //The tabindex will increment by the size of the  displayItemGroup List
      ...
      //We have to create an extra thead column for the Remove Row button, if
      //the table involves repeating rows
      List<Element> thTags =
        (repeatFlag ? createTheadContentsFromDisplayItems(currentDisplayItems,
          true)
          : createTheadContentsFromDisplayItems(currentDisplayItems, false));
      ...
      if (repeatFlag && ! (isDataEntry && hasStoredRepeatedRows)) {
        row = repeatManager.addParentRepeatAttributes(row,
          repeatParentId,
          repeatNumber,
          displayItemGroup.getGroupMetaBean().getRepeatMax());
      }
      ...
      for (DisplayItemBean displayBean : currentDisplayItems) {
        ...
        String responseName = displayBean.getMetadata().getResponseSet().
        ...
        if (displayBean.getMetadata().getResponseLayout().
          ...
          Element[] elements = cellFactory.createCellContentsForChecks(responseName,
            displayBean,
          ...
          for (Element el : elements) {
            ...
            if (repeatFlag) {
              el = repeatManager.addChildRepeatAttributes(el, repeatParentId,
                displayBean.getItem().getId(), null);
            }
            row.addContent(el);
          }
        ...
        td = cellFactory.createCellContents(td, responseName,
          displayBean, ++tabindex, hasDiscrepancyMgt);
        if (repeatFlag) {
      ...
      if(hasStoredRepeatedRows)  {
        List<Element> storedRepeatedRows =
          builderUtil.generatePersistentMatrixRows(ordinalItemDataMap,
            currentDisplayItems,tabindex,repeatParentId,hasDiscrepancyMgt);
      ...
      //Create a row for the Add Row button, if the group includes any repeaters
      if (repeatFlag) {
        builderUtil.createAddRowControl(tbody, repeatParentId,
          (builderUtil.calcNumberofColumns(displayItemGroup) + 1));
  ...
  private void addResponseLayoutRow(Element thRow,
                                    List<DisplayItemBean> displayBeans) {
    ...
    for (DisplayItemBean dBean : displayBeans) {
      ...
      if(responseLayout==null) { responseLayout="";}
      //You could have a radio or checkbox whose layout is *not* horizontal,
      //next to a rad or check with a hor layout.
      if ((responseName.equalsIgnoreCase("radio") ||
        responseName.equalsIgnoreCase("checkbox")) &&
        responseLayout.equalsIgnoreCase("horizontal")) {
        ...
        //create empty cells for non-radios or checks, or rads and checks
  ...
  public Element createTHTagFromItemMeta(
    ItemFormMetadataBean itemFormBean) {

    //include quest number in the header
    Element thTag;
    boolean hasQuestNumber = ! "".equalsIgnoreCase(itemFormBean.
    ...
    if (itemFormBean.getResponseLayout().equalsIgnoreCase("horizontal")) {
  ...
  public List<Element> createTheadContentsFromDisplayItems(
    ...
    //Create an extra clumn for the cells that contain a Remove Row button
  ...
  public void setTabindexSeed(int tabindexSeedint) {
    ...
    if (tabindexSeedint > 1) this.tabindexSeed = tabindexSeedint;
  }

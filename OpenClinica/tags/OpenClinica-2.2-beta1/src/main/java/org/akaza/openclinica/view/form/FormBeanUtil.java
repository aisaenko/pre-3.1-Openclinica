...
public class FormBeanUtil {
  ...
  public Element createXHTMLTableFromNonGroup(List<DisplayItemBean> items,
    ...
    //or several columns
    if(numberOfColumns == 1) {
      createSingleColumn(tbody,items,tabindex, hasDiscrepancyMgt);
    }   else {
      createMultipleCols(tbody,multiRowMap,tabindex, hasDiscrepancyMgt, numberOfColumns);
    }
    return table;
  }
  ...
  private void createSingleColumn(Element tbody,
    ...
    for(DisplayItemBean disBean : items) {
      ...
      cellFactory.createCellContents(tdCell,
        responseName,disBean,++tabindex, hasDiscrepancyMgt);
   ...
   * other row, so numberOfColumns would equal five).
   */
  ...
  public DisplaySectionBean createDisplaySectionWithItemGroups(
    int sectionId,
    int crfVersionId,
    SessionManager sm,int eventDefinitionCRFId) {

...
public class RemoveStudyUserRoleServlet extends SecureController {
  ...
  private String sendEmail(UserAccountBean u, StudyUserRoleBean sub) throws Exception {
    ...
    String body = u.getFirstName() + " " + u.getLastName() + "("+resword.getString("username")+ ": " + u.getName()
        + ") "+ respage.getString("has_been_removed_from_the_study_site") + study.getName() + " "+ respage.getString("with_role")+" "+
        sub.getRoleName() + ". "+ respage.getString("the_user_will_no_longer_access_to_the_study");
    try {
      EmailEngine em = new EmailEngine(EmailEngine.getSMTPHost());
      //to user
      em.process(u.getEmail().trim(), EmailEngine.getAdminEmail(), respage.getString("remove_user_role"), body);

      em = null;//otherwise keep sending emails??
      em = new EmailEngine(EmailEngine.getSMTPHost());
      //to study director
      em.process(ub.getEmail().trim(), EmailEngine.getAdminEmail(), respage.getString("remove_user_role"), body);

      em = null;
      em = new EmailEngine(EmailEngine.getSMTPHost());
      //to admin
      em
          .process(EmailEngine.getAdminEmail(), EmailEngine.getAdminEmail(), respage.getString("remove_user_role"),
              body);

...
public class ViewStudySubjectServlet extends SecureController {
  ...
  public void processRequest() throws Exception {
    ...
    if (studySubId == 0) {
      ...
      String[] columns = {resword.getString("event"), resword.getString("start_date1"),  resword.getString("location"), resword.getString("status"), resword.getString("actions"), resword.getString("CRFs_atrib")};
    ...
    EventDefinitionCRFDAO edcdao = new EventDefinitionCRFDAO(ds);
  	...
  	for (i = 0; i < eventCRFs.size(); i++) {
        ...
        EventDefinitionCRFBean edc = (EventDefinitionCRFBean)
        	edcdao.findByStudyEventDefinitionIdAndCRFId(
        			studyEventDefinitionId,
        			cb.getId());
  ...
  public void mayAccess() throws InsufficientPermissionException {
	...
	if(studySubId > 0) {
		if(!entityIncluded(studySubId, ub.getName(), subdao, sm.getDataSource())) {
			addPageMessage("Required " + resword.getString("study_subject") + respage.getString("not_belong_to_studies"));

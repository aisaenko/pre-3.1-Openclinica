...
<td class="table_cell" nowrap>
  <c:choose>
    <c:when test="${isInError_Dat}">
      <fmt:message key="interview_date" bundle="${resword}"/>: <span class="aka_exclaim_error">! </span>&nbsp;<br />(<fmt:message key="date_format" bundle="${resformat}"/>)
    </c:when>
    <c:otherwise>
      <fmt:message key="interview_date" bundle="${resword}"/>: *&nbsp;<br />(<fmt:message key="date_format" bundle="${resformat}"/>)
    </c:otherwise>
  </c:choose>
</td><!--</a>-->
<td class="table_cell_left">
      ...
      <td valign="top" nowrap><a href="#" onClick=
        ...
        <c:if test="${study.studyParameterConfig.discrepancyManagement=='true'}">
          <a href="#" onClick="openDNoteWindow('CreateDiscrepancyNote?id=<c:out value="${toc.eventCRF.id}"/>&name=eventCrf&field=interviewDate&column=date_interviewed','spanAlert-interviewDate'); return false;">
            <img name="interviewDate" src="images/icon_noNote.gif" border="0" alt="Discrepancy Note" title="<fmt:message key="discrepancy_note" bundle="${resword}"/>" title="<fmt:message key="discrepancy_note" bundle="${resword}"/>"></a>
        </c:if>
      </td>
    </tr>
    <tr>

...
<h1><span class="title_manage"><fmt:message key="manage_study" bundle="${resworkflow}"/> <a href="javascript:openDocWindow('help/4_0_manageStudy_Help.html')"><img src="images/bt_Help_Manage.gif" border="0" alt="<fmt:message key="help" bundle="${resword}"/>" title="<fmt:message key="help" bundle="${resword}"/>"></a></span></h1>


<h2><fmt:message key="recent_activity" bundle="${restext}"/><span style="font-size:12px"><fmt:message key="select_to_manage" bundle="${restext}"/></span></h2>
				...
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					...
					<tr valign="top">   
						...
						<c:when test="${sub.updatedDate != null}">
						 <fmt:formatDate value="${sub.updatedDate}" dateStyle="short"/>
						 </c:when>
						 <c:otherwise>
						  <fmt:formatDate value="${sub.createdDate}" dateStyle="short"/>
						 </c:otherwise>
						</c:choose>
						</td>
					...
					<tr valign="top">
					 <td class="table_cell" align="right" colspan="3">
					   <c:if test="${subCount>0}">
					    <a href="ListStudySubject"><fmt:message key="show_all" bundle="${resword}"/></a> | 
					   </c:if>
		...
		<div class="box_T"><div class="box_L"><div class="box_R"><div class="box_B"><div class="box_TL"><div class="box_TR"><div class="box_BL"><div class="box_BR">
			...
			<div class="tablebox_center">
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					...
					<tr valign="top">
					 <td class="table_cell" align="right" colspan="3"> <c:if test="${userCount>0}"><a href="ListStudyUser"><fmt:message key="show_all" bundle="${resword}"/></a> | </c:if><a href="AssignUserToStudy"><fmt:message key="add_new" bundle="${resword}"/></a> </td>
					</tr>					
				</table>
					...
					<tr valign="top">   
						...
						<c:choose>
						 <c:when test="${site.updatedDate != null}">
						  <fmt:formatDate value="${site.updatedDate}" dateStyle="short"/> 
						 </c:when>
						 <c:otherwise>
						   <fmt:formatDate value="${site.createdDate}" dateStyle="short"/> 
						 </c:otherwise>
						</c:choose> 
						</td>
					     ...
					     <tr valign="top">   
						  ...
						  <td class="table_cell">
						   ...
						   <c:when test="${def.updatedDate != null}">
						    <fmt:formatDate value="${def.updatedDate}" dateStyle="short"/>
						   </c:when>
						   <c:otherwise>
						     <fmt:formatDate value="${def.createdDate}" dateStyle="short"/>
						   </c:otherwise>
						  </c:choose> 
						  </td>
...
<div style="width: 250px">
  ...
  <a name="log"><a href="AuditLogStudy">View Study Audit Log</a></a>
						...
						<c:forEach var="audit" items="${audits}">
						  <tr>
							...
							<c:if test="${audit.auditDate != null}">
							 <fmt:formatDate value="${audit.auditDate}" dateStyle="short"/>
							</c:if>&nbsp;
							</td>

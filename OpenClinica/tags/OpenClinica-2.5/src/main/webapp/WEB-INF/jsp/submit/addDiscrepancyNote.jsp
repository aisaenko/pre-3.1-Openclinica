   ...
   <c:if test="${discrepancyNote.crfName !=''}">
    ...
    <td><c:out value="${discrepancyNote.crfName}"/>&nbsp;</td>
       ...
       <tr valign="top">
           ...
           <c:when test="${parent == null || parent.id ==0 }">         
            ...
            <c:choose>             
             <c:when test="${typeId1 == type.id}">
              <input type="radio" name="typeId" value="<c:out value="${type.id}"/>" checked  onclick ="javascript:setStatus(<c:out value="${type.id}"/>);"><c:out value="${type.name}"/><br>
             </c:when>
             <c:otherwise>
              <input type="radio" name="typeId" value="<c:out value="${type.id}"/>" onclick ="javascript:setStatus(<c:out value="${type.id}"/>);"><c:out value="${type.name}"/><br>
             </c:otherwise>
             </c:choose>
            </c:forEach>
        ...
        <tr valign="top">
            <td colspan="2"><a href="ViewDiscrepancyNote?subjectId=<c:out value="${discrepancyNote.subjectId}"/>&itemId=<c:out value="${item.id}"/>&id=<c:out value="${discrepancyNote.entityId}"/>&name=<c:out value="${discrepancyNote.entityType}"/>&field=<c:out value="${discrepancyNote.field}"/>&column=<c:out value="${discrepancyNote.column}"/>&enterData=<c:out value="${enterData}"/>&monitor=<c:out value="${monitor}"/>&blank=<c:out value="${blank}"/>">

...
public class DefineStudyEventServlet extends SecureController {
  ...
  public void mayProceed() throws InsufficientPermissionException {
    ...
    if (currentStudy.getParentStudyId() > 0) {
      ...
      throw new InsufficientPermissionException(Page.STUDY_EVENT_DEFINITION_LIST, resexception.getString("not_top_study"),
          "1");
    }
    ...
    if (StringUtil.isBlank(action)) {
      ...
      if ("confirm".equalsIgnoreCase(action)) {
        confirmWholeDefinition();
    	  ...
    	  //put a try catch in here to fix task # 1642 in Mantis, added 092007 tbh
        try {
   ...
   * NullPointer catch added by tbh 092007, mean to fix task #1642 in Mantis
   * 
   */

...
<c:forEach var="note" items="${discrepancyNotes}">
  ...
  <img src="images/bullet.gif">&nbsp;<b><c:out value="${note.value.description}"/></b>&nbsp;&nbsp;
   ...
   <c:if test="${note.value.id>0}">
    <a href="CreateDiscrepancyNote?id=<c:out value='${note.value.entityId}'/>&name=<c:out value='${note.value.name}'/>&field=<c:out value='${note.value.field}'/>&column=<c:out value='${note.value.column}'/>&parentId=<c:out value='${note.value.id}'/>&enterData=<c:out value='${enterData}'/>&monitor=<c:out value='${monitor}'/>&blank=<c:out value='${blank}'/>">

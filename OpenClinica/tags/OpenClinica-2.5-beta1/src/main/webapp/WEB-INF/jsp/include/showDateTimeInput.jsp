...
<div class="formfieldS_BG">
	<input type="text" name="<c:out value="${dateFieldName}"/>" value="<c:out value="${date}" />" class="formfieldS" id="dateField"/>
</div>
</td>
<td valign="top">
    <a href="#" >
        <img src="images/bt_Calendar.gif" alt="<fmt:message key="show_calendar" bundle="${resword}"/>" title="<fmt:message key="show_calendar" bundle="${resword}"/>" border="0" id="dateTrigger" />
        <script type="text/javascript">
        Calendar.setup({inputField  : "dateField", ifFormat    : "<fmt:message key="date_format_calender" bundle="${resformat}"/>", button      : "dateTrigger" });
        </script>
...
<% if (resformat.getString("date_time_format_string").contains("HH") || resformat.getString("date_time_format_string").contains("kk")) { %>
	<c:forEach var="currHour" begin="1" end="24" step="1">
		<c:choose>

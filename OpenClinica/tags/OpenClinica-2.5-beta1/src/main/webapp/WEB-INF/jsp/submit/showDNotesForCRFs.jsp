...
<c:forEach var="event" items="${currRow.bean.studyEvents}">
<!-- currRow.bean is a DisplayStudySubjectBean;
...
<td class="table_cell"><fmt:formatDate value="${event.studyEvent.dateStarted}" dateStyle="short"/>&nbsp;</td>
    <%--event = DisplayStudyEventBean
...
<%-- include an icon for displaying the eventCRf status
...
<c:if test="${! (empty event.studyEvent.discBeanList)}"><c:set var="hasDiscNotes" value="${true}"/></c:if>
        ...
        <c:choose>
            <c:when test="${! boolResStatus}">
                ...
                <c:if test="${hasDiscNotes}"><a href=
                  "javascript:openDocWindow('ChooseDownloadFormat?discNoteType=${discNoteType}&module=${module}&subjectId=<c:out value="${currRow.bean.studySubject.id}"/>')"><img
            ...
            <c:otherwise>
                ...
                <c:if test="${hasDiscNotes}"><a href=
                  "javascript:openDocWindow('ChooseDownloadFormat?subjectId=<c:out value="${currRow.bean.studySubject.id}"/>&discNoteType=${discNoteType}&module=${module}&resolutionStatus=<c:out value="${resolutionStatusFromServlet}"/>')"><img
                  hspace="2" name="bt_View1" src="images/bt_Download.gif" border="0" alt="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>" title="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>"></a>

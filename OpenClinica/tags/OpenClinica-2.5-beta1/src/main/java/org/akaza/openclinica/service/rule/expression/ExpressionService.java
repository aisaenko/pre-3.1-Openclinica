...
public class ExpressionService {
    ...
    public ExpressionService(DataSource ds) {
        ...
        pattern[3] = Pattern.compile("^[^\\[\\]]+|\\S+\\[(ALL|\\d+)\\]$"); // StudyEventDefinition
        pattern[2] = Pattern.compile("^[^\\[\\]\\s]+$"); // Crf
        pattern[1] = Pattern.compile("^[^\\[\\]]+|\\S+\\[(ALL|\\d+)\\]$"); // ItemDataGroup
        pattern[0] = Pattern.compile("^[^\\[\\]\\s]+$"); // ItemData
        this.expressionWrapper = null;
        this.ds = ds;
    }
    ...
    public ExpressionService(ExpressionObjectWrapper expressionWrapper) {
        ...
        this.ds = expressionWrapper.getDs();
    }
    ...
    public String getValueFromDbb(String expression, String ruleSetExpression) throws OpenClinicaSystemException {
        ...
        try {
            // Get the studyEventId from RuleSet Target so we can know which StudySubject we are dealing with.
    ...
    private String constructFullExpressionIfPartialProvided(String expression, String ruleSetTargetExpression) {
        ...
        case 4:
            return expression;
        default:
            throw new OpenClinicaSystemException("Full Expression cannot be constructed from provided expression");
        }
    }
    ...
    private String getItemGroupOidFromExpression(String expression) throws OpenClinicaSystemException {
        return getOidFromExpression(expression, 1, 1).replaceAll("\\[(ALL|\\d+)\\]", "");
    }
    ...
    private String getItemGroupOidOrdinalFromExpression(String expression) throws OpenClinicaSystemException {
        ...
        if (itemGroupOid.matches("\\S+\\[(ALL|\\d+)\\]")) {
            itemGroupOidOrdinal = itemGroupOid.trim().replaceAll("\\S+\\[", "").trim().replaceAll("\\]", "");
        }
    ...
    private String getStudyEventDefinitionOidFromExpression(String expression) throws OpenClinicaSystemException {
        return getOidFromExpression(expression, 3, 3).replaceAll("\\[(ALL|\\d+)\\]", "");
    }
    ...
    public String getStudyEventDefinitionOidOrdinalFromExpression(String expression) throws OpenClinicaSystemException {
        ...
        if (studyEventDefinitionOid.matches("\\S+\\[(ALL|\\d+)\\]")) {
            studyEventDefinitionOidOrdinal = studyEventDefinitionOid.trim().replaceAll("\\S+\\[", "").trim().replaceAll("\\]", "");
        }
    ...
    public String getGroupOrdninalCurated(String expression) {
        ...
        String ordinal = originalOrdinal.equals("ALL") ? "" : originalOrdinal;
        return ordinal;
    }
    ...
    public String replaceGroupOidOrdinalInExpression(String expression, Integer ordinal) {
        ...
        if (ordinal == null) {
            ...
            replacement += getItemGroupOidFromExpression(expression) + "[" + ordinal + "]" + SEPERATOR + getItemOidFromExpression(expression);
        }
    ...
    public String getCustomExpressionUsedToCreateView(String expression, int sampleOrdinal) {
        return getStudyEventDefenitionOid(expression) + "[" + sampleOrdinal + "]" + SEPERATOR + "XXX" + SEPERATOR + getGroupOidWithItemOid(expression);
    }

    public String replaceStudyEventDefinitionOIDWith(String expression, String replacement) {
        replacement = getStudyEventDefinitionOidFromExpression(expression) + "[" + replacement + "]";
    ...
    /**
     * 
     * Use this method to convert TEST[3] to TEST[replacement]
     * 
     * @param expression
     * @param replacement
     * @return expression with first instance of digit replaced.
     */
    public String replaceFirstIntegerBy(String expression, String replacement) {
        return expression.trim().replaceFirst("\\[\\d+\\]", "[" + replacement + "}");
    }
    ...
    public StudyEventDefinitionBean getStudyEventDefinitionFromExpression(String expression) {
        ...
        logger.info("Expression : " + getStudyEventDefinitionOidFromExpression(expression));
        logger.info(" Study Bean : " + expressionWrapper.getStudyBean().getId());
        StudyEventDefinitionBean studyEventDefinition =
            getStudyEventDefinitionDao().findByOidAndStudy(getStudyEventDefinitionOidFromExpression(expression), expressionWrapper.getStudyBean().getId());
        return studyEventDefinition != null ? studyEventDefinition : null;
    }
    ...
    public ItemGroupBean getItemGroupExpression(String expression) {
        ...
        ItemGroupBean itemGroup = getItemGroupDao().findByOid(getItemGroupOidFromExpression(expression));
        return itemGroup;
    }
    ...
    public ItemBean getItemExpression(String expression, ItemGroupBean itemGroup) {
        logger.debug("Expression : " + expression);
        logger.debug("Expression : " + getItemOidFromExpression(expression));
        ItemBean item = getItemDao().findItemByGroupIdandItemOid(itemGroup.getId(), getItemOidFromExpression(expression));
        return item;
    }
    ...
    public void isExpressionValid(String expression) {
        ...
        EventDefinitionCRFBean eventDefinitionCrf = getEventDefinitionCRFDao().findByStudyEventDefinitionIdAndCRFId(studyEventDefinition.getId(), crf.getId());

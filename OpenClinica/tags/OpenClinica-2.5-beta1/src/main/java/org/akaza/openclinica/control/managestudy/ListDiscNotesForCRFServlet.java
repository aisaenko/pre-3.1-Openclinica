    ...
    public void processRequest() throws Exception {
        ...
        for (int i = 0; i < subjects.size(); i++) {
            ...
            discNoteUtil.injectDiscNotesIntoDisplayStudyEvents(displayEvents,
              resolutionStatus,sm.getDataSource(), discNoteType);

            //Create a request attribute indicating whether or not there are any disc notes for the study
            List<DiscrepancyNoteBean> allDiscNotes = discNoteUtil.getDNotesForStudy(this.currentStudy,
              resolutionStatus,sm.getDataSource(), discNoteType);
        ...
        table.setQuery("ListDiscNotesForCRFServlet?module=" + module + "&defId=" + definitionId + "&tab=" + tabId, new HashMap());

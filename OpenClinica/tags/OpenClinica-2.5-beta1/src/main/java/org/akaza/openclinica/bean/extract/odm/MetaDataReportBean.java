...
public class MetaDataReportBean extends OdmXmlReportBean {
    ...
    public void addNodeStudy() {
        StringBuffer xml = this.getXmlOutput();
        String indent = this.getIndent();
        xml.append(indent + "<Study OID=\"" + odmstudy.getOid() + "\">");
        xml.append("\n");
        addStudyGlobalVariables();
        addStudyMetaDataVersion();
    ...
    public void addStudyMetaDataVersion() {
        StringBuffer xml = this.getXmlOutput();
        String indent = this.getIndent();
        String currentIndent = indent + indent;
        MetaDataVersionBean meta = odmstudy.getMetaDataVersion();
        xml.append(currentIndent + "<MetaDataVersion OID=\"" + meta.getOid() + "\" Name=\"" + meta.getName() + "\">");
        xml.append("\n");
        // for <Include>,
        ...
        if (meta.getInclude() != null) {
            ...
            if (pmOid != null && pmOid.length() > 0) {
                ...
                if (psOid != null && psOid.length() > 0) {
                    xml.append("<Include StudyOID =\"" + psOid + "\"");
                } else {
                    xml.append("<Include StudyOID =\"" + odmstudy.getOid() + "\"");
                }
                xml.append(" MetaDataVersionOID=\"" + pmOid + "\"/>");
                xml.append("\n");
            }
        }
        //
    ...
    public void addProtocol(String currentIndent) {
        ...
        for (ElementRefBean seref : odmstudy.getMetaDataVersion().getProtocol().getStudyEventRefs()) {
            ...
            xml.append(currentIndent + indent + "<StudyEventRef StudyEventOID=\"" + seref.getElementDefOID() + "\" Mandatory=\"Yes\"/>");
            xml.append("\n");
        }
        xml.append(currentIndent + "</Protocol>");
        xml.append("\n");
    ...
    public void addStudyEventDef(String currentIndent) {
        ...
        for (StudyEventDefBean sed : seds) {
            xml.append(currentIndent + "<StudyEventDef OID=\"" + sed.getOid() + "\"  Name=\"" + StringEscapeUtils.escapeXml(sed.getName()) + "\" Repeating=\""
                + sed.getRepeating() + "\" Type=\"" + sed.getType() + "\">");
            xml.append("\n");
            ArrayList<ElementRefBean> forms = (ArrayList<ElementRefBean>) sed.getFormRefs();
            for (ElementRefBean form : forms) {
                xml.append(currentIndent + indent + "<FormRef FormOID=\"" + form.getElementDefOID() + "\" Mandatory=\"" + form.getMandatory() + "\"/>");
                xml.append("\n");
            }
    ...
    public void addFormDef(String currentIndent) {
        ...
        for (FormDefBean form : forms) {
            xml.append(currentIndent + "<FormDef OID=\"" + form.getOid() + "\" Name=\"" + StringEscapeUtils.escapeXml(form.getName()) + "\" Repeating=\""
                + form.getRepeating() + "\">");
            xml.append("\n");
            ArrayList<ElementRefBean> igs = (ArrayList<ElementRefBean>) form.getItemGroupRefs();
            for (ElementRefBean ig : igs) {
                xml.append(currentIndent + indent + "<ItemGroupRef ItemGroupOID=\"" + ig.getElementDefOID() + "\" Mandatory=\"" + ig.getMandatory() + "\"/>");
                xml.append("\n");
            }
    ...
    public void addItemGroupDef(String currentIndent) {
        ...
        for (ItemGroupDefBean ig : igs) {
            xml.append(currentIndent + "<ItemGroupDef OID=\"" + ig.getOid() + "\" Name=\"" + StringEscapeUtils.escapeXml(ig.getName()) + "\" Repeating=\""
                + ig.getRepeating() + "\" SASDatasetName=\"" + this.getSasNameValidatory().getValidSasName(ig.getPreSASDatasetName()) + "\">");
            xml.append("\n");
            ...
            if (item.getRangeCheck() != null) {
                ...
                for (RangeCheckBean rc : rcs) {
                    if (rc.getComparator().length() > 0) {
                        ...
                        xml.append(currentIndent + indent + "<RangeCheck Comparator=\"" + rc.getComparator() + "\" SoftHard=\"" + rc.getSoftHard() + "\">");
                        xml.append("\n");
                        xml.append(currentIndent + indent + indent + "<CheckValue>" + rc.getCheckValue() + "</CheckValue>");
                        xml.append("\n");

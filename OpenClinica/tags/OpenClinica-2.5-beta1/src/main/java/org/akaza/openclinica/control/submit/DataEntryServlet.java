...
public abstract class DataEntryServlet extends SecureController {
    ...
    protected void processRequest() throws Exception {
        ...
        try {
            ...
            ArrayList<DiscrepancyNoteBean> allNotes = dndao.findAllItemNotesByEventCRF(ecb.getId());
        ...
        if (!isSubmitted) {
            ...
            if (errors.isEmpty() && shouldRunRules()) {
                ...
                if (session.getAttribute("kk") != null && session.getAttribute("kk").toString().equals("1")) {
                    session.setAttribute("kk", null);
                } else {
                    errors = ruleValidator.validate();
                    if (errors.size() > 0) {
                        session.setAttribute("kk", "1");
                    }
...
/*                if(studyEventBean.getSubjectEventStatus().equals(SubjectEventStatus.SIGNED)){
        ...
        if (ecb == null) {
            ...
            if (eventCRFId > 0) {
                ...
                try {
                    ...
                    // logger.info("did not find an event crf id at line 731,
                    // created an ecb id "
                    // + ecb.getId());
                    request.setAttribute(INPUT_EVENT_CRF, ecb);
                    // }
            ...
            if (itemWithGroup.isInGroup()) {
                ...
                for (int i = 0; i < digbs.size(); i++) {
                    ...
                    for (DisplayItemBean dib : items) {
                        ...
                        logger.info("itemDataId:" + itemDataId);
                        logger.info("numNotes:" + numNotes);
                        // String inputName =
    ...
    private ExpressionService getExpressionService() {
        expressionService = this.expressionService != null ? expressionService : new ExpressionService(sm.getDataSource());
        return expressionService;
    }

    private DiscrepancyNoteService getDiscrepancyNoteService() {
        discrepancyNoteService = this.discrepancyNoteService != null ? discrepancyNoteService : new DiscrepancyNoteService(sm.getDataSource());

...
public class CreateNewStudyEventServlet extends SecureController {
    ...
    public static final String INPUT_ENDDATE_PREFIX_SCHEDULED = "endScheduled";
    ...
    protected void processRequest() throws Exception {
        ...
        if (!fp.isSubmitted()) {
            ...
            presetValues.put(INPUT_ENDDATE_PREFIX_SCHEDULED + "Hour", new Integer(-1));
            presetValues.put(INPUT_ENDDATE_PREFIX_SCHEDULED + "Minute", new Integer(-1));
            presetValues.put(INPUT_ENDDATE_PREFIX_SCHEDULED + "Half", new String(""));
            ...
            String strEndScheduled = fp.getDateTimeInputString(INPUT_ENDDATE_PREFIX_SCHEDULED);
            logger.info("string end and string end scheduled: " + strEnd + "; " + strEndScheduled);
            Date start = getInputStartDate();
            logger.info("just got start date; " + start.toString());
            Date end = null;
            Date startScheduled = getInputStartDateScheduled();
            Date endScheduled = null;
            // YW >>
            ...
            if (!StringUtil.isBlank(fp.getString(INPUT_STUDY_EVENT_DEFINITION_SCHEDULED))) {
                ...
                v.addValidation(INPUT_STUDY_EVENT_DEFINITION_SCHEDULED, Validator.ENTITY_EXISTS_IN_STUDY, seddao, studyWithEventDefinitions);
                v.addValidation(INPUT_SCHEDULED_LOCATION, Validator.NO_BLANKS);
                v.addValidation(INPUT_SCHEDULED_LOCATION, Validator.LENGTH_NUMERIC_COMPARISON, NumericComparisonOperator.LESS_THAN_OR_EQUAL_TO, 2000);
                ...
                if (!strEndScheduled.equals("")) {
                    v.addValidation(INPUT_ENDDATE_PREFIX_SCHEDULED, Validator.IS_DATE_TIME);
                    v.alwaysExecuteLastValidation(INPUT_ENDDATE_PREFIX_SCHEDULED);
                }
            ...
            if (hasScheduledEvent) {
                definitionScheduled = (StudyEventDefinitionBean) seddao.findByPK(fp.getInt(INPUT_STUDY_EVENT_DEFINITION_SCHEDULED));
            }
            ...
            if (hasScheduledEvent && !subjectMayReceiveStudyEvent(sm.getDataSource(), definitionScheduled, studySubject)) {
                Validator.addError(errors, INPUT_STUDY_EVENT_DEFINITION_SCHEDULED, restext.getString("not_added_since_event_not_repeating"));
            }
            ...
            if (hasScheduledEvent && !errors.containsKey(INPUT_STARTDATE_PREFIX_SCHEDULED) && !errors.containsKey(INPUT_ENDDATE_PREFIX_SCHEDULED)) {
                if (definitionScheduled.getId() == studyEventDefinitionId) {
                    // here the precondition of evaluating if two fields entered
                    // a same day is that
                    // Date formats are same for both fields.
                    if (fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED + "Date").equals(fp.getString(INPUT_STARTDATE_PREFIX + "Date"))) {
                        // if one of start or startScheduled is datetime, then
                        // another one will be forced to input datetime
                        boolean schStartTime = fp.timeEntered(INPUT_STARTDATE_PREFIX_SCHEDULED);
                        boolean startTime = fp.timeEntered(INPUT_STARTDATE_PREFIX);
                        if (!schStartTime && !startTime || schStartTime && startTime) {
                            if (startScheduled.before(start)) {
                                v.addError(errors, INPUT_STARTDATE_PREFIX_SCHEDULED, resexception
                                        .getString("input_provided_not_occure_after_previous_start_date_time"));
                            }
                        } else {
                            v.addError(errors, INPUT_STARTDATE_PREFIX_SCHEDULED,
                                    "When you scheduled the same date for start date/time of two events at this time, "
                                        + "if you choose time for one event, you should choose time for another event.");
                            v.addError(errors, INPUT_STARTDATE_PREFIX, "When you scheduled the same date for start date/time of two events at this time, "
                                + "if you choose time for one event, you should choose time for another event.");
                        }
                    } else {
                        if (startScheduled.before(start)) {
                ...
                endScheduled = fp.getDateTime(INPUT_ENDDATE_PREFIX_SCHEDULED);
                if (!strEndScheduled.equals("")) {
                    endScheduled = fp.getDateTime(INPUT_ENDDATE_PREFIX_SCHEDULED);
                    if (!fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED + "Date").equals(fp.getString(INPUT_ENDDATE_PREFIX + "Date"))) {
                        if (endScheduled.before(startScheduled)) {
                            v.addError(errors, INPUT_ENDDATE_PREFIX_SCHEDULED, resexception
                                    .getString("input_provided_not_occure_after_previous_start_date_time"));
                        }
                    } else {
                        // if in same date, only check when both had time
                        // entered
                        if (fp.timeEntered(INPUT_STARTDATE_PREFIX_SCHEDULED) && fp.timeEntered(INPUT_ENDDATE_PREFIX_SCHEDULED)) {
                            if (endScheduled.before(startScheduled) || endScheduled.equals(startScheduled)) {
                                v.addError(errors, INPUT_ENDDATE_PREFIX_SCHEDULED, resexception
                                        .getString("input_provided_not_occure_after_previous_start_date_time"));
                            }
                        }
            ...
            if (!errors.isEmpty()) {
                ...
                String prefixes[] = { INPUT_STARTDATE_PREFIX, INPUT_ENDDATE_PREFIX, INPUT_STARTDATE_PREFIX_SCHEDULED, INPUT_ENDDATE_PREFIX_SCHEDULED };
                ...
                ArrayList subjectsExistingEvents = sed.findAllByStudyAndStudySubjectId(currentStudy, studySubject.getId());
                ...
                if (hasScheduledEvent) {
                    // logger.info(restext.getString("has_scheduled_event"));
                    if (subjectMayReceiveStudyEvent(sm.getDataSource(), definitionScheduled, studySubject)) {
                        ...
                        // YW 11-14-2007
                        if ("-1".equals(fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED + "Hour"))
                            && "-1".equals(fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED + "Minute"))
                            && "".equals(fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED + "Half"))) {
                        ...
                        // YW, 3-12-2008, 2220 fix<<
                        if (!"".equals(strEndScheduled)) {
                            endScheduled = fp.getDateTime(INPUT_ENDDATE_PREFIX_SCHEDULED);
                            if ("-1".equals(fp.getString(INPUT_ENDDATE_PREFIX_SCHEDULED + "Hour"))
                                && "-1".equals(fp.getString(INPUT_ENDDATE_PREFIX_SCHEDULED + "Minute"))
                                && "".equals(fp.getString(INPUT_ENDDATE_PREFIX_SCHEDULED + "Half"))) {
                                studyEventScheduled.setEndTimeFlag(false);
                            } else {
                                studyEventScheduled.setEndTimeFlag(true);
                            }
                        }
                        studyEventScheduled.setDateEnded(endScheduled);
                        // YW >>
                        ...
                        subjectsExistingEvents = sed.findAllByStudyAndStudySubjectId(currentStudy, studySubject.getId());
                        ...
                        AddNewSubjectServlet.saveFieldNotes(INPUT_SCHEDULED_LOCATION, fdn, dndao, studyEventScheduled.getId(), "studyEvent", currentStudy);
                        // YW 3-12-2008, 2220 fix <<
                        AddNewSubjectServlet.saveFieldNotes(INPUT_STARTDATE_PREFIX_SCHEDULED, fdn, dndao, studyEventScheduled.getId(), "studyEvent",
                                currentStudy);
                        AddNewSubjectServlet
                                .saveFieldNotes(INPUT_ENDDATE_PREFIX_SCHEDULED, fdn, dndao, studyEventScheduled.getId(), "studyEvent", currentStudy);
                        // YW >>
                    } else {
                        addPageMessage(restext.getString("scheduled_event_definition") + definitionScheduled.getName() + restext.getString("X_and_subject")
                            + studySubject.getName() + restext.getString("not_created_since_event_not_repeating")
                ...
                // redirecting
                response.sendRedirect(response.encodeRedirectURL("EnterDataForStudyEvent?eventId=" + studyEvent.getId()));
                return;
            }
        }
    }
     ...
     * <li>The subject does not yet have a study event for the given study
     * event definition
     * </ul>
     * 
     ...
     * @return <code>true</code> if the subject may receive an additional
     *         study event, <code>false</code> otherwise.
     */
    ...
    private void setupBeans(ArrayList subjects, ArrayList eventDefinitions) throws Exception {
        addEntityList("subjects", subjects, restext.getString("cannot_create_event_because_no_subjects"), Page.SUBMIT_DATA_SERVLET);
        addEntityList("eventDefinitions", eventDefinitions, restext.getString("cannot_create_event_because_no_event_definitions"), Page.SUBMIT_DATA_SERVLET);

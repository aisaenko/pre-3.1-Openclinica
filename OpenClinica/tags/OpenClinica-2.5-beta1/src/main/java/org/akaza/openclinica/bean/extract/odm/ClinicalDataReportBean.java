...
public class ClinicalDataReportBean extends OdmXmlReportBean {
    ...
    public void addNodeClinicalData() {
        ...
        xml.append(indent + "<ClinicalData StudyOID=\"" + clinicalData.getStudyOID() + "\" MetaDataVersionOID=\"" + this.clinicalData.getMetaDataVersionOID()
            + "\">");
        xml.append("\n");
        ArrayList<SubjectDataBean> subs = (ArrayList<SubjectDataBean>) this.clinicalData.getSubjectData();
        for (SubjectDataBean sub : subs) {
            xml.append(indent + indent + "<SubjectData SubjectKey=\"" + sub.getSubjectOID() + "\">");
            xml.append("\n");
            //
            ArrayList<StudyEventDataBean> ses = sub.getStudyEventData();
            for (StudyEventDataBean se : ses) {
                xml.append(indent + indent + indent + "<StudyEventData StudyEventOID=\"" + se.getStudyEventOID() + "\"");
                ...
                for (FormDataBean form : forms) {
                    xml.append(indent + indent + indent + indent + "<FormData FormOID=\"" + form.getFormOID() + "\">");
                    xml.append("\n");
                    //
                    ArrayList<ImportItemGroupDataBean> igs = form.getItemGroupData();
                    for (ImportItemGroupDataBean ig : igs) {
                        xml.append(indent + indent + indent + indent + indent + "<ItemGroupData ItemGroupOID=\"" + ig.getItemGroupOID() + "\" ");

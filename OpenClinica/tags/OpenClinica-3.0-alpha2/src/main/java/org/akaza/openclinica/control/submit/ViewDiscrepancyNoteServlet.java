...
public class ViewDiscrepancyNoteServlet extends SecureController {
    ...
    public static final String LOCKED_FLAG = "isLocked";// if an event crf is locked, so don't allow to enter notes
    ...
    protected void processRequest() throws Exception {
        ...
        //BWP 5/13/2009 3468 WHO; update the resolution status of  parent disc notes based
        ...
        if (newNotes != null && !newNotes.getNotes(field).isEmpty()) {
            ...
            for (int i = 0; i < newFieldNotes.size(); i++) {
                ...
                if (pId == 0) {// we can only keep one unsaved note because note.id == 0
        ...
        for (int i = 0; i < notes.size(); i++) {
            ...
            // System.out.println("found assigned user id: " + note.getAssignedUserId());
            // UserAccountBean assignedUser = (UserAccountBean) udao.findByPK(note.getAssignedUserId());
        ...
        // noteTree is a Hashmap mapping note id to a parent note, with all the child notes
        // stored in the children List.
        // BWP 3029>>make sure the parent note has an updated resolution status and
        // updated date
    ...
    /**
     * Update a parent DiscrepancyNoteBean's resolution status and updated date to that of the latest child DiscrepancyNoteBean.
     *
     * @param noteTree
     *            A HashMap of an Integer representing the DiscrepancyNoteBean, pointing to a parent DiscrepancyNoteBean.
     */

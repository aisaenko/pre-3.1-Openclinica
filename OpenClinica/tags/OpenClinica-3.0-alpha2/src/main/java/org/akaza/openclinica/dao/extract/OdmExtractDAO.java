...
public class OdmExtractDAO extends DatasetDAO {
    ...
    // public HashSet<Integer> getMetadata(int parentStudyId, int studyId,
    // MetaDataVersionBean metadata, String odmVersion) {
    public void getMetadata(int parentStudyId, int studyId, MetaDataVersionBean metadata, String odmVersion) {
        ...
        logger.error("getItemGroupandItemMetasql= " + this.getItemGroupAndItemMetaSql(cvIds));
        ...
        while (it.hasNext()) {
            ...
            if (!itdset.contains(itId)) {
                ...
                if (regexp != null && regexp.startsWith("func:")) {
                    idef.setRangeCheck(MetadataUnit.getItemRangeCheck(regexp.substring(5).trim(), metadata.getSoftHard(), regexpErr));
                }
    ...
    public void getClinicalData(StudyBean study, DatasetBean dataset, OdmClinicalDataBean data, String odmVersion) {
        ...
        ArrayList viewRows = select(getEventGroupItemSql(studyIds, sedIds, itemIds, dateConstraint, datasetItemStatusId));
        logger.warn("getEventGroupItemSql : " + getEventGroupItemSql(studyIds, sedIds, itemIds, dateConstraint, datasetItemStatusId));
        Iterator iter = viewRows.iterator();
        ExportSubjectDataBean sub = new ExportSubjectDataBean();
        ExportStudyEventDataBean se = new ExportStudyEventDataBean();
        ExportFormDataBean form = new ExportFormDataBean();

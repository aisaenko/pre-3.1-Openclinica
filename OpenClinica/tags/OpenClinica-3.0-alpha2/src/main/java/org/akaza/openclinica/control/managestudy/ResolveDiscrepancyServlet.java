...
public class ResolveDiscrepancyServlet extends SecureController {
    ...
    public Page getPageForForwarding(DiscrepancyNoteBean note, boolean isCompleted) {
         ...
         * BWP>> 2966, commented this out: else if ("eventcrf".equalsIgnoreCase(entityType)) { return Page.TABLE_OF_CONTENTS_SERVLET; //
    ...
    public boolean prepareRequestForResolution(HttpServletRequest request, DataSource ds, StudyBean currentStudy, DiscrepancyNoteBean note, boolean isCompleted) {
        ...
        if ("subject".equalsIgnoreCase(entityType)) {
            ...
            // If the request is passed along to ViewSectionDataEntryServlet, that code needs
            // an event crf id; the (ecb.getId()+"") is necessary because FormProcessor throws
    ...
    protected void processRequest() throws Exception {
        ...
        if (noteIsClosed(discrepancyNoteBean)) {
        ...
        //BWP 03/17/2009 3166: if it's not an ItemData type note, redirect Monitors to View Subject or
        //View Study Events <<
        if(currentRole.getRole().equals(Role.MONITOR) &&
           ! "itemdata".equalsIgnoreCase(entityType) && ! "eventcrf".equalsIgnoreCase(entityType)) {
        ...
        String createNoteURL = CreateDiscrepancyNoteServlet.getAddChildURL(discrepancyNoteBean, ResolutionStatus.CLOSED, toView);
    ...
    /**
     * Determines if a discrepancy note is closed or not. The note is closed if it has status closed, or any of its children have closed status.
     *
     * @param note
    ...
    protected void mayProceed() throws InsufficientPermissionException {
        ...
        //Integer subjectId = (Integer) session.getAttribute("subjectId");
        /*
         * BWP: This caused a problem with page refreshing (the subjectId was lost); so I had to comment it out if(subjectId != null){
        ...
        // BWP 11/03/2008 3029: redirect monitor user to ViewStudySubject if they click "add note to thread" link>>
        // if (currentRole.getRole().equals(Role.MONITOR)) {
        // addPageMessage(respage.getString("no_have_permission_to_resolve_discrepancy"));
        //
        // RequestDispatcher dispatcher = request.getRequestDispatcher("/ViewStudySubject?id=" + subjectId + "&module=" + module);
        // try {
        ...
        // return;
        // }
        // tbh 02/2009: now removed, to allow for the Query workflow and allow a Monitor to Resolve a Query.
    ...
    /**
     * Redirect the request to another page if the user is a Monitor type and the
     * discrepancy note is a type other than item data or event crf.
    ...
    private void redirectMonitor(String module,DiscrepancyNoteBean discrepancyNoteBean){
        ...
        if(discrepancyNoteBean != null){
            ...
            //This String will determine whether the type is other than itemdata.
            String entityType = discrepancyNoteBean.getEntityType().toLowerCase();
...
//                addPageMessage(resword.getString("monitors_do_not_have_permission_to_resolve_discrepancy_notes"));
                if("studySub".equalsIgnoreCase(entityType)){
                    ...
                    int studySubId = discNoteUtil.getStudySubjectIdForDiscNote(discrepancyNoteBean,
                      sm.getDataSource(),currentStudy.getId());
                    ...
                    dispatcher = request.getRequestDispatcher("/ViewStudyEvents?&module=" + module);

                }

                //This code creates the URL for a popup window, which the processing Servlet will initiate.
                //'true' parameter means that ViewDiscrepancyNote is the handling Servlet.
                createNoteURL =

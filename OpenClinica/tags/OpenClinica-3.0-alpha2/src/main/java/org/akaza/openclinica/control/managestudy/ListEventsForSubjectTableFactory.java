...
public class ListEventsForSubjectTableFactory extends AbstractTableFactory {
    ...
    protected void configureColumns(TableFacade tableFacade, Locale locale) {
        ...
        configureColumn(row.getColumn(columnNames[2]), resword.getString("gender"), null, null);

        // group class columns
        for (int i = 3; i < 3 + studyGroupClasses.size(); i++) {
            StudyGroupClassBean studyGroupClass = studyGroupClasses.get(i - 3);
        ...
        configureColumn(row.getColumn(columnNames[3 + studyGroupClasses.size()]), "Event Status", new EventStatusCellEditor(),
                new SubjectEventStatusDroplistFilterEditor(), true, false);
        configureColumn(row.getColumn(columnNames[4 + studyGroupClasses.size()]), "Event Date", new EventStartDateCellEditor(), null);

        // crf columns
        for (int i = 5 + studyGroupClasses.size(); i < columnNames.length - 1; i++) {
            CRFBean crfBean = crfBeans.get(i - (5 + studyGroupClasses.size()));
    ...
    private ArrayList<CRFBean> getCrfs(StudyEventDefinitionBean eventDefinition) {
        if (this.crfBeans == null) {
            ...
            for (EventDefinitionCRFBean eventDefinitionCrf : (List<EventDefinitionCRFBean>) getEventDefintionCRFDAO().findAllActiveByEventDefinitionId(
                ...
                crfBeans.add((CRFBean) getCrfDAO().findByPK(eventDefinitionCrf.getCrfId()));

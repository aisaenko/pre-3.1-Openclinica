...
public class ListStudySubjectTableFactory extends AbstractTableFactory {
    ...
    protected void configureColumns(TableFacade tableFacade, Locale locale) {
        ...
        configureColumn(row.getColumn(columnNames[2]), resword.getString("rule_oid"), null, null);
        configureColumn(row.getColumn(columnNames[3]), resword.getString("gender"), null, null);
        configureColumn(row.getColumn(columnNames[4]), resword.getString("secondary_label"), null, null);
        // group class columns
        for (int i = 5; i < 5 + studyGroupClasses.size(); i++) {
            StudyGroupClassBean studyGroupClass = studyGroupClasses.get(i - 5);
            // configureColumn(row.getColumn(columnNames[i]), studyGroupClass.getName(), new StudyGroupClassCellEditor(studyGroupClass), null, false, false);
        ...
        for (int i = 5 + studyGroupClasses.size(); i < columnNames.length - 1; i++) {
            StudyEventDefinitionBean studyEventDefinition = studyEventDefinitions.get(i - (5 + studyGroupClasses.size()));
        ...
        configureColumn(row.getColumn(columnNames[columnNames.length - 1]), actionsHeader, new ActionsCellEditor(), null, false, false);
    ...
    /**
     * A very custom way to sort the items. The PresidentSort acts as a command for the Hibernate criteria object. There are probably many ways to do this, but
     * this is the most flexible way I have found. The point is you need to somehow take the Limit information and sort the rows.
     * 
     * @param limit
     *            The Limit to use.
     */
    ...
    private class StudyEventDefinitionCellEditor implements CellEditor {
        ...
        private String getCount() {
            return studyEvents.size() < 2 ? "" : "&nbsp;&nbsp;&nbsp;(" + String.valueOf(studyEvents.size() + ")");
        }
        ...
        public Object getValue(Object item, String property, int rowcount) {
            ...
            if (studySubjectId != null) {
                ...
                if (getCurrentRole().getRole() != Role.MONITOR) {
                    ...
                    if ((getCurrentRole().getRole() == Role.STUDYDIRECTOR || getCurrentRole().getRole() == Role.COORDINATOR)

...
public class DiscrepancyNoteUtil {
     ...
     * @param discrepancyNoteThreads A List of DiscrepancyNoteThreads
     */
    public void injectDiscNotesIntoDisplayStudyEvents(List<DisplayStudyEventBean>
      displayStudyBeans, Set<Integer> resolutionStatusIds,
        ...
        for(DisplayStudyEventBean dStudyEventBean : displayStudyBeans) {
            ...
            for(EventCRFBean eventCrfBean : eventCRFBeans){
                ...
                foundDiscNotes =
                  discrepancyNoteDAO.findItemDataDNotesFromEventCRF(eventCrfBean);
                //return only parent beans
                foundDiscNotes = filterDiscNotesForOnlyParents(foundDiscNotes,discrepancyNoteThreads);
                //update the resolution status of these parent discrepancy notes
    ...
    /**
     * Cycle through the DiscrepancyNoteThread objects, and update the resolution status
     * of each discrepancy note bean, based on the related DiscrepancyNoteThread's
     *  latestResolutionStatus property.
     * @param foundDiscNotes  A List of parent DiscrepancyNoteBeans.
     * @param discrepancyNoteThreads A List of DiscrepancyNoteThreads.
     */
    public void updateStatusUsingThread(List<DiscrepancyNoteBean> foundDiscNotes,
        ...
        beginFor:
        for(DiscrepancyNoteThread discrepancyNoteThread : discrepancyNoteThreads){
            ...
            for(DiscrepancyNoteBean tmpBean : foundDiscNotes){
                //each of these disc beans is a parent; therefore, we find its thread by
                //comparing its id to the id of the first bean in the thread (which are parents)
    ...
    /**
     * Take a List of DiscrepancyNoteBeans, and filter the List to return only "parent"
     * DiscrepancyNoteBeans. the parameter List<DiscrepancyNoteThread> contains each
     * parent the DiscrepancyNoteBeans are compared to in order to filter the original List.
     * @param dnBeans  A List of DiscrepancyNoteBeans.
     * @param dnThreads  A List of DiscrepancyNoteThreads.
     * @return  The filtered List of DiscrepancyNoteBeans.
     */
    public List<DiscrepancyNoteBean> filterDiscNotesForOnlyParents(
            ...
            for(DiscrepancyNoteThread dnThread : dnThreads){
                ...
                if(tempBean != null) {
                    if(tempBean.getId() == noteBean.getId()) {
                        ...
                        //a noteBean matches a parent, so continue with the next noteBean
                        continue outer;
                    }
                }
            }
    ...
    public void injectParentDiscNotesIntoDisplayStudyEvents(List<DisplayStudyEventBean>
      displayStudyBeans, Set<Integer> resolutionStatusIds,
                                                            DataSource dataSource, int discNoteType) {
        ...
        for(DisplayStudyEventBean dStudyEventBean : displayStudyBeans) {
            ...
            for(EventCRFBean eventCrfBean : eventCRFBeans){
                //Find ItemData type notes associated with an event crf
                foundDiscNotes =
                  discrepancyNoteDAO.findParentItemDataDNotesFromEventCRF(eventCrfBean);
    ...
    /**
     * Get all discrepancy notes for a study, including
     * "threads" of parent and child discrepancy notes.
     * @param currentStudy    A StudyBean object.
     * @param resolutionStatusIds   A HashSet object consisting of resolution status ids (i.e., 1,2,3).
     * @param dataSource A DataSource used for various DAO methods.
     * @param discNoteType  An int discrepancy note type id; only DiscrepancyNoteBeans will be returned if
    ...
    public List<DiscrepancyNoteBean> getThreadedDNotesForStudy(StudyBean currentStudy,
        ...
        if(parentStudyId > 0){
            ...
            parentItemDataNotes = discrepancyNoteDAO.findAllItemDataByStudy(parentStudy);
    ...
    public List<DiscrepancyNoteThread> getDNotesForStudyAsThreads(StudyBean currentStudy,
        ...
        List<DiscrepancyNoteBean> allDiscNotes =
          new ArrayList<DiscrepancyNoteBean>();
        List<DiscrepancyNoteThread> threadedNotes =
          new ArrayList<DiscrepancyNoteThread>();
        if(currentStudy == null) return threadedNotes;
        ...
        //convert or encapsulate List of notes in List of disc note thread objects
        threadedNotes = createThreadsOfParents(allDiscNotes,dataSource,currentStudy, null, 0, false);

        return threadedNotes;
    }

    public List<DiscrepancyNoteThread> createThreadsOfParents(
      List<DiscrepancyNoteBean> allDiscNotes,
      DataSource dataSource, StudyBean currentStudy,
      ...
      boolean includeEventCRFNotes){
        ...
        if(currentStudy == null) { currentStudy = new StudyBean(); }
        ...
        for(DiscrepancyNoteBean discBean : allDiscNotes){
            ...
            childDiscBeans = discrepancyNoteDAO.
              findAllItemDataByStudyAndParent(currentStudy,discBean);
            if(includeEventCRFNotes) {
                eventCRFChildDiscBeans =
                  discrepancyNoteDAO.findAllEventCRFByStudyAndParent(currentStudy,discBean);
            }
            if(! eventCRFChildDiscBeans.isEmpty()){
                childDiscBeans.addAll(eventCRFChildDiscBeans);
                //sort the discnote beans, because we have eventCRF and ItemDate types mixed here.
                Collections.sort(childDiscBeans);
            }
            resolutionStatusId= discBean.getResolutionStatusId();
            //the thread's status id is the parent's in this case, when there are no children
            tempDNThread.setLatestResolutionStatus(
              this.getResolutionStatusName(resolutionStatusId));

            if(! childDiscBeans.isEmpty()) {

                for(DiscrepancyNoteBean childBean : childDiscBeans){
                    if(childBean.getResolutionStatusId() != resolutionStatusId) {
                        //BWP issue 3468 WHO 5/2009: this local variable needs to be updated>>
                        resolutionStatusId = childBean.getResolutionStatusId();
                        //<<
                        tempDNThread.setLatestResolutionStatus(
                          this.getResolutionStatusName(childBean.getResolutionStatusId()));
                    }
        ...
        //Do the filtering here; remove any DN threads that do not have any notes
    ...
    public List<DiscrepancyNoteThread> removeEmptyDNThreads(
      List<DiscrepancyNoteThread> allDNThreads){
     ...
     * @return  Only any DiscrepancyNoteBeans that have this type id.
     */
    public int getDiscNoteCountByStatusEventCRFId(
      List<DiscrepancyNoteBean> discrepancyNoteBeans,
      int resStatusId,int eventCRFId) {
        int typeCount=0;
        for( DiscrepancyNoteBean dBean : discrepancyNoteBeans){
            if(dBean.getResolutionStatusId() == resStatusId &&
              dBean.getEventCRFId() == eventCRFId){
                typeCount++;
            }
        }
    ...
    public Map<Integer,Map<String,Integer>>  createDiscNoteMapByEventCRF(
    ...
    private Map<String,Integer> getDiscNoteCountFromDisplayEvents(
      List<DisplayStudyEventBean> disBeans, int eventCRFId){
        ...
        for(DisplayStudyEventBean eventBean : disBeans) {
            ...
            for(String statusName : RESOLUTION_STATUS.keySet()) {
                discNoteMap.put(statusName,
                  getDiscNoteCountByStatusEventCRFId(dnBeans,
                    RESOLUTION_STATUS.get(statusName),
                    eventCRFId));
            }
        }
    ...
    private SortedSet<Integer> getEventCRFIdsFromDisplayEvents(
      List<DisplayStudyEventBean>
        displayEvents) {
    ...
    public List<DiscrepancyNoteBean> filterDiscNotesBySubjectOrStudySubject(
      List<DiscrepancyNoteBean> allDiscNotes,
      int subjectId,  int studySubjectId) {
        //This collection will hold the beans that are associated with either the subject id or
        //study subject id
    ...
    public int getStudySubjectIdForDiscNote(DiscrepancyNoteBean discrepancyNoteBean,
                                            DataSource dataSource,int studyId){
        if(discrepancyNoteBean == null) return 0;
        ...
        if("subject".equalsIgnoreCase(discrepancyNoteBean.getEntityType())) {
            studySubjectBeans = studySubjectDAO.findAllBySubjectId(
              discrepancyNoteBean.getEntityId());

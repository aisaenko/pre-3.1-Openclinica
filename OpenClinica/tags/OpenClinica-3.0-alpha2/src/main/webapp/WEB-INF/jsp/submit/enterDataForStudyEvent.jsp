    ...
    <table>
        <tr>
            <c:choose>
                ...
                <c:when test="${!userRole.monitor && studySubject.status.name != 'removed'&& studySubject.status.name != 'auto-removed' && study.status.available}">
                    <td class="table_cell">
...
<form method="POST" action="ViewStudySubject">
    ...
    <input type="button" onclick="confirmExit('ListStudySubjectsSubmit');"  name="exit" value="<fmt:message key="exit" bundle="${resword}"/>   " class="button_medium"/>
</form>

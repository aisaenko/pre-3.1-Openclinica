...
<h1><span class="title_submit"><fmt:message key="validation_for" bundle="${resword}"/> <c:out value="${section.crf.name}" /> <c:out value="${section.crfVersion.name}" /></span></h1>
<%--</div>--%>
...
<span id="mbm">
Marking this CRF complete will finalize data entry. You will be allowed to edit the data later but this data entry stage is completed. If Double Data Entry is required, you or another user may need to complete this CRF again before it is verified as complete. If you are sure you want to mark this CRF complete, please enter your password in the box below. Then select OK.
</span><br>
...
function DisplaySectionTabs()
{
...

    {
        ...
        url = "DoubleDataEntry?eventCRFId=" + <c:out value="${section.eventCRF.id}"/> + "&sectionId=" + sectionId + "&tab=" + TabID;
...
<td>&nbsp;
    <div class="formfieldM_BG_noMargin"><select class="formfieldM" name="sectionName" size="1" onchange="gotoLink();">
        ...
        <c:forEach var="sec" items="${toc.sections}" >
            <c:set var="tabUrl" value = "DoubleDataEntry?eventCRFId=${section.eventCRF.id}&sectionId=${sec.id}&tab=${tabCount}"/>

...
<fmt:setBundle basename="org.akaza.openclinica.i18n.format" var="resformat"/>
  ...
  <table border="1" cellpadding="2" cellspacing="0" style="background-color:#CCCCCC">
    ...
    <tr >
			  ...
			  <input type="hidden" name="addWithEvent" value="1"/>
				<c:choose>
			      ...
			      <c:when test="${study.studyParameterConfig.subjectIdGeneration =='auto editable'}">
			       <input onfocus="this.select()" type="text" name="label" tabindex ="<c:out value="${tabCount}"/>" value="<c:out value="${label}"/>" size="12" class="formfield">
			      </c:when>
			      <c:otherwise>
			        <input onfocus="this.select()" type="text" name="label" tabindex ="<c:out value="${tabCount}"/>" value="<fmt:message key="study_subject_ID" bundle="${resword}"/>" size="12" class="formfield">
			      </c:otherwise>
			    </c:choose>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					...
					<td valign="top" align="right">
					 <input type="text" name="location" tabindex="<c:out value="${tabCount}"/>" size="8" value="<fmt:message key="location" bundle="${resword}"/>" class="formfield" />*
					</td>
					 <c:set var="tabCount" value="${tabCount+1}"/>
					<td valign="top">
					<input type="text" name="startDate" size="15" value="<fmt:message key="eventMMDDYYYY" bundle="${resword}"/>" tabindex="<c:out value="${tabCount}"/>" class="formfield" id="enrollmentDateField"/>
					<span class="formlabel">*</span></td>
					 <c:set var="tabCount" value="${tabCount+1}"/>
			...
			<td valign="top" align="right" class="table_cell" nowrap>
				<input type="button" onclick="hm('box');" name="cancel" value="<fmt:message key="cancel" bundle="${resword}"/>" />
                <input type="submit" name="addSubject" value="<fmt:message key="add" bundle="${resword}"/>" />
			</td>
		...
		<tr>
			<td valign="top" align="left" colspan="2" class="table_cell_left" nowrap>
			 <c:choose>
	           <c:when test="${study.studyParameterConfig.subjectPersonIdRequired =='required'}">
			    &nbsp;<input type="text" name="uniqueIdentifier" value="<fmt:message key="person_ID" bundle="${resword}"/>" size="12" tabindex="<c:out value="${tabCount}"/>" class="formfield"><span class="formlabel">*</span>
			   </c:when>
			   <c:when test="${study.studyParameterConfig.subjectPersonIdRequired =='optional'}">
			    &nbsp;<input type="text" name="uniqueIdentifier" value="<fmt:message key="person_ID" bundle="${resword}"/>" size="12" tabindex="<c:out value="${tabCount}"/>" class="formfield">
			   </c:when>
			   <c:otherwise>
				...
				<tr>
					<td valign="top" align="right">
					 <c:choose>
						...
						&nbsp;<input type="text" name="dob" size="20" value="<fmt:message key="DOB" bundle="${resword}"/>" tabindex="<c:out value="${tabCount}"/>" class="formfield" id="dobField" ><span class="formlabel">*</span>
						</td>
					    ...
					    <c:when test="${study.studyParameterConfig.collectDob == '2'}">
					     &nbsp;<input type="text" name="yob" size="15" value="YOB: YYYY" class="formfield" tabindex="<c:out value="${tabCount}"/>"/><span class="formlabel">*</span>
					    </c:when>
					    <c:otherwise>
			...
			<tr>
			 <td valign="top" align="left">
			  <input type="text" name="enrollmentDate" size="20" value="<fmt:message key="enrollMMDDYYYY" bundle="${resword}"/>" tabindex="<c:out value="${tabCount}"/>" class="formfield" id="enrollmentDateField2"/>
			  <span class="formlabel">*</span></td>
					 <c:set var="tabCount" value="${tabCount+1}"/>
				     <td valign="top"><A HREF="#" >
			...
			<td valign="top" nowrap align="right" colspan="8" class="table_cell">
             <a href="AddNewSubject"><fmt:message key="enter_full_record_details" bundle="${resword}"/></a></td>
		</tr>
    </form>
</table>

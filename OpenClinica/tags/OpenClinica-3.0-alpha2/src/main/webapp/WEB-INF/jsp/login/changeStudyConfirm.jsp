...
<h1><fmt:message key="confirm_changing_study" bundle="${resword}"/></h1>


<form action="ChangeStudy" method="post">
<input type="hidden" name="action" value="submit">
<input type="hidden" name="studyId" value="<c:out value="${studyId}"/>">
<p><fmt:message key="choosing_switch_to_study" bundle="${restext}"/>: <c:out value="${studyWithRole.studyName}"/> <fmt:message key="with_a_role_of" bundle="${restext}"/> <c:out value="${studyWithRole.role.description}"/>.</p>
<br>

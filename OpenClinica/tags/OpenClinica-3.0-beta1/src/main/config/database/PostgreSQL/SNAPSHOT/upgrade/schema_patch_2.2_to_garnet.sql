...
update study set oc_oid = substring(upper(regexp_replace(unique_identifier, E'\s+|\W+', '','g')),0,9) || study_id;

-- generate OIDs for study_subject
update study_subject set oc_oid = substring(upper(regexp_replace(label, E'\s+|\W+', '','g')),0,9) || study_subject_id;
...
----
---new trigger to log the case of a new repeating row is added during DDE/ADE
---

INSERT INTO audit_log_event_type VALUES (30,'Item data inserted for repeating row');
...
--
-- Add show_secondary_id to dataset ; Add secondary_label to extract_data_table
--  
ALTER TABLE dataset ADD COLUMN show_secondary_id BOOLEAN DEFAULT false;
ALTER TABLE extract_data_table ADD COLUMN secondary_label character;

...
<c:if test='${inputType=="file"}'>
	...
	<c:otherwise>
		...
		<c:otherwise>
			...
			<a href="javascript:openDocWindow('DownloadAttachedFile?eventCRFId=<c:out value="${section.eventCRF.id}"/>&fileName=${fn:replace(filename,sep,sep2)}')" id="a<c:out value="${itemId}"/>"><c:out value="${inputTxtValue}"/></a>
		</c:otherwise>
		</c:choose>
	</c:otherwise>
	</c:choose>

	...
	<c:choose>
		<c:when test="${isAnExternalValue == '1'}">
			...
			<script>
				...
				{
					window.attachEvent("onmessage", receiver<c:out value="${inputName}"/>); // for IE
				}
				else 
				{
					window.addEventListener("message", receiver<c:out value="${inputName}"/>, false); // for FF
				}
				function receiver<c:out value="${inputName}"/>(e) {

...
<c:if test='${inputType=="file"}'>
	...
	<c:otherwise>
		...
		<input id="rp<c:out value="${itemId}"/>" type="button" value="<fmt:message key="replace" bundle="${resword}"/>" onClick="replaceSwitch('<c:out value="${section.eventCRF.id}"/>','<c:out value="${itemId}"/>','rp','value','Replace','Cancel Replace','<c:out value="${inputTxtValue}"/>','<c:out value="${pathAndName}"/>','notFound')">
		<input id="rm<c:out value="${itemId}"/>" type="button" value="<fmt:message key="remove" bundle="${resword}"/>" onClick="removeSwitch('<c:out value="${section.eventCRF.id}"/>','<c:out value="${itemId}"/>','rm','value','Remove','Cancel Remove','<c:out value="${inputTxtValue}"/>','<c:out value="${pathAndName}"/>','notFound')">
		</c:when>
		<c:otherwise>
			<a href="DownloadAttachedFile?eventCRFId=<c:out value="${section.eventCRF.id}"/>&fileName=<c:out value="${pathAndName}"/>" id="a<c:out value="${itemId}"/>"><c:out value="${inputTxtValue}"/></a>
		</div><br>
    ...
    <c:forEach var="option" items="${displayItem.metadata.responseSet.options}">
        ...
        <c:choose>
            <c:when test="${! printDefaultFirst}">
                <option value="<c:out value="${option.value}" />" <c:if test="${option.selected}">
                      selected="selected"
                  </c:if>
                  >

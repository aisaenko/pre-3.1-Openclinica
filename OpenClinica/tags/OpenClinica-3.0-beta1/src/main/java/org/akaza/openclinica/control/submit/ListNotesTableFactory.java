...
public class ListNotesTableFactory extends AbstractTableFactory {
    ...
    protected void configureColumns(TableFacade tableFacade, Locale locale) {
        ...
        configureColumn(row.getColumn("discrepancyNoteBean.resolutionStatus"), "Resolution status", new ResolutionStatusCellEditor(), new ResolutionStatusDroplistFilterEditor(), true, false);
    ...
    public void setDataAndLimitVariables(TableFacade tableFacade) {
        ...
        if (!limit.isComplete()) {
            int totalRows = getDiscrepancyNoteDao().getCountWithFilter(listNotesFilter, getCurrentStudy());
        ...
        ArrayList<DiscrepancyNoteBean> items =
            getDiscrepancyNoteDao().getWithFilterAndSort(getCurrentStudy(), listNotesFilter, listNotesSort, rowStart, rowEnd);
        Collection<HashMap<Object, Object>> theItems = new ArrayList<HashMap<Object, Object>>();
        populateRowsWithAttachedData(items);
        ...
        public Object getValue(Object item, String property, int rowcount) {
            ...
            if(!dnb.getResStatus().isClosed() && !dnb.getResStatus().isNotApplicable()) {
                if (dnb.getEntityType() != "eventCrf") {
                    ...
                    builder.close();
                    builder.img().name("bt_Reassign1").src("images/bt_Reassign_d.gif").border("0").alt(resword.getString("resolve_discrepancy_note")).title(resword.getString("resolve_discrepancy_note")).align("left").append("hspace=\"6\"").close();
                    builder.aEnd();
                } else {
                    if (dnb.getStageId() == 5) {
                        builder.a().href("ResolveDiscrepancy?noteId=" + dnb.getId());

...
public class ListDiscNotesSubjectTableFactory extends AbstractTableFactory {
            ...
            for (StudyEventDefinitionBean studyEventDefinition : getStudyEventDefinitions()) {
                ...
                if (studyEvents.size() < 1) {
                    ...
                    for (StudyEventBean studyEventBean : studyEvents) {
                        List<DiscrepancyNoteBean> discs = getDiscrepancyNoteDAO().findAllByStudyEventWithConstraints(studyEventBean, constraints);
    ...
    private class StudyEventDefinitionMapCellEditor implements CellEditor {
        ...
        private String getCount() {
            return studyEvents.size() < 2 ? "" : "&nbsp;&nbsp;&nbsp;<strong>[" + String.valueOf(studyEvents.size() + "]</strong>");
        }
        ...
        public Object getValue(Object item, String property, int rowcount) {
            ...
            StringBuilder url = new StringBuilder();
            // url.append(eventDivBuilder(subject, rowcount, studyEvents,
            // studyEventDefinition, studySubjectBean));
            url.append("<img src='" + imageIconPaths.get(subjectEventStatus.getId()) + "' border='0' style='position: relative; left: 7px;'>");
            url.append(getCount() + "<br/>");
            for (ResolutionStatus key : discCounts.keySet()) {
                url.append(discNoteIconPaths.get(key.getId()) + "(" + discCounts.get(key) + ") <br/>");
            }

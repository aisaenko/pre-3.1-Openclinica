...
public class SpreadSheetTableClassic implements SpreadSheetTable {// extends
                        ...
                        if (unit != null && unit.length() > 0) {
                            ...
                            if (this.existingUnits.size() > 0) {
                                if (this.existingUnits.contains(unit.toUpperCase())) {
                                    ...
                                    muSql = this.getMUInsertSql(unit, ub.getId(), dbName);
                                    queries.add(muSql);
    ...
    private String getMUInsertSql(String measurementUnitName, int ownerId, String dbName) {
        String oid = "";
        try {
            oid = new MeasurementUnitOidGenerator().generateOid(measurementUnitName);
        } catch (Exception e) {
        ...
        if (dbName.equals("oracle")) {
            muSql =
                "insert into measurement_unit (oc_oid, name, owner_id, date_created) values ('" + oid + "', '" + stripQuotes(measurementUnitName) + "', "
                    + ownerId + ", sysdate')";
        } else {
            muSql =
                "insert into measurement_unit (oc_oid, name, owner_id, date_created) values ('" + oid + "', '" + stripQuotes(measurementUnitName) + "', "

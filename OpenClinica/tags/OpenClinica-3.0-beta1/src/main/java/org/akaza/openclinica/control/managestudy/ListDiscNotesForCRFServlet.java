...
public class ListDiscNotesForCRFServlet extends SecureController {
    ...
    public void processRequest() throws Exception {
        ...
        ArrayList eventDefinitionCRFs = (ArrayList) edcdao.findAllActiveParentsByEventDefinitionId(definitionId);
         ...
         * discNoteUtil.getThreadedDNotesForStudy(sbean,
         * resolutionStatusIds,sm.getDataSource(), discNoteType, true);
         */

        // Get disc note threads for the study
        List<DiscrepancyNoteThread> allThreadedDiscNotes =
            discNoteUtil.getDNotesForStudyAsThreads(sbean, resolutionStatusIds, sm.getDataSource(), discNoteType);

        // Don't loop through the subjects if there aren't any notes in the
        // study
        if (!(allThreadedDiscNotes == null || allThreadedDiscNotes.isEmpty())) {
            for (int i = 0; i < subjects.size(); i++) {
                ...
                for (int k = 0; k < events.size(); k++) {
                    ...
                    DisplayStudyEventBean dseb =
                        ListStudySubjectServlet.getDisplayStudyEventsForStudySubject(studySub, seb, sm.getDataSource(), ub, currentRole, sbean);
                ...
                discNoteUtil.injectDiscNotesIntoDisplayStudyEvents(displayEvents, resolutionStatusIds, sm.getDataSource(), discNoteType, allThreadedDiscNotes);

                // Create a request attribute indicating whether or not there
                // are any disc notes for the study
                /*
                 * allDiscNotes =
                 * discNoteUtil.getDNotesForStudy(this.currentStudy,
                 * resolutionStatusIds,sm.getDataSource(), discNoteType);
                 */
                boolean studyHasDiscNotes = allThreadedDiscNotes != null && !allThreadedDiscNotes.isEmpty();
                ...
                // if there the studysubject bean does not have any DN's then do
                // not add it to this list.
        ...
        // table.setQuery("ListDiscNotesForCRFServlet?tab=" + tabId, new
        // HashMap());
        ...
        factory.setStudyHasDiscNotes(allThreadedDiscNotes != null && !allThreadedDiscNotes.isEmpty());

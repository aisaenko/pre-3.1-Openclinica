        ...
        if (!isSubmitted) {
                ...
                if (diwb.isInGroup()) {
                        ...
                        for (DisplayItemBean displayItem : items) {
                            ...
                            if (responseTypeId == 8 || responseTypeId == 9) {
                                ...
                                if (robBean.getValue().startsWith("func: getexternalvalue") || robBean.getValue().startsWith("func: getExternalValue")) {
                                    ...
                                    //System.out.println("found in fp line 816: "
                                    // + fp.getString(displayItem.getItem().
                                    // getName()));
                                    // System.out.println(
                                    // "scoreitemdata LINE 817: " +
                    ...
                    if (responseTypeId == 8 || responseTypeId == 9) {
                        ...
                        if (robBean.getValue().startsWith("func: getexternalvalue") || robBean.getValue().startsWith("func: getExternalValue")) {
                            ...
                            // System.out.println("found in fp: " +
                            // fp.getString(dib.getItem().getName()));
            ...
            if (this.isAdminForcedReasonForChange() && this.isAdministrativeEditing()) {
                ...
                if (changedItemsList.size() > 0) {
                    System.out.println("found admin force reason for change: changed items " + changedItems.toString());
                    ...
                    for (DisplayItemBean displayItem : changedItemsList) {
                        ...
                        if (existingNotes > 0) {
                            ...
                            // numNotes++;
                            String itemName = displayItem.getItem().getName();

                            // ArrayList notes = discNotes.getNotes(itemName);
                            ArrayList<DiscrepancyNoteBean> dnNotes = dndao.findAllDNotesByItemNameAndEventCRF(ecb, itemName);
                            int nonforcedChangesList = 0;
                            for (DiscrepancyNoteBean dnNote : dnNotes) {

                                if (!dnNote.getDisType().equals(DiscrepancyNoteType.REASON_FOR_CHANGE)) {
                                    nonforcedChangesList++;
                                }
                            }
                            System.out.println("non forced changes: " + nonforcedChangesList + " and dnNotes size: " + dnNotes.size());
                            if (nonforcedChangesList == dnNotes.size()) {
                                // if it's less it has at least one reason for
                                // change DN
                                nonforcedChanges++;
                                // v.addError(errors, itemName,
                                // "Please enter a reason for change discrepancy note before saving."
                                // );
                            ...
                            // numNotes++;
                            ArrayList<DiscrepancyNoteBean> onlineNotes = (ArrayList) idNotes.get(idb.getId());
                            int nonforcedChangesOnline = 0;
                            for (DiscrepancyNoteBean dnNote : onlineNotes) {
                                if (!dnNote.getDisType().equals(DiscrepancyNoteType.REASON_FOR_CHANGE)) {
                                    nonforcedChangesOnline++;
                                }
                                System.out.println("non forced changes online: " + nonforcedChangesOnline + " and online Notes size: " + onlineNotes.size());
                            }
                            if (nonforcedChangesOnline == onlineNotes.size()) {
                                // if it's less it has at least one reason for
                                // change DN
                                nonforcedChanges++;
                                // v.addError(errors, itemName,
                        ...
                        // System.out.println("disc notes: " +
                        // discNotes.getNotes(formName).toString());
                        // if
                        // (discNotes.getNotes(displayItem.getItem().getName()
                        // ).size() == 0) {
                        // String itemName = displayItem.getItem().getName();
                        //            				
                        // // ArrayList notes = discNotes.getNotes(itemName);
                        // ArrayList<DiscrepancyNoteBean> dnNotes =
                        // dndao.findAllDNotesByItemNameAndEventCRF(ecb,
                        // itemName);
                        // int nonforcedChangesList = 0;
                        // for (DiscrepancyNoteBean dnNote: dnNotes) {
                        //            					
                        // if (!dnNote.getDisType().equals(DiscrepancyNoteType.
                        // REASON_FOR_CHANGE)) {
                        // nonforcedChangesList++;
                        // }
                        // }
                        // System.out.println("non forced changes: " +
                        // nonforcedChangesList + " and dnNotes size: " +
                        // dnNotes.size());
                        // if (nonforcedChangesList == dnNotes.size()) {
                        // nonforcedChanges++;
                        // // v.addError(errors, itemName,
                        ...
                        // }
                        // } else {
                        // // need to check to see if 'live' dn's exist
                        // nonforcedChanges++;
                        // // v.addError(errors, itemName,
                        // "Please enter a reason for change discrepancy note before saving."
                        // );

...
public class SubjectIdSDVFactory extends AbstractTableFactory {
    ...
    protected void configureColumns(TableFacade tableFacade, Locale locale) {
        ...
        String[] allTitles =
            new String[] { "SDV Status", "Study Subject ID", " Site ID", "Person ID", "Study Subject Status", "Group", "# of CRFs Completed or Greater",
                "# of CRFs SDV'd", "Total Event CRFs", "Actions" };
        sdvUtil.setTitles(allTitles, (HtmlTable) tableFacade.getTable());
        sdvUtil.turnOffFilters(tableFacade, new String[] { "sdvStatus", "personId", "numberCRFComplete", "numberOfCRFsSDV", "totalEventCRF", "actions" });
    ...
    */
    public int getTotalRowCount() {
        ...
        return studySubDAO.countAllByStudySDV(studyId, studyId);

    }

    @Override
    public void configureTableFacadePostColumnConfiguration(TableFacade tableFacade) {
        tableFacade.setToolbar(new SDVToolbarSubject());
    }

    @SuppressWarnings("unchecked")
    private Collection<SubjectAggregateContainer> getFilteredItems(FilterSet filterSet, SortSet sortSet, int rowStart, int rowEnd) {

        List<SubjectAggregateContainer> rows = new ArrayList<SubjectAggregateContainer>();
        StudySubjectDAO studySubjectDAO = new StudySubjectDAO(dataSource);
        List<StudySubjectBean> studySubjectBeans = studySubjectDAO.findAllByStudySDV(studyId, studyId, rowEnd - rowStart, rowStart);
    ...
    String getIconForCrfStatusPrefix() {
        String prefix = "../";
        return "<img hspace='2' border='0'  title='SDV Status' alt='SDV Status' src='" + prefix + "images/icon_";
    }

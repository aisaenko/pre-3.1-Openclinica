    ...
    <tr id="AddSubjectRow1" style="display:none">
		<form id="subjectForm" name="subjectForm" action="AddNewSubject" method="post">
			  ...
			  <input type="hidden" name="addWithEvent" value="1"/>
				<c:choose>
			      <c:when test="${study.studyParameterConfig.subjectIdGeneration =='auto non-editable'}">
			       <input type="text" value="<c:out value="${label}"/>" size="12" class="formfield" disabled>
			      ...
			      <c:when test="${study.studyParameterConfig.subjectIdGeneration =='auto editable'}">
			       <input onfocus="this.select()" type="text" name="label" value="<c:out value="${label}"/>" size="12" class="formfield">
			      </c:when>
			      <c:otherwise>
			        <!--<input type="text" name="label" value="<c:out value="${label}"/>" size="5" class="formfield">-->
			        <input onfocus="this.select()" type="text" name="label" value="<fmt:message key="study_subject_ID" bundle="${resword}"/>" size="12" class="formfield">
			      </c:otherwise>
			    </c:choose>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					...
					<td valign="top" align="right">
					 <input onfocus="this.select()" type="text" name="location" size="8" value="<fmt:message key="location" bundle="${resword}"/>" class="formfield" />
					</td>
					<td valign="top">
					<input onfocus="this.select()" type="text" name="enrollmentDate" size="15" value="<fmt:message key="eventMMDDYYYY" bundle="${resword}"/>" class="formfield" />
					</td>				
			...
			<td valign="top" align="right" class="table_cell" nowrap>
				<input type="submit" name="addSubject" value="<fmt:message key="add" bundle="${resword}"/>" class="button_search" /></td>
			</td>
			</td>
		</tr>
			...
			<td valign="top" align="left" colspan="2" class="table_cell_left" nowrap>
			 <c:choose>
	           <c:when test="${study.studyParameterConfig.subjectPersonIdRequired =='required'}">
			    &nbsp;<input onfocus="this.select()" type="text" name="uniqueIdentifier" value="<fmt:message key="person_ID" bundle="${resword}"/>" size="12" class="formfield"><span class="formlabel">*</span>
			   </c:when>
			   <c:when test="${study.studyParameterConfig.subjectPersonIdRequired =='optional'}">
			    &nbsp;<input onfocus="this.select()" type="text" name="uniqueIdentifier" value="<fmt:message key="person_ID" bundle="${resword}"/>" size="12" class="formfield">
			   </c:when>
			   <c:otherwise>
				...
				<tr>
					<td valign="top" align="right">
					 <c:choose>
						...
						&nbsp;<input onfocus="this.select()" type="text" name="dob" size="20" value="<fmt:message key="DOB" bundle="${resword}"/>" class="formfield"><span class="formlabel">*</span>
						</td>
					    ...
					    <c:when test="${study.studyParameterConfig.collectDob == '2'}">
					     &nbsp;<input onfocus="this.select()" type="text" name="yob" size="15" value="YOB: YYYY" class="formfield" /><span class="formlabel">*</span>
					    </c:when>
					    <c:otherwise>

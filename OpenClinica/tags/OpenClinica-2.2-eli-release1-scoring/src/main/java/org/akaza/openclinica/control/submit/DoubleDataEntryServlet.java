...
public class DoubleDataEntryServlet extends DataEntryServlet {
  ...
  protected void mayProceed() throws InsufficientPermissionException {
    ...
    //BWP 12/2/07>> The following COUNT_VALIDATE session attribute is not accessible,
    // for unknown reasons (threading problems?), when
    //double-data entry displays error messages; it's value is always 0; so I have to create my
    ...
    boolean hasVisitedSection = doubleDataProgress.getSectionVisited(tabNumber,
      eventCRFId);
    ...
    DataEntryStage stage = ecb.getStage();
     if(stage.equals(DataEntryStage.INITIAL_DATA_ENTRY_COMPLETE) &&
      (! hasVisitedSection)){
      //if the user has not entered this section yet in Double Data Entry, then
  ...
  protected DisplayItemBean validateDisplayItemBean(
   ...
   // we only give warning to user if data entered in DDE is different from IDE when the first 
    //time user hits 'save'
   int keyId = ecb.getId();
   ...
   if (!isSingleItem) {
    ...
    if (rt.equals(org.akaza.openclinica.bean.core.ResponseType.TEXT)
      ...
      //logger.info("### found a response set count of "+inputName+" "+rsBean.getOptions().size());
      ...
      if (validationCount == null || validationCount.intValue() ==0) {
         v.addValidation(inputName, 
    		  Validator.MATCHES_INITIAL_DATA_ENTRY_VALUE, 
    		  valueToCompare,
    		  true);
        ...
        for (int u = 0; u < options.size(); u++) {
    	  ...
    	  if (errorValue.contains(rob.getValue())) {
              ...
              if (u < options.size()-1){
                //the values for multi-select are seperated by comma
	  ...
	  if (rt.equals(org.akaza.openclinica.bean.core.ResponseType.CALCULATION)
		|| rt.equals(org.akaza.openclinica.bean.core.ResponseType.GROUP_CALCULATION)) {
		  ...
		  if (validationCount == null || validationCount.intValue() ==0) {
			  ...
			  sv.setErrorMessage(respage.getString("value_you_specified")+ " "
					  + valueToCompare.getValue() + " " + respage.getString("from_initial_data_entry"));
		  }
	  }

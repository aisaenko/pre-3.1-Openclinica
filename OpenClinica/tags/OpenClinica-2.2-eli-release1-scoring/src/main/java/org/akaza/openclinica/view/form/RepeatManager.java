...
/**
 * This class adds the required repetition-model related attributes to an Element. These
 * attributes involve repeating form elements, and use a JavaScript library. See
 * the repetition-model specification at
...
public class RepeatManager {
  ...
  * repeating form elements. This method adds the required attributes to
  * the repeater parameter than returns the changed element. */
  public Element addParentRepeatAttributes(Element repeater,
                                           String parentRepeaterId,
                                           Integer repeatStart, Integer repeatMax) {
...
* repeating form elements. This method adds the required attributes to
* the repeater parameter than returns the changed element. */
  public Element addChildRepeatAttributes(Element repeater,
                                          String parentId, Integer itemId,
    ...
    //for the preview, where the item id is 0, we have to generate random IDs,
    ...
    //if the element does not have an input "name" attribute, then create a new one; otherwise
    //remove and edit the existing one to add this required repeat information.
    ...
    for(Element input : inputs)   {
      ...
      if(attribute == null ||  attribute.getValue() == null ||
        (! attribute.getValue().equalsIgnoreCase("hidden"))) {
      ...
      //the input Element has to use the id of its "repeat parent" element
      input.setAttribute("name", nameVal.toString());
    }
    return repeater;
  }

  public static Element createrepeatButtonControl(String type,
                                                  String templateName){

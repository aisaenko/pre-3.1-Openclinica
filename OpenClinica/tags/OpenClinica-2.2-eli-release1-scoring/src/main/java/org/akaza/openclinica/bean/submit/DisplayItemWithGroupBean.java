...
//this is an array of same item groups, which reflects multiple item group rows on the front end
  private List<DisplayItemGroupBean> itemGroups= new ArrayList<DisplayItemGroupBean>();
  //always keeps the group rows from DB, so we can keep track of which row is removed
  ...
  /**
   * @param dbItemGroups the dbItemGroups to set
   */

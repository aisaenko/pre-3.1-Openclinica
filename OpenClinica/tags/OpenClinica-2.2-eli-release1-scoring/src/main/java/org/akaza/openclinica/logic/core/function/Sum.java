...
public class Sum extends AbstractFunction {
     ...
     * 
     * All arguments should has been parsed to numbers before this execute() method is called.
     * 
    ...
    public void execute() {
        ...
        for (int i = 0; i < argumentCount(); i++) {
            ...
            try {
                ...
                //errors.put(new Integer(errorCount++), e.getMessage() + " when evaluate " + "Sum(); ");
            	errors.put(new Integer(errorCount++), "Unparseable number:" + " " + arg + " " + "in evaluation of" + " Sum(); ");
            }
        }
        if(errors.size() > 0){
            logger.severe("The following errors happended when Sum() evaluation was performed: " + errors);
            value = "";
            return;
        }
        double v = 0;

...
public class ScoreValidator {
	 ...
	 * @return
	 */
	public static boolean isValidExpression(String expression, StringBuffer errors, ArrayList<String> allVariables) {
		...
		while(info.pos < contents.length){
			...
			if(c == ' '){
				...
				if(!noCommaEnds(token)) {
					errors.append("One comma in '" + token + "' is invalid" + "; ");
				}
				token = token.trim();
	...
	public static boolean isValidFunction(char[] contents, ScoreUtil.Info info, String func, StringBuffer errors, ArrayList<String> allVariables) {
	...
	public static boolean isValidArgument(String term, StringBuffer errors, ArrayList<String> allVariables) {
		if(isNumber(term)) {
			return true;

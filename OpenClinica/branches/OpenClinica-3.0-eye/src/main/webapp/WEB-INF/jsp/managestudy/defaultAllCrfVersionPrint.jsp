...
<head>
    ...
    <style>
        ...
        .infotab th{border-right:1px #CCCCCC solid;border-bottom:3px #CCCCCC solid; padding:2px 4px 2px 4px}
        .infotab tr td{border-right:1px #CCCCCC solid;border-bottom:1px #CCCCCC solid; padding:2px 4px 2px 4px}
...
<th width=100%>
        <table class="infotab" cellpadding="0" cellspacing="0">
            <tr>
                <th width="15%"><h3><fmt:message key="study_name" bundle="${resword}"/></h3> </th>
                <th width="15%"><h3><fmt:message key="event_definition_name" bundle="${resword}"/></h3></th>
                <th width="15%"><h3><fmt:message key="site" bundle="${resword}"/></h3></th>
                <th width="15%"><h3><fmt:message key="study_subject_ID" bundle="${resword}"/></h3></th>
                <th width="15%"><h3><fmt:message key="date_interviewed" bundle="${resword}"/></h3></th>
                <th width="15%"><h3>Global ID </h3></th>
                <th width="10%"><h3><fmt:message key="interviewer_name" bundle="${resword}"/></h3></th>

            </tr>
            <tr>
                <td><c:out value="${studyName}"/></td>
                <td><c:out value="${item.key.name}"/></td>
                <td>&nbsp;<c:out value="${site}"/></td>
                <td>&nbsp;<h1></h1></td>
                <td>&nbsp;<h1></h1></td>
                <td>&nbsp;<h1></h1></td>
                <td>&nbsp;<h1></h1></td>
...
<c:out value="${printCrfBean.crfBean.name}" /> <c:out value="${printCrfBean.crfVersionBean.name}" />
        ...
        <div>
            &nbsp;&nbsp;<b><c:out value="${printCrfBean.crfSectionMessage}" escapeXml="false"/></b>
        </div>
    <c:choose>

 ...
 * <P>
 * The most important method here is the toNIB() method, which returns the NewInstrumentBean which in turn creates a new instrument version in the database.
 * 
 * 
 * @author thickerson with help from Brian Gilman @ the Whitehead Institute modified by jxu
 * @version CVS: $Id: SpreadSheetTableRepeating.java 12563 2009-03-19 20:22:08Z kkrumlian $
 */
...
public class SpreadSheetTableRepeating implements SpreadSheetTable {
    ...
    public NewCRFBean toNewCRF(javax.sql.DataSource ds, ResourceBundle resPageMsg) throws IOException {
        ...
        for (int j = 0; j < numSheets; j++) {
            ...
            if (sheetName.equalsIgnoreCase("Instructions")) {
                ...
                /*
                 * current strategem: build out the queries by hand and revisit this as part of the data loading module. We begin to check for errors here and
                 * look for blank cells where there should be data, tbh, 7/28
                 */
                ...
                if (sheetName.equalsIgnoreCase("Items")) {
                    ...
                    for (int k = 1; k < numRows; k++) {
                        ...
                        if (!secNames.contains(secName)) {
                            /*
                             * errors .add("The SECTION_LABEL column is not a valid section at row " + k + ", Items worksheet. " + "Please check the Sections
                             * worksheet to see that there is a valid LABEL for this SECTION_LABEL."); htmlErrors.put(j + "," + k + ",5", "NOT A VALID LABEL");
                             */
                        ...
                        /**
                         * The application will show error on page if two identical RESPONSE_LABEL has different RESPONSE_OPTIONS_TEXT
                         */
                        ...
                        if (responseTypeId == 8 || responseTypeId == 9) {
                            ...
                            if (resValArray.length != numberOfOptions) {
                                /*
                                 * errors.add("There are an incomplete number of option-value pairs in " + "RESPONSE_OPTIONS and RESPONSE_VALUES at row
                                 * " + k + ", questions worksheet; perhaps you are missing a comma? If there is a comma in any option text/value itself, please
                                 * use \\, instead."); htmlErrors.put(j + "," + k + ",15", "NUMBER OF OPTIONS DOES NOT MATCH"); htmlErrors.put(j + "," + k +
                        ...
                        /**
                         * The application will show error on page if two identical RESPONSE_LABEL has different REPONSE_VALUES
                         */
                        ...
                        /*
                         * Adding two columns here for the repeating rows, REsPONSE_LAYOUT and DEFAULT_VALUE TBH, 06/05/2007 YW 08-02-2007: move default_value
                         * down after data_type
                         */
                        ...
                        if (dbName.equals("oracle")) {
                            sql =
                                "INSERT INTO RESPONSE_SET (LABEL, OPTIONS_TEXT, OPTIONS_VALUES, " + "RESPONSE_TYPE_ID, VERSION_ID)" + " VALUES ('"
                                    + stripQuotes(responseLabel) + "', '" + stripQuotes(resOptions) + "','" + stripQuotes(resValues.replace("\\\\", "\\"))
                    ...
                    for (int gk = 1; gk < numRows; gk++) {
                        ...
                        /*
                         * BWP>>commented out after removal of borders column cell = sheet.getRow(gk).getCell((short) 7); String groupBorders = getValue(cell);
                         * Integer borders = 0; try { borders = Integer.valueOf(groupBorders); if (borders.intValue() <0) { errors.add("The BORDERS column must
                         * be a positive integer. " + groupBorders + " at row " + gk + ", Groups worksheet."); htmlErrors.put(j + "," + gk + ",7",
                         * "INVALID FIELD"); } } catch (NumberFormatException ne) { errors.add("The BORDERS column must be a positive integer. " + groupBorders
                         * + " at row " + gk + ", Groups worksheet."); htmlErrors.put(j + "," + gk + ",7", "INVALID FIELD"); } >>
                         */
                    ...
                    if (crfId == 0) {
                        ...
                        try {
                            ...
                            /*
                             * We are selecting the crf id which will be used to save the new CRF. Selecting the crf id in advance will not cause any problem in
                             * a multi threaded environment because the nextVal() method always returns unique values. So there is no chance of processing two
                             * CRF simultaneously with same crf id.
                             */
                            ResultSet nextIdRs;
                            if (dbName.equals("oracle")) {
                    ...
                    if (checkCRFVersions.containsKey(version)) {
                        ...
                        /*
                         * errors.add("The VERSION column is not unique. This can cause confusion in " + "selecting the correct CRF. Please make sure you change
                         * the " + "version name so that it can be uniquely identified by users in the system. " + "Otherwise, the previous same version will be
                         * deleted from database."); htmlErrors.put(j + ",1,2", "NOT UNIQUE");
                         */
    ...
    /**
     * stripQuotes, utility function meant to replace single quotes in strings with double quotes for SQL compatability. Don't -> Don''t, for example.
     * 
     * @param subj
     *            the subject line

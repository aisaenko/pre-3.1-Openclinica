...
public class EnterDataForStudyEventServlet extends SecureController {
  ...
  protected void processRequest() throws Exception {
    ...
    //removing the below row in exchange for the ViewStudySubjectServlet version, for two
    //reasons:
  ...
  private void populateUncompletedCRFsWithCRFAndVersions(ArrayList uncompletedEventDefinitionCRFs) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (cb.getStatus().equals(Status.AVAILABLE)) {
    	  //the above does not allow us to show the CRF as a thing with status of 'invalid' so we have to
   ...
   */
  private ArrayList getDisplayEventCRFs(
		  	ArrayList eventCRFs, 
    ...
    for (i = 0; i < eventCRFs.size(); i++) {
      ...
      logger.info("found subj event status: "+status.getName()+
    		  " cb status: "+cb.getStatus().getName()+
    		  " cvb status: "+cvb.getStatus().getName());
      ...
      try {
    	  ...
    	  //event definitions
    	  EventDefinitionCRFBean edcb = (EventDefinitionCRFBean) 
    	  	definitionsByCRFId.get(new Integer(cb.getId()));

 ...
 * 
 * Typically this happens when the user must choose some entity in a form, but no
 * entities are available to be selected. 

 * For example, the user may want to add a new subject to a genetic study.
 * However, in this use case, a father and mother must be specified for the subject.
 * If there are no female subjects in the system, then it will not be possible to
 * specify a mother for the subject.  Consequently, the user may not proceed.
 * 
 * @author ssachs
 */
public class InconsistentStateException extends OpenClinicaException {
	...
	public InconsistentStateException(Page goTo, String message, String type,

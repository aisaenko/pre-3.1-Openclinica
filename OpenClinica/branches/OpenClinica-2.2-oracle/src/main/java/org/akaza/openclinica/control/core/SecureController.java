...
public abstract class SecureController extends HttpServlet implements SingleThreadModel {
  ...
  protected static ResourceBundle resadmin, resaudit, resexception, resformat, respage, resterm, restext, resword, resworkflow;
  ...
  public static final String MODULE = "module";// to determine which module user is in
    ...
    try {
      ...
      if ((currentStudy == null) || (currentStudy.getId() <= 0)) {
        ...
        session.setAttribute("study", currentStudy);
    	  ...
    	  //YW 06-20-2007<< set site's parentstudy name when site is restored
      ...
      if ((currentRole == null) || (currentRole.getId() <= 0)) {
    	...
    	//if current study has been "removed", current role will be kept as "invalid" -- YW 06-21-2007 
    	if (ub.getId() > 0 && currentStudy.getId() > 0 && !currentStudy.getStatus().getName().equals("removed")) {
          ...
          if(currentStudy.getParentStudyId() > 0) {
          //Checking if currentStudy has been removed or not will ge good enough -- YW 10-17-2007
      ...
      //YW << For the case that current role is not "invalid" but current active study has been removed.
      ...
      //logging added to find problems with adding subjects, tbh 102007
    	...
    	if (checkTrail) {
    		...
    		if (session != null ){//added bu jxu, fixed bug for log out
    			...
    			if (panel.isStudyInfoShown()){
    			  ...
    			  setToPanel(resword.getString("number_of_subjects"),
  ...
  protected void setPopUpURL(String url) {
	...
	protected boolean entityIncluded(int entityId, String userName, AuditableEntityDAO adao, DataSource ds) {
		...
		for(int i=0; i<studies.size(); ++i) {
			...
			//Here follow the current logic - study subjects at sites level are visible to parent studies.

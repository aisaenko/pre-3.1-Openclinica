    ...
    if (crfVersionId == 0) {
      ...
      for (int i = 0; i < sections.size(); i++) {
        ...
        for(int j=0; j<igs.size(); ++j) {
        	ArrayList<ItemGroupMetadataBean> igms = (ArrayList<ItemGroupMetadataBean>)igmdao.findMetaByGroupAndSection(
						igs.get(j).getId(), section.getId());
        	if(!igms.isEmpty()) {
        		//Note, the following logic has been adapted here - "for a given crf version, 
        		//all the items in the same group have the same group metadata
      ...
      //YW 08-22-2007, if this crf_version_id doesn't exist in item_group_metadata table, 

...
public class StudyInfoPanel {
    ...
    public void setData(Page page, HttpSession session, HttpServletRequest request) {
        ...
        logger.info("found date format string: " + ResourceBundleProvider.getFormatBundle().getString("date_format_string"));
        logger.info("found date format string with locale: " + ResourceBundleProvider.getFormatBundle(locale).getString("date_format_string"));

        try {
            // defaults, can be reset inadverdantly by running through one page,
            // tbh
            ...
            if (page.equals(Page.CREATE_DATASET_1)) {
                ...
                || page.equals(Page.CREATE_DATASET_CRF_ATTR) || page.equals(Page.CREATE_DATASET_GROUP_ATTR)) {
            ...
            } else if (page.equals(Page.LIST_STUDY_SUBJECT) || page.equals(Page.LIST_STUDY_SUBJECT_SERVLET) || page.equals(Page.SUBMIT_DATA)
    ...
    public ArrayList generateTreeFromBeans(ArrayList rows, ArrayList displayData, StudySubjectBean studySubject, EventCRFBean ecb) {
        ...
        while (itRows.hasNext()) {
            ...
            displayData.add(new StudyInfoPanelLine("<img src='images/bt_View.gif' alt='View'>", "<a href='EnterDataForStudyEvent?eventId=" + seBean.getId()
    ...
    public ArrayList generateTreeFromBeansWithoutLink(ArrayList rows, ArrayList displayData, StudySubjectBean studySubject, EventCRFBean ecb) {
        ...
        while (itRows.hasNext()) {
            ...
            displayData.add(new StudyInfoPanelLine("<img src='images/bt_View.gif' alt='View'>", seBean.getSubjectEventStatus().getName(), false, false, false));
            ArrayList displayCRFs = dseBean.getDisplayEventCRFs();
            int count = 0;
            Iterator displayIt = displayCRFs.iterator();
            while (displayIt.hasNext()) {
    ...
    private ArrayList generatePermissionsEventTree(HashMap eventlist) {
        ...
        for (Iterator keyIt = eventlist.keySet().iterator(); keyIt.hasNext();) {
            ...
            for (int i = 0; i < crfs.size(); i++) {
                CRFBean crf = (CRFBean) crfs.get(i);
                if (ordinal_crf < crfs.size()) {
                    displayData.add(new StudyInfoPanelLine("CRF", "<a href='" + SERVLET_NAME + "?crfId=" + crf.getId() + "&defId=" + sed.getId() + "'>" + crf.getName()
                        + "</a>", false, false));
                } else {
                    displayData.add(new StudyInfoPanelLine("CRF", "<a href='" + SERVLET_NAME + "?crfId=" + crf.getId() + "&defId=" + sed.getId() + "'>" + crf.getName()
                        + "</a>", false, true));

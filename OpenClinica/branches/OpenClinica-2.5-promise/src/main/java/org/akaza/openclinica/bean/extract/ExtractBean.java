...
public class ExtractBean {
    ...
    //java.text.SimpleDateFormat sdf = new SimpleDateFormat(ResourceBundleProvider.getFormatBundle().getString("date_format_string_ds"));
     ...
     * - separate the subjects from study subjects
     * - leave the subjects array name 
     * - add subjectsnostudy for original subjects
     * The two array could be different because they are 
     * not synchronized in the code
     */
    ...
    /**
     * @vbc 08/06/2008 NEW EXTRACT DATA IMPLEMENTATION
     * - remove subjectsAdded - it is not used
     * - add InKeysHelper - a HashMap that will speed up
     * the data extract display
     */
    ...
    // >> tbh data extract permissions implementation
    private PermissionsMatrixBean permissions;
    
    private Role currentRole;
    // << tbh 08/2009

    public ExtractBean(DataSource ds, Role role) {
        this.ds = ds;
        ...
        permissions = new PermissionsMatrixBean();
    }

    public ExtractBean(DataSource ds, Role role, SimpleDateFormat sdf, SimpleDateFormat long_sdf) {
        this.sdf = sdf;
    ...
    public void computeReportData(ReportBean answer) {
        ...
        permissions = filterDao.findPermissionsByStudy(study);
        // << tbh 08/2009
        
        // item column headers here
        // this is where we will have to filter by permissions? yes
        for (int i = 1; i <= numSEDs; i++) {
            int numSamples = getSEDNumSamples(i);
        ...
        for (int h = 1; h <= getNumSubjects(); h++) {
            ...
            for (int i = 1; i <= numSEDs; i++) {
                ...
                for (int j = 1; j <= numSamples; j++) {
                    ...
                    if (dataset.isShowSubjectAgeAtEvent() && currentSubject.getDateOfBirth() != null) {
                    ...
                    for (int k = 1; k <= numSEDCRFs; k++) {
                        ...
                        for (int l = 1; l <= numItems; l++) {
                            ...
                            for (java.util.Iterator iter = groupNames.entrySet().iterator(); iter.hasNext();) {
                                ...
                                String groupName = (String) groupEntry.getKey();
                                if (inKeys(i, j, k, l, groupName)) {
                                    //logger.info("found group name at line 703: " + groupName);
                                    Integer groupCount = (Integer) groupEntry.getValue();
                                    //logger.info("found groupCount of " + groupCount);
                                    for (int m = 1; m <= groupCount.intValue(); m++) {
    ...
    /**
     * @vbc 08/06/2008 NEW EXTRACT DATA IMPLEMENTATION
     * replaced the old one with a new function
     */
    ...
    /*
    public void addStudySubjectData(Integer studySubjectId, String studySubjectLabel, Date dateOfBirth, String gender, Integer subjectStatusId,
            Boolean dobCollected, String uniqueIdentifier, String subjectSecondaryLabel) {
    ...
    /**
     * @vbc 08/06/2008 NEW EXTRACT DATA IMPLEMENTATION 
     * Combines the two  HashMaps into eventData entries
     * - the data is already filtered for null values
     */
    public void addStudyEventData()
    {
    	
    	/**
    	 * The two ArrayList are synchronized because they are extracted with "ORDER BY aitem_data_id"
    	 */
    	boolean isfnd = false;
    	...
    	for (int ik = 0;  ik < aBASE_ITEMDATAID.size(); ik++)
    	{
    		...
    		if ( ( itemdataid_objgrp.intValue() == itemdataid.intValue()) && (itemdataid_objev.intValue() ==itemdataid.intValue() ) )
			{
    	        ...
    	        event.setSampleOrdinal(objev.sampleOrdinal.intValue());  //sampleOrdinal.intValue()
    	        event.setStudyEventDefinitionId(objev.studyEvenetDefinitionId.intValue());  //studyEventDefinitionId.intValue()
    	        event.setStudySubjectId( objev.studySubjectId.intValue());  //studySubjectId.intValue()
    	        event.setStartTimeFlag(objev.studyEventStartTimeFlag.booleanValue()); //se.getStartTimeFlag()
    	        event.setEndTimeFlag(objev.studyEventEndTimeFlag.booleanValue());  //se.getEndTimeFlag()
    	        // below needs to be added, tbh
    	        //TODO - @vbc - validate this
    	        event.setStatus(Status.get(objev.studyEventStatusId.intValue()));  //se.getStatus()
    	        event.setSubjectEventStatus(SubjectEventStatus.get(objev.studyEventSubjectEventStatusId.intValue()));  //se.getSubjectEventStatus()
    	        ...
    	        // end tbh, 03/08
    	        eventCRF.setCompletionStatusId(objgrp.eventCrfCompletionStatusId);//completionStatusId.intValue()
    	        ...
    	        crfVersion.setStatusId(objgrp.crfVersionStatusId.intValue());	//crfVersionStatusId.intValue()
    ...
    public void addItemData()
   {
    	...
    	for (int ik = 0;  ik < aBASE_ITEMDATAID.size(); ik++)
    	{
    		...
    		if ( ( itemdataid_objgrp.intValue() == itemdataid.intValue()) && (itemdataid_objev.intValue() ==itemdataid.intValue() ) )
			{
		        ...
		        if (!"".equals(/*itemGroupName*/ objgrp.itemGroupName)) 
		        {
		        	...
		        	/**
		        	 * @vbc
		        	 * NOTE: the "item_data_ordinal" is the id.ordinal AS item_data_ordinal
		        	 * which is the item_data table.
		        	 * Here we retrieve the item_group_metadata.repeat_number
		        	 * 
		        	 * TODO - validate the logic
		        	 */
		            String key =
		                getDataKey(/*studySubjectId.intValue()*/ objev.studySubjectId.intValue(), 
		                		/*studyEventDefinitionId.intValue()*/ objev.studyEvenetDefinitionId.intValue(), 
		                		/*sampleOrdinal.intValue()*/ objev.sampleOrdinal.intValue(), 
		            ...
		            //logger.info("*** just put in data for " + key + " and value " + objgrp.itemValue/*itemValue*/);
		            // groups.put(key, itemGroupName);
		            int maxOrdinal = getMaxOrdinal(/*studyEventDefinitionId.intValue()*/ objev.studyEvenetDefinitionId.intValue());
		            if (maxOrdinal <  objev.sampleOrdinal.intValue()) { ///*sampleOrdinal.intValue()*/
		            ...
		            selectedSEDCRFs.put(objev.studyEvenetDefinitionId.intValue() + "_" + objgrp.crfid.intValue()
		            		/*studyEventDefinitionId.intValue() + "_" + crfId.intValue()*/, Boolean.TRUE);
		            ...
		            if ( /*itemDataOrdinal.intValue()*/ objgrp.itemGroupRepeatNumber.intValue() > getMaxItemDataBeanOrdinal()) {
		                setMaxItemDataBeanOrdinal(objgrp.itemGroupRepeatNumber.intValue()/*itemDataOrdinal.intValue()*/);
    ...
    private void setMaxOrdinal(int studyEventDefinitionId, int sampleOrdinal) {
        ...
        //logger.info("*** set max ordinal: " + sampleOrdinal + " for key " + key.toString());
    ...
    private String getSEDCRFStatus(int h, int sedInd, int crfInd) {// BADS Flag
        ...
        StudyEventBean seb = getEvent(h, sedInd, crfInd);
        DataEntryStage stage = seb.getStage();
        ...
        try {
            ...
            status = seb.getSubjectEventStatus();// SubjectEventStatus.get(eventCRF.getCompletionStatusId());
        ...
        //
        logger.info("STATUS: " + status.getName() + " crf version: " + crfv.getStatus().getName() + " data entry stage: " + stage.getName()
            + " event crf status: " + ecStatus.getName());
        ...
        if (status.equals(SubjectEventStatus.LOCKED) || status.equals(SubjectEventStatus.SKIPPED) || status.equals(SubjectEventStatus.STOPPED)) {
            ...
            crfVersionStatus = DataEntryStage.LOCKED.getName();
        } else if (status.equals(SubjectEventStatus.COMPLETED)) {
            crfVersionStatus = SubjectEventStatus.COMPLETED.getName();
    ...
    protected boolean inKeys(int sedInd, int sampleOrdinal, int crfInd, int itemInd, String groupName) {
        ...
        /**
         * @vbc 08/06/2008 NEW EXTRACT DATA IMPLEMENTATION
         * change it into a simple HashMap
         * 
         * TODO - verify if the itemOrdinalItem is required - in the previous code is set to 1 !?!!?
         */
        // >> tbh 08/2009 changing this for the PEP implementation
        // access the permissions bean, use the ids below to see if we have permissions or not
        ...
        boolean isPermitted = permissions.isPermitted(currentDef.getId(), currentCRF.getId(), currentItem.getId(), currentRole.getId());
        System.out.println("found " + isPermitted + " from " + currentDef.getId() + " " + 
        		currentCRF.getId() + " " + currentItem.getId() + " " + currentRole.getId());
        // << tbh 08/2009
        if (issavedkey == null){
        	//the "True" is not found for key - set to false
        	return false;
        } else if (!isPermitted){
        	System.out.println("returned false for permissions");
        	return false;
        } else {
        	return true;
        }//if
        ...
        for (Iterator iter = data.entrySet().iterator(); iter.hasNext();) {
            String key = (String) ((java.util.Map.Entry) iter.next()).getKey() + "_Ungrouped";
            String testKey = getDataKey(0, currentDef.getId(), sampleOrdinal, currentCRF.getId(), currentItem.getId(), 1, groupName).substring(2);
            testKey = groupName.equals("Ungrouped") ? testKey + "_Ungrouped" : testKey;
     ...
     * ********************************************************************************
     * @vbc 08/06/2008 NEW EXTRACT DATA IMPLEMENTATION 
     *  - a new section that uses a different way to access the data
	...
	public void addEntryBASE_ITEMGROUPSIDE(
			...
			Integer pitemdataordinal,
			 ...
			 String pitemValue,
			 String pitemUnits,
			 String pcrfVersionName,
			 Integer pcrfVersionStatusId,
			 Date pdateInterviewed,
			 String pinterviewerName,
			 Timestamp peventCrfDateCompleted,
			 Timestamp peventCrfDateValidateCompleted,
			 Integer peventCrfCompletionStatusId,
			 Integer pitemGroupRepeatNumber,
			 Integer pcrfId,
			 Integer pstudySubjectId,
			 Integer peventCrfId,
			 Integer pitemId,
			 Integer pcrfVersionId)
	{
		extractDataset_ITEMGROUPSIDE obj = new extractDataset_ITEMGROUPSIDE();
		
		obj.setSQLDatasetBASE_ITEMGROUPSIDE(pitemDataId, pitemdataordinal,
				pitemGroupId, pitemGroupName, 
				pitemDescription, pitemName, pitemValue, pitemUnits, 
				pcrfVersionName, pcrfVersionStatusId, pdateInterviewed, 
				pinterviewerName, peventCrfDateCompleted, 
				peventCrfDateValidateCompleted, peventCrfCompletionStatusId, 
				pitemGroupRepeatNumber, pcrfId,
				pstudySubjectId, peventCrfId, pitemId, pcrfVersionId);
	 ...
	 * @param pstudyEventId
	 */
	public void addEntryBASE_EVENTSIDE(
					Integer pitemDataId,
					Integer pstudySubjectId,
					Integer psampleOrdinal,
					 Integer pstudyEvenetDefinitionId,
					 String pstudyEventDefinitionName,
					 String pstudyEventLoacation,
					 Timestamp pstudyEventDateStart,
					 Timestamp pstudyEventDateEnd,
					 Boolean pstudyEventStartTimeFlag,
					 Boolean pstudyEventEndTimeFlag,
					 Integer pstudyEventStatusId,
					 Integer pstudyEventSubjectEventStatusId,
					 Integer pitemId,
					 Integer pcrfVersionId,
					 Integer peventCrfId,
					 Integer pstudyEventId)
	
	{
		extractDataset_EVENTSIDE obj = new extractDataset_EVENTSIDE();
		
			
		obj.setSQLDatasetBASE_EVENTSIDE(pitemDataId, pstudySubjectId, 
				psampleOrdinal, pstudyEvenetDefinitionId, 
				pstudyEventDefinitionName, pstudyEventLoacation, 
				pstudyEventDateStart, pstudyEventDateEnd, 
				pstudyEventStartTimeFlag, pstudyEventEndTimeFlag,
				pstudyEventStatusId,pstudyEventSubjectEventStatusId,
	...
	/**
	 * This class captures the data from getSQLDatasetBASE_EVENTSIDE
	 *  		SELECT 
			...
			study_event.location, 
			study_event.date_start, 
			study_event.date_end,
			
			study_event.start_time_flag
			study_event.end_time_flag
			study_event.status_id
	...
	public class extractDataset_EVENTSIDE {
		...
		public void setSQLDatasetBASE_EVENTSIDE(Integer pitemDataId,
									Integer pstudySubjectId,
									Integer psampleOrdinal,
									 Integer pstudyEvenetDefinitionId,
									 String pstudyEventDefinitionName,
	 ...
	 * 			SELECT 
			itemdataid, 
			item_group_metadata.item_group_id , 
			item_group.name, 
			itemdesc, 
			itemname, 
			itemvalue, 
			...
			for value apply:
			// YW 12-06-2007 << do convert for date data-type item.
            String itemValue = Utils.convertedItemDateValue((String) row.get("value"), oc_df_string, local_df_string);
	...
	public class extractDataset_ITEMGROUPSIDE {
		...
		public void setSQLDatasetBASE_ITEMGROUPSIDE(Integer pitemDataId,
											 Integer pitemdataordinal,
											 Integer pitemGroupId,
											 String pitemGroupName,
											 String pitemDescription,
											 ...
											 Timestamp peventCrfDateValidateCompleted,
											 Integer peventCrfCompletionStatusId,
											 Integer pitemGroupMetatdatrepeatNumber,
											 Integer pcrfId,
											 Integer pstudySubjectId,
											 Integer peventCrfId,
											 Integer pitemId,
											...
											)
		{
			//assign
			 ...
			 //Utils.convertedItemDateValue((String) itemValue, oc_df_string, local_df_string);;

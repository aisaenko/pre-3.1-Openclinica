...
public class SelectItemsServlet extends SecureController {
    ...
    public void processRequest() throws Exception {
        ...
        PermissionsMatrixBean permissions = filterDao.findPermissionsByStudy(currentStudy);
        
        ArrayList items = idao.findAllActiveByCRF(crf);
        ArrayList apprItems = new ArrayList();
        for (int i = 0; i < items.size(); i++) {
            ItemBean item = (ItemBean) items.get(i);
            if (permissions.isPermitted(sed.getId(), crf.getId(), item.getId(), ub.getActiveStudyRole().getId())) {
            	...
            	logger.info("removed unapproved item " + i);
            	// items.remove(i);
            }
            /*
        ...
        for (int i = 0; i < apprItems.size(); i++) {
            ItemBean item = (ItemBean) apprItems.get(i);
            ...
            if (!itemMap.containsKey(defId + "_" + item.getId())) {
                ...
                logger.info("checking approved items " + i);
        ...
        /*
         * communication with the user - if we have no items or some items
         * possible change here will be to include the items, but in a 
         * grayed-out fashion
         * >> tbh 08/2009
         */
        if (apprItems.isEmpty()) {
        	addPageMessage(respage.getString("no_access_to_any_items"));
        } else if (apprItems.size() < items.size()) {
        	addPageMessage(respage.getString("some_access_to_extract_items"));
        }
        // << tbh 08/2009

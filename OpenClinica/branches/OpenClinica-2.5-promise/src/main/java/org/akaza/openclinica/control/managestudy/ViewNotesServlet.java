    ...
    */
    ...
    protected void processRequest() throws Exception {
        ...
        if(parentStudyId > 0){
            ...
            parentItemDataNotes = dndao.findAllItemDataByStudy(parentStudy);
        ...
        String[] columns =
          { resword.getString("study_subject_ID"), resword.getString("date_created"), resword.getString("date_updated"),resword.getString("event_date"), resword.getString("event"),
        ...
        //after resolving a note, user wants to go back to view notes page, we save the current URL
    ...
    private void addUpdatedDateToNotes(
      List<DiscrepancyNoteBean> discrepancyNoteBeans){
        ...
        DiscrepancyNoteDAO discrepancyNoteDAO =
          new DiscrepancyNoteDAO(sm.getDataSource());

        //The updated date is the creation date for notes with no parents or children
        for(DiscrepancyNoteBean dnBean : discrepancyNoteBeans){
            Date createDate = dnBean.getCreatedDate();
            if(isUnaffiliatedDNote(dnBean,discrepancyNoteDAO) &&
              (createDate != null)){
            ...
            List<DiscrepancyNoteBean> childNoteBeans =
              discrepancyNoteDAO.findAllByParent(dnBean);
            ...
            if(parentId == 0)  {
                //There is no reason to move on if there is not a valid parent id
                continue;
            }
            parentBean = (DiscrepancyNoteBean)
              discrepancyNoteDAO.findByPK(parentId);
    ...
    private boolean isUnaffiliatedDNote(DiscrepancyNoteBean dnBean,
                                        DiscrepancyNoteDAO discrepancyNoteDAO){

        if(dnBean == null || discrepancyNoteDAO == null) return true;
        ...
        if(isChildless && isOrphan) return true;

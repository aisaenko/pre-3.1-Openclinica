      ...
      if ("begin".equalsIgnoreCase(action)) {
        ...
        if (!errors.isEmpty()) {
          ...
          if (fp.getString("submit").equals(resword.getString("continue_to_apply_filter"))) {
            ...
            //YW, 2-20-2008 <<these attributes will show on the CREATE_DATASET_4
        ...
        if (!errors.isEmpty()) {
          ...
          //YW 2-21-2008 << editing dataset becomes creating a new dataset if dataset name has been changed.
	      ...
	      for (int j = 0; j < allGroups.size(); j++) {
	    	  ...
	    	  if (!StringUtil.isBlank(checked) 
	              ...
	              logger.info("just set a group to true: "+
	            		  sgclass.getName());
	              //YW, 2-26-2008 << let subjectGroupIds contain only selected StudyGroupClass_id <<
	    		  ...
	    		  //wasn't checked
	    		  //YW, 2-26-2008 << delete StudyGroupClass_id from subjectGroupIds if appliable<<
	    		  if(db.getSubjectGroupIds()!=null && db.getSubjectGroupIds().contains(sgclass.getId())) {

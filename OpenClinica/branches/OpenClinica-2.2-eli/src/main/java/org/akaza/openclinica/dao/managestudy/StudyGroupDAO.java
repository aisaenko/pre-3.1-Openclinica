...
public class StudyGroupDAO extends AuditableEntityDAO {
  ...
  /*
   * created for Extract Bean, we give the method a study subject bean
   * and it returns for us a hash map of class ids pointing to 
   * study group beans, tbh July 2007
   */
  public HashMap findByStudySubject(StudySubjectBean studySubject) {
	  ...
	  if (it.hasNext()) {
		  StudyGroupBean sgbean = 
			  (StudyGroupBean) 
			  this.getEntityFromHashMap((HashMap) it.next());
		  classBeanMap.put(new Integer(sgbean.getStudyGroupClassId()), 
				  sgbean);
	  }
	  return classBeanMap;
  }
  ...
  public HashMap findSubjectGroupMaps(int studyId) {
	  System.out.println("testing with variable: "+studyId);
	  ...
	  while (it.hasNext()) {
		  System.out.println("iteration over answers...");
		  subjectGroupMap = new HashMap();
		  
		  HashMap answers = (HashMap)it.next();
		  
		  Integer studySubjectId = (Integer)answers.get("study_subject_id");
		  System.out.println("iteration over answers..."+studySubjectId.intValue());
		  if (subjectGroupMaps.containsKey(studySubjectId)) {
			  groupMaps = (ArrayList)subjectGroupMaps.get(studySubjectId);
			  //subjectGroupMap = (HashMap)subjectGroupMaps.get(studySubjectId);
		  } else {
			  groupMaps = new ArrayList();
		  }
		  StudyGroupBean sgbean = 
			  (StudyGroupBean) 
			  this.getEntityFromHashMap(answers);
		  
		  subjectGroupMap.put(new Integer(sgbean.getStudyGroupClassId()), 
				  sgbean);
		  groupMaps.add(subjectGroupMap);
		  System.out.println("subjectgroupmaps: just put in "+sgbean.getStudyGroupClassId());

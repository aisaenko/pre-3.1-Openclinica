...
public class SPSSReportBean extends ReportBean {
  ...
  //YW 10-31-2007, more titles have been added because of additional attrubutes added for extracting data\
  private static final String[] builtin = {"SubjID", "ProtocolID", "DOB", "YOB", "Gender", "SubjectStatus", "UniqueID",
      "Location", "StartDate", "EndDate", "SubjectEventStatus", "AgeAtEvent", "InterviewDate", "InterviewerName", 
      "CRFVersionStatus", "VersionName"};
  
  //YW <<
  private static final String[] builtinType = { "A", "A", "ADATE10", "ADATE10", "A", "A", "A",  
	  "A", "ADATE10", "ADATE10", "A", "F8.0", "ADATE10", "A", "A", "A" };
  ...
  public String getMetadataFile(SPSSVariableNameValidationBean svnvbean, ExtractBean eb) {
	itemNames.clear();
	String[] attributes = createAttributes(eb); //Is it necessary to validate StudyGroupClassBean names? YW
    ...
    if (data.size() <= 0) {
      ...
      for (int i = 0; i < columns.size(); i++) {
        ...
        //YW << Why do we need "V_" here? Right now it has been removed, because:
      ...
      for(int i=0; i<startItem; ++i) {
        ...
        if((String)descriptions.get(itemNames.get(i))!=null) {
        	...
        	for(int j=0; j<eb.getStudyGroupClasses().size(); ++j) {
        		answer += "\t" + varLabel + " \"" + (String)descriptions.get(((StudyGroupClassBean)eb.getStudyGroupClasses().get(j)).getName()) + "\"\n";
        	}
        }
      }
      //items

 ...
 * double data entry. When the user views the section for the first time, any defauts
 * field values are shown on the form, and the visit is recorded and stored in
 * a session attribute. Default values are not displayed on the form thereafter, to prevent
 * default values from being displayed, for instance, when the form is redisplayed with
 * error messages.
 * @see SectionVisit
...
public class DoubleDataProgress implements EnterDataProgress {
  ...
  public boolean getSectionVisited(int sectionNumber,int eventCRFId) {
    for(SectionVisit secVisit : synchronizedList ) {
      if(secVisit.getEventCRFId() == eventCRFId &&
        secVisit.getSectionNumber() == sectionNumber) return secVisit.isVisitedOnce();
    }
    return false;
  }
  ...
  public void setSectionVisited(int eventCRFId, int sectionNumber,boolean hasVisited) {
    for(SectionVisit secVisit : synchronizedList ) {
      if(secVisit.getEventCRFId() == eventCRFId &&
        secVisit.getSectionNumber() == sectionNumber){

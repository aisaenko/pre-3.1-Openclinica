...
public class Validator {
	...
	public static ValidatorRegularExpression getDateRegEx(){
		...
		return new ValidatorRegularExpression(
				resformat.getString("date_format"),resformat.getString("date_regexp"));
	}
	...
	public static ValidatorRegularExpression getDateTimeRegEx(){
		ResourceBundle resformat = ResourceBundleProvider.getFormatBundle();
		return new ValidatorRegularExpression(
				resformat.getString("date_time_format"),resformat.getString("date_time_regexp"));
	}
	...
	public static ValidatorRegularExpression getPhoneRegEx(){
		ResourceBundle resformat=ResourceBundleProvider.getFormatBundle();
		return new ValidatorRegularExpression(
				resformat.getString("phone_format"),resformat.getString("phone_regexp"));
	}
	...
	//YW 06-26-2007 << throw error message:"You have to choose different value" if one field 
	//   has the same value as another field.
	//   e.g. new expired password should be different from the old password; otherwise, throw error
    ...
    //YW, 2-5-2008 << For scoring. If this type has been set, add error message which will be set out of this class
	...
	protected void addError(String fieldName, Validation v) {
		...
		if (v.isErrorMessageSet()) {
			...
			case DATE_IS_AFTER_OR_EQUAL:
				...
				String earlierDateValue = (String) getFieldValue(earlierDateFieldName);
			...
			case MATCHES_INITIAL_DATA_ENTRY_VALUE:
				...
				//errorMessage = v.getErrorMessage();//should be set at the DDE stage, tbh 112007
		...
		//System.out.println("<<<error added: "+errorMessage+" to "+fieldName);
	...
	protected String getFieldValue(String fieldName) {
		return request.getParameter(fieldName) == null ? request
				.getAttribute(fieldName) == null ? null : request
     ...
     * @param fieldName The name of a field containing some text string.
     * @return <code>true</code> if the field contains a valid date in "MM/dd/yyyy" format;
     * <code>false</code> if the given input is not date formatted or null or empty .
     */
	...
	protected boolean isDateTime(String prefix) {
		...
		//YW 08-17-2007 << If no input for any of these 3 fields, it means that time is not specified.
		// In this case, 12:00 am has been set as default according to original setting
	...
	protected boolean valueMatchesInitialValue(String fieldName, String oldValue, boolean isMultiple) {
		...
		if (isMultiple) {
			...
			try {
				...
				System.out.println(
						"line 1444: validator: found NPE with "+
						fieldName);
				return false;
			}
		...
		if (fieldValue == null) {
          ...
          //when the value from page is null, we save "" in DB,so should consider they match
          if ("".equals(oldValue)) {
            return true;
          }
			return false;
	...
	public void setErrorMessage(String message) {
		...
		//System.out.println("set error message successfully: "+message);
	...
	public static Validation processCRFValidationRegex(String inputRegex) throws Exception {
		...
		//YW 10-23-2007, << 
		//inputRegex should be looking like "regexp:/expression/" if it goes this far

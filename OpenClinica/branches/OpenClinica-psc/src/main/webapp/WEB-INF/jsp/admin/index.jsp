...
<fmt:message key="business_administrator" bundle="${resword}"/> <a href="javascript:openDocWindow('help/5_0_administerSystem_Help.html')"><img src="images/bt_Help_Admin.gif" border="0" alt="<fmt:message key="help" bundle="${resword}"/>" title="<fmt:message key="help" bundle="${resword}"/>"></a>
</span></h1>
						...
						<td class="table_cell">
						 <c:choose>
						  <c:when test="${subject.updatedDate != null}">
						   <fmt:formatDate value="${subject.updatedDate}" dateStyle="short"/>
						  </c:when>
						  <c:otherwise>
						    <fmt:formatDate value="${subject.createdDate}" dateStyle="short"/>
						  </c:otherwise>
						 </c:choose> 
						</td>
					 ...
					 <tr valign="top">   
						...
						<c:choose>
						 <c:when test="${user.updatedDate != null}">
						  <fmt:formatDate value="${user.updatedDate}" dateStyle="short"/>
						 </c:when> 
						 <c:otherwise>
						    <fmt:formatDate value="${user.createdDate}" dateStyle="short"/>
						 </c:otherwise>
						</c:choose>
						</td>
					...
					<tr valign="top">   
						...
						<c:choose>
						 <c:when test="${study.updatedDate != null}">
						  <fmt:formatDate value="${study.updatedDate}" dateStyle="short"/>
						 </c:when>
						 <c:otherwise>
						  <fmt:formatDate value="${study.createdDate}" dateStyle="short"/>
						 </c:otherwise>
						</c:choose> 
						</td>
		...
		<div class="box_T"><div class="box_L"><div class="box_R"><div class="box_B"><div class="box_TL"><div class="box_TR"><div class="box_BL"><div class="box_BR">
			...
			<div class="tablebox_center">
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
						...
						<td class="table_cell">
						 <c:choose>
						  <c:when test="${crf.updatedDate != null}">
						   <fmt:formatDate value="${crf.updatedDate}" dateStyle="short"/>
						  </c:when>
						  <c:otherwise>
						    <fmt:formatDate value="${crf.createdDate}" dateStyle="short"/>
						  </c:otherwise>
						 </c:choose>  
						</td>
					...
					<tr valign="top">
					 <td class="table_cell" align="right" colspan="3"><c:if test="${crfCount>0}"><a href="ListCRF"><fmt:message key="show_all" bundle="${resword}"/></a> | </c:if><a href="CreateCRF"><fmt:message key="add_new" bundle="${resword}"/></a> </td>
					</tr>
				</table>

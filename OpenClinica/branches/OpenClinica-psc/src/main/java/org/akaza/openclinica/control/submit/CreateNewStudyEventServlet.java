...
public class CreateNewStudyEventServlet extends SecureController {
    ...
    protected void processRequest() throws Exception {
        ...
        if (!fp.isSubmitted()) {
            ...
            int[] scheduledDefinitionIds = new int[this.ADDITIONAL_SCHEDULED_NUM];
            if (hasScheduledEvent) {
                for(int i=0; i<this.ADDITIONAL_SCHEDULED_NUM; ++i) {
                    int pk = fp.getInt(this.INPUT_STUDY_EVENT_DEFINITION_SCHEDULED[i]);
                    if(pk>0) {
            ...
            for(int i=0; i<this.ADDITIONAL_SCHEDULED_NUM; ++i) {
                if (scheduledDefinitionIds[i]>0 && !errors.containsKey(INPUT_STARTDATE_PREFIX_SCHEDULED[i]) && !errors.containsKey(INPUT_ENDDATE_PREFIX_SCHEDULED[i])) {
                    if (scheduledSeds.containsKey(scheduledDefinitionIds[i])) {
                        ...
                        prevStartPrefix = prevStart==-1?this.INPUT_STARTDATE_PREFIX:this.INPUT_STARTDATE_PREFIX_SCHEDULED[prevStart];
                        ...
                        if (fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED[i] + "Date").equals(fp.getString(prevStartPrefix+"Date"))) {
                            //if in same day, only check when both have time inputs.
                            ...
                            if (schStartTime && startTime) {
                                if (startScheduled[i].before(prevStartDate)) {
                                    v.addError(errors, INPUT_STARTDATE_PREFIX_SCHEDULED[i], resexception
                    ...
                    if (!strEndScheduled[i].equals("")) {
                        ...
                        if (!fp.getString(INPUT_STARTDATE_PREFIX_SCHEDULED[i] + "Date").equals(fp.getString(prevEndPrefix + "Date"))) {
                            if (endScheduled[i].before(startScheduled[i])) {
                                v.addError(errors, INPUT_ENDDATE_PREFIX_SCHEDULED[i], resexception
                                        .getString("input_provided_not_occure_after_previous_start_date_time"));
                            }
                        } else {
            ...
            if (!errors.isEmpty()) {
                ...
                String prefixes[] = new String[2+2*this.ADDITIONAL_SCHEDULED_NUM];
                prefixes[0] = INPUT_STARTDATE_PREFIX;
                prefixes[1] = INPUT_ENDDATE_PREFIX;
                int b = this.ADDITIONAL_SCHEDULED_NUM+2;
                for(int i=2; i<b; ++i) {
                    prefixes[i] = INPUT_STARTDATE_PREFIX_SCHEDULED[i-2];
                }
                for(int i=b; i<this.ADDITIONAL_SCHEDULED_NUM+b; ++i) {
                ...
                Date startScheduled2[] = new Date[this.ADDITIONAL_SCHEDULED_NUM];
                for(int i=0; i<this.ADDITIONAL_SCHEDULED_NUM; ++i) {
                ...
                if (hasScheduledEvent) {
                    for(int i=0; i<this.ADDITIONAL_SCHEDULED_NUM; ++i) {
                        ...
                        if (scheduledDefinitionIds[i]>0 && subjectMayReceiveStudyEvent(sm.getDataSource(), definitionScheduleds.get(i), studySubject)) {
                            ...
                            AddNewSubjectServlet
                                    .saveFieldNotes(INPUT_ENDDATE_PREFIX_SCHEDULED[i], fdn, dndao, studyEventScheduled.getId(), "studyEvent", currentStudy);
                            // YW >>
                        } else {
                            addPageMessage(restext.getString("scheduled_event_definition") + definitionScheduleds.get(i).getName() + restext.getString("X_and_subject")

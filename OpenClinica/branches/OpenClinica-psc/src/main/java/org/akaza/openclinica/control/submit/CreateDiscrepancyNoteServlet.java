...
public class CreateDiscrepancyNoteServlet extends SecureController {
    ...
    protected void processRequest() throws Exception {
        ...
        if (!fp.isSubmitted()) {
            ...
            if (parent.getDiscrepancyNoteTypeId()>0) {
...
//                  
                    ...
                    //if not creating a nre thread(note), update exsiting notes if necessary           
                    if ("itemData".equalsIgnoreCase(entityType) && !isNew) {                
                      System.out.println("Create:find parent note for item data:" + note.getEntityId());
                     
                      ArrayList parentNotes = (ArrayList)dndao.findExistingNotesForItemData(note.getEntityId());
                      for (int j=0; j<parentNotes.size(); j++) {
                          DiscrepancyNoteBean pNote = (DiscrepancyNoteBean)parentNotes.get(j);

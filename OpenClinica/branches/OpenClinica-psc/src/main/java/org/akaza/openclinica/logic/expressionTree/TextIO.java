...
public class TextIO {
    ...
    /*
     * private void fillBuffer() { // Wait for user to type a line and // press
     * return, try { buffer = in.readLine(); } catch (Exception e) { if
     * (readingStandardInput) throw new IllegalArgumentException("Error while
     * reading standard input???"); else if (inputFileName != null) throw new
     * IllegalArgumentException("Error while attempting to read from file \"" +
     * inputFileName + "\"."); else throw new IllegalArgumentException("Errow
     * while attempting to read form an input stream."); } pos = 0; floatMatcher =
     * null; integerMatcher = null; }
     */
    ...
    private void outputError(String message) { // Report an error on

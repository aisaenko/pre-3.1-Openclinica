...
<div id="Lock_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>" style="position: absolute; visibility: hidden; z-index: 3; width: 50px; height: 30px; top: 0px;">
 ...
 <c:choose>
  ...
  <c:when test="${crfStatus ==initial_data_entry_completei18n || crfStatus =='initial data entry complete'}">
  ...
  <c:when test="${crfStatus ==double_data_entryi18n || crfStatus =='double data entry'}">
	<!-- double data entry -->
   <a href="javascript:leftnavExpand('Menu_on_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>'); javascript:leftnavExpand('Menu_off_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>');" onMouseOver="layersShowOrHide('visible','Event_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>');
  ...
  <c:when test="${crfStatus ==lockedi18n || crfStatus =='locked'}">
	<!-- locked, we show nothing -->     
  </c:when>
  
  <c:when test="${crfStatus ==no_startedi18n || crfStatus =='Not Started'}">	
  <!-- we have not started -->
   <a href="javascript:leftnavExpand('Menu_on_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>'); javascript:leftnavExpand('Menu_off_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>');" onMouseOver="layersShowOrHide('visible','Event_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>');
...
<div id="Event_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>" style="position: absolute; visibility: hidden; z-index: 3; width: 180px; top: 0px;">
			...
			<div class="ViewSubjectsPopup" style="color:#5b5b5b">
					  ...
					  <table border="0" cellpadding="0" cellspacing="0" width="100%">		
						...
						<c:choose>
                         ...
                         crfStatus =='administrative editing' || crfStatus =='data entry complete'}">
						...
						<tr id="Menu_off_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>" style="display: all">
										...
										<c:choose>
										 ...
										 <c:when test="${eventId<=0 && (crfStatus==no_startedi18n || crfStatus =='Not Started')}">										  
										  <i><fmt:message key="click_to_enter_data" bundle="${restext}"/><br><fmt:message key="to_use_another_version_click" bundle="${restext}"/></i>
										 </c:when>
										 <c:otherwise>
						...
						<tr id="Menu_on_<c:out value="${eblRowCount}"/>_<c:out value="${count}"/>_<c:out value="${subjectName}"/>" style="display: none">
							...
							<table border="0" cellpadding="0" cellspacing="0" width="100%">		
							   <c:choose>
							    <c:when test="${crfStatus ==data_entry_completei18n || crfStatus =='data entry complete' ||  crfStatus =='administrative editing'}">
                                  ...
                                  <c:if test="${userBean.sysAdmin}">
								   <tr valign="top"><td class="table_cell_left"><a href="DeleteEventCRF?action=confirm&ssId=<c:out value="${subjectId}"/>&ecId=<c:out value="${eventCrfId}"/>"><img src="images/bt_Delete.gif" border="0" align="left"></a>&nbsp;&nbsp; <a href="DeleteEventCRF?action=confirm&ssId=<c:out value="${subjectId}"/>&ecId=<c:out value="${eventCrfId}"/>"><fmt:message key="delete" bundle="${resword}"/></a></td></tr>
                                  </c:if>
							    ...
							    <c:otherwise>
							      <c:choose>
							        ...
							        crfStatus==double_data_entryi18n || crfStatus =='data entry complete' || crfStatus =='initial data entry complete'}">
									<tr valign="top"><td class="table_cell_left"><a href="DoubleDataEntry?eventCRFId=<c:out value="${eventCrfId}" />">
									<img src="images/bt_Edit.gif" border="0" align="left"></a>&nbsp;&nbsp; <a href="DoubleDataEntry?eventCRFId=<c:out value="${eventCrfId}" />"><fmt:message key="enter_data" bundle="${resword}"/></a></td></tr>
									 
									</c:when>
									<c:otherwise>
									 <tr valign="top"><td class="table_cell_left"><a href="InitialDataEntry?eventCRFId=<c:out value="${eventCrfId}" />">
									 <img src="images/bt_Edit.gif" border="0" align="left"></a>&nbsp;&nbsp; <a href="InitialDataEntry?eventCRFId=<c:out value="${eventCrfId}" />"><fmt:message key="enter_data" bundle="${resword}"/></a></td></tr>
         ...
         <c:choose>
         <c:when test="${crfStatus ==data_entry_completei18n ||  crfStatus =='administrative editing' || crfStatus =='data entry complete'}">

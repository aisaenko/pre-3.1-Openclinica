...
/**
 *BeanFactory contains methods for generating beans representing non-persistent data. the
 * data derives largely from spreadsheets defining Case Report Forms.
...
public class BeanFactory {
  ...
  protected static final Logger logger = Logger.getLogger(
    "org.akaza.openclinica.control.managestudy.BeanFactory");
  ...
  //the Items that have a response_label value, but leave the options text and values
  //blank and use a response set previously defined in the spreadsheet template.
  private static Map<String,Map<String,String[]>> cachedResponseLabels=Collections.
    synchronizedMap(
      new HashMap<String,Map<String,String[]>>());

  /**
   * This method creates a List of DisplayItemGroupBeans. The DisplayItemGroupBeans are
   * listed in the order their items are defined in the CRF spreadsheet. These
   * beans include the items that are and are not associated with a group.  If an item is defined in a
   * spreadsheet but does not have a group label, that item is included in a DisplayItemGroupBean,
   * but not linked to any groups (i.e., that DisplayItemGroupBean has an empty
   * FormGroupBean object, with an empty String for a group label).
   * For example, if the spreadsheet defines two items
   * first, but does not assign them a group, then
   * those items will make up the first DisplayItemGroupBean. The spreadsheet then might
   * define a <em>group</em> of items
   * followed by more items that are not part of a group. The former items will represent the
   * second DisplayItemGroupBean. The third DisplayItemGroupBean will include the last collection
   * of "orphaned" items, accompanied by an empty FormGroupBean object.
   * @param itemsMap  A Map containing rows of Item information from the spreadsheet. The
   * items are in the order they were defined in the spreadsheet.
   * @param sectionLabel   A String specifying the name of the section we are displaying.
   * @param groupsMap  A Map containing rows of Group information from the spreadsheet.
   * @param crfName The name of the CRF, a String.
   * @see org.akaza.openclinica.bean.submit.ItemGroupBean
   * @return  A  List of DisplayItemGroupBeans
   */
  public List<DisplayItemGroupBean> createGroupBeans(
    Map<Integer,Map<String,String>> itemsMap,
    ...
    if((groupsMap == null || groupsMap.isEmpty()) ||
      (sectionLabel == null || sectionLabel.length() < 1 || sectionLabel.equalsIgnoreCase(UNGROUPED)) ||
      (itemsMap == null || itemsMap.isEmpty()))  {
      return  groupBeans;
    }

    //First, separate the items into those only associated with this section
    Map<String,String> innermap;
    List<DisplayItemBean> displayItems;
    Map.Entry<Integer,Map<String,String>> me;
    //This Map will hold the DisplayitemBeans that are associated with this section
    Map<Integer,Map<String,String>> newMap = new HashMap<Integer,Map<String,String>>();
    String lab;
    for(Iterator<Map.Entry<Integer,Map<String,String>>> iter =
      itemsMap.entrySet().iterator();iter.hasNext();)  {
      me = iter.next();
    ...
    int ordinal = 0;
    //Examine each Item; if it doesn't have a group label, store the item as a
    //DisplayItemBean in a new DisplayFormGroupBean. Once items defining a
    //group begin, create a new DisplayFormGroupBean for those group-related
    //items and place the first DisplayFormGroupBean the code created in the List.
    for(DisplayItemBean disBean : displayItems) {

      latestGroupLabel = disBean.getMetadata().getGroupLabel();
      //If the group label is a valid String, then the displayItem is related
      //to a group. Store it in a FormGroupBean.
      if(latestGroupLabel != null && latestGroupLabel.length() > 0
        && !latestGroupLabel.equalsIgnoreCase(UNGROUPED)) {
        //set this flag to true, indicating that the items being processed now
        //are associated with a valid group
        validGroupFlag=true;
        //If lastGroupLabel is not yet stored in the List of DisplayGroupBeans
        //then the beans associated with that label (DisplayGroupBeans and
        ...
        //if there is no group label associated with the DisplayItemBean, then it
        //does not have a group; it is an "orphaned" item. In this case, create a
        //DisplayFormGroup with a group label signified with an empty string. This
        //"group" will hold the orphaned items. What if there are orphaned items
        //in the spreadsheet that are divided by grouped items? Then these orphaned
        //items have to be in separate DisplayFormGroups. To handle this case, the
        //code checks the validGroupFlag boolean variable. if "true," indicating that
        // non-orphaned beans have just been processed, then create a new
        ...
        if(validGroupFlag  || isFirst) {
          ...
          //is the FormGroupBean containing the highest ordinal, and with a group label
  ...
  private boolean isFirstUngroupedItem(
    List<DisplayItemGroupBean> groupBeans){

    for (DisplayItemGroupBean digBean : groupBeans ) {
      if(digBean.getItemGroupBean().getName().
        equalsIgnoreCase(UNGROUPED)){
        return false;
      }
    }
    return true;
  ...
  private DisplayItemGroupBean getLatestDisFormBeanForOrphanedItems(
    List<DisplayItemGroupBean> beans){

    if(beans == null || beans.isEmpty())  { return new  DisplayItemGroupBean(); }
    ...
    for(DisplayItemGroupBean dBean : beans) {
      if(dBean.getItemGroupBean().getName().length() < 1
        || dBean.getItemGroupBean().getName().equalsIgnoreCase(UNGROUPED))  {
        list.add(dBean);
      }
    }
    //sort the list
    Collections.sort(list,new Comparator<DisplayItemGroupBean>(){
      ...
      public int compare(DisplayItemGroupBean displayFormGroupBean,
                         DisplayItemGroupBean displayFormGroupBean1) {
        return displayFormGroupBean.getGroupMetaBean().getOrdinal().compareTo(
          displayFormGroupBean1.getGroupMetaBean().getOrdinal()
        );
      }
    });
   ...
   * @param groupLabel  The String specifying a group label.
   * @param groupMap    The Map containing the data for a group.
   * @return  ItemGroupBean
   */
  private ItemGroupBean initFormGroupFromMap(String groupLabel,
                                             Map<Integer,Map<String,String>> groupMap)  {
    ...
    String tempValue;
    for(Iterator<Map.Entry<Integer,Map<String,String>>> iter =
      groupMap.entrySet().iterator();iter.hasNext();) {
      groupValues = iter.next().getValue();
      if(groupValues.get("group_label").equalsIgnoreCase(groupLabel)){
        ...
        if("0.0".equalsIgnoreCase(tempValue)) { tempValue="22000";}
        try {

          igMetaBean.setRepeatMax(new Integer(
            numFormatter.format(Double.parseDouble(
              tempValue))));

        } catch(NumberFormatException nfe) {
          //BWP >>an arbitrarily large number allows infinite repeats; it could also be -1
        ...
        //This line is necessary because "0" will not throw an
        //exception, but is still unacceptable
        if("0.0".equalsIgnoreCase(tempValue)) { tempValue="1";}
        try {
          igMetaBean.setRepeatNum(new Integer(numFormatter.format(
  ...
  private DisplayItemGroupBean getGroupFromLabel(String label,
                                                 List<DisplayItemGroupBean> groupBeans)  {

    if(groupBeans == null || groupBeans.isEmpty()) { return new DisplayItemGroupBean(); }
  ...
  public ItemGroupBean createFormGroupBeanFromMap(
    ...
    numFormatter.setMaximumFractionDigits(0);
    String tempValue;
    for (Iterator<Map.Entry<Integer, Map<String, String>>> iter = groupMap.
      entrySet().
      ...
      if("0.0".equalsIgnoreCase(tempValue)) { tempValue="1";}
      try {
        fBean.getMeta().setRepeatNum(new Integer(numFormatter.format(
          Double.parseDouble(tempValue))));
      } catch(NumberFormatException nfe) {
        //BWP 10-13-07
      ...
      if("0.0".equalsIgnoreCase(tempValue)) { tempValue="22000";}
      try{
        fBean.getMeta().setRepeatMax(new Integer(
          numFormatter.format(Double.parseDouble(
            tempValue))));
      } catch(NumberFormatException nfe) {
        //BWP >>an arbitrarily large number allows infinite repeats; it could also be -1
  ...
  /*Generate the list of children for a DisplayItemBean. Currently, this method must return an ArrayList,
* because that is the data type for an ItemFormMetaDataBean's children field.  */
  public ArrayList<DisplayItemBean> createChildren(List<DisplayItemBean> childrenItems,
                                                   DisplayItemBean parent) {
    ArrayList<DisplayItemBean> children = new ArrayList<DisplayItemBean>();
    //If the ItemBean's parent label is the same as an ItemBean's  name, then the  former ItemBean is
    //a child of that paent ItemBean.
    for (DisplayItemBean bean : childrenItems){
      if (bean.getMetadata().getParentLabel().
        equalsIgnoreCase(parent.getItem().getName())){
        children.add(bean);
      }
    }

    //sort in ascending order based on the column number of
    //the ItemFormMetadataBean. This is specified by the DisplayItemBean API.
    Collections.sort(children,new Comparator<DisplayItemBean>(){

      public int compare(DisplayItemBean displayItemBean,
                         DisplayItemBean displayItemBean1) {
        return (new Integer(displayItemBean.getMetadata().getColumnNumber()).compareTo(
          displayItemBean1.getMetadata().getColumnNumber())
        );      }
    });
    return children;
  }
  /* Create an ItemBean from a Map specifying its property values, as in {item_name: Item1, ...) etc.*/
  ...
  public ResponseSetBean createResponseSetBean(Map<String,String> itemValuesMap,
                                               String crfName) {
    ResponseSetBean rBean = new ResponseSetBean();
    //If an item has a response label that has already been defined (by previous
    //item definitions) then use the response_options_text and response_values associated
    ...
    //Find out if the response text and option values are already cached for
    ...
    //if the item has a response label but no values for  response_options_text
    //or response_values, provide the item with the cached values for these properties
    String[] tmpArr;
    if((responseLab != null && responseLab.length() !=0)  &&
      (optionsTxt != null && optionsTxt.length()==0) &&
      (optionsVals != null && optionsVals.length()==0)) {
  ...
  /*Create an ItemFormMetadataBean from the values referred to by a Map.Entry object.
   *  The Map.Entry contains an integer pointing to a Map of Item values from the
   * Sreadsheet template, as in
   * (item_name: Item1, etc...} */
  public ItemFormMetadataBean createItemFormMetadataBean(
    Map.Entry<Integer,Map<String,String>> me,String crfName) {
    if(me == null)  return new ItemFormMetadataBean();
    ...
    try{
      itemPageNum=numFormatter.format(Double.
        parseDouble(itemPageNum));
    } catch(NumberFormatException nfe)  {
      itemPageNum="";
    }
    ...
    try{
      columnNum=numFormatter.format(Double.
        parseDouble(columnNum));
    ...
    try{
      questNum=numFormatter.format(Double.
        parseDouble(questNum));
    } catch(NumberFormatException nfe)  {
      questNum="";
    }
  ...
  /**
   *  Create an ItemFormMetadataBean from the values referred to by a Map object.
   * The Map contains the Item values from the
   * Sreadsheet template, as in
   * (item_name: Item1, etc...}
   * @param map   A Map involving the column names and values Of items.
   * @param ordinal The ordinal property for the meta data bean.
   * @param crfName   The CRF name.
   * @return  An ItemFormMetadataBean representing the values from the Map.
   */
  public ItemFormMetadataBean createMetadataBean(
    Map<String,String> map, int ordinal,
    String crfName) {
    if(map == null)  return new ItemFormMetadataBean();
    ItemFormMetadataBean metadataBean = new ItemFormMetadataBean();
    //the POI library returns all numbers as doubles (2.0), we round
    //off the numbers to 2, for instance
    NumberFormat numFormatter = NumberFormat.getInstance();
  ...
  public DisplaySectionBean createDisplaySectionBean(
    ...
    String instructions,
    String crfName) {
    /*each map row or entry looks like this:
     * 36: {subheader=, validation=, page_number=2.0, response_type=text, left_item_text=Other Body Sysytem/Site,
     * response_options_text=, required=, response_values=, question_number=35.0, phi=0.0,
     * validation_error_message=, header=, units=, data_type=ST, response_label=text,
     * item_name=can_base36, column_number=2.0, section_label=Specify Other Body System/Site,
     * description_label=label, parent_item=can_base34, right_item_text=} */
    //Play defense!
    if(map == null)  return new DisplaySectionBean();
    ...
    Map.Entry<Integer,Map<String,String>> me;
    Map<String,String> innerMap;
    for(Iterator<Map.Entry<Integer,Map<String,String>>> iter = map.
      ...
      if(itemSecLabelMatchesSection(me,sectionLabel))   {
        ...
        if(parentName.length() > 0 &&
          (! displayItem.getItem().getName().equalsIgnoreCase(parentName))) {
    ...
    for(DisplayItemBean parentBean : allDisplayItems)  {
      parentBean.setChildren(this.createChildren(childrenItems,
        parentBean));
    }
  ...
  //to interfaces), because the SectionBean.setitems method takes an ArrayList
  public ArrayList createItemBeanList(Map<Integer,Map<String,String>> itemsMap,
                                      String secLabel,
                                      String crfName){
    if(itemsMap==null) return new ArrayList();
    ...
    //for "1" in a spreadsheet cell, so make sure only "1" is displayed using
    //this NumberFormat object
    NumberFormat numFormatter = NumberFormat.getInstance();
    numFormatter.setMaximumFractionDigits(0);
    for(Iterator<Map.Entry<Integer,Map<String,String>>> iter = itemsMap.
      entrySet().iterator();iter.hasNext();){
      me = iter.next();
  ...
  public List<SectionBean> createSectionBeanList(Map<Integer, Map<String, String>> sectionsMap,
                                                 Map<Integer, Map<String, String>> itemsMap,
                                                 String crfName, Map<Integer, Map<String, String>> groupsMap){
    ...
    NumberFormat numFormatter = NumberFormat.getInstance();
    numFormatter.setMaximumFractionDigits(0);
    for(Iterator<Map.Entry<Integer,Map<String,String>>> iter = sectionsMap.
      entrySet().iterator();iter.hasNext();) {
      ...
      secBean.setParent(createSectionBean(
        sectionVals.get("parent_section")
      ));
  ...
  public List<SectionBean> addPreviewGroupsInfo(List<SectionBean> secsList,
                                                Map<Integer, Map<String, String>> groupsMap) {
    if(secsList == null || secsList.isEmpty()) { return secsList; }
    ItemGroupBean groupBean = new  ItemGroupBean();
    Map<String,String> groupValues;
    String groupLabel ="";
    //Find out which groups belong with which section
    for(Iterator<Map.Entry<Integer,Map<String,String>>> iter =
      groupsMap.entrySet().iterator();iter.hasNext();) {
      groupValues = iter.next().getValue();
  ...
  private boolean sectionHasGroup(String groupLabel, SectionBean secBean) {
    //if an item in a section matches with the group label, then the group belongs to
    //that section
  ...
  private boolean itemSecLabelMatchesSection(Map.Entry me,
                                             String sectionLabel) {
  ...
  public List<DisplayItemBean> createDisplayItemBeansFromMap(
    Map<Integer,Map<String,String>> itemsMap,String crfName)  {

 ...
 * @auther ywang
 *
 * Create CDISC ODM XML format for OpenClinica data based on CDISC ODM Version
 * 1.2.1.
 *
 * <p>
 * At this point, it generates snapshot type file. It includes some requires
 * elements and attributes of general node, metadata node and clinical node. It
 * generates one MetaDataVersion in one file.
 * </p>
 * <p>
 * The generated data file is ready for PROC CDISC to create a SAS data set.
 * </p>
 * <p>
 * For this class to work properly, another data type (an addition row) is added
...
public class CdiscOdmXmlBean {
    ...
    // private SimpleDateFormat sdf_sqldate = new
    // SimpleDateFormat("yyyy-MM-dd");
    private SasNameValidationBean sasname = new SasNameValidationBean();
    private ODMSASFormatNameValidator formatname = new ODMSASFormatNameValidator();
    ...
    /**
     * This method constructs all information about an ODM tag and stores them
     * in the form of Strings in the ArrayList xmlOutput. This method is for
     * Snapshot filetype and still under construction so that some attributes
     * are not available. User should call this method to start the processing
     * of converting relational-database-info into XML-formatted info
     *
     * @param eb
     *            ExtractBean
     */
    public void createCdiscOdmXML(ExtractBean eb) {
        ...
        String root_line =
            "<ODM xmlns=\"" + this.xmlns + "\" " + "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" " + "xsi:schemaLocation=\"" + this.xmlns + " "
                + this.schemaLocation + "\" ";
        root_line +=
            "ODMVersion=\"1.2\" " + "FileOID=\"" + eb.getDataset().getId() + "D" + (new SimpleDateFormat("yyyyMMddHHmmss")).format(creationDateTime) + "\" "
        ...
        root_line += "CreationDateTime=\"" + sdf_datetime.format(creationDateTime) + "\" >";
        // for jdk1.5
        // "CreationDateTime=\"" +
        // sdf_datetime.format(System.currentTimeMillis()) + "\" >";
        String ending_line = "</ODM>";

        // first of all, set up the information about the ODM node, including
        // schema file
     ...
     *
     * @param eb ExtractBean @param currentIndent String
     */
    private void addNodeStudy(ExtractBean eb, String currentIndent) {
        DatasetBean dataset = eb.getDataset();

        /*
         * I hope that the studyID got from dataset is exactly the same as the
         * studyID in currentstudy, this is what I am counting on.
         */
    ...
    private void addStudyBasicDefinitions(org.akaza.openclinica.bean.extract.ExtractBean eb, String currentIndent) {
        /*
         * String line=""; add(currentIndent, "<Basicdefinitions>"); line += "<MeasurementUnit
         * OID= Name=>" + "????" + "</MeasurementUnit>";
         * add(currentIndent+indent, line); add(currentIndent, "</Basicdefinitions>");
         */
    }
    ...
    private void addStudyMetaDataVersion(org.akaza.openclinica.bean.extract.ExtractBean eb, String currentIndent) {
        ...
        // for <Include>,
        // 1. In order to have <Include>, previous metadataversionOID must be
        // given.
        // 2. If there is no previous study, then previous study OID is as the
    ...
    private void addMetaDataVersionProtocol(ExtractBean eb, String currentIndent) {
        ...
        for (int i = 1; i < eb.getNumSEDs() + 1; ++i) {
            ...
            // At this point, I don't know how to handle Mandatory. Since it is
            // required and sed is selected, I assign it 'yes'.
    ...
    private void addMetaDataVersionFormRef(ExtractBean eb, StudyEventDefinitionBean sed, String currentIndent) {
        ...
        ArrayList crfBeans = sed.getCrfs();
    ...
    private void addMetaDataVersionFormDef(ExtractBean eb, Hashtable sedCrfIgItem, String currentIndent) {
        for (int i = 1; i < eb.getNumSEDs() + 1; ++i) {
            ...
            for (int k = 1; k < eb.getSEDNumCRFs(i) + 1; ++k) {
                ...
                add(currentIndent, "<FormDef OID=\"FORM." + crfBean.getId() + "\" Name=\"" + StringEscapeUtils.escapeXml(crfBean.getName())
                    + "\" Repeating=\"No\">");
    ...
    private void addMetaDataVersionItemGroupRef(String crfId, Hashtable crfIgItem, String currentIndent) {
        ...
        while (iter.hasNext()) {
            ...
            // If there is at least one item is required, then Mandatory of a
            // section/ItemGroup is "Yes"
            ArrayList aa = (ArrayList) sectionItemData.get(sectionID + "");
            for (int i = 0; i < aa.size(); ++i) {
                TempItemInfo tii = (TempItemInfo) aa.get(i);
                ItemFormMetadataBean ifmBean = tii.imfb;
            ...
            for (int k = 1; k < eb.getSEDNumCRFs(i) + 1; ++k) {
                ...
                while (iter.hasNext()) {
                    ...
                    add(currentIndent, "<ItemGroupDef OID=\"IG." + sb.getId() + "\" Name=\"" + StringEscapeUtils.escapeXml(sb.getLabel())
                        + "\" Repeating=\"No\" SASDatasetName=\"" + sasname.getValidSasName(sb.getLabel()) + comment + "\">");
    ...
    private void addMetaDataVersionItemRef(String sectionId, Hashtable sectionItemData, String currentIndent) {
        String line;
        ArrayList aa = (ArrayList) sectionItemData.get(sectionId + "");
        // ItemRef, a reference to an ItemDef as it occurs within a specific
        // ItemGroupDef.
        for (int i = 0; i < aa.size(); ++i) {
            TempItemInfo tii = (TempItemInfo) aa.get(i);
            ItemFormMetadataBean ifmBean = tii.imfb;
    ...
    private void addMetaDataVersionItemDef(ExtractBean eb, Hashtable sedCrfIgItem, String currentIndent) {
        ...
        CodeListInfo cli = new CodeListInfo();
        cli.cls = new HashMap<Integer, ResponseSetBean>();
        cli.datatypes = new HashMap<Integer, String>();
        for (int i = 1; i < eb.getNumSEDs() + 1; ++i) {
            StudyEventDefinitionBean sed = (StudyEventDefinitionBean) eb.getStudyEvents().get(i - 1);
            Hashtable crfIgItem = (Hashtable) sedCrfIgItem.get(sed.getId() + "");
            for (int k = 1; k < eb.getSEDNumCRFs(i) + 1; ++k) {
                ...
                while (iter.hasNext()) {
                    ...
                    for (int j = 0; j < aa.size(); ++j) {
                        ...
                        //
                        ResponseSetBean rs = ifmBean.getResponseSet();
                        boolean needCodeList = needCodeList(rs.getResponseTypeId(), ib.getItemDataTypeId());
                        // add(currentIndent, "<ItemDef OID=\"IT." +
                        // tii.itemName +
                        // "\" Name=\"" + stringCut(ifmBean.getLeftItemText()) +
                        // "\"
                        // DataType=\"" + datatype + "\" SASFieldName=\"" +
                        // sasname.getValidSasName(ifmBean.getLeftItemText()) +
                        // "\"/>" );
                        String line = "";
                        if (datatype.equalsIgnoreCase("text")) {
                            ...
                            line +=
                                "<ItemDef OID=\"IT." + StringEscapeUtils.escapeXml(tii.itemName) + "." + tii.crfId + "\" Name=\""
                                    + StringEscapeUtils.escapeXml(tii.itemName) + "\" DataType=\"" + datatype + "\" Length=\"" + length + "\" SASFieldName=\""
                                    + sasname.getValidSasName(tii.itemName) + comment;
                            ...
                            if (needCodeList) {
                                length = getLength(rs);
                                significantDigits = getSignificantDigits(rs);
                            }
                            line +=
                                "<ItemDef OID=\"IT." + StringEscapeUtils.escapeXml(tii.itemName) + "." + tii.crfId + "\" Name=\""
                                    + StringEscapeUtils.escapeXml(tii.itemName) + "\" DataType=\"" + datatype + "\" Length=\"" + length
                                    + "\" SignificantDigits=\"" + significantDigits + "\" SASFieldName=\"" + sasname.getValidSasName(tii.itemName) + comment;
                        } else {
                            line +=
                                "<ItemDef OID=\"IT." + StringEscapeUtils.escapeXml(tii.itemName) + "." + tii.crfId + "\" Name=\""
                                    + StringEscapeUtils.escapeXml(tii.itemName) + "\" DataType=\"" + datatype + "\" SASFieldName=\""
                                    + sasname.getValidSasName(tii.itemName) + comment;
                        }
                        ...
                        if (!(header.equals("") && left.equals("") && right.equals(""))) {
                            ...
                            add(currentIndent + indent, "<Question>");
                            add(currentIndent + indent + indent, "<TranslatedText>");
                            add(currentIndent + indent + indent, StringEscapeUtils.escapeXml(header + left + right));
                            add(currentIndent + indent + indent, "</TranslatedText>");
                            add(currentIndent + indent, "</Question>");
                        }
                        // add RangeCheck
                        String validation = ifmBean.getRegexp();
                        if (validation != null && validation.startsWith("func:")) {
                            if (!hasNode) {
                                hasNode = true;
                                line += "\">";
                                add(currentIndent, line);
                            ...
                            String func = validation.substring(5).trim();
                            String errmsg = ifmBean.getRegexpErrorMsg();
                            String softhard = getSoftHard(eb);
                            addRangeCheck(func, softhard, errmsg, currentIndent);
                        }
                        // add CodeListRef
                        if (needCodeList) {
                            if (!hasNode) {
                            ...
                            cli.cls.put(Integer.valueOf(rs.getId()), rs);
                            if (cli.cls.containsKey(rs.getId())) {
                                cli.datatypes.put(Integer.valueOf(rs.getId()), datatype);
                            }
                            line += "<CodeListRef CodeListOID=\"CL." + ifmBean.getResponseSetId() + "\"/>";
                            add(currentIndent + indent, line);
                            line = "";
                        }
                        if (hasNode) {
        ...
        addMetadataCodeList(cli, currentIndent);

    }

    private void addMetadataCodeList(CodeListInfo cli, String currentIndent) {
        if (cli.cls.size() > 0) {
            String line = "";
            String indent = "    ";
            ArrayList<ResponseSetBean> sortedCL = new ArrayList<ResponseSetBean>();
            TreeSet<Integer> sorted = new TreeSet<Integer>(cli.cls.keySet());
            Object[] setArray = sorted.toArray();
            for (int i = 0; i < setArray.length; ++i) {
                sortedCL.add(cli.cls.get(setArray[i]));
            }
            for (ResponseSetBean cl : sortedCL) {
                String datatype = cli.datatypes.get(cl.getId());
                boolean isString = datatype.equalsIgnoreCase("text") ? true : false;
                line +=
                    "<CodeList OID=\"CL." + cl.getId() + "\" Name=\"" + StringEscapeUtils.escapeXml(cl.getLabel()) + "\" DataType=\"" + datatype
                        + "\" SASFormatName=\"" + formatname.getValidSASFormatName(cl.getLabel(), isString) + "\">";
                // line += "<CodeList OID=\"CL." + cl.getId() + "\" Name=\"" +
                // cl.getLabel() + "\" DataType=\"" +
                // cli.datatypes.get(cl.getId()) + "\">";
                add(currentIndent, line);
                line = "";
                ArrayList<ResponseOptionBean> codelists = cl.getOptions();
                // check if the first option is empty or not
                ResponseOptionBean c0 = codelists.get(0);
                if (!c0.getText().trim().equals("") && !c0.getValue().trim().equals("")) {
                    add(currentIndent + indent, "<CodeListItem CodedValue=\"" + StringEscapeUtils.escapeXml(c0.getValue()) + "\">");
                    add(currentIndent + indent + indent, "<Decode>");
                    add(currentIndent + indent + indent + indent, "<TranslatedText>" + StringEscapeUtils.escapeXml(c0.getText()) + "</TranslatedText>");
                    add(currentIndent + indent + indent, "</Decode>");
                    add(currentIndent + indent, "</CodeListItem>");
                }
                for (int i = 1; i < codelists.size(); ++i) {
                    ResponseOptionBean c = codelists.get(i);
                    add(currentIndent + indent, "<CodeListItem CodedValue=\"" + StringEscapeUtils.escapeXml(c.getValue()) + "\">");
                    add(currentIndent + indent + indent, "<Decode>");
                    add(currentIndent + indent + indent + indent, "<TranslatedText>" + StringEscapeUtils.escapeXml(c.getText()) + "</TranslatedText>");
                    add(currentIndent + indent + indent, "</Decode>");
                    add(currentIndent + indent, "</CodeListItem>");
                }
                add(currentIndent, "</CodeList>");
            }
        }
    }

    /**
     * *---------- Methods related to AdminData nodes. However, it is not
     * available currently ---------*
     */
    ...
    /**
     * *---------- Methods related to ReferenceData nodes. However, it is not
     * available currently ---------*
     */
    ...
    /*
     * @param eb ExtractBean @param currentIndent String the indent space for
     * the clinical data nodes
     */
    private void addNodeClinicalData(ExtractBean eb, String currentIndent) {
        ...
        for (int w = 1; w < ss.size() + 1; ++w) {
            ...
            add(currentIndent + indent, "<SubjectData SubjectKey=\"" + StringEscapeUtils.escapeXml(studySubjectBean.getLabel()) + "\">");
            ...
            for (int i = 1; i < eb.getNumSEDs() + 1; ++i) {
                ...
                for (int j = 1; j < eb.getSEDNumSamples(i) + 1; ++j) {
                    ...
                    for (int k = 1; k < eb.getSEDNumCRFs(i) + 1; ++k) {
                        ...
                        // the key is the sectionID, the value is an arrayList
                        // of TempItemInfo
                        Hashtable sectionItemData = new Hashtable();
                        // 1. from the item_form_metadata table, get section_id,
                        // and from section_id get SectionBean
                        // if item has value
                        TempItemInfo tti;
                        SectionDAO sdao = new SectionDAO(this.ds);
                        for (int n = 1; n < eb.getNumItems(i, k) + 1; ++n) {
                            // adding an extra loop here for repeating items,
                            // tbh
                            // for (int o=0; o < eb.getMaxItemDataBeanOrdinal();
                            // o++) {
                            for (java.util.Iterator iter = groupNames.entrySet().iterator(); iter.hasNext();) {
                                ...
                                for (int m = 1; m <= size; m++) {
                                    ...
                                    // TODO NO guard clause here like in extract
                                    // bean, need to review and do, tbh
                                    // YW 11-02-2007, at this time,
                                    // !itemvalue.equals("") is good enough for
                                    // avoiding groupName not
                                    // belong to an item.
                                    String itemvalue = eb.getDataByIndex(w, i, j, k, n, m, groupName); // o+1
                                    // since
                                    // o
                                    // start
                                    // from
                        ...
                        // 2. From the hashtable, find all keys and then all
                        // values, print them into the XML
                        TreeSet sortedKeys = new TreeSet();
                        sortedKeys.addAll(sectionItemData.keySet());
                        Iterator iter = sortedKeys.iterator();
                        while (iter.hasNext()) {
                            String sectionID = (String) iter.next();
                            ArrayList aa = (ArrayList) sectionItemData.get(sectionID);
                            // double make sure that this section/itemGroup has
                            // something
                            if (aa.size() > 0) {
                                add(currentIndent + indent + indent + indent + indent, "<ItemGroupData ItemGroupOID=\"IG." + sectionID
                                    + "\" TransactionType=\"Insert\" >");
                                for (int m = 0; m < aa.size(); ++m) {
                                    // for(int m=aa.size()-1; m>=0; --m){
                                    tti = (TempItemInfo) aa.get(m);
                                    ItemFormMetadataBean ifmBean = tti.imfb;
                                    ItemBean ib = (ItemBean) idao.findByPK(ifmBean.getItemId());
                                    ...
                                    add(currentIndent + indent + indent + indent + indent + indent, "<ItemData ItemOID=\"IT."
                                        + StringEscapeUtils.escapeXml(tti.itemName) + "." + tti.crfId + "\" Value=\"" + StringEscapeUtils.escapeXml(value)
                                        + "\"/>");
    ...
    // --------------------------------------
    /*
     * This method return a Hashtable 'sedCrfIgItem'. In 'sedCrfIgItem', the key
     * is sedId and the value is a Hashtable 'crfIgItem'. In 'crfIgItem', the
     * key is CRFId and the value is a Hashtable 'sectionItemData'. In
     * 'sectionItemData', the key is sectionId and the value is an arrayList of
     * TempItemInfo
     *
     * @param eb ExtractBean @param currentIndent String the indent space for
     * the clinical data nodes
        ...
        for (int i = 1; i < eb.getNumSEDs() + 1; ++i) {
            ...
            for (int j = 1; j < eb.getSEDNumSamples(i) + 1; ++j) {
                ...
                for (int k = 1; k < eb.getSEDNumCRFs(i) + 1; ++k) {
                    ...
                    // the hashtable used to organize the section/item group
                    // the key is the sectionID, the value is an arrayList of
                    // TempItemInfo
                    Hashtable sectionItemData = new Hashtable();
                    // 1.1 from the item_form_metadata table, get section_id,
                    // and
                    // from section_id get SectionBean
                    // if item has value
                    TempItemInfo tti;
                    SectionDAO sdao = new SectionDAO(this.ds);
                    for (int n = 1; n < eb.getNumItems(i, k) + 1; ++n) {
                    ...
                    // 1.2 put Hashtable sectionItemData into Hashtable
                    // crfSectionItem
    ...
    class CodeListInfo {
        HashMap<Integer, ResponseSetBean> cls;
        HashMap<Integer, String> datatypes; // <response_set_id, item_datatype
    }
    ...
    private String getODMDate(String d, String locale_df_string) {
        try {
            return new SimpleDateFormat("yyyy-MM-dd").format(new SimpleDateFormat(locale_df_string).parse(d));
        } catch (Exception fe) {
            // fe.printStackTrace();
            // This way will fail user's ODM xml against ODM1-2-1 schema and
            // force user to check.
            return d;
        }
    }

    private boolean needCodeList(int id, int datatypeid) {
        if ((id == 3 || id == 5 || id == 6 || id == 7) && (datatypeid == 5 || datatypeid == 6 || datatypeid == 7)) {
            return true;
        }
        return false;
    }

    private int getLength(ResponseSetBean rs) {
        int len = 0;
        for (ResponseOptionBean rso : (ArrayList<ResponseOptionBean>) rs.getOptions()) {
            len = Math.max(len, rso.getValue().length());
        }
        return len;
    }

    private int getSignificantDigits(ResponseSetBean rs) {
        int d = 0;
        for (ResponseOptionBean rso : (ArrayList<ResponseOptionBean>) rs.getOptions()) {
            String v = rso.getValue();
            if (v != null && v.length() > 0) {
                d = Math.max(d, BigDecimal.valueOf(Double.parseDouble(rso.getValue())).scale());
            }
        }
        return d;
    }

    private String getSoftHard(ExtractBean eb) {
        int studyId = eb.getDataset().getStudyId();
        StudyParameterValueDAO spvdao = new StudyParameterValueDAO(this.ds);
        StudyParameterValueBean spv = spvdao.findByHandleAndStudy(studyId, "discrepancyManagement");
        if (spv.getValue().equalsIgnoreCase("true")) {
            return "Soft";
        } else {
            return "Hard";
        }
    ...
    private String lineRangeCheckAttributes(String comparator, String softhard) {
        return "<RangeCheck Comparator=\"" + comparator + "\" SoftHard=\"" + softhard + "\">";
    }

    private String lineCheckValue(String value) {
        return "<CheckValue>" + value + "</CheckValue>";
    }

    private String lineErrorMessage(String errormessage) {
        return "<ErrorMessage><TranslatedText>" + errormessage + "</TranslatedText></ErrorMessage>";
    }

    private void addOneRangeCheck(String comparator, String softhard, String value, String errormessage, String currentIndent) {
        add(currentIndent, lineRangeCheckAttributes(comparator, softhard));
        add(currentIndent + indent, lineCheckValue(value));
        /*
         * line += "<MeasurementUnitRef MeasurementUnitOID=/>";
         * add(currentIndent+indent+indent, line); line="";
         */
        add(currentIndent + indent, lineErrorMessage(errormessage));
        add(currentIndent, "</RangeCheck>");
    }

    private void addRangeCheck(String func, String softhard, String errormessage, String currentIndent) {
        ...
        if (s[0].equalsIgnoreCase("range")) {
            ...
            addOneRangeCheck("GE", softhard, smaller, errormessage, currentIndent);
            addOneRangeCheck("LE", softhard, larger, errormessage, currentIndent);
        } else {
            String value = s[1].trim();
            value = value.substring(0, value.length() - 1);
            if (s[0].equalsIgnoreCase("gt")) {
                addOneRangeCheck("GT", softhard, value, errormessage, currentIndent);
            } else if (s[0].equalsIgnoreCase("lt")) {
                addOneRangeCheck("LT", softhard, value, errormessage, currentIndent);
            } else if (s[0].equalsIgnoreCase("gte")) {
                addOneRangeCheck("GE", softhard, value, errormessage, currentIndent);
            } else if (s[0].equalsIgnoreCase("lte")) {
                addOneRangeCheck("LE", softhard, value, errormessage, currentIndent);
            } else if (s[0].equalsIgnoreCase("ne")) {
                addOneRangeCheck("NE", softhard, value, errormessage, currentIndent);
            } else if (s[0].equalsIgnoreCase("eq")) {
                addOneRangeCheck("EQ", softhard, value, errormessage, currentIndent);
            }
        }
    }
     ...
     */
    /*
     * All information of from the database is organized in an array of String
     * in order. If print all the strings in this ArrayList in order, everything
     * should be fine.
     */
    ...
    /**
     * Set the heading of the xml file. By default, it is <?xml version="1.0"
     * encoding="UTF-8"?>
     *
     * @param heading
     *            String
    ...
    /**
     * Set the namespace of the xml file. By default, it is
    ...
    /**
     * Return the used-indent value. This value is used to indent all the XML
     * information.
     *
     * @return String
     */
    ...
    /**
     * Set the indent information for displaying the xml information. If you
     * prefer having no indentation, please set it to empty string. By default,
     * it is 4 spaces.
     *
     * @param indent
     *            String

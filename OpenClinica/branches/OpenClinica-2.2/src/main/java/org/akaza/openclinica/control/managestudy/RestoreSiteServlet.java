    ...
    if (StringUtil.isBlank(idString)) {
      ...
      forwardPage(Page.SITE_LIST_SERVLET);
        ...
        if ("confirm".equalsIgnoreCase(action)) {
        	//site can be restored when its parent study is not "removed" -- YW -6-21-2007
        	...
        	if(!"removed".equals(parentstudy.getStatus().getName())) {
        		...
        		addPageMessage("The chosen site \"" + study.getName() + "\" can not be restored because its study \"" + parentstudy.getName() + "\" has been removed.");
        ...
        for (int i = 0; i < userRoles.size(); i++) {
          ...
          //YW << So study_user_role table status_id field can be updated
          udao.updateStudyUserRole(role, role.getUserName());
          }
          //YW 06-18-2007 >>
        }
        
        //YW 06-19-2007 << Meanwhile update current active study attribute of session if restored study is current active study
        if(study.getId() == currentStudy.getId()) {
      	  	currentStudy.setStatus(Status.AVAILABLE);
      	  	
      	  	StudyUserRoleBean r = (StudyUserRoleBean) (new UserAccountDAO(sm.getDataSource())).findRoleByUserNameAndStudyId(ub.getName(), currentStudy.getId());
      	  	StudyUserRoleBean rInParent = (StudyUserRoleBean) (new UserAccountDAO(sm.getDataSource())).findRoleByUserNameAndStudyId(ub.getName(), currentStudy.getParentStudyId());
      	  	//according to logic in SecureController.java: inherited role from parent study, pick the higher role
        ...
        for (int i = 0; i < subjects.size(); i++) {
          ...
          ArrayList events = (ArrayList) sedao.findAllByStudySubject(subject);

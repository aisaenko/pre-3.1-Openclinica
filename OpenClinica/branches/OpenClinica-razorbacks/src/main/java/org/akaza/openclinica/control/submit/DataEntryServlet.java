...
public abstract class DataEntryServlet extends SecureController {
    ...
    /**
     * This methods will create an array of DisplayItemGroupBean,
     * which contains multiple rows for an item group on the data entry form.
     *
     * @param digb
     ...
     * @return new constructed formGroups, compare to dbGroups,
     * some rows are update, some new ones are added and some are removed
     */
    protected List<DisplayItemGroupBean> loadFormValueForItemGroup(
      DisplayItemGroupBean digb, List<DisplayItemGroupBean> dbGroups,
     ...
     * @return
     */
    private List<DisplayItemBean> processInputForGroupItem(FormProcessor fp,
                                                           List<DisplayItemBean> dibs, int i,
        ...
        for (int j = 0; j < dibs.size(); j++) {
            ...
            if (rt.equals(org.akaza.openclinica.bean.core.ResponseType.CHECKBOX) ||
              rt.equals(org.akaza.openclinica.bean.core.ResponseType.SELECTMULTI)) {
                ...
                //BWP issue 3257 << This seems to be an apt place to check any single-select options as "selected"
                //if the data in the displayItem matches the options text; although this code is very
                //confusing and murky, ; refactoring of form handling is necessary here
    ...
    private String ifValueIsDate(ItemBean itemBean, String value) {
        ...
        for (DisplayItemWithGroupBean displayItemWithGroupBean : allItems) {
            ...
            if (displayItemWithGroupBean.getSingleItem() != null) {
                if (displayItemWithGroupBean.getSingleItem().getItem().getOid() != null) {
                    ...
                    c.variableAndValue.put(displayItemWithGroupBean.getSingleItem().getItem().getOid(), ifValueIsDate(displayItemWithGroupBean.getSingleItem()
                      .getItem(), displayItemWithGroupBean.getSingleItem().getData().getValue()));
                    ...
                    for (Object displayItemBean : displayItemWithGroupBean.getSingleItem().getChildren()) {
                        ...
                        String value = ifValueIsDate(((DisplayItemBean) displayItemBean).getItem(), ((DisplayItemBean) displayItemBean).getData().getValue());
            ...
            for (DisplayItemGroupBean itemGroupBean : displayItemWithGroupBean.getItemGroups()) {
                ...
                for (DisplayItemBean displayItemBean : itemGroupBean.getItems()) {
                    ...
                    c.variableAndValue.put(key1, ifValueIsDate(displayItemBean.getItem(), displayItemBean.getData().getValue()));

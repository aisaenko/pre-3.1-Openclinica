...
public class StudyDAO extends AuditableEntityDAO {
    ...
    /**
     * <b>update </b>, the method that returns an updated study bean after it updates the database. Note that we can use the three stages from our creation use
     * case.
     * 
    ...
    /**
     * <P>
     * updateStepOne, the update method for the database. This method takes the place of createStepOne, since it runs an update and assumes you already have a
     * primary key in the study bean object.
     * 
     * @param sb
     ...
     * <P>
     * note: create is split up into four custom functions, per the use case; we are creating the standard create function here which calls all four functions
     * at once, but the seperate functions may be required in the control servlets.
     * 
    ...
    /**
     * <P>
     * createStepOne, per the 'Detailed use case for administer system document v1.0rc1' document. We insert the study in this method, and then update the same
     * study in the next three steps.
     * <P>
    ...
    /*
     * public HashMap fillVariables(StudyBean sb) { HashMap variables = new HashMap(); variables.put(new Integer(1),new Integer(sb.getParentStudyId()));
     * variables.put(new Integer(2),sb.getLabel()); return variables; }
     */
     ...
     * @return A HashMap where the keys are Integers whose intValue are studyIds and the values are ArrayLists; each element of the ArrayList is a StudyBean
     *         representing a child of the study whose id is the key
     *         <p>
     *         e.g., if A has children B and C, then this will return a HashMap h where h.get(A.getId()) returns an ArrayList whose elements are B and C
     */

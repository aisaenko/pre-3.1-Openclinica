 ...
 * 
 * <p>
 * There are 15 types of validations possible; below there are details on the semantics as well as the syntax of the addValidation call:
 * 
 * <ul>
 ...
 * </ul>
 * <li>IS_A_DATE – test that a given input is in "MM/DD/YYYY" format. Note that this format is specified as Validator.DATE.getDescription() (DATE is a static
 * ValidationRegularExpression object)
 * <ul>
 * <li>addValidation(fieldname, Validator.IS_A_DATE)
 * </ul>
 * <li>IS_DATE_TIME – test that a given input is in "MM/DD/YYYY HH:MM a" format. Note that this format is specified as Validator.DATE_TIME.getDescription()
 * (DATE is a static ValidationRegularExpression object)
 * <ul>
 * <li>addValidation(prefix, Validator.IS_A_DATE)
 * <li>This validation is a bit different from the others. It assumes that you are attempting to validate a *group* of inputs, all of which have the same
 ...
 * <li>Half ("am" or "pm")
 * </ul>
 * <li>e.g., if you have a start datetime and end datetime on the same form, you might have startDate, startHour, startMinute, startHalf, and similarly for end.
 ...
 * </ul>
 * <li>CHECK_SAME – test that one field equals the value of another. This is null-safe meaning that if both fields are blank, no errors are reported.
 * <ul>
 ...
 * </ul>
 * <li>IS_A_EMAIL – test that one field is in proper e-mail format, ie "username@institution.domain". Note that this format is specified as
 * Validator.EMAIL.getDescription() (EMAIL is a static ValidationRegularExpression object)
 * <ul>
 * <li>addValidation(fieldname, Validator.IS_A_EMAIL);
 * </ul>
 * <li>IS_A_PHONE_NUMBER - test that one field is in proper phone number format, ie "123-456-7890". Note that this format is specified as
 * Validator.PHONE_NUMBER.getDescription() (PHONE_NUMBER is a static ValidationRegularExpression object)
 * <ul>
 * <li>addValidation(fieldname, Validator.IS_A_PHONE_NUMER);
 * </ul>
 * <li>LENGTH_NUMERIC_COMPARISON – test that the length of a string meets some numeric comparison test, e.g. ">= 8 characters long".
 * <ul>
 * <li>addValidation(fieldname, Validator.LENGTH_NUMERIC_COMPARISON, operator, compareTo);
 * <li>operator is a NumericComparisonOperator object. (NumericComparisonOperator is a typesafe enumeration which implements a controlled vocabulary of “=”,
 * “!=”, “ <”, “ <=”, “>”, “>=”)
 ...
 * </ul>
 * <li>ENTITY_EXISTS – not yet implemented, but it will test that the value of an input field holds the primary key of some entity in the database; the entity
 * type is implicitly specified by an EntityDAO passed to addValidation
 ...
 * </ul>
 * <li>COMPARES_TO_STATIC_VALUE – test that an input field is an integer which matches a Boolean comparison to some fixed integer value
 * <ul>
 ...
 * <li>note that while the input name on the form should include brackets (e.g. "input123[]"), brackets should not be included in the call to addValidation
 ...
 * 
 * <p>
 * We will take, as an example, a new type of validation which checks that the input field contains a multiple of some integer (specified by the control.) For
 * example, if you are asking the user to input time in 15-minute increments, you can use this validation to ensure that the user enters a multiple of 15.
 * 
 * <ul>
 * <li>assign the type a public static int, e.g. IS_MULTIPLE
 * <li>if necessary, write an addValidation method to store that type of validation in the validations HashMap. make sure to note that the addValidation method
 * you wrote is for the validation type in the comments. <br/>
 * a new addValidation method is necessary if there are no other addValidation methods which accept the kind of arguments you need. in this case, we need only
 * the multiple argument (15, in the example used above.) In fact, there is no such method (at the time of writing), so we add one: <br/>
 * <br/>
 * <code>
 ...
 * <br/>
 * <br/>
 * Note that in the code above, we added a comment to indicate which validation type the addValidation method is used for. This is a courtesy to other
 * developers who are using the class.
 * 
 * <li>if it's not necessary to write an addValidation method, determine which addValidation will be used to store the type of validation you are writing, and
 * note this fact in the comments for the appropriate addValidation method.
 * 
 * <li>if necessary, write a method to execute the validation, similar to isBlank, isNumber, etc. note that the utility functions intComparesToStaticValue and
 * matchesRegex can handle a wide variety of validations. don't reinvent the wheel! <br/>
 ...
 * </code>
 * 
 * <li>in validate(String fieldName, Validation v, HashMap errors), add a case to respond to the new validation type be sure to get all necessary arguments,
 * determine if the input field passes validation, and call addError(fieldName, v) if the field does not pass validation <br/>
 * For example: <br/>
 * <br/>
 * 
 * <code>
 ...
 * </code>
 * 
 * <li>in addError(String fieldName, Validation v), add a case to set a default error message which will be displayed to the user if an input violates the
 * validation rule. <br/>
 * For example: <br/>
 * <br/>
 * 
...
public class Validator {
    ...
    /*
     * use for: NO_BLANKS, IS_A_NUMBER, IS_A_DATE, IS_A_EMAIL, IS_AN_INTEGER, IS_A_PASSWORD, IS_A_USERNAME, IS_A_PHONE_NUMBER, IS_DATE_TIME, NO_BLANKS_SET
     * DATE_IN_PAST
     */
    ...
    /**
     * Adds an error to a <code>HashMap</code> of errors generated by validate. This can be used for "one-off" validations, e.g.:
     * 
     * <code>
     * errors = v.validate();
     * 
     ...
     * @return <code>true</code> if the field contains a valid date in "MM/dd/yyyy" format; <code>false</code> otherwise.
     */
     ...
     * @param fieldName
     *            The name of a field containing some text string.
     * @return <code>true</code> if the field contains a valid date in "MM/dd/yyyy" format; <code>false</code> if the given input is not date formatted or null
     *         or empty .
     */
     ...
     * @param fieldName
     *            The name of a field containing some text string.
     * @return <code>true</code> if the field contains a valid date in "MM/dd/yyyy" format and is in the past; <code>false</code> otherwise.
     */
     ...
     * @return <code>true</code> if the fields encode a valid date/time in "date_time_format_string" format. <code>false</code> otherwise.
     */
    ...
    public static Validation processCRFValidationFunction(String inputFunction) throws Exception {
        ...
        if (!fname.equalsIgnoreCase("range") && !fname.equalsIgnoreCase("gt") && !fname.equalsIgnoreCase("lt") && !fname.equalsIgnoreCase("gte")
            && !fname.equalsIgnoreCase("lte") && !fname.equalsIgnoreCase("eq") && !fname.equalsIgnoreCase("ne")) {

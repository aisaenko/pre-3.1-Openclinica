...
<c:when test="${panel.orderedData}">
    ...
    --%>
    <a href='ViewSelected'><b><fmt:message key="view_selected_items" bundle="${resword}"/></b></a><br><br>
	<c:if test="${!EditSelectedSubmitted && !ManagePrivileges}"><a href='javascript:void 0' onclick="if(confirm('<fmt:message key="there_a_total_of" bundle="${resword}"><fmt:param value="${numberOfStudyItems}"/></fmt:message>')){document.getElementById('selectAllItems').submit();}"><b><fmt:message key="select_all_items_in_study" bundle="${resword}"/></b></a>
   </c:if> 
     </c:when>
     <c:when test="${panel.permissions}">
           <b><fmt:message key="study_events" bundle="${resword}"/>:</b><br>
           <c:set var="count" value="0"/>
           <c:set var="newEvent" value="0"/>
           <c:set var="eventCount" value="0"/>            

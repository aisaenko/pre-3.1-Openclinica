...
public abstract class DataEntryServlet extends SecureController {
    ...
    protected void processRequest() throws Exception {
        ...
        if (!isSubmitted) {
            ...
            HashMap<String, ArrayList<String>> groupOrdinalPLusItemOid = runRules(allItems, ruleSets, true, shouldRunRules(),ecb);
            ...
            for (int i = 0; i < allItems.size(); i++) {
                ...
                if (diwb.isInGroup()) {
                    ...
                    if (isChanged(dib, oldItemdata, attachedFilePath)) {
                        changedItems.add(itemName);
                    ...
                    for (int j = 0; j < children.size(); j++) {
                        ...
                        if (isChanged(child, oldItemdata, attachedFilePath)) {
                            changedItems.add(itemName);
            ...
            if (!errors.isEmpty()) {
                ...
                */
                ...
                runRules(allItems, ruleSets, false, shouldRunRules(),ecb);
    ...
    protected List<DisplayItemGroupBean> loadFormValueForItemGroup(DisplayItemGroupBean digb, List<DisplayItemGroupBean> dbGroups,
        ...
        for (int i = 0; i < repeatMax; i++) {
            ...
            List<DisplayItemBean> dibs = FormBeanUtil.getDisplayBeansFromItems(itBeans, sm.getDataSource(), ecb.getCRFVersionId(), sb.getId(), edcb, 0);
    ...
    private ArrayList getParentDisplayItems(boolean hasGroup, SectionBean sb, EventDefinitionCRFBean edcb, ItemDAO idao, ItemFormMetadataDAO ifmdao,
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (dib != null) {
                dib.setData(idb);
                logger.debug("just set " + idb.getId() + " " + idb.getValue());
                displayItems.put(new Integer(idb.getItemId()), dib);
            } else {
                // this can be the case with import, tbh
                logger.debug("found a null, setting here: " + idb.getValue());
                //            	DisplayItemBean displayItem = new DisplayItemBean();
                //          	displayItem.setData(idb);
                //          	ItemBean item = (ItemBean)idao.findByPK(idb.getItemId());
                //          	displayItem.setItem(item);
                //          	displayItems.put(idb.getItemId(), displayItem);

            }
        }

        logger.debug("found this list of display item beans:");
        ...
        while (hmIt.hasNext()) {
            ...
            answer.add(dib);
            logger.debug("*** getting with key: " + key + " display item bean with value: " + dib.getData().getId() + " " + dib.getData().getValue());
        }
                        ...
                        for (DisplayItemBean dib : displayGroup.getItems()) {
                            for (int j = 0; j < data.size(); j++) {
                                ...
                                if (idb.getItemId() == dib.getItem().getId() && !idb.isSelected()) {
                                    ...
                                    dib.setData(idb);
                                    // logger.debug("+++about to check if db values is true here");
                                    if (shouldLoadDBValues(dib)) {
    ...
    private HashMap<String, ArrayList<String>> runRules(List<DisplayItemWithGroupBean> allItems, List<RuleSetBean> ruleSets, Boolean dryRun,
            Boolean shouldRunRules,EventCRFBean ecb) {
        if (shouldRunRules) {
            ...
            return getRuleSetService().runRulesInDataEntry(ruleSets, dryRun, currentStudy, ub, c.variableAndValue,ecb);
        } else {

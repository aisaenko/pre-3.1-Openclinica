...
<!-- EDIT !! -->
	...
	<tr valign="top">
	  	<td class="formlabel">Pii Privilege:</td>
		<td valign="top">
			<table border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td valign="top"><div class="formfieldM_BG">
						<select name="piiPrivilegeId" class="formfieldM">
						<c:forEach var="currPii" items="${piiPrivileges}">
								<c:choose>
									<c:when test="${piiPrivilegeId == currPii.id}">
										<option value='<c:out value="${currPii.id}" />' selected><c:out value="${currPii.name}" /></option>
									</c:when>
									<c:otherwise>
										<option value='<c:out value="${currPii.id}" />'><c:out value="${currPii.name}" /></option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</select>			
    ...
    <tr valign="top">
	  <td class="formlabel">User Password:<!-- <br />(generated by system) --></td>
         ...
         <c:when test="${displayPwd == 'no'}">
            ...
            <div style="padding-top: 4px; padding-right: 4px; padding-left: 2px; padding-bottom: 4px;"><input type="hidden" name="displayPwd" value="no">A system generated password will be sent via Email</div>
         </c:when>
         <c:otherwise>
            ...
            <div style="padding-top: 4px; padding-right: 4px; padding-left: 2px; padding-bottom: 4px;"><input type="hidden" name="displayPwd" value="yes">A system generated password will be shown in the &quot;Alerts&nbsp;&amp;&nbsp;Messages&quot; section on the next screen.</div>
         </c:otherwise>
       </c:choose>
      </td>
	</tr>

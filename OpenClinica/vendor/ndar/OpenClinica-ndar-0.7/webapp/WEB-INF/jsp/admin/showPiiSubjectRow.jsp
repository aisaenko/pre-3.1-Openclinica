...
<jsp:useBean scope="request" id="currRow" class="org.akaza.openclinica.bean.admin.DisplayPiiSubjectRow" />

<tr valign="top">        
      <td class="table_cell_left"><c:out value="${currRow.bean.person.personId}"/>&nbsp;</td>
      <td class="table_cell"><c:out value="${currRow.bean.person.surname}"/>,&nbsp;<c:out value="${currRow.bean.person.givenName}"/></td>
      ...
      <td class="table_cell"><c:out value="${currRow.bean.labelIds}"/>&nbsp;</td>
      ...
      <tr>
      <td>
      <a href="ViewPiiSubject?id=<c:out value="${currRow.bean.person.personId}"/>"
	  ...
	  <c:choose>	  
       <c:when test="${currRow.bean.subject.status.name !='removed'}">
        <td><a href="EditSubject?uniqueIdentifier=<c:out value="${currRow.bean.person.personId}"/>"
	  ...
	  <c:choose>
       <c:when test="${currRow.bean.subject != null}">
		<td><a href="AssignPiiSubject?id=<c:out value="${currRow.bean.subject.id}"/>"
			onMouseDown="setImage('bt_Assign1','images/bt_Assign_d.gif');"
			onMouseUp="setImage('bt_Assign1','images/bt_Assign.gif');"><img 
			name="bt_Assign1" src="images/bt_Assign.gif" border="0" alt="Add Study" title="Add to Another Study" align="left" hspace="6"></a>
		</td>        

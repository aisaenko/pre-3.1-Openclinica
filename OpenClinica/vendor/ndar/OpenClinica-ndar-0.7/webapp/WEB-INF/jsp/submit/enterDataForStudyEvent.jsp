		...
		<c:forEach var="dedc" items="${uncompletedEventDefinitionCRFs}">
			<c:set var="getQuery" value="action=ide_s&eventDefinitionCRFId=${dedc.edc.id}&studyEventId=${studyEvent.id}&subjectId=${studySubject.subjectId}&eventCRFId=${dedc.eventCRF.id}" />
				...
				<c:choose>
				  ...
				  <c:otherwise>
				    <form name="startForm<c:out value="${dedc.edc.crf.id}"/>" action="AssessmentInfo?<c:out value="${getQuery}"/>" method="POST">
				  </c:otherwise>
				</c:choose> 
				  <td class="table_cell">
				  ...
				  <c:choose>
				    ...
				    <c:otherwise>
					<select name="versionId<c:out value="${dedc.edc.crf.id}"/>" onchange="changeQuery<c:out value="${dedc.edc.crf.id}"/>();">  
				      <c:forEach var="version" items="${dedc.edc.versions}">   
					...
					<td class="table_cell">
						<a href="javascript:void(0);" onclick="document.startForm<c:out value="${dedc.edc.crf.id}"/>.submit();"
							onMouseDown="setImage('bt_EnterData<c:out value="${rowCount}"/>','images/bt_EnterData_d.gif');"
							onMouseUp="setImage('bt_EnterData<c:out value="${rowCount}"/>','images/bt_EnterData.gif');"
					...
					<c:choose>
						<c:when test='${actionQuery == "" && dec.stage.name =="invalid" }'>
							<a href="ViewSectionDataEntry?ecId=<c:out value="${dec.eventCRF.id}"/>&tabId=1"
								onMouseDown="setImage('bt_View<c:out value="${rowCount}"/>','images/bt_View.gif');"
								onMouseUp="setImage('bt_View<c:out value="${rowCount}"/>','images/bt_View.gif');"
							...
							<a href="javascript:openDocWindow('PrintDataEntry?ecId=<c:out value="${dec.eventCRF.id}"/>')"
								onMouseDown="setImage('bt_Print<c:out value="${rowCount}"/>','images/bt_Print.gif');"
								onMouseUp="setImage('bt_Print<c:out value="${rowCount}"/>','images/bt_Print.gif');"
							...
							<a href="RestoreEventCRF?action=confirm&id=<c:out value="${dec.eventCRF.id}"/>&studySubId=<c:out value="${studySubject.id}"/>"
								onMouseDown="setImage('bt_Restore<c:out value="${rowCount}"/>','images/bt_Restore.gif');"
								onMouseUp="setImage('bt_Restore<c:out value="${rowCount}"/>','images/bt_Restore.gif');"
						...
						<c:otherwise>
							...
							<a href="RemoveEventCRF?action=confirm&id=<c:out value="${dec.eventCRF.id}"/>&studySubId=<c:out value="${studySubject.id}"/>"
								onMouseDown="setImage('bt_Remove<c:out value="${rowCount}"/>','images/bt_Remove.gif');"
								onMouseUp="setImage('bt_Remove<c:out value="${rowCount}"/>','images/bt_Remove.gif');"
							...
							<a href="DeleteEventCRF?action=confirm&ssId=<c:out value="${studySubject.id}"/>&ecId=<c:out value="${dec.eventCRF.id}"/>"
								onMouseDown="setImage('bt_Delete<c:out value="${rowCount}"/>','images/bt_Delete.gif');"
								onMouseUp="setImage('bt_Delete<c:out value="${rowCount}"/>','images/bt_Delete.gif');"

...
<c:choose>
	<c:when test="${param.displayMode==1}">
<c:set var="showSchedule" value="no" />
<c:set var="showDataEntry" value="yes" />
	</c:when>
	<c:when test="${param.displayMode==2}">
<c:set var="showSchedule" value="yes" />
<c:set var="showDataEntry" value="yes" />
	</c:when>
	<c:otherwise>
<c:set var="showSchedule" value="yes" />
<c:set var="showDataEntry" value="no" />
	</c:otherwise>
</c:choose>
...
<c:url var="tabUrl" value="ListEventsForSubject">
  <c:param name="displayMode"><c:out value="${param.displayMode}" /></c:param>
</c:url>
		url = '<c:out value="${tabUrl}" />&defId=' + defID + '&tab=' + TabID;
		if (TabID<=TabsShown)
			{
...
<c:url var="reloadUrl" value="ListStudySubjectsSubmit">
  <c:param name="defId"><c:out value="${param.defId}" /></c:param>
  <c:param name="tab"><c:out value="${param.tab}" /></c:param>
</c:url>
 <td>&nbsp;&nbsp;&nbsp;View&nbsp;<select name="displayMode" onchange="document.location='<c:out value="${reloadUrl}" />&displayMode='+this.value;">
	<option value="0" <c:choose><c:when test="${param.displayMode==0}">selected="selected"</c:when></c:choose>>Event
	<option value="1" <c:choose><c:when test="${param.displayMode==1}">selected="selected"</c:when></c:choose>>Data Entry
	<option value="2" <c:choose><c:when test="${param.displayMode==2}">selected="selected"</c:when></c:choose>>Combined
</select>&nbsp;status
 </td>
...
document.write('<img src="images/spacer.gif" width="' + (document.body.clientWidth - 180) + '" height=' + (document.body.clientHeight - 271) + '" border="0" alt="">');
document.write('</div>');
...
<c:import url="../include/showTableWithTabForSubject.jsp">
	...
	<c:param name="showSchedule" value="${showSchedule}" />
	<c:param name="showDataEntry" value="${showDataEntry}" />
</c:import>

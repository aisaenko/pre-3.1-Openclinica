...
<c:forEach var="note" items="${discrepancyNotes}">
  ...
  <tr>
  <td>
  <img src="images/bullet.gif" alt=">" title="">&nbsp;<b><c:out value="${note.value.description}"/></b>&nbsp;&nbsp;
   <c:if test="${note.value.saved==false}">
   <span class="alert">[not saved] <a href="DeleteUnsavedDiscrepancyNote?id=<c:out value='${note.value.id}'/>&field=<c:out value="${note.value.field}"/>">[Delete]</a></span>
   </c:if>
  </td><td>

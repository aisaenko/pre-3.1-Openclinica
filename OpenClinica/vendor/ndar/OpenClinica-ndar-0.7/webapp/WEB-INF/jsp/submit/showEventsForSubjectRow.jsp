     ...
     <!-- <td class="table_cell"><c:out value="${count+1}"/></td>-->
      <td class="table_cell_shaded">  
        <c:import url="../submit/eventLayer.jsp">
           ...
           <c:param name="summaryDataEntryStatus" value="${event.studyEvent.summaryDataEntryStatus.name}"/>
      ...
      <c:choose>
        ...
        <c:when test="${event.studyEvent.subjectEventStatus.id==9}">
         <img src="images/icon_Locked.gif" border="0" style="position: relative; left: 7px;" alt="locked" title="" />   
        
        </c:when>    
        </c:choose> 
          </a><img name="ExpandIcon_<c:out value="${currRow.bean.studySubject.label}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount+count}"/>" src="images/icon_blank.gif" width="15" height="15" style="position: relative; left: 8px;" alt="" />            
      </td>
      ...
      <c:when test="${dec.eventCRF.id > 0}">
       ...
       <c:choose>		 
		 <c:when test="${dec.stage.name=='initial data entry'}">		  
		  <img name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>" src="images/CRF_status_icon_Started.gif" border="0" alt="started" title="" />
		  </a>
		 </c:when>
		 <c:when test="${dec.stage.name=='initial data entry complete'}">
		  <img name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>" src="images/CRF_status_icon_InitialDEcomplete.gif" border="0" alt="initial data entry complete" title="" />
		  </a>
		 </c:when>
		 <c:when test="${dec.stage.name=='double data entry'}">
		    <img name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>" src="images/CRF_status_icon_DDE.gif" border="0" alt="double data entry" title="" />
		   </a>
		 </c:when>
		 <c:when test="${dec.stage.name=='data entry complete'}">
		   <img name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>" src="images/CRF_status_icon_Complete.gif" border="0" alt="complete" title="" />
		   </a>
		 </c:when>
		 ...
		 <c:when test="${dec.stage.name=='locked'}">
		   <img name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>" src="images/icon_Locked.gif" border="0" alt="locked" title="" />
		   </a>
		 </c:when>
		 <c:otherwise>
		    <img name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>" src="images/CRF_status_icon_Invalid.gif" border="0" alt="invalid" title="" />
		   </a>
		 </c:otherwise>
		</c:choose>	
	  ...
	  <c:choose>	  
       <c:when test="${currRow.bean.studySubject.status.name !='removed'}">
		...
		<td><a href="ReassignStudySubject?id=<c:out value="${currRow.bean.studySubject.id}"/>"
			...
			name="bt_Reassign1" src="images/bt_Reassign.gif" border="0" alt="Transfer Subject" title="Transfer Subject to Another Site" align="left" hspace="6" /></a>
		</td>        

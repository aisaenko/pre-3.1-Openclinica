...
<tr id="sidebar_Instructions_open" style="display: all">
        ...
        Confirm removal of this event from Study <c:out value="${study.name}"/>. The event and all data associated with it in this Study/Site will be removed.
        It may be restored by a system administrator.
...
<h1><span class="title_manage">Remove Event from Study</span></h1>
   ...
   <c:choose>
    <c:when test="${!empty displayEvent.displayEventCRFs}">
     <form action='RemoveStudyEvent?action=submit&id=<c:out value="${displayEvent.studyEvent.id}"/>&studySubId=<c:out value="${studySub.id}"/>' method="POST">
      <input type="submit" name="submit" value="Remove Event from Study" class="button_xlong" onClick='return confirm("This event has CRF data shown on page. Are you sure you want to remove this event?");'>
     </form>    
    </c:when>
    <c:otherwise>      
      <form action='RemoveStudyEvent?action=submit&id=<c:out value="${displayEvent.studyEvent.id}"/>&studySubId=<c:out value="${studySub.id}"/>' method="POST">
        <input type="submit" name="submit" value="Remove Event from Study" class="button_xlong" onClick='return confirm("Are you sure you want to remove it?");'>
      </form> 
    </c:otherwise>
   </c:choose>  

...
<jsp:useBean scope="request" id="currRow" class="org.akaza.openclinica.bean.managestudy.DisplayStudyEventRow" />   
   <tr>       
     ...
     <td class="table_cell"><c:out value="${currRow.bean.studyEvent.summaryDataEntryStatus.name}"/></td>         
     <td class="table_cell">
			...
			<tr valign="top">
			  ...
			  <td class="table_cell" width="180"><c:out value="${dedc.edc.crf.name}" /></td>
				...
				<c:choose>
				 ...
				 <c:otherwise>
				     <form name="startForm<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>" action="AssessmentInfo?<c:out value="${getQuery}"/>" method="POST">
				...
				<td class="table_cell" with="100">
				 ...
				 <c:otherwise>
				<select name="versionId<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>" onchange="changeQuery<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>();">  
				 <c:forEach var="version" items="${dedc.edc.versions}">   
				...
				<td class="table_cell" width="140">				
				 ...
				 <a href="javascript:void(0);" onclick="document.startForm<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>.submit();"
				  onMouseDown="setImage('bt_EnterData1','images/bt_EnterData_d.gif');"
				  onMouseUp="setImage('bt_EnterData1','images/bt_EnterData.gif');"><img 
	...
	<tr>
		...
		<td>
		 <a href="DeleteEventCRF?action=confirm&ssId=<c:out value="${studySub.id}"/>&ecId=<c:out value="${dec.eventCRF.id}"/>"
			onMouseDown="setImage('bt_Delete1','images/bt_Delete_d.gif');"
			onMouseUp="setImage('bt_Delete1','images/bt_Delete.gif');"><img 

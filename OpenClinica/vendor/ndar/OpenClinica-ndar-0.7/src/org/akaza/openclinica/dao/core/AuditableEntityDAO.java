...
public abstract class AuditableEntityDAO extends EntityDAO {
	...
	public AuditableEntityDAO(DataSource ds) {
		super(ds);
		setDigesterName();
//		System.out.println("digester name set to " + digesterName);
		digester = SQLFactory.getInstance().getDigester(digesterName);
//		System.out.println("digester null? " + (digester == null));
	}
	
	
	/*public AuditableEntityBean getEntityAuditInformation(HashMap hm) {
	 AuditableEntityBean aeb = new AuditableEntityBean();
	...
	public abstract void setTypesExpected();
	  ...
	  /**
	   * <p>
	   * getCountFromHashMap, the method that gets the count from the database
	   * query.
	   */
	  public int getCountFromHashMap(HashMap hm) {
	    return ((Integer) hm.get("count(*)")).intValue();
	  }
	
	/**
	 * Note: The subclass must define findAllByStudyName before calling this method.  Otherwise an empty array will be returned.
	 *
	...
	public void setEntityAuditInformation(AuditableEntityBean aeb, HashMap hm) {
		...
		if (aeb != null) {
			...
			aeb.setStatus(Status.get(statusId.intValue()));
			aeb.setOwnerId(ownerId.intValue());
			aeb.setUpdaterId(updateId==null ? 0 : updateId.intValue());
		}
	}
	

	/**
	 * This method executes a "findAll-style" query.
	 * Such a query has two characteristics:
	 * <ol>
	 * <li> The columns SELECTed by the SQL are all of the columns in the table relevant to the DAO, and only those columns.
	 * (e.g., in StudyDAO, the columns SELECTed are all of the columns in the study table, and only those columns.)
	...
	/**
	 * This method executes a "count-style" query.
	 * Such a query has two characteristics:
	 * <ol>
	 * <li> The column SELECTed by the SQL is a count(*), and only the count column.
	 * <li> It returns number.
	 * </ol>
	 * 
	 * Note that queries which join two tables may be included in the definition
	 * of "count-style" query, as long as the first criterion is met.
	 * 
	 * @param queryName The name of the query which should be executed.
	 * @param variables The set of variables used to populate the PreparedStatement; should be empty if none are needed.
	 * @return A count of AuditableEntityBeans selected by the query.
	 */
	public int executeCountQuery(String queryName, HashMap variables) {
		int answer = -1;

		this.setCountTypeExpected();
	    ...
	    while (it.hasNext()) {
	      answer=(getCountFromHashMap((HashMap) it.next()));
	    }
	...
	/**
	 * This method executes a "count-style" query which does not accept any variables.
	 * 
	 * @param queryName The name of the query which selects the AuditableEntityBeans.
	 * @return A count of AuditableEntityBeans selected by the query.
	 */
	public int executeCountQuery(String queryName) {
		return executeCountQuery(queryName, new HashMap());
	}	
}
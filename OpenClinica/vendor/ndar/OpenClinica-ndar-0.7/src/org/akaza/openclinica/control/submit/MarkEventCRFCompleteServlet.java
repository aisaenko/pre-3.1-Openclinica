...
protected void processRequest() throws Exception {
    ...
    Page errorPage = Page.SUBMIT_DATA_SERVLET;
    ...
    /*if (StringUtil.isBlank(ecb.getInterviewerName())) {
      if ((discNotes == null) || discNotes.getNotes(TableOfContentsServlet.INPUT_INTERVIEWER).isEmpty()){
        throw new InconsistentStateException(errorPage,
        "You may not mark this Event CRF complete, because interviewer name is blank.");
      }
    }
    ...
    if (ecb.getDateInterviewed() == null) {
      if ((discNotes == null) || (discNotes.getNotes(TableOfContentsServlet.INPUT_INTERVIEW_DATE).isEmpty()) ) {
        throw new InconsistentStateException(errorPage,
        "You may not mark this Event CRF complete, because interview date is blank.");
      }
    }*/
    ...
    if (stage.equals(DataEntryStage.UNCOMPLETED)
      ...
      throw new InconsistentStateException(
          errorPage,
          "You may not mark this Event CRF complete, because it is not in the initial data entry or validation stage.");
    }
    ...
    if (stage.equals(DataEntryStage.INITIAL_DATA_ENTRY_COMPLETE)
      ...
      if (!edcb.isDoubleEntry()) {
        throw new InconsistentStateException(
            errorPage,
            "You may not mark this Event CRF complete, because it has already been marked complete for data entry, and validation is not allowed for this Event Definition CRF.");
      }
    }
    ...
    if (!isEachSectionReviewedOnce()) {
      throw new InconsistentStateException(
          errorPage,
          "You may not mark this Event CRF complete, because there are some sections which have not been reviewed once.");
    }

    if (!isEachRequiredFieldFillout()) {
      throw new InconsistentStateException(
          errorPage,
          "You may not mark this Event CRF complete, because there are some required entries which have not been filled out.");
    }
    ...
    if (!fp.isSubmitted()) {
      DisplayTableOfContentsBean toc = TableOfContentsServlet.getDisplayBean(ecb, sm
          .getDataSource(), currentStudy);
      request.setAttribute(BEAN_DISPLAY, toc);

      resetPanel();
      panel.setStudyInfoShown(false);
      panel.setOrderedData(true);
      setToPanel("Subject", toc.getStudySubject().getLabel());
      setToPanel("Study Event Definition", toc.getStudyEventDefinition().getName());

      StudyEventBean seb = toc.getStudyEvent();
      setToPanel("Location", seb.getLocation());
      setToPanel("Start Date", seb.getDateStarted().toString());
      setToPanel("End Date", seb.getDateEnded().toString());

      setToPanel("CRF", toc.getCrf().getName());
      setToPanel("CRF Version", toc.getCrfVersion().getName());
      ...
      if (markComplete) {
        ...
        ArrayList allEDCs = 
          (ArrayList)edcdao.findAllActiveByEventDefinitionId(seb.getStudyEventDefinitionId());
        ...
        if (eventCompleted && (allCRFs.size() >= allEDCs.size())) {
          	//TODO: Change to only set complete if all required are complete --AJF
            seb.setSummaryDataEntryStatus(SummaryDataEntryStatus.COMPLETED);
        }

        seb = (StudyEventBean) sedao.update(seb);

        addPageMessage("The Event CRF was marked complete.");
        request.setAttribute(EnterDataForStudyEventServlet.INPUT_EVENT_ID, String.valueOf(ecb
            .getStudyEventId()));
        ...
        addPageMessage("The Event CRF was not marked complete.");
  ...
  protected void mayProceed() throws InsufficientPermissionException {
    ...
    if (ecb.getStage().equals(DataEntryStage.INITIAL_DATA_ENTRY)) {
      if (ecb.getOwnerId() != ub.getId() && !r.equals(Role.COORDINATOR)
          && !r.equals(Role.STUDYDIRECTOR)) {
        request.setAttribute(TableOfContentsServlet.INPUT_EVENT_CRF_BEAN, ecb);
        addPageMessage("You may not mark this Event CRF complete, because you did not start the data entry and you are not the Study Director or Study Coordinator.");
      ...
      if (ecb.getValidatorId() != ub.getId() && !r.equals(Role.COORDINATOR)
          && !r.equals(Role.STUDYDIRECTOR)) {
        request.setAttribute(TableOfContentsServlet.INPUT_EVENT_CRF_BEAN, ecb);
        addPageMessage("You may not mark this Event CRF complete, because you are not the validator and you are not the Study Director or Study Coordinator.");
        throw new InsufficientPermissionException(Page.TABLE_OF_CONTENTS_SERVLET,
            "not study owner", "1");
      }
    }

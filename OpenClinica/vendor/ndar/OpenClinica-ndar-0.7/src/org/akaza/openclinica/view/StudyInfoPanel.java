...
/**
 * To create a flexible panel of information that will change while
 * the user manages his or her session.
 * 
 * @author thickerson 
 *  
 */
public class StudyInfoPanel {
  ...
  /**
   * An array of StudyInfoPanelLine objects. This is only used if orderedData is
   * flipped on.
   */
  ...
  private boolean submitDataModule = false; //if it is submit data module, need
                                            // to show tree panel
  ...
  private boolean manageSubject = false;// added to control the group of side icons
  ...
  public void setData(String key, TreeMap value) {
    data.put(key, value);
  }
  ...
  public void setData(Page page, HttpSession session, HttpServletRequest request) {
    try {
      //defaults, can be reset inadverdantly by running through one page, tbh
      ...
      if (page.equals(Page.CREATE_DATASET_1)) {
        ...
        //this.setData("Number of Steps", "5");
          ...
          || page.equals(Page.CREATE_DATASET_SUB_ATTR)
          || page.equals(Page.CREATE_DATASET_VIEW_SELECTED)) {
        ...
        this.setData("Items Selected", new Integer(ev_count).toString());
        ...
        this.setSubmitDataModule(false);
        this.setExtractData(false);
        this.removeData("Beginning Date");
        this.removeData("Ending Date");
        DatasetBean dsb = (DatasetBean) session.getAttribute("newDataset");
        int ev_count = dsb.getItemIds().size();
        this.setData("Items Selected", new Integer(ev_count).toString());
       
        if ("01/01/1900".equals(sdf.format(dsb.getDateStart()))) {
          this.setData("Beginning Date", "Not Specified");
        } else {
          this.setData("Beginning Date", sdf.format(dsb.getDateStart()));
        }
        if ("12/31/2100".equals(sdf.format(dsb.getDateEnd()))) {
          this.setData("Ending Date", "Not Specified");
        } else {
          this.setData("Ending Date", sdf.format(dsb.getDateEnd()));
        }
        ...
        this.setExtractData(false);       
        DatasetBean dsb = (DatasetBean) session.getAttribute("newDataset");
        this.setData("Dataset Name", dsb.getName());
        this.setData("Dataset Description", dsb.getDescription());
        int ev_count = dsb.getItemIds().size();
        this.setData("Items Selected", new Integer(ev_count).toString());
        ...
        /*
         * set up the side info panel to create the following upon entry from
         * the ViewStudyServlet Study X Subject Y StudyEventDef Z1 StudyEventDef
         * Z2 <status-tag> CRF A1 <status-tag> CRF A2 Z1 should be
        ...
        StudySubjectBean studySubject = (StudySubjectBean) request.getAttribute("studySub");
        ...
        ArrayList beans = DisplayStudyEventBean.generateBeansFromRows(rows);
        ...
        */
        ...
        this.setIconInfoShown(false);        
          ...
          || page.equals(Page.TABLE_OF_CONTENTS_SERVLET)
          || page.equals(Page.INITIAL_DATA_ENTRY) ||page.equals(Page.INITIAL_DATA_ENTRY_SERVLET)
          || page.equals(Page.DOUBLE_DATA_ENTRY) || page.equals(Page.DOUBLE_DATA_ENTRY_SERVLET)
          || page.equals(Page.ADMIN_EDIT) || page.equals(Page.ADMIN_EDIT_SERVLET)) {
        /*
         * pages designed to also follow the above format; check to see if they
         * are in the session already, and does not refresh. TODO refine and
         * test
         */
        StudyBean study = (StudyBean) session.getAttribute("study");
        StudySubjectBean studySubject = (StudySubjectBean) request.getAttribute("studySubject");
        ...
        this.setCreateDataset(false);
        this.setData("Dataset Name", dsb.getName());
        this.setData("Date Created", sdf.format(dsb.getCreatedDate()));
        this.setData("Dataset Owner", dsb.getOwner().getName());
        this.setData("Date Last Run", sdf.format(dsb.getDateLastRun()));
        ...
        this.setCreateDataset(false);
          ...
          page.equals(Page.LIST_STUDY_SUBJECT_SERVLET) ||
          page.equals(Page.LIST_EVENTS_FOR_SUBJECT)||
          page.equals(Page.SUBMIT_DATA) ||
      ...
      } else if (page.equals(Page.VIEW_SECTION_DATA_ENTRY) ||
          page.equals(Page.VIEW_SECTION_DATA_ENTRY_SERVLET)) {
   ...
   */
  public String getStageImageText(String reference) {
    String answer = "";
    if (reference.equalsIgnoreCase("initial data entry")) {
      answer = "<img src=\"images/icon_InitialDE.gif\" alt=\"Initial Data Entry\" />";
    } else if (reference.equalsIgnoreCase("initial data entry complete")) {
      answer = "<img src=\"images/icon_InitialDEcomplete.gif\" alt=\"Initial Data Entry Complete\" />";
    } else if (reference.equalsIgnoreCase("double data entry")) {
      answer = "<img src=\"images/icon_DDE.gif\" alt=\"Double Data Entry\" />";
    } else if (reference.equalsIgnoreCase("data entry complete")) {
      answer = "<img src=\"images/icon_DEcomplete.gif\" alt=\"Data Entry Complete\" />";
    } else if (reference.equalsIgnoreCase("administrative editing")) {
      answer = "<img src=\"images/icon_AdminEdit.gif\" alt=\"Administrative Editing\" />";
    } else if (reference.equalsIgnoreCase("locked")) {
  ...
  public void addStudyEventTree(StudyBean study, StudySubjectBean studySubject,
    ...
    ArrayList displayData = new ArrayList();
   // displayData.add(new StudyInfoPanelLine("Study", study.getName(), true, false));
    //displayData.add(new StudyInfoPanelLine("<span class=\"alert\" />Subject", studySubject.getLabel() + "</span>", true, false));
    if (withLink) {
   ...
   * @return
   */
  public ArrayList generateTreeFromBeans(ArrayList rows, ArrayList displayData,
    ...
    while (itRows.hasNext()) {
      ...
      StudyEventBean seBean = (StudyEventBean) dseBean.getStudyEvent();
      //checks whether the event is the current one
      if ((ecb != null) && (ecb.getStudyEventId()==seBean.getId()) ) {
         displayData.add(new StudyInfoPanelLine("Study Event", seBean.getStudyEventDefinition()
      ...
      displayData.add(new StudyInfoPanelLine("<img src=\"images/bt_View.gif\" alt=\"View\" />",
          "<a href=\"EnterDataForStudyEvent?eventId=" + seBean.getId() + "\">"
      ...
      while (displayIt.hasNext()) {
        ...
        if ((count == displayCRFs.size() - 1) && dseBean.getUncompletedCRFs().size() == 0) {
          ...
          displayData.add(new StudyInfoPanelLine("" + getStageImageText(dec.getStage().getName()),
              "<span class=\"alert\">" + dec.getEventCRF().getCrf().getName() + " "
                  + dec.getEventCRF().getCrfVersion().getName() + "</span>", false, true,true));
          } else {
            displayData.add(new StudyInfoPanelLine("" + getStageImageText(dec.getStage().getName()),
                "<a href=\"" + getTOCLink(dec) + "\">" + dec.getEventCRF().getCrf().getName() + " "
      ...
      while (uncompIt.hasNext()) {
        ...
        if ((count == uncompleted.size() - 1)) {
          if ((ecb != null) && (ecb.getId() == dedc.getEventCRF().getId())  
            ...
            //System.out.println("ecb id*******" + ecb.getId() + dedc.getEventCRF().getId());
          displayData.add(new StudyInfoPanelLine(
              "<img src=\"images/icon_NotStarted.gif\" alt=\"Not Started\" />",
              "<span class=\"alert\">" + dedc.getEdc().getCrf().getName()
                  + "</span>", false, true,true));
          } else {
            displayData.add(new StudyInfoPanelLine(
                "<img src=\"images/icon_NotStarted.gif\" alt=\"Not Started\" />",
                "<a href=\"InitialDataEntry?eventDefinitionCRFId=" + dedc.getEdc().getId()
                    + "&studyEventId=" + seBean.getId() + "&subjectId=" + studySubject.getSubjectId()
                    + "&eventCRFId=" + dedc.getEventCRF().getId() + "&crfVersionId="
                    + dedc.getEdc().getDefaultVersionId() + "\">" + dedc.getEdc().getCrf().getName()
                    + "</a>", false, true,false));
  ...
  public ArrayList generateTreeFromBeansWithoutLink(ArrayList rows, ArrayList displayData,
    ...
    while (itRows.hasNext()) {
      ...
      displayData.add(new StudyInfoPanelLine("<img src=\"images/bt_View.gif\" alt=\"View\" />",
                   seBean.getSubjectEventStatus().getName(), false, false,false));
      ArrayList displayCRFs = dseBean.getDisplayEventCRFs();
      int count = 0;
      Iterator displayIt = displayCRFs.iterator();
      while (displayIt.hasNext()) {
      ...
      while (uncompIt.hasNext()) {
        ...
        if ((count == uncompleted.size() - 1)) {
          ...
          displayData.add(new StudyInfoPanelLine(
            ...
            displayData.add(new StudyInfoPanelLine(
                "<img src=\"images/icon_NotStarted.gif\" alt=\"Not Started\" />",
                dedc.getEdc().getCrf().getName(), false, true,false));
          ...
          displayData.add(new StudyInfoPanelLine(
            ...
            displayData.add(new StudyInfoPanelLine(
                "<img src=\"images/icon_NotStarted.gif\" alt=\"Not Started\" />",
                dedc.getEdc().getCrf().getName(), false, false,false));
  ...
  private ArrayList generateDatasetTree(ExtractBean eb, DatasetBean db) {
    ...
    for (int i = 0; i < seds.size(); i++) {
      ...
      displayData.add(new StudyInfoPanelLine("Study Event Definition", sed.getName() + repeating,
          true, false));
  ...
  private ArrayList generateEventTree(HashMap eventlist) {
    ...
    //System.out.println("how many events =" + eventlist.size());
    ...
    System.out.println("how many definitions =" + count);
    return displayData;

...
public class EditUserAccountServlet extends SecureController {
  ...
  public static final String PII_PRIVILEGE_ID = "piiPrivilegeId";
  
  public static final String PII_PRIVILEGE_NAME = "piiPrivilegeName";
  ...
  protected void processRequest() throws Exception {
    ...
    if (!fp.isSubmitted()) {
     ...
     //only root user can grant pii privilege
      if (ub.getName().equals("root")) {
        ArrayList piiPrivileges = PiiPrivilege.toArrayList();
        addEntityList("piiPrivileges", piiPrivileges, "A user cannot be edited, because there are no pii privilege within the active study for the user.", Page.ADMIN_SYSTEM);
      }
      ...
      if (errors.isEmpty()) {
        ...
        if (ub.getName().equals("root") && !StringUtil.isBlank(fp.getString("piiPrivilegeId"))) {
          user.setPiiPrivilege(PiiPrivilege.get(fp.getInt("piiPrivilegeId")));
          fp.addPresetValue(PII_PRIVILEGE_NAME, user.getPiiPrivilege().getName());
        }
       ...
       //only root user can grant pii privilege
        if (ub.getName().equals("root")) {
          ...
          if (!StringUtil.isBlank(fp.getString("piiPrivilegeId"))) {
            user.setPiiPrivilege(PiiPrivilege.get(fp.getInt("piiPrivilegeId")));
            fp.addPresetValue(PII_PRIVILEGE_NAME, user.getPiiPrivilege().getName());
          }
        }
  ...
  private void loadPresetValuesFromBean(FormProcessor fp, UserAccountBean user) {
    ...
    fp.addPresetValue(PII_PRIVILEGE_ID, user.getPiiPrivilege().getId());
    fp.addPresetValue(PII_PRIVILEGE_NAME, user.getPiiPrivilege().getName());
  }
  ...
  private void loadPresetValuesFromForm(FormProcessor fp) {
    ...
    String textFields[] = { ARG_USERID, INPUT_FIRST_NAME, INPUT_LAST_NAME, INPUT_EMAIL,
        INPUT_INSTITUTION, INPUT_DISPLAY_PWD,PII_PRIVILEGE_NAME };
    fp.setCurrentStringValuesAsPreset(textFields);

    String ddlbFields[] = { INPUT_USER_TYPE, INPUT_RESET_PASSWORD, PII_PRIVILEGE_ID};

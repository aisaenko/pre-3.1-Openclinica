 ...
 * 
 * @author jxu
 * @version CVS: $Id: UpdateStudyServlet.java,v 1.25 2006/11/01 23:38:19 jxu Exp $
 * 
 */
public class UpdateStudyServlet extends SecureController {
  ...
  public void processRequest() throws Exception {
    ...
    if (StringUtil.isBlank(action)) {
      ...
      if ("confirm".equalsIgnoreCase(action)) {
        ...
        submitStudy();
        addPageMessage("The study has been updated successfully.");
        forwardPage(Page.STUDY_LIST_SERVLET);
  ...
  private void confirmStudy1() throws Exception {
    ...
    if (fp.getString("officialTitle").trim().length()>255){
      Validator.addError(errors, "officialTitle", "The maximum length of Official Title is 255 characters.");
    }
    ...
    if (errors.isEmpty()) {
      ...
      fp.addPresetValue(INPUT_START_DATE, sdf.format(newStudy.getDatePlannedStart()));
      if (newStudy.getDatePlannedEnd() != null){
        fp.addPresetValue(INPUT_END_DATE, sdf.format(newStudy.getDatePlannedEnd()));
      } 
      fp.addPresetValue(INPUT_VER_DATE, sdf.format(newStudy.getProtocolDateVerification()));
  ...
  private void submitStudy() {
    ...
    spv.setValue(study1.getStudyParameterConfig().getReschedule());
    updateParameter(spvdao,spv); 
  ...
  /**
   * Parses the intetventions of a study and divides it into different type and
   * name pairs type and name are separated by '/', and interventions are
  ...
  private void updateParameter(StudyParameterValueDAO spvdao, StudyParameterValueBean spv){
    ...
    //logger.info("study id:" + spv.getStudyId());
    if (spv1.getId() >0) {
      spvdao.update(spv);
    } else {
      spvdao.create(spv);

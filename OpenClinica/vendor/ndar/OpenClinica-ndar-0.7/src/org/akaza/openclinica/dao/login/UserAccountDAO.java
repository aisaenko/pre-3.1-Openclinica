...
public class UserAccountDAO extends AuditableEntityDAO {
  ...
  public void setTypesExpected() {
    ...
    this.setTypeExpected(13, TypeNames.TIMESTAMP);//lastvisit, changed from date
    ...
    this.setTypeExpected(20, TypeNames.INT);//pii_privilege_id
  }
  ...
  public EntityBean update(EntityBean eb) {
    ...
    if (uab.getPasswdTimestamp() == null) {
      nullVars.put(new Integer(11), new Integer(Types.DATE));
    ...
    variables.put(new Integer(16), new Integer(uab.getPiiPrivilege().getId()));

    variables.put(new Integer(17), new Integer(uab.getId()));
    ...
    if (!this.isQuerySuccessful()) {
      ...
      //for bug testing, tbh
      //logger.warning("query succeeded: "+sql+" variables: "+variables.toString());
    }
  ...
  public EntityBean create(EntityBean eb) {
    ...
    variables.put(new Integer(15), new Integer(uab.getPiiPrivilege().getId()));
  ...
  public Object getEntityFromHashMap(HashMap hm, boolean findOwner) {
    ...
    if (findOwner) {
      UserAccountBean owner = (UserAccountBean) this.findByPK(eb.getOwnerId(), false);
      eb.setOwner(owner);
      if (eb.getUpdaterId()!=0) {
        UserAccountBean updater = (UserAccountBean) this.findByPK(eb.getUpdaterId(), false);
    ...
    Integer piiPrivilegeId = (Integer) hm.get("pii_privilege_id");
    eb.setPiiPrivilege(PiiPrivilege.get(piiPrivilegeId == null ? 0 : piiPrivilegeId));
  ...
  /*
   * next on our list, how can we affect the query??? SELECT * FROM USER_ACCOUNT
   * ORDER BY ? DESC?
   * 
  ...
  public ArrayList findStudyByUser(String userName, ArrayList allStudies) {
    ...
    for (int i = 0; i < allStudies.size(); i++) {
      ...
      if (allStudyUserRoleBeans.containsKey(studyId)) {
        ...
        parentAdded = true;
      } else { // we do this so that we can compute Role.max below without
    ...
    //	get all of the user's StudyUserRoleBean for each study that is a child of
    // P
    //	foreach of the user's StudyUserRoleBean for each study that is a child of
    // P
    ...
    //	        // since the user doesn't have a role in sb's parent, their role in sb
    ...
    //	      
    //	      if (roleInChild != null) { // that is, the user has a role in the child
    // study
    //	        Role maxRole = Role.max(roleInChild.getRole(), sur.getRole());
    //	        roleInChild.setRole(maxRole);
    //	        result.add(roleInChild);
    //	      }
    //	      else { // the user does not have a role in the child study - he inherits
  ...
  public ArrayList findAllUsersByStudy(int studyId) {
	 ...
	 //AND (sur.study_id=? 
	 //OR sur.study_id in (select s.study_id FROM study s WHERE s.parent_study_id=? ))

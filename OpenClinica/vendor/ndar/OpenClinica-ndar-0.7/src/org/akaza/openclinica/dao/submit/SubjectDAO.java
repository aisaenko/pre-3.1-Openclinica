...
public class SubjectDAO extends AuditableEntityDAO {
  ...
  public EntityBean create(EntityBean eb) {
    ...
    int id = getNextPK();
    variables.put(new Integer(1), new Integer(id));
    variables.put(new Integer(2), new Integer(sb.getFatherId()));
    variables.put(new Integer(3), new Integer(sb.getMotherId()));
    ...
    if (sb.getDateOfBirth() == null) {
      ...
      variables.put(new Integer(5), sb.getDateOfBirth());
    }
    if (sb.getGender() != 'm' && sb.getGender() != 'f') {
      nullVars.put(new Integer(6), new Integer(Types.CHAR));
      variables.put(new Integer(6), null);
    } else {
      variables.put(new Integer(6), new Character(sb.getGender()));
    }
    variables.put(new Integer(7), sb.getUniqueIdentifier());
    //DATE_CREATED is now()
   ...
   * @param withParents
   *          <code>true</code> if the father and mother id have been properly
   *          set; primarily for use with genetic studies. <code>false</code>
   *          if the father and mother id have not been properly set; primarily
   *          for use with non-genetic studies.
   * @return
   */
  public SubjectBean create(SubjectBean sb) {
    ...
    if (sb.getGender() != 'm' && sb.getGender() != 'f') {
      ...
      variables.put(new Integer(ind), new Character(sb.getGender()));
    }
    ind++;
    if (sb.getUniqueIdentifier() == null) {
  ...
  /**
   * Create a subject whose father and mother id have been properly set. This is
   * primarily for use when creating subjects in genetic studies.
   * 
   * @param sb
   *          The subject to create.
   * @return The created subject, with id set according to the insert id if the
   *         operation was successful, or id set to 0 otherwise.
   * @deprecated
   */
  ...
  public EntityBean update(EntityBean eb) {
    ...
    if (sb.getGender() != 'm' && sb.getGender() != 'f') {
      ...
      variables.put(new Integer(5), new Character(sb.getGender()));
    }

	...
	protected void processRequest() throws Exception {
       //find last 5 modifed studies
	    ...
	    if (allStudies.size()>0){
	      setToPanel("Studies", new Integer(allStudies.size()).toString());
	    }
	    if (allCrfs.size()>0){
	      setToPanel("CRFs", new Integer(allCrfs.size()).toString());
	    }
	    CRFBean crf = (CRFBean)cdao.findByName("ADOS - Autism Diagnostic Observation Schedule - General,  Module 2 - Phrase Speech");
	    if (crf!=null && crf.isActive()) {
	    	request.setAttribute("crfId", new Integer(crf.getId()));
	    	CRFVersionDAO crfvdao = new CRFVersionDAO(sm.getDataSource());
	    	ArrayList<CRFVersionBean> versions = new ArrayList<CRFVersionBean>(crfvdao.findAllActiveByCRF(crf.getId()));
	    	if (!versions.isEmpty()) {
	    		request.setAttribute("versionId", new Integer(versions.get(0).getId()));
	    	}
	    }

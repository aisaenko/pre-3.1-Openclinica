 ...
 * @author jxu
 *
 * Views details of a Study Template
 */
public class ViewSubjectGroupClassServlet  extends SecureController {
  ...
  public void processRequest() throws Exception {
    ...
    if (classId == 0) {
      ...
      addPageMessage("Please choose a group template to view.");
      ...
      for ( int i= 0; i<groups.size(); i++) {
        ...
        ArrayList subjectMaps =
          sgmdao.findAllByStudyGroupClassAndGroup(stb.getId(),sg.getId());
      ...
      //reorder events in the template, put any removed ones to the bottom
      ArrayList availDefs = new ArrayList();
      ArrayList otherDefs = new ArrayList();
      for (int i=0; i<templateDefs.size(); i++) {
        StudyTemplateEventDefBean sted = 
          (StudyTemplateEventDefBean)templateDefs.get(i);
      ...
      availDefs.addAll(otherDefs);
     
      request.setAttribute("group", stb);
      request.setAttribute("templateDefs",  availDefs);

...
public class UserAccountBean extends AuditableEntityBean {
	...
	/*since we extend entity bean, we already have the following:
	 * user_id, user_name, owner_id, date_created, 
	...
	private PiiPrivilege piiPrivilege;
	...
	//
	private boolean sysAdmin; // this is true if the user is the business dmin, false otherwise 
	...
	//		then rolesByStudy has a key which is an Integer whose intValue is s.getStudyId
	//			and the value of that key is an Integer whose intValue is i
	//	in other words, rolesByStudy is a hashmap whose keys are studyIds and whose values
	...
	// key is Integer whose intValue is a studyId, value is StudyUserRoleBean for that study
	private HashMap rolesByStudy = new HashMap();
	
	private String notes; //not in the DB, only for showing some notes for this acocunt on page
	...
	public UserAccountBean() {
		...
		notes="";
		piiPrivilege=PiiPrivilege.NONE;
	}
	...
	/**
	 * @param passwdChallengeAnswer The passwdChallengeAnswer to set.
	 */
	...
	/**
	 * @param passwdChallengeQuestion The passwdChallengeQuestion to set.
	 */
	...
	/**
	 * @param passwdTimestamp The passwdTimestamp to set.
	 */
	...
	public void addUserType(UserType u) {
		...
		// i.e. it may be possible for users to have multiple UserTypes in the future
		if (userTypes.size() > 0) {
			userTypes.clear();
		}
		...
		/*
		 * currently set tech admin superior to sys admin, i.e
		 * anything a sysadmin can do, a tech admin can do too.
		 */
	...
	/**
	 * @param activeStudyId The activeStudyId to set.
	 */
	...
	public StudyUserRoleBean getRoleByStudy(int studyId) {
		...
		if (rolesByStudy.containsKey(key)) {
			...
			if ((s != null) && !s.getStatus().equals(Status.DELETED)) {
				return s;
			}
		}
...
//	}
	...
	/**
	 * @param numVisitsToMainMenu The numVisitsToMainMenu to set.
	 */
  ...
  /**
   * @return the piiPrivilege
   */
  public PiiPrivilege getPiiPrivilege() {
	return piiPrivilege;
  }

  /**
   * @param piiPrivilege the piiPrivilege to set
   */
  public void setPiiPrivilege(PiiPrivilege piiPrivilege) {
	this.piiPrivilege = piiPrivilege;
  }
}
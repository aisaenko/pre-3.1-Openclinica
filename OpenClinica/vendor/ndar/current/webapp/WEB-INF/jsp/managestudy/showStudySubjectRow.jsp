...
<%-- end if show data entry, but not schedule --%>
	  ...
	  <c:choose>	  
       <c:when test="${currRow.bean.studySubject.status.name !='removed'}">
		<td><a href="AssignStudySubject?id=<c:out value="${currRow.bean.studySubject.id}"/>"
			onMouseDown="javascript:setImage('bt_Assign1','images/bt_Assign_d.gif');"
			onMouseUp="javascript:setImage('bt_Assign1','images/bt_Assign.gif');"><img 

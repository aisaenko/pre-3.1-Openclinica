...
<c:set var="displayPwd" value="yes" />
...
<c:choose>
 <c:when test="${userBean.name=='root'}"> 
	<tr valign="top">
	<input type="hidden" name="piiPrivilegeName" value="<c:out value="${piiPrivilegeName}"/>">
	  	<td class="table_header_column">Pii Privilege:</td>
		<td valign="top">
			<table border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td valign="top"><div class="formfieldM_BG">
						<select name="piiPrivilegeId" class="formfieldM">
							<c:forEach var="pii" items="${piiPrivileges}">
								<c:choose>
									<c:when test="${piiPrivilegeId == pii.id}">
										<option value='<c:out value="${pii.id}" />' selected><c:out value="${pii.name}" /></option>
									</c:when>
									<c:otherwise>
										<option value='<c:out value="${pii.id}" />'><c:out value="${pii.name}" /></option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</select>
				...
				<tr>
					<td colspan="2"><jsp:include page="../showMessage.jsp"><jsp:param name="key" value="piiPrivilegeId" /></jsp:include></td>
				</tr>
			</table>
		</td>
	</tr>
  ...
  <tr valign="top">
	...
	<td colspan="2"> 
         ...
         <c:when test="${displayPwd == 'no'}">            
            ...
            <div style="padding-left: 6px; padding-right: 6px; padding-top: 3px; padding-bottom: 3px;"><input type="hidden" name="displayPwd" value="no">On reset, a new password will be sent via Email</div>
         </c:when>
         <c:otherwise>
            ...
            <div style="padding-left: 6px; padding-right: 6px; padding-top: 3px; padding-bottom: 3px;"><input type="hidden" name="displayPwd" value="yes">On reset, a new password will be shown in the &quot;Alerts &amp; Messages&quot; section on a subsequent screen.</div>
         </c:otherwise>
       </c:choose>
      </td>
	</tr>

...
public class ReassignStudySubjectServlet extends SecureController {
  ...
  public void processRequest() throws Exception {
    ...
    if (studySubId == 0) {
      addPageMessage("Please choose a subject to reassign.");
      ...
      if (StringUtil.isBlank(action)) {
        ...
        if (studyId == 0) {
          addPageMessage("Please choose a study/site to reassign the subject.");
          forwardPage(Page.REASSIGN_STUDY_SUBJECT);
          return;
        }
        ...
        if ("confirm".equalsIgnoreCase(action)) {
          ...
          if (sub1.getId() > 0) {
            addPageMessage("The Study Subject ID has been used by another subject in the destination study/site. Study Subject ID must be unique within a study, so please change the ID before reassign this subject.");
            forwardPage(Page.REASSIGN_STUDY_SUBJECT);
            return;
          }
          if (groupMaps.size() > 0) {
            addPageMessage("Warning: This subject has Group data assoicated with current study,"
                + "the group data will be lost if it is reassigned to another study.");
          }
          ...
          addPageMessage("The subject " + studySub.getLabel() + " has been reassigned to " + 
              "study/site " + st.getName() + ".");
          forwardPage(Page.LIST_STUDY_SUBJECT_SERVLET);

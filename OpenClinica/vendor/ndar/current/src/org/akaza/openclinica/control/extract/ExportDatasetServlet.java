...
//	private static String WEB_DIR = "/WEB-INF/datasets/";
	...
	public void processRequest() throws Exception {
	    ...
	    if (StringUtil.isBlank(action)) {  
    	    ...
    	    setToPanel("Study Name", eb.getStudy().getName());
    	    setToPanel("Protocol ID",eb.getStudy().getIdentifier());
    	    setToPanel("Dataset Name", db.getName());
    	    setToPanel("Date Created", sdf.format(db.getCreatedDate()));
    	    setToPanel("Dataset Owner", db.getOwner().getName());
    	    setToPanel("Date Last Run", sdf.format(db.getDateLastRun()));
...
//								parentStudy);
	    		...
	    		String SASFileName = db.getName()+"_sas.sas";
	    	  	...
	    	  	finalTarget = finalTarget.cloneWithNewFileName(generalFileDir+SASFileName);
	    		...
	    		String schemaLocation = "http://www.cdisc.org/models/odm/v1.2.1/ODM1-2-1-foundation.xsd";
	    		eb = dsdao.getDatasetData(eb);
	    		eb.getMetadata();
...
//							parentStudy);
           	...
           	//html based dataset browser
	    	    ...
	    	    eventDescs.put("SubjID", "Study Subject ID");
	    	    eventDescs.put("ProtocolID", "Protocol ID_Site ID");
	    	    eventDescs.put("DOB", "Date Of Birth");
	    	    eventDescs.put("YOB", "Year Of Birth");
	    	    eventDescs.put("Gender", "Gender");   
                answer.setDescriptions(eventDescs);
	    	   
	    	    ArrayList generatedReports = new ArrayList();
	    	    generatedReports.add(answer.getDataFile());
	    	    generatedReports.add(answer.getMetadataFile());
...
//	    	  			ExportFormatBean.EXCELFILE);
	    	  	...
	    	  	finalTarget = Page.EXPORT_DATA_CUSTOM.cloneWithNewFileName("/GeneratedFileDataset");
...
//	    	  	response.setContentType("application/vnd.ms-excel");
	    	...
	    	if (!finalTarget.equals(Page.GENERATE_DATASET_HTML)) {
	    		...
	    		finalTarget = finalTarget.cloneWithNewFileName("" +

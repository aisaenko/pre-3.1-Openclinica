...
public class CdiscOdmXmlBean {
  ...
  private void addStudyGlobalVariables(org.akaza.openclinica.bean.extract.ExtractBean eb, String currentIndent){
	  ...
	  add(currentIndent + indent + indent, eb.getParentStudySummary());
	  add(currentIndent + indent, "</StudyDescription>");
...
private void addMetaDataVersionItemRef(String sectionId, Hashtable sectionItemData, String currentIndent) {
	...
	for(int i=0; i<aa.size(); ++i) {
		TempItemInfo tii = (TempItemInfo)aa.get(i);
		ItemFormMetadataBean ifmBean = (ItemFormMetadataBean)tii.imfb; 
		String name = tii.itemName;
		line = "<ItemRef ItemOID=\"IT." + name + "\"";
		if(ifmBean.isRequired()) {
			line +=  " Mandatory=\"Yes\"";
		} else {
			line +=  " Mandatory=\"No\"";
		...
		for(int k=1; k<eb.getSEDNumCRFs(i)+1; ++k) {
			...
			while(iter.hasNext()) {
				...
				for(int j=0; j<aa.size(); ++j) {
					...
					String datatype = getODMDataType(ib.getItemDataTypeId());
					String header = "";
					String left = "";
					String right = "";
					if(ifmBean.getHeader().length()>0) {
						header = ifmBean.getHeader() + ".";
					} 
					if(ifmBean.getLeftItemText().length()>0) {
						left = " " + ifmBean.getLeftItemText() + ".";
					} 
					if(ifmBean.getRightItemText().length()>0) {
						right = " " + ifmBean.getRightItemText();
					}
					if(header.equals("") && left.equals("") && right.equals("")) {
						...
						add(currentIndent, "<ItemDef OID=\"IT." + tii.itemName + "\" Name=\"" + stringCut(ib.getDescription()) + "\" DataType=\"" + datatype + "\" SASFieldName=\"" + sasname.getValidSasName(ib.getDescription()) + "\"/>" );
					} else {
						add(currentIndent, "<ItemDef OID=\"IT." + tii.itemName + "\" Name=\"" + stringCut(ib.getDescription()) + "\" DataType=\"" + datatype + "\" SASFieldName=\"" + sasname.getValidSasName(ib.getDescription()) + "\">" );
		  				  ...
		  				  if(aa.size() > 0){
			  				  ...
			  				  //for(int m=aa.size()-1; m>=0; --m){
				  			  	  ...
				  			  	  if(ocDatatypeId != 9) {
				  			  		  ...
				  			  		  value = getODMDate(tti.itemValue);
				  			  	  }
				  			  	  add(currentIndent+indent+indent+indent+indent+indent, "<ItemData ItemOID=\"IT." + tti.itemName + "\" Value=\"" + value + "\"/>");
...
private Hashtable getSedCrfIgItem(ExtractBean eb){
  ...
  private String getODMDataType(int OCDataTypeId) {
	  switch (OCDataTypeId) {
	  	  //BL		//BN	//ED	//TEL	//ST	  
  ...
  //convert openclinica Date format String (MM/DD/YYYY) to ODM Date format String (YYYY-MM-DD)
  private String getODMDate(String d) {
	  String[] ds = d.split("\\/");

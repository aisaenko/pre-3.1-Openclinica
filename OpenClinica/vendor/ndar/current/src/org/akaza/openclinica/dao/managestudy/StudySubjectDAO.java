...
//		 study_subject_id | integer               | not null default nextval('public.study_subject_study_subject_id_seq'::text)
...
//		 update_id        | numeric               | 
		...
		int ind = 1;
		this.setTypeExpected(ind, TypeNames.INT); ind++; // study_subject_id
		this.setTypeExpected(ind, TypeNames.STRING); ind++; // label
		this.setTypeExpected(ind, TypeNames.STRING); ind++; // secondary_label
		this.setTypeExpected(ind, TypeNames.INT); ind++; // subject_id
		this.setTypeExpected(ind, TypeNames.INT); ind++; // study_id
		this.setTypeExpected(ind, TypeNames.INT); ind++; // status_id
		
		this.setTypeExpected(ind, TypeNames.DATE); ind++; // enrollment_date
		this.setTypeExpected(ind, TypeNames.DATE); ind++; // date_created
		this.setTypeExpected(ind, TypeNames.DATE); ind++; // date_updated
		this.setTypeExpected(ind, TypeNames.INT); ind++; // owner_id
		this.setTypeExpected(ind, TypeNames.INT); ind++; // update_id
	}
    ...
    public int findTheGreatestLabel() {
    	return findTheGreatestLabel(null);
    }

    public int findTheGreatestLabel(String prefix) {
        this.setTypesExpected();
        String sql = digester.getQuery("findAll");
        ArrayList alist = this.select(sql);
        ArrayList answer = new ArrayList();
        ...
        while (it.hasNext()) {
            StudySubjectBean eb = (StudySubjectBean) this.getEntityFromHashMap((HashMap) it.next());
            if (prefix==null || prefix.length() == 0 || eb.getLabel().startsWith(prefix)) {
                answer.add(eb);
            }
        }
    ...
    public EntityBean findAnotherBySameLabel(String label, int studyId,
        int studySubjectId) {
      StudySubjectBean eb = new StudySubjectBean();
        this.setTypesExpected();
...
//		   VALUES (?,?,?,?,NOW(),?,?,?)
		...
		int ind = 1;
		int id = getNextPK();
		variables.put(new Integer(ind), new Integer(id)); ind++;
		variables.put(new Integer(ind), sb.getLabel()); ind++;
		variables.put(new Integer(ind), new Integer(sb.getSubjectId())); ind++;
		variables.put(new Integer(ind), new Integer(sb.getStudyId())); ind++;
		variables.put(new Integer(ind), new Integer(sb.getStatus().getId())); ind++;
		//Date_created is now()
		variables.put(new Integer(ind), new Integer(sb.getOwnerId())); ind++;
		//    variables.put(new Integer(ind), new Integer(sb.getStudyGroupId())); ind++;
		if (sb.getEnrollmentDate() == null) {
			nullVars.put(new Integer(ind), new Integer(Types.DATE));
			variables.put(new Integer(ind), null); ind++;
		} else {
			variables.put(new Integer(ind), sb.getEnrollmentDate()); ind++;
		}
		variables.put(new Integer(ind), sb.getSecondaryLabel()); ind++;
	 ...
	 * @param withGroup
	 *          <code>true</code> if the group id has been set (primarily for
	 *          use with genetic studies); <code>false</false> otherwise.
	...
	public EntityBean update(EntityBean eb) {
		...
		variables.put(new Integer(ind), new java.util.Date()); ind++;
		variables.put(new Integer(ind), new Integer(sb.getUpdater().getId())); ind++;
		variables.put(new Integer(ind), sb.getSecondaryLabel()); ind++;
		variables.put(new Integer(ind), new Integer(sb.getId())); ind++;
	...
	public ArrayList findAllByStudyIdAndLimit(int studyId,boolean isLimited) {
		...
		this.setTypesExpected();
		int ind = 1;
		this.setTypeExpected(ind, TypeNames.STRING); ind++; //unique_identifier
		this.setTypeExpected(ind, TypeNames.CHAR); ind++; //gender
		this.setTypeExpected(ind, TypeNames.INT); ind++; // study_subject_id
		this.setTypeExpected(ind, TypeNames.STRING); ind++; // label
		this.setTypeExpected(ind, TypeNames.STRING); ind++; // secondary_label
		this.setTypeExpected(ind, TypeNames.INT); ind++; // subject_id
		...
		this.setTypeExpected(ind, TypeNames.INT); ind++; // update_id
		this.setTypeExpected(ind, TypeNames.STRING); ind++; // secondary_label
		this.setTypeExpected(ind, TypeNames.STRING); ind++; // studyName

...
public class StudyDAO extends AuditableEntityDAO {
  ...
  public void setTypesExpected() {
    ...
    this.setTypeExpected(55, TypeNames.BOOL);//collect_dob
  ...
  public StudyBean updateStepOne(StudyBean sb) {
    ...
    // id
    //variables.put(new Integer(19), sb.getStatus())//need to get a function
    //to get the id
    //variables.put(new Integer(19), sb.getCreatedDate());
    variables.put(new Integer(21), new Integer(sb.getUpdaterId()));//owner id
   ...
   * v1.0rc1' document. We insert the study in this method, and then update the
   * same study in the next three steps.
   * <P>
  ...
  public StudyBean createStepOne(StudyBean sb) {
    ...
    variables.put(new Integer(16), sb.getFacilityRecruitmentStatus());
  ...
  public StudyBean createStepTwo(StudyBean sb) {
    ...
    variables.put(new Integer(2), sb.getProtocolType());
    ...
    variables.put(new Integer(5), sb.getPhase());
    ...
    variables.put(new Integer(16), sb.getGender());
   ...
   // variables.put(new Integer(21), new Boolean(sb.isDiscrepancyManagement()));
  ...
  public StudyBean createStepThree(StudyBean sb) {
    HashMap variables = new HashMap();
    variables.put(new Integer(1), sb.getPurpose());
    variables.put(new Integer(2), sb.getAllocation());
    variables.put(new Integer(3), sb.getMasking());
    variables.put(new Integer(4), sb.getControl());
    variables.put(new Integer(5), sb.getAssignment());
    variables.put(new Integer(6), sb.getEndpoint());
    variables.put(new Integer(7), sb.getInterventions());
  ...
  public StudyBean createStepFour(StudyBean sb) {
    HashMap variables = new HashMap();
    variables.put(new Integer(1), sb.getDuration());
    variables.put(new Integer(2), sb.getSelection());
    variables.put(new Integer(3), sb.getTiming());
  ...
  public ArrayList<StudyBean> findAllStudiesBySubjectId(int subjectId) {
	ArrayList<StudyBean> answer = new ArrayList<StudyBean>();
	...
	String sql = digester.getQuery("findAllStudiesBySubject");
	ArrayList alist = this.select(sql, variables);
	
	for (Iterator it = alist.iterator(); it.hasNext(); ) {
		StudyBean sb = (StudyBean) this.getEntityFromHashMap((HashMap) it.next());
		answer.add(sb);
	}
  ...
  public Collection findAllByLimit(boolean isLimited) {
    ...
    String sql = null;
    if (isLimited) {
      sql = digester.getQuery("findAllLimit5");
  ...
  public Collection findAllByPermission(Object objCurrentUser, int intActionType) {
	 ...
	 * and the values are ArrayLists; each element of the ArrayList is a StudyBean
	 * representing a child of the study whose id is the key
	 * <p>e.g., if A has children B and C, then this will return a HashMap h where
	 * h.get(A.getId()) returns an ArrayList whose elements are B and C
	 */
	public HashMap getChildrenByParentIds(ArrayList allStudies) {
	  HashMap answer = new HashMap();
	  
	  if (allStudies == null) { return answer; }

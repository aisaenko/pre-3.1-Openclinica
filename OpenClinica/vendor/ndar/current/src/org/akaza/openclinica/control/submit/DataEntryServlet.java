...
import org.akaza.openclinica.dao.submit.ScoreUsageCountDAO;
import org.akaza.openclinica.dao.submit.ScoreUsageCountDetailDAO;
import org.akaza.openclinica.dao.submit.SectionDAO;
import org.akaza.openclinica.dao.submit.SubjectDAO;
import org.akaza.openclinica.exception.InconsistentStateException;
import org.akaza.openclinica.exception.InsufficientPermissionException;
import org.akaza.openclinica.logic.core.ScoreCalculator;
import org.akaza.openclinica.logic.core.Skipper;
...
public abstract class DataEntryServlet extends SecureController {
  ...
  public static final String INPUT_RUN_SCORING = "runScoring";
...
//  Get the study then the parent study
    ...
    setPresetValues(fp.getPresetValues());
        ...
        Skipper skipper = new Skipper(sm, ecb, edcb, ub);
        skipper.doSkip();
		section.setSkipRules(skipper.getDisplayBeans(section.getItems()));
		skipper.setSkipped((List<DisplayItemBean>)section.getItems());
...
//					addPageMessage("Please notice that you must enter data for the <b>required</b> entries.");
        // }
        ...
        boolean runScoring = fp.getString(INPUT_RUN_SCORING).equals(VALUE_YES);
        ...
        if (!success) {
          ...
          if (runScoring) {
              // Start to score the derived items
              sc.doCalculation();
              ScoreUsageCountDAO sucdao = new ScoreUsageCountDAO(sm.getDataSource());
              int crfVersionId = ecb.getCRFVersionId();
              int scoreUsageCountId=0;
              ScoreUsageCountBean sucb = (ScoreUsageCountBean)sucdao.findByCRFVersion(crfVersionId);
              if (sucb==null) {
            	  sucb = new ScoreUsageCountBean();
              }
              if (!sucb.isActive()) {
            	  sucb.setCRFVersionId(crfVersionId);
            	  sucb = (ScoreUsageCountBean)sucdao.create(sucb);
              }
              scoreUsageCountId=sucb.getId();
              if (!sucdao.incrementCountUsageByCrfVersion(sucb)) {
            	  logger.warning("Exceeded number of scores purchased (purchased="+sucb.getCountPurchased()+", used="+sucb.getCountUsed()+") for crfVersionId="+crfVersionId);
              }
              ScoreUsageCountDetailDAO sucddao = new ScoreUsageCountDetailDAO(sm.getDataSource());
              ScoreUsageCountDetailBean sucdb = new ScoreUsageCountDetailBean();
              sucdb.setDateScored(new Date());
              sucdb.setScoreUsageCountId(scoreUsageCountId);
              sucdb.setUserId(ub.getId());
              sucdb.setEventCRFId(ecb.getId());
              sucddao.create(sucdb);
          } else {
        	  sc.eraseCalculations();
          }
  ...
  private EventCRFBean createEventCRF() throws InconsistentStateException {
    ...
    if (eventCRFId == 0) {// no event CRF created yet
      ...
      if (newEcb.getFormUsageCountId()==0) {
          int formUsageCountId=0;
          FormUsageCountDAO fucdao = new FormUsageCountDAO(sm.getDataSource());
          FormUsageCountBean fuc = (FormUsageCountBean)fucdao.findByCRFVersion(crfVersionId);
          if (fuc==null) {
...
//  TODO: Also need to change/set eventStatus? --AJF
  ...
  protected boolean markCRFComplete() throws Exception {
    ...
    boolean ide = true; //initial data entry (true=ide, false=dde)
...
//    System.out.println("*** countCrf="+ecdao.countAllRequiredByStudyEvent(seb)+" and countDef="+edcdao.countAllRequiredActiveByEventDefinitionId(seb.getStudyEventDefinitionId()));
    for (EventCRFBean ec: (ArrayList<EventCRFBean>)allCRFs) {
      if (!ec.getStatus().equals(Status.UNAVAILABLE) && !ec.getStatus().equals(Status.LOCKED) && !ec.getStatus().equals(Status.DELETED)) {
        eventCompleted = false;
        break;
      }
    }
    if (eventCompleted && (ecdao.countAllRequiredByStudyEvent(seb) == edcdao.countAllRequiredActiveByEventDefinitionId(seb.getStudyEventDefinitionId()))) {
      seb.setSummaryDataEntryStatus(SummaryDataEntryStatus.COMPLETED);
    }

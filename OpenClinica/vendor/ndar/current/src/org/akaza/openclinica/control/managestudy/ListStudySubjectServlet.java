...
public abstract class ListStudySubjectServlet extends SecureController {
  ...
  public void processRequest() throws Exception {
    ...
    ArrayList studyGroupClasses = stdao.findAllActiveByStudy(currentStudy);
    
    //for all the study groups for each group class
    for (int i=0; i<studyGroupClasses.size();i++) {
      StudyTemplateBean stb = (StudyTemplateBean)studyGroupClasses.get(i);
      ArrayList groups = sgdao.findAllByGroupClass(stb);
      stb.setStudyGroups(groups);
    }
    ...
    for (int i = 0; i < subjects.size(); i++) {
      ...
      for (int j = 0; j < studyGroupClasses.size(); j++) {
        StudyTemplateBean stb = (StudyTemplateBean) studyGroupClasses.get(j);
        boolean hasClass = false;
        for (int k = 0; k < groups.size(); k++) {
          SubjectGroupMapBean sgmb = (SubjectGroupMapBean) groups.get(k);
          if (sgmb.getGroupClassName().equalsIgnoreCase(stb.getName())) {
      ...
      for (int j = 0; j < allDefs.size(); j++) {
        ...
        if (!hasDef) {
          ...
          blank.setSummaryDataEntryStatus(SummaryDataEntryStatus.NOT_STARTED);
    ...
    columnArray.add("Study Subject ID");
    columnArray.add("Subject Status");
    columnArray.add("Gender");
    for (int i = 0; i < studyGroupClasses.size(); i++) {
      StudyTemplateBean stb = (StudyTemplateBean) studyGroupClasses.get(i);
      columnArray.add(stb.getName());
    }
  ...
  public static DisplayStudyEventBean getDisplayStudyEventsForStudySubject(
    ...
    de.setDisplayEventCRFs(ViewStudySubjectServlet.getDisplayEventCRFs(ds, eventCRFs,
        eventDefinitionCRFs, ub, currentRole));

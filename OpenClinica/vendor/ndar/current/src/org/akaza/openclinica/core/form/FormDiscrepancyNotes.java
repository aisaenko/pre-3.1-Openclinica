...
public class FormDiscrepancyNotes {
	...
	public boolean deleteUnsavedNote(String field, DiscrepancyNoteBean note) {
		List newFieldNotes=new ArrayList();
		if (!fieldNotes.containsKey(field)) {
			return false;
		}

		boolean deleted=false;
		for (DiscrepancyNoteBean aNote : ((List<DiscrepancyNoteBean>)fieldNotes.get(field))) {
			if (!deleted && !(aNote.equals(note))) {
				newFieldNotes.add(aNote);
			} else {
				deleted=true;
			}
		}
		fieldNotes.put(field, newFieldNotes);
		return deleted;
	}

...
public class DatasetDAO extends AuditableEntityDAO {
  ...
  public EntityBean create(EntityBean eb) {
    ...
    variables.put(new Integer(2), new Integer(db.getStudyId()));
    variables.put(new Integer(3), new Integer(db.getStatus().getId()));
    variables.put(new Integer(4), db.getName());
    variables.put(new Integer(5), db.getDescription());
    variables.put(new Integer(6), db.getSQLStatement());
    variables.put(new Integer(7), new Integer(db.getOwnerId()));
    variables.put(new Integer(8), new Integer(db.getNumRuns()));
    variables.put(new Integer(9), new Boolean(db.isShowEventLocation()));
    variables.put(new Integer(10), new Boolean(db.isShowEventStart()));
    variables.put(new Integer(11), new Boolean(db.isShowEventEnd()));
    variables.put(new Integer(12), new Boolean(db.isShowSubjectDob()));
    variables.put(new Integer(13), new Boolean(db.isShowSubjectGender()));    
  ...
  public EntityBean findByNameAndStudy(String name,StudyBean study) {
    ...
    if (it.hasNext()) {
      eb = (DatasetBean) this.getEntityFromHashMap((HashMap) it.next());
    	...
    	logger.warning("found no object: "+sql+" with name="+name);
    }
    return eb;
  }
  ...
  public ExtractBean getDatasetData(ExtractBean eb) {
  	...
  	while (it.hasNext()) {
		...
		String gender = (String)row.get("gender");
  		...
  		Boolean repeating = (Boolean) row.get("study_event_def_repeating");
...
//  			Integer studyEventDefinitionId = (Integer) row.get("study_event_definition_id");
//  			String studyEventDefinitionName = (String) row.get("study_event_definition_name");
//  			Boolean studyEventDefinitionRepeating = (Boolean) row.get("study_event_def_repeating");
//  			

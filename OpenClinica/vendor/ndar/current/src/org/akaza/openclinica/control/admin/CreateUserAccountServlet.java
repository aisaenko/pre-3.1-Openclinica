...
public class CreateUserAccountServlet extends SecureController {
	...
	protected void processRequest() throws Exception {
        ...
        if (ub.getName().equals("root")) {
          ...
          addEntityList("piiPrivileges", piiPrivileges, "A user cannot be created, because there are no pii privilege within the active study for the user.", Page.ADMIN_SYSTEM);
        }
		  ...
		  forwardPage(Page.CREATE_ACCOUNT);
			...
			if (errors.isEmpty()) {
                ...
                if (ub.getName().equals("root") && !StringUtil.isBlank(fp.getString("piiPrivilegeId"))) {
                  createdUserAccountBean.setPiiPrivilege(PiiPrivilege.get(fp.getInt("piiPrivilegeId")));
                }
				...
				String ddlbFields[] = { INPUT_STUDY, INPUT_ROLE,INPUT_TYPE,PII_PRIVILEGE_ID };
	...
	private void sendNewAccountEmail(UserAccountBean createdUserAccountBean, String password) throws Exception {
		...
		body += "A new user account has been created for you on the OpenClinica system.  Your login information follows:\n\n";

...
public class StudyConfigService {
  ...
  /**
   * – true if the study has a value defined for this parameter o if studyId is
   * a parent study, then this is true iff there is a row for this
  ...
  public String hasDefinedParameterValue(int studyId, String parameterHandle) {
    ...
    if (spv.getId() > 0) {// there is a row for that study, no matter it is a
      // top study or not
      return spv.getValue();
    }
    ...
    for (int i = 0; i < parameters.size(); i++) {
      ...
      if (spv.getId() > 0) {
        if (handle.equalsIgnoreCase("collectDob")) {
          ...
          //System.out.println("subjectPersonIdRequired" +  spc.getSubjectPersonIdRequired());
          ...
          //System.out.println("subjectIdGeneration" +  spc.getSubjectIdGeneration());
        ...
        } else if (handle.equalsIgnoreCase("reschedule")){
          spc.setReschedule(spv.getValue());
        }
      }
    }
  ...
  public StudyBean setParameterValuesForStudy(StudyBean study) {    
    StudyParameterValueDAO spvdao = new StudyParameterValueDAO(ds);
    ArrayList parameters = spvdao.findParamConfigByStudy(study);
    study.setStudyParameters(parameters);
    return study;
  ...
  public StudyBean setParametersForSite(StudyBean site) { 
    ...
    for ( int i=0; i<siteParameters.size(); i++) {
      ...
      } else if (parameter.equalsIgnoreCase("reschedule")) {
        site.getStudyParameterConfig().setReschedule(spvb.getValue());
      } 

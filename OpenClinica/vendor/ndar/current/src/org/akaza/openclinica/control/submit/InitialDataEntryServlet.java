...
public class InitialDataEntryServlet extends DataEntryServlet {
	...
	protected DisplayItemBean validateDisplayItemBean(DiscrepancyValidator v,
		...
		DisplayItemBean newDib = loadFormValue(dib);
		...
		if (rt.equals(org.akaza.openclinica.bean.core.ResponseType.TEXT) ||
			...
			newDib = validateDisplayItemBeanText(v, newDib);
		}
			...
			newDib = validateDisplayItemBeanSingleCV(v, newDib);
		}
			...
			newDib = validateDisplayItemBeanMultipleCV(v, newDib);
	    } else if (rt.equals(org.akaza.openclinica.bean.core.ResponseType.CALCULATION)) {
	        //for now, treat calculation like any other text input -- eventually this might need to be customized
	        newDib = validateDisplayItemBeanText(v, newDib);
		}
	...
	protected Page getServletPage() {
        ...
        return Page.INITIAL_DATA_ENTRY_SERVLET;
          ...
          Page target = Page.INITIAL_DATA_ENTRY_SERVLET.cloneWithNewFileName(Page.INITIAL_DATA_ENTRY_SERVLET.getFileName()+"?eventCRFId="
        	  + eventCRFId + "&sectionId="+ sectionId + "&tab=" + tabId);
          return target;
        }
	}

...
//        }
        ...
        DisplaySubjectBean dsb = null;
        for (int i = 0; i < allSubjects.size(); i++) {
            // Extract the subject from the hash map
            SubjectBean subject = (SubjectBean) allSubjects.get(i);
            SubjectBean prevSubject = i > 0 ? (SubjectBean)allSubjects.get(i - 1) : null;
            // Create protocolSubjectIds
            if (prevSubject == null || subject.getId() != prevSubject.getId()) {
                dsb = new DisplaySubjectBean();

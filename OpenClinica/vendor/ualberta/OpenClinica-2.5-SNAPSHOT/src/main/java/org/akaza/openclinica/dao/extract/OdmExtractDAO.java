...
public class OdmExtractDAO extends DatasetDAO {
    ...
    public void setSubjectEventFormDataTypesExpected(String odmVersion) {
        if ("1.2".equals(odmVersion) || "1.3".equals(odmVersion)) {
            ...
            this.setTypeExpected(8, TypeNames.INT);// definition_order;
            this.setTypeExpected(9, TypeNames.STRING);// definition_oid;
            this.setTypeExpected(10, TypeNames.BOOL);// definition_repeating
            this.setTypeExpected(11, TypeNames.INT);// sample_ordinal
            this.setTypeExpected(12, TypeNames.STRING);// se_location (study
            // event)
            this.setTypeExpected(13, TypeNames.DATE);// date_start (study
            // event)
            this.setTypeExpected(14, TypeNames.DATE);// date_end (study
            // event)
            this.setTypeExpected(15, TypeNames.BOOL);// start_time_flag
            this.setTypeExpected(16, TypeNames.BOOL);// end_time_flag
            this.setTypeExpected(17, TypeNames.INT);// event_status_id
            this.setTypeExpected(18, TypeNames.INT);// crf_order;
            this.setTypeExpected(19, TypeNames.STRING);// crf_version_oid
            this.setTypeExpected(20, TypeNames.STRING);// crf_version
            this.setTypeExpected(21, TypeNames.INT);// crf_status_id
            this.setTypeExpected(22, TypeNames.INT);// event_crf_id
            this.setTypeExpected(23, TypeNames.DATE);// date_interviewed
            this.setTypeExpected(24, TypeNames.STRING);// interviewer_name
        }
    }
    ...
    public HashSet<Integer> getMetadata(int studyId, MetaDataVersionBean metadata, String odmVersion) {
        ...
        // System.out.println("getStudyEventAndFormMetaSQl= " +
        // this.getStudyEventAndFormMetaSql(studyId));
        ...
        // System.out.println("getItemGroupandItemMetasql= " +
        // this.getItemGroupAndItemMetaSql(cvIds));
        ...
        while (it.hasNext()) {
            ...
            String datatype = MetaDataCollector.getOdmItemDataType(rsTypeId, dataTypeId, odmVersion);
            ...
            if (!itdset.contains(itId)) {
                ...
                if (rsTypeId == 3 || rsTypeId == 7) {// 3:checkbox;
                    ...
                    idef.setLength(hasCode ? MetaDataCollector.getDataTypeLength(codes.keySet()) : maxLengths.containsKey(itId) ? maxLengths.get(itId)
                        : MetaDataCollector.getTextLength());
                } else if ("integer".equalsIgnoreCase(datatype)) {
                    idef.setLength(hasCode ? MetaDataCollector.getDataTypeLength(codes.keySet()) : 10);
                } else if ("float".equalsIgnoreCase(datatype)) {
                    idef.setLength(hasCode ? MetaDataCollector.getDataTypeLength(codes.keySet()) : 32);
                } else {
                    idef.setLength(0);
                }
                idef.setSignificantDigits(MetaDataCollector.getSignificantDigits(datatype, codes.keySet(), hasCode));
                itemDefs.add(idef);
            }
    ...
    public void getClinicalData(StudyBean study, DatasetBean dataset, OdmClinicalDataBean data, String odmVersion) {
        ...
        if (odmVersion.startsWith("oc")) {
            // System.out.println("getOCSubjectEventFormSql=" +
            // getOCSubjectEventFormSql(studyIds, sedIds, itemIds,
            // dateConstraint));
            this.setSubjectEventFormDataTypesExpected(odmVersion);
            ArrayList viewRows = select(getOCSubjectEventFormSql(studyIds, sedIds, itemIds, dateConstraint));
            Iterator iter = viewRows.iterator();
            this.setDataWithOCAttributes(study, dataset, data, odmVersion, iter, oidPoses);
        } else {
            // System.out.println("getSubjectEventFormSql=" +
            // getSubjectEventFormSql(studyIds, sedIds, itemIds,
            // dateConstraint));
        ...
        ArrayList viewRows = select(getEventGroupItemSql(studyIds, sedIds, itemIds, dateConstraint));
        Iterator iter = viewRows.iterator();
        ExportSubjectDataBean sub = new ExportSubjectDataBean();
        ExportStudyEventDataBean se = new ExportStudyEventDataBean();
        ExportFormDataBean form = new ExportFormDataBean();
        ...
        boolean goon = true;
        String itprev = "";
        HashMap<String, String> nullValueMap = ClinicalDataCollector.getNullValueMap();
        while (iter.hasNext()) {
            HashMap row = (HashMap) iter.next();
        ...
        while (iter.hasNext()) {
            ...
            if (subprev.equals(studySubjectLabel)) {
                ...
                if (dataset.isShowSubjectSecondaryId()) {
                    sub.setSecondaryId((String) row.get("secondaryId"));
                }
                ...
                if (dob != null) {
                    ...
                    if (startDate != null && dataset.isShowSubjectAgeAtEvent()) {
                        sub.setAgeAtEvent(Utils.getAge(dob, startDate));
                    }
                }
            ...
            if (!seprev.equals(key) || !sepos.containsKey(key + sampleOrdinal)) {
                ...
                if (dataset.isShowEventLocation()) {
                    se.setLocation((String) row.get("location"));
                }
            ...
            if (formprev.equals(key)) {
                ...
                if (dataset.isShowCRFstatus()) {
                    // for odm, it shows only crf version with not empty item
                    // data value
                    // so, here use event_crf status which could be 2 or 6 only.
                    // However, status names are different:
                    // 2 - completed; 6 - locked.
                    Integer statusId = (Integer) row.get("crf_status_id");
                    form.setStatus(statusId == 2 ? "Completed" : "Locked");
                }
    ...
    private String getCrfVersionStatus(int seSubjectEventStatusId, int crfVersionStatusId) {
        // Logic here was copied from the method "getSEDCRFStatus" of
        // ExtractBean.java
        String crfVersionStatus = "";
        if (seSubjectEventStatusId == 7 || seSubjectEventStatusId == 6 || seSubjectEventStatusId == 5) {
            // locked,skipped,stopped
            crfVersionStatus = DataEntryStage.LOCKED.getName();
        } else if (seSubjectEventStatusId == 0) { // invalid
            crfVersionStatus = DataEntryStage.LOCKED.getName();
        } else if (seSubjectEventStatusId == 4) { // COMPLETED
            crfVersionStatus = SubjectEventStatus.COMPLETED.getName();
        } else if (crfVersionStatusId >= 0) {
            if (!Status.AVAILABLE.equals(Status.get(crfVersionStatusId))) {
                crfVersionStatus = DataEntryStage.LOCKED.getName();
            } else {// /??????
                crfVersionStatus = DataEntryStage.get(crfVersionStatusId).getName();
            }
        }
    ...
    protected String getOCSubjectEventFormSql(String studyIds, String sedIds, String itemIds, String dateConstraint) {
        return "select ss.oc_oid as study_subject_oid, ss.label, ss.unique_identifier, ss.secondary_label, ss.gender, ss.date_of_birth, ss.status_id,"
            + " sed.ordinal as definition_order, sed.oc_oid as definition_oid, sed.repeating as definition_repeating, se.sample_ordinal as sample_ordinal,"
            + " se.se_location, se.date_start, se.date_end, se.start_time_flag, se.end_time_flag,"
            + " se.subject_event_status_id as event_status_id, edc.ordinal as crf_order, "
            + " cv.oc_oid as crf_version_oid, cv.name as crf_version, ec.status_id as crf_status_id,"
            + " ec.event_crf_id, ec.date_interviewed, ec.interviewer_name from (select study_event_id, study_event_definition_id, study_subject_id,"
            + " \"location\" as se_location, sample_ordinal, date_start, date_end, subject_event_status_id, start_time_flag, end_time_flag"
            + " from study_event where study_event_definition_id in " + sedIds + " and study_subject_id in"
            + " (select ss.study_subject_id from study_subject ss where ss.study_id in (" + studyIds + ") " + dateConstraint
            + ")) se, (select ss.oc_oid, ss.study_subject_id, ss.label, ss.secondary_label, ss.subject_id, s.unique_identifier, "
            + " s.gender, s.date_of_birth, s.status_id from study_subject ss, subject s where ss.study_id in (" + studyIds + ") " + dateConstraint
            + " and ss.subject_id = s.subject_id) ss, study_event_definition sed, event_definition_crf edc,"
            + " (select event_crf_id, crf_version_id, study_event_id, status_id, date_interviewed, interviewer_name from event_crf where event_crf_id in ("
            + getEventCrfIdsByItemDataSql(studyIds, sedIds, itemIds, dateConstraint) + ")) ec, crf_version cv" + " where sed.study_event_definition_id in "
            + sedIds + " and sed.study_event_definition_id = se.study_event_definition_id and se.study_subject_id = ss.study_subject_id"
            + " and sed.study_event_definition_id = edc.study_event_definition_id and se.study_event_id = ec.study_event_id"
            + " and edc.crf_id = cv.crf_id and ec.crf_version_id = cv.crf_version_id order by ss.oc_oid, sed.ordinal, se.sample_ordinal, edc.ordinal";
